"""Instantiate, prepare, save and enroll PCB cases; never dispatch execution."""
import argparse
import json
...
def main():
    ...
    template = request(args.api, "GET", "/api/workspace/workflow-source-templates/sensor-interface-pcb")["template"]
    catalog = request(args.api, "GET", "/api/workspace/workflow-sources/tools?session_id=" + args.session)
    catalog["tools"] = [tool for tool in catalog["tools"] if tool["server_id"] == args.server_id or (tool["server_id"] == "wright-workspace-files" and tool["tool_name"] == "copy_file")]
    tool_path = root / "pcb-tool-catalog.json"
    tool_path.write_text(json.dumps(catalog, indent=2) + "\n", encoding="utf-8")
    cases = []
    for index in range(1, 4):
        identity = f"sensor-interface-pcb-{index:02d}"
        ...
        if saved.exists():
            ...
            instance = request(args.api, "POST", "/api/workspace/workflow-source-templates/sensor-interface-pcb/instances", payload)
        ...
        if not prepared.exists():
            subprocess.run([sys.executable, "scripts/prepare-pcb-dataset-campaign.py", "--workspace-root", args.workspace_root, "--draft-root", str(draft_root), "--server-id", args.server_id, "--attempt", args.attempt, "--scenario", identity, "--instance-source", str(original), "--tool-catalog", str(tool_path)], check=True, timeout=120)
        ...
        cases.append(json.loads(execution.read_text(encoding="utf-8")))
    combined = root / f"pcb-{args.attempt}.json"
