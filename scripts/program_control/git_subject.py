...
class GitReader:
    ...
    def resolve_commit(self, revision: str) -> str:
        if not revision or revision.startswith("-") or any(ch.isspace() for ch in revision):
            raise GitSubjectError("unsafe revision")
        value = self._run(["rev-parse", "--verify", f"{revision}^{{commit}}"]).decode().strip()
    ...
    def blob_id(self, commit: str, path: str) -> str:
        normalized = normalize_repo_path(path)
        output = self._run(["ls-tree", commit, "--", normalized]).decode("utf-8", errors="strict")
    ...
    def list_files(self, commit: str, root: str) -> list[str]:
        ...
        raw = self._run(["ls-tree", "-r", "-z", "--name-only", commit, "--", normalized])
        try:
    ...
    def worktree_observation(self) -> dict[str, object]:
        ...
        autocrlf_raw = self._run(["config", "--get", "core.autocrlf"]).decode().strip().lower()
        autocrlf = autocrlf_raw if autocrlf_raw in {"true", "false", "input"} else "unset"
        return {
    ...
    def manifest(self, commit: str, program_root: str) -> tuple[list[dict[str, str]], str]:
        ...
        for path in self.list_files(commit, root):
            if path in excluded or "/evidence/verification/EPP-F01-dashboard-delivery.json" in path:
                continue
