"""Register selected isolated robot tools through normal local API; explicit opt-in."""
import argparse
import json
...
def main():
    ...
    base = ["docker", "exec", "-i", "-e", "PYTHONPATH=", "-e", "WRIGHT_ROBOT_WORKSPACE=/workspace", "wright-081-robot-runtime"]
    definitions = {
        "wright-campaign-rosbag": ("ROSBag MCP — isolated engineering dataset campaign", base+["/tmp/robot-env/bin/rosbag-mcp"], "https://pypi.org/project/rosbag-mcp/0.2.0/"),
        "wright-campaign-robot-operations": ("Offline robot tracking operations — engineering dataset campaign", base+["/tmp/robot-env/bin/python", "/operations/robot_tracking_mcp.py"], None),
    }
    for alias, (name, command, source) in definitions.items():
        if alias not in report["server_map"]:
            created = request(args.api, "POST", "/api/mcp/servers", {"name": name, "type": "stdio", "command": command, "category": "robotics", "description": "Selected local disposable dataset runtime. Actual offline CSV/ROS2 operations; no physical robot or production graph access. Local scope only; not a public catalog qualification.", "source_url": source, "default_enabled": False, "risk_level": "medium", "deployment_mode": "local-only"})
    ...
    report["status"] = "installed_discovered" if len(report["tools"]) == 33 else "tool_discovery_incomplete"
    persist()
