...
const checkout = resolve(
  process.env.WRIGHT_RIVET2_CHECKOUT || resolve(runnerRoot, '..', 'spike', '.work', 'rivet2'),
);
...
if (!existsSync(resolve(checkout, '.git'))) {
  throw new Error('Pinned Rivet 2 checkout is missing. Run the editor acquire script first.');
}
if (run('git', ['rev-parse', 'HEAD'], { cwd: checkout, capture: true }) !== revision) {
...
writeFileSync(resolve(runnerRoot, 'manifest.json'), `${JSON.stringify(manifest, null, 2)}\n`, 'utf8');
