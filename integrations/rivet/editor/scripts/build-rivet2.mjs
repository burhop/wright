...
const patchPaths = [
  ...
  resolve(editorRoot, "patches", "rivet2-wright-ai-bridge.patch"),
  ...
  resolve(editorRoot, "patches", "rivet2-workspace-deliverables.patch"),
];
const wrappers = [
  "App.tsx",
  "WrightAiRuntime.ts",
  "WrightEditorBridge.tsx",
  "index.html",
];
...
const expectedChangedPaths = new Set([
  ...
  "packages/app/src/features/graphBuilder/authoringSemantics.test.ts",
  ...
  "packages/app/src/components/AiGraphCreatorInput.tsx",
  "packages/app/src/components/GraphBuilderSessionPanel.test.tsx",
  "packages/app/src/components/GraphBuilderSessionPanel.tsx",
  "packages/app/src/domain/graphBuilder/graphBuilderSchemas.ts",
  "packages/app/src/state/graphBuilder.ts",
  "packages/app/src/features/graphBuilder/authoringSemantics.ts",
  "packages/app/src/features/graphBuilder/editorGateway.test.ts",
  "packages/app/src/features/graphBuilder/editorGateway.ts",
  "packages/app/src/features/graphBuilder/evaluation/runtimeAdapters.ts",
  "packages/app/src/features/graphBuilder/legacyDraftRunnerGateway.test.ts",
  "packages/app/src/features/graphBuilder/planBSessionRuntime.test.ts",
  "packages/app/src/features/graphBuilder/planBSessionRuntime.ts",
  "packages/app/src/features/graphBuilder/policyPrompt.ts",
  "packages/app/src/features/graphBuilder/policyRunner.providerContract.test.ts",
  "packages/app/src/features/graphBuilder/policyRunner.test.ts",
  "packages/app/src/features/graphBuilder/requestedDeliverable.test.ts",
  "packages/app/src/features/graphBuilder/sessionController.test.ts",
  "packages/app/src/features/graphBuilder/sessionController.ts",
  "packages/app/src/features/graphBuilder/sessionPresentation.test.ts",
  "packages/app/src/hooks/usePlanBGraphBuilder.ts",
...
const wrapperTargets = new Map([
  ...
  [
    "WrightEditorBridge.tsx",
    resolve(checkout, "packages", "app", "src", "WrightEditorBridge.tsx"),
  ],
  [
    "WrightAiRuntime.ts",
    resolve(checkout, "packages", "app", "src", "WrightAiRuntime.ts"),
  ],
...
if (dirty) {
  ...
  const onlyExpectedPaths = changedPaths.every((path) =>
    expectedChangedPaths.has(path),
  );
...
run(
  process.execPath,
  ["--max-old-space-size=8192", yarn, "workspace", packageName, "run", "build"],
  { cwd: checkout },
);
...
if (resolvedOutput !== resolve(editorRoot, "dist")) {
  throw new Error(
    `Refusing to replace unexpected artifact path: ${resolvedOutput}`,
  );
}
...
const publicAssetPattern =
  /https?:\/\/(?:fonts\.(?:googleapis|gstatic)\.com|cdn\.|unpkg\.|jsdelivr\.)/i;
...
writeFileWithRetry(
  resolve(editorRoot, "manifest.json"),
