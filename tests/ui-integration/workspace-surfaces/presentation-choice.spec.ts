...
test.describe("panel and browser presentation choice", () => {
  test("shares state, remembers choice, and closes a view without stopping", async ({
    ...
    await context.route("http://s-*.localhost:5173/__wright/bootstrap", (route) =>
      route.fulfill({ contentType: "text/html", body: previewAppHtml }),
    );
    let presentation = 0;
    await page.route("**/api/workspace/surfaces/surface-app/presentations", async (route) => {
    ...
    await expect(page.getByText(/preferred presentation: browser/i)).toBeVisible();
    ...
    await expect(page.getByText(/closing this view keeps.*running/i)).toBeVisible();
  });

  test("browser-open failure preserves an already active panel", async ({ page }) => {
    await mockWorkspaceShell(page, [liveSurface()]);
    await page.addInitScript(() => {
      window.open = () => null;
    });
    await page.route("**/api/workspace/surfaces/surface-app/presentations", async (route) => {
      const kind = String((route.request().postDataJSON() as { kind: string }).kind);
      await route.fulfill({
        status: 201,
        json: {
    ...
    await page.route("**/presentations/*", (route) => route.fulfill({ status: 204 }));
    await page.goto("/workspace/ws-1");
    await page.getByTestId("surface-open-panel").click();
    await expect(page.locator('iframe[title="Shareable app"]')).toBeVisible();
    await page.getByTestId("surface-open-browser").click();
    await expect(page.getByRole("alert")).toContainText(/refused to open/i);
    await expect(page.locator('iframe[title="Shareable app"]')).toBeVisible();
  });
});
