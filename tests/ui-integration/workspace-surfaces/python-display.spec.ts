...
const projection = (revision: number, value: number) => ({
  ...
  representations: [
    ...
    },
    {
      ...
      encoding: "json",
      data: { columns: ["Time", "Load"], data: [[0, 10], [1, value]] },
      fallbackRank: 10,
    },
  ],
});
...
test.describe("Python display journey", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route("**/api/agent/health", (route) => route.fulfill({ json: { status: "ok" } }));
    await page.route("**/api/inference/health", (route) => route.fulfill({ json: { status: "ok" } }));
    ...
    await page.route("**/api/agent/active", (route) => route.fulfill({ json: "hermes" }));
    await page.route("**/api/agent/sessions*", (route) =>
      route.fulfill({ json: { sessions: [{ session_id: "session-1", title: "Default" }] } }),
    );
    ...
    await page.route("**/api/workspace/files?*", (route) =>
      route.fulfill({
        json: { workspace: { name: "graphs", path: "/", type: "directory", children: [] } },
      }),
    );
    ...
    await page.route("**/api/workspace/surfaces/surface-loads/display*", async (route) => {
    ...
    await page.route("**/api/workspace/surfaces/surface-loads/history", (route) =>
      route.fulfill({
        json: {
          items: [
            { artifactId: "artifact-1", revision: 1, current: false, createdAt: "2026-07-30T12:00:00Z" },
            { artifactId: "artifact-2", revision: 2, current: true, createdAt: "2026-07-30T12:01:00Z" },
          ],
        },
      }),
    );
    await page.route("**/api/workspace/surfaces/surface-loads/verification", (route) =>
      route.fulfill({
        json: {
  ...
  test("shows, updates, preserves history, and discloses destructive retention", async ({ page }) => {
    ...
    await expect(page.getByTestId("surface-tab-surface-loads")).toHaveText("Measured load");
    await expect(page.getByRole("img", { name: "Load reaches 15 N." })).toBeVisible();

    releaseEvent();
    await expect(page.getByRole("img", { name: "Load reaches 18 N." })).toBeVisible();
    await expect(page.getByTestId("surface-tab-surface-loads")).toHaveCount(1);

    await page.getByTestId("surface-history").click();
    const history = page.getByRole("dialog", { name: "Display revision history" });
    ...
    const verification = page.getByRole("dialog", { name: "Artifact verification" });
    ...
    await expect(page.getByRole("dialog")).toContainText(/cannot be recovered/i);
  ...
  test("shows a stable actionable renderer error", async ({ page }) => {
    ...
    await page.route("**/api/workspace/surfaces/surface-loads/display*", (route) =>
      route.fulfill({
        json: {
          ...projection(1, 15),
    ...
    await page.goto("/workspace/ws-1");
    await expect(page.getByRole("alert")).toContainText(/display.*unavailable/i);
    await expect(page.getByTestId("surface-diagnostics")).toBeVisible();
  });

  test("live installed-wheel display path", { tag: "@live" }, async ({ page }) => {
    await page.goto("/workspace/ws-1");
