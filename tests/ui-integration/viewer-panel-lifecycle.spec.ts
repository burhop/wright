...
test.describe("Pluggable Viewer Panel Lifecycle E2E", () => {
  ...
  test("should open and mount the 3D STL viewer when clicking cube.stl", async ({
    page,
  }) => {
  ...
  test("should open and mount the Code Editor when clicking script.py", async ({
    page,
  }) => {
    await page.goto("/workspace/ws-1");
    ...
    await expect(textarea).toHaveValue(
      "print('Hello Playwright Pluggable Viewer')",
    );

    // Verify syntax highlighting overlay has print keyword and string styled
    const overlay = page.locator(
      '[data-testid="code-editor-highlight-overlay"]',
    );
    ...
    const consoleBody = page.locator(
      '[data-testid="code-editor-console-body"]',
    );
  ...
  test("should open and mount the PDF Document Viewer when clicking document.pdf", async ({
    page,
  }) => {
    await page.goto("/workspace/ws-1");
  ...
  test("should open and mount the Image Viewer when clicking image.png", async ({
    page,
  }) => {
    await page.goto("/workspace/ws-1");
  ...
  test("should open and mount the HTML Previewer when clicking sandbox.html", async ({
    page,
  }) => {
    await page.goto("/workspace/ws-1");
  ...
  test("should show fix button on error output and trigger agent chat on click", async ({
    ...
    expect(lastSentMessage).toContain(
      "When executing @error_script.py we got this output. Fix any problems:",
    );
