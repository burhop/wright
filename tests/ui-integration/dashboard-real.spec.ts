...
test.describe("Dashboard Page - Real Backend Integration @live", () => {
  test("should support clicking + Create Workspace and selecting an existing workspace", async ({
    page,
  }) => {
    ...
    const workspaceData = workspaceResponse.ok()
      ...
      : { workspaces: [] };
    const hasWorkspaces = Boolean(workspaceData.workspaces?.length);

    if (hasWorkspaces) {
      console.log("Workspaces found. Clicking the first workspace card...");
      // Click the first card
      const workspaceCard = page
        .locator('[data-testid^="card-workspace-"]')
        .first();
