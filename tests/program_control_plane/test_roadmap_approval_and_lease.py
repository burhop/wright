...
from program_control.validation import evaluate_approval_history, validate_roadmap_approval_and_lease
...
def test_current_pointer_wip_dates_and_lease_are_consistent(repository_root: Path) -> None:
...
def test_expired_or_wrong_worktree_lease_and_unsafe_paths_fail(repository_root: Path) -> None:
    documents = copy.deepcopy(current_documents(repository_root))
...
def test_decision_and_risk_dates_are_machine_structured(repository_root: Path) -> None:
    decisions = load(
        repository_root / "docs/programs/engineering-process-platform/decision-register.json"
    )["records"]
    risks = load(repository_root / "docs/programs/engineering-process-platform/risk-register.json")[
        "risks"
    ]
    assert all(
        set(row["due"]) == {"before_event", "due_at", "last_reviewed_at"} for row in decisions
    )
    assert all(
...
def test_exact_two_scope_approval_bundle_is_current(repository_root: Path) -> None:
    root = repository_root / "docs/programs/engineering-process-platform/evidence/approvals"
    approvals = [
        load(root / "APR-EPP-F01-MC-002.json"),
        load(root / "APR-EPP-F01-IMPL-002.json"),
    ]
...
def test_approval_revocation_expiry_scope_and_subject_drift_fail(repository_root: Path) -> None:
    root = repository_root / "docs/programs/engineering-process-platform/evidence/approvals"
    material = load(root / "APR-EPP-F01-MC-002.json")
    implementation = load(root / "APR-EPP-F01-IMPL-002.json")
