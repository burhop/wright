...
async def check_custom_health(url: str = Query(..., description="LLM URL to test")):
    url = url.strip()
    if not url:
        return HealthCheckResponse(status="unhealthy", latency_ms=0.0, error="URL is empty")
...
async def reset_setup(request: Request):
    ...
    try:
        if os.path.exists(DATABASE_PATH):
            conn = sqlite3.connect(DATABASE_PATH)
            conn.execute("DELETE FROM system_settings WHERE key IN ('llm_api_url', 'active_agent')")
            conn.commit()
            conn.close()
    except Exception as e:
        raise HTTPException(
