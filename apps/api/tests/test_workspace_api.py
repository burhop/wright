...
def client(workspace_setup) -> TestClient:
    ...
    try:
        ...
        conn.execute("DELETE FROM engineering_workspaces WHERE session_id = 'test-session'")
        conn.commit()
        conn.close()
    except Exception:
        pass
...
def test_create_file_and_directory(client):
    ...
    response = client.post(
        "/api/workspace/files",
        json={"session_id": "test-session", "path": "/new_folder/test.txt", "type": "file"}
    )
    assert response.status_code == 201
...
def test_move_file(client):
    ...
    client.post(
        "/api/workspace/files",
        json={"session_id": "test-session", "path": "/move_folder", "type": "directory"}
    )
    client.post(
        "/api/workspace/files",
    ...
    response = client.put(
        ...
        json={"session_id": "test-session", "source_path": "/to_move.txt", "destination_path": "/move_folder/moved.txt"}
    )
    assert response.status_code == 200
...
def test_workspace_config_and_remote_operations(client):
    ...
    try:
        ...
        client.post(
            "/api/workspace/files",
            json={"session_id": "test-session", "path": "/push_test.txt", "type": "file"}
        )
        client.post(
            "/api/workspace/git/commit",
        ...
        res = subprocess.run(["git", "log", "-n1", "--oneline"], cwd=remote_dir, capture_output=True, text=True, check=True)
        ...
        try:
            ...
            branch_res = subprocess.run(["git", "branch", "--show-current"], cwd=clone_dir, capture_output=True, text=True, check=True)
            ...
            subprocess.run(["git", "commit", "-m", "remote update"], cwd=clone_dir, check=True)
            subprocess.run(["git", "push", "origin", current_branch], cwd=clone_dir, check=True)
            ...
            subprocess.run(["git", "add", "push_test.txt"], cwd=clone_dir, check=True)
            subprocess.run(["git", "commit", "-m", "conflicting remote update"], cwd=clone_dir, check=True)
            subprocess.run(["git", "push", "origin", current_branch], cwd=clone_dir, check=True)
            ...
            client.post(
                "/api/workspace/git/commit",
                json={"session_id": "test-session", "message": "conflicting local update"}
            )
...
def test_save_file_content_success(client):
    ...
    response = client.put(
        "/api/workspace/files/content",
        json={"session_id": "test-session", "path": "/save_test.py", "content": "print('hello world')"}
    )
    assert response.status_code == 200
    assert response.json()["success"] is True
