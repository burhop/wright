...
def test_catalog_browsing_never_probes_the_computer(capability_client, monkeypatch):
    ...
    with sqlite3.connect(database_path) as connection:
        assert (
            connection.execute(
                "SELECT COUNT(*) FROM machine_compatibility_observations"
            ).fetchone()[0]
            == 0
        )


def test_repeated_catalog_reads_do_not_rebuild_bundled_snapshot(
    ...
    monkeypatch.setattr(
        "tool_registry.catalog_snapshots.bundled_snapshot", forbid_rebuild
    )
    assert client.get("/api/mcp/capabilities").status_code == 200
