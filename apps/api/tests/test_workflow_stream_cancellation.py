...
def test_disconnect_drains_real_file_checkpoint_without_recancelling_producer(tmp_path, monkeypatch, checkpoint_times_out):
    if checkpoint_times_out:
        monkeypatch.setattr('workspace_service.workflow_run_record._FINAL_CHECKPOINT_SECONDS', .1)
    text = section('workflow', 'disconnect', name='Disconnect control', purpose='Controlled cancellation',
                   discipline='engineering', reviewed_ai_suggestions=False)
    for key, fmt in [('first', 'text'), ('second', 'html')]:
        settings = {'output_format': fmt, 'save_output': True, 'file_policy': 'indexed',
                    'output_filename': key + ('.txt' if fmt == 'text' else '.html')}
        if key == 'second':
            settings.update(prompt_source='connection', prompt_input='second_prompt')
        text += section('task', key, name=key, purpose='Controlled task', step_type='work', group=None,
                        tool=None, reusable_step=None, performed_by='ai_assisted', prompt='Write a report',
                        inputs=[port(key + '_prompt')], outputs=[port(key + '_response')], settings=settings)
    text += section('connection', 'edge', type='item', **{'from': 'first.first_response', 'to': 'second.second_prompt'},
                    label='Prompt', when=None)
    path = 'workflows/disconnect.workflow.wflow'

    async def scenario():
        executor = BoundedExecutor(max_workers=1)
        svc = SimpleNamespace(
            files=WorkspaceFileUseCases(str(tmp_path / 'unused.db'), executor, LocalWorkspaceFiles),
            workflow_sources=WorkspaceWorkflowSourceUseCases(executor),
            lifecycle=SimpleNamespace(get_by_session=lambda _: {'workspace_id': 'controlled', 'local_path': str(tmp_path)}),
        ...
        response = await router.run_workflow_source_endpoint(
            WorkflowSourceRunRequest(session_id='s', path=path, expected_storage_digest=saved.storage_digest),
            Request(scope), svc)
        sent = []

        async def receive():
            await started.wait()
            asyncio.get_running_loop().call_later(.25 if checkpoint_times_out else .05, executor._capacity.release)
        ...
        try:
            ...
            raw = json.loads(next((tmp_path / 'runs/disconnect').glob('*.json')).read_text())
            ...
            assert not any(event['kind'] == 'step_completed' and event.get('task_id') == 'second'
                           for event in raw['events'])
            assert (tmp_path / 'first.txt').read_text() == 'Completed first task'
            assert not (tmp_path / 'second.html').exists()
            assert recent_workflow_runs(tmp_path, path)[0]['status'] == ('interrupted' if checkpoint_times_out else 'cancelled')
            assert not any(b'"kind": "completed"' in item.get('body', b'') for item in sent)
        finally:
