...
def test_latest_snapshot_preserves_authoritative_review_and_response_contract(tmp_path):
    ...
    async def scenario():
        pending = await router.workflow_recent_runs(
            "s", "workflows/rfq.workflow.wflow", svc, latest_only=True
        )
        run = pending["runs"][0]
        assert (
            run["status"] == "pending_review"
            and run["execution"]["active_task_id"] is None
        )
        assert (
            run["source_matches_current"] is True and run["run_id"] == result["run_id"]
        )
        ...
        await router.workflow_artifact_review_decision(
            ...
            WorkflowArtifactReviewDecisionRequest(
                session_id="s",
                expected_package_digest=result["review"]["package_digest"],
                decision="changes_requested",
                reason="Controlled test missing dimension",
            ),
            svc,
        )
        changed = await router.workflow_recent_runs(
            "s", "workflows/rfq.workflow.wflow", svc, latest_only=True
        )
        ...
        stale = await router.workflow_recent_runs(
            "s", "workflows/rfq.workflow.wflow", svc, latest_only=True
        )
