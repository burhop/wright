...
describe("engineering workflow source", () => {
  ...
  it("rehydrates the friendly projection against the exact accepted envelope", () => {
    ...
    expect(canonicalDefinitionBytes(parsed.workflow!)).toBe(
      canonicalDefinitionBytes(initialWorkflow),
    );
  ...
  it("keeps canonical software identities out of public connection points", () => {
    const proposal = applyRecoveryBatch(
      initialWorkflow,
      initialLayout,
      aiDrawingProposal(initialWorkflow),
    );
    expect(proposal.ok).toBe(true);
    const proposedWorkflow = acceptRecoveryResult(
      initialWorkflow,
      initialLayout,
      proposal,
    )!.workflow;
  ...
  it("fails closed when an internal connection-point type has no public kind", () => {
    ...
    expect(() => formatRecoveryAuthoringSource(unsupported)).toThrow(
      "WFR-SOURCE-PORT-KIND-UNSUPPORTED:type.unsupported.internal",
    );
  });

  it("rejects an unknown public connection-point kind", () => {
    const source = formatRecoveryAuthoringSource(initialWorkflow).text.replace(
      '"kind":"reference_images"',
      '"kind":"unknown_engineering_kind"',
    );
    const parsed = parseRecoveryAuthoringSource(source, initialWorkflow);
    expect(parsed.ok).toBe(false);
    expect(parsed.workflow).toBeNull();
    expect(parsed.diagnostics).toContainEqual(
      expect.objectContaining({ code: "WFR-SOURCE-PORT-KIND" }),
    );
  });
  ...
  it("turns an engineering-source prompt edit into a contained command", () => {
    ...
    const edited = formatted.replace(
      "Create the bracket with two mounting holes and one slotted interface.",
      "Create the bracket with two mounting holes and two slotted interfaces.",
    );
  ...
  it("rejects host-managed authority fields and retains the accepted definition", () => {
    ...
    expect(parsed.ok).toBe(false);
    expect(parsed.diagnostics).toContainEqual(
      expect.objectContaining({ code: "WFR-SOURCE-FIELD-MANAGED" }),
    );
    expect(parsed.workflow).toBeNull();
  });
  ...
  it("selects the engineering task instead of an overlapping internal port span", () => {
    ...
    expect(
      recoveryAuthoringSemanticIdAtOffset(formatted.sourceMap, offset),
    ).toBe("block.export-step");
  });
  ...
  it.each([
    ...
    [
      ...
      () =>
        apply(initialWorkflow, [
          { kind: "disconnect", relationshipId: "rel.review-to-export" },
        ]),
    ],
    [
      "connect",
      () => {
        const disconnected = apply(initialWorkflow, [
          { kind: "disconnect", relationshipId: "rel.review-to-export" },
        ]);
        return apply(disconnected, [
          {
            kind: "connect",
            relationship: structuredClone(
              initialWorkflow.relationships.find(
                (item) => item.id === "rel.review-to-export",
              )!,
            ),
          },
        ]);
    ...
    "cold-reopens a %s command from the one visible source",
    (_name, candidate) => {
      expectColdRoundTrip(candidate());
    },
  );
  ...
  it("keeps groups optional while preserving a group rename when groups exist", () => {
    const renamed = apply(initialWorkflow, [
      {
        kind: "set_phase_name",
    ...
    expect(formatRecoveryAuthoringSource(renamed).text).toContain(
      'name: "Engineering review"',
    );
    expectColdRoundTrip(renamed);

    const withoutGroupSections = formatRecoveryAuthoringSource(
      initialWorkflow,
    ).text.replace(/group (define|verify|deliver)\n(?: {2}.+\n)+end\n\n/g, "");
    const parsed = parseRecoveryAuthoringSource(
      withoutGroupSections,
      initialWorkflow,
    );
    expect(parsed.ok).toBe(true);
    expect(semanticBytes(parsed.workflow!)).toBe(
      semanticBytes(initialWorkflow),
    );
    ...
    const ungroupedResult = validateRecoveryAuthoringRoundTrip(
      ungrouped,
      ungrouped,
    );
  ...
  it("lowers a source-added ungrouped task through the atomic command set", () => {
    ...
    expect(
      parsed.ok,
      parsed.diagnostics.map((item) => item.code).join(", "),
    ).toBe(true);
    ...
    expect(commands).toContainEqual(
      expect.objectContaining({
        ...
        block: expect.objectContaining({
          id: "block.record-release-note",
          phaseId: null,
        }),
      }),
    );
    const accepted = apply(initialWorkflow, commands as RecoveryCommand[]);
    expect(accepted.blocks).toContainEqual(
      expect.objectContaining({
        id: "block.record-release-note",
        phaseId: null,
      }),
  ...
  it("cold format-parse-formats a newly authored input without changing it into a task", () => {
    const authored = parseRecoveryAuthoringSource(
      newlyAuthoredInputSource(),
      initialWorkflow,
    );
    expect(
      authored.ok,
      authored.diagnostics.map((item) => item.code).join(", "),
    ).toBe(true);
    ...
    expect(
      reopened.ok,
      reopened.diagnostics.map((item) => item.code).join(", "),
    ).toBe(true);
    expect(formatRecoveryAuthoringSource(reopened.workflow!).text).toBe(
      firstFormat,
    );
    expect(semanticBytes(reopened.workflow!)).toBe(
      semanticBytes(authored.workflow!),
    );
  });
  ...
  it.each([
    [
      "workflow discipline",
      {
        ...
        patch: { engineeringDomain: "mechanical_design.custom" },
      } as const,
    ],
    [
      "engineering item",
      {
        ...
        artifactId: "artifact.design-intent",
        patch: {
          name: "Loads and design intent",
          description:
            "Engineer-entered loads, interfaces, constraints, units, and material decisions.",
        },
      } as const,
    ],
    [
    ...
    [
      "tool assignment",
      {
        ...
        toolId: "tool.create-production-bracket",
      } as const,
    ],
    [
      "connection details",
      {
        ...
        relationshipId: "rel.review-to-export",
        patch: {
          label: "released CAD model",
          condition: "Engineer approval recorded",
        },
      } as const,
    ],
  ...
  it("lowers a source-added task and source-created connection to the shared command set", () => {
    ...
    const parsed = parseRecoveryAuthoringSource(
      formatRecoveryAuthoringSource(connected).text,
      initialWorkflow,
    );
    expect(parsed.ok).toBe(true);
    const commands = textEditCommands(initialWorkflow, parsed.workflow!);
    expect(commands).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          kind: "add_block",
          block: expect.objectContaining({ id: "block.drawing-1" }),
        }),
        expect.objectContaining({
          kind: "connect",
  ...
  it.each([
    ...
    "lowers a source-driven %s to the shared command set",
    (_name, candidate, expectedKind) => {
      const before = initialWorkflow;
      const parsed = parseRecoveryAuthoringSource(
        formatRecoveryAuthoringSource(candidate()).text,
        before,
      );
      expect(parsed.ok).toBe(true);
  ...
  it("rejects a candidate whose semantic facts are not represented by the public source", () => {
    ...
    candidate.artifactContracts.find(
