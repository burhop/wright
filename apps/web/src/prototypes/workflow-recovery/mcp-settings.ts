...
export function hiddenMcpInputs(workflow: RecoveryWorkflow): Set<string> {
  const connected = new Set(workflow.relationships.map(edge => edge.targetId));
  return new Set(workflow.blocks.filter(isMcpBlock).flatMap(block => block.inputPortIds.filter(id =>
    !connected.has(id) && !(block.configuration.mcp_arguments_source === "connection" && sourceKey(id) === block.configuration.mcp_arguments_input)
  )));
}
...
export function workflowMcpServerName(tool: Pick<WorkflowMcpTool, "server_name">): string {
...
export async function listWorkflowMcpTools(
  ...
  const serverNames = fetch("/api/mcp/servers", { credentials: "same-origin" })
    .then(async response => {
      ...
      return new Map<string, string>((data.servers ?? []).map(
        (server: { server_id: string; name: string }) => [server.server_id, server.name],
      ));
  ...
  if (!response.ok)
    throw new Error("Tools could not be loaded. Check Tool Registry and retry.");
