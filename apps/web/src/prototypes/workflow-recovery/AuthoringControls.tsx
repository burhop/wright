...
export function AuthoringCreateRail({ onCreate, disabled, sessionId, onCreateServer }: { readonly onCreate: (templateId: string) => void; readonly disabled: boolean; sessionId?: string; onCreateServer?: (server: McpServerChoice) => void }) {
  const [group, setGroup] = useState<string | null>(null);
  const [tools, setTools] = useState<WorkflowMcpTool[]>([]);
  const [toolMessage, setToolMessage] = useState("");
  useEffect(() => { if (group !== "tool" || !sessionId) return; let current = true; setToolMessage("Loading available servers…"); void listWorkflowMcpTools(sessionId).then((value) => { if (current) { setTools(value); setToolMessage(value.length ? "" : "Enable a server in Tool Registry to add it here."); } }).catch(() => { if (current) setToolMessage("Could not load servers. Open Tool Registry to check the connection."); }); return () => { current = false; }; }, [group, sessionId]);
  ...
  return <aside ref={rootRef} className="recovery-create-rail" aria-label="Create workflow objects" data-testid="workflow-recovery-palette" onKeyDown={(event) => { if (event.key === "Escape") { const id = group; setGroup(null); rootRef.current?.querySelector<HTMLButtonElement>(`[data-group="${id}"]`)?.focus(); } }}>
    ...
    {AUTHORING_GROUPS.map((item) => <button type="button" key={item.id} data-group={item.id} className={`recovery-create-group ${group === item.id ? "is-selected" : ""}`} aria-label={item.label} aria-expanded={group === item.id} aria-controls="recovery-create-options" data-testid={`workflow-recovery-create-group-${item.id}`} disabled={disabled} title={item.description} onClick={() => setGroup((current) => current === item.id ? null : item.id)}><WorkflowObjectIcon kind={item.id} /><b aria-hidden="true">{item.label}</b></button>)}</div>
    {group && <section id="recovery-create-options" className="recovery-create-popover" aria-label={`${AUTHORING_GROUPS.find((item) => item.id === group)?.label} templates`}>
      ...
      <p>{group === "tool" ? "Choose a server, then describe what you want it to do." : "Add an object to this workflow. Nothing runs when you add it."}</p>
      {AUTHORING_TEMPLATES.filter((item) => item.group === group && !["mcp-tool", "mcp-task", "engineering-step"].includes(item.id)).map((item) => <button type="button" key={item.id} className="recovery-create-template" data-testid={`workflow-recovery-create-template-${item.id}`} onClick={() => { onCreate(item.id); setGroup(null); }}><b>{item.label}</b><span>{item.description}</span></button>)}
      {group === "tool" && <>
        {toolMessage && <p role="status">{toolMessage}</p>}
        {mcpServerChoices(tools).map(server => <button type="button" key={server.id} className="recovery-create-template" data-testid={`workflow-recovery-create-server-${server.id}`} onClick={() => { onCreateServer?.(server); setGroup(null); }}><b>{server.name}</b><span>AI task</span></button>)}
        <a href="/tool-registry" target="_blank" rel="noreferrer">Add or enable a server</a>
      </>}
    </section>}
  </aside>;
}
...
export function AuthoringOverview({ block, workflow, onSelect }: { readonly block: RecoveryBlock; readonly workflow: RecoveryWorkflow; readonly onSelect: (id: string) => void }) {
  ...
  return <section className="recovery-overview" data-testid={`workflow-recovery-overview-${block.id}`}>
    ...
    <details className="recovery-technical-details"><summary data-testid={`workflow-recovery-overview-contracts-${block.id}`}>Inputs and outputs</summary>
      {[...block.inputPortIds, ...block.outputPortIds].map((id) => { const port = findPort(workflow, id); return port ? <button type="button" key={id} data-testid={`workflow-recovery-overview-port-${id}`} onClick={() => onSelect(id)}>{port.direction === "input" ? "Input" : "Output"}: {port.name}</button> : null; })}
    </details>
  </section>;
}

export function AuthoringSettings({ block, workflow, readOnly, onApply, onDelete, direct = false }: { direct?: boolean; readonly block: RecoveryBlock; readonly workflow: RecoveryWorkflow; readonly readOnly: boolean; readonly onApply: (commands: RecoveryCommand[]) => boolean; readonly onDelete: () => void; readonly onListWorkspaceFiles?: () => Promise<AuthoringWorkspaceFile[]> }) {
  ...
  const isPromptFirst = !isInput && (block.executionKind === "ai_capable" || isReview);
  const promptRef = useRef<HTMLTextAreaElement>(null);
  useEffect(() => { (isPromptFirst ? promptRef.current : titleRef.current)?.focus(); }, [block.id, isPromptFirst]);
  ...
  const setConfig = (key: string, value: string | number | boolean) => {
    if (direct) {
      try { if (!readOnly) onApply(authoringConfigurationCommands(block, { [key]: value })); } catch (error) { setMessage(error instanceof Error ? error.message : "This parameter could not be updated."); }
  ...
  const stepNameField = <label>Step name<input ref={titleRef} data-testid={`workflow-recovery-block-title-${block.id}`} value={title} readOnly={readOnly} onChange={(event) => setTitle(event.target.value)} /></label>;
  const instructionsField = !isInput ? <label className={isPromptFirst ? "recovery-authoring-settings__prompt" : undefined}>{block.executionKind === "ai_capable" ? "AI prompt" : block.executionKind === "human" ? "Engineer checklist" : "Tool instructions"}<textarea ref={isPromptFirst ? promptRef : undefined} rows={isPromptFirst ? 8 : 5} data-testid={`workflow-recovery-block-instructions-${block.id}`} value={instructions} readOnly={readOnly} onChange={(event) => { if (direct) { if (!readOnly) onApply([{ kind: "set_block_definition", blockId: block.id, patch: { instructions: event.target.value } }]); } else setInstructions(event.target.value); }} /><small className="recovery-field-help">{isReview ? "Connect one document as the final step. These instructions appear with the saved files for your approval or change request." : "This prompt is saved in the workflow and is the instruction sent when this step runs."}</small></label> : null;
  return <section className="recovery-authoring-settings">
    {direct ? instructionsField : isPromptFirst ? instructionsField : !isWorkspaceFileInput && stepNameField}
    {isInput && <fieldset className="recovery-input-editor"><legend>Input content</legend>
      {isWorkspaceFileInput ? <><label>Workspace file path<input data-testid={`workflow-recovery-input-file-${block.id}`} value={String(configuration.workspace_file ?? "")} readOnly={readOnly} placeholder="requirements.pdf" onChange={(event) => setConfiguration((current) => ({ ...current, input_mode: "workspace-file", workspace_file: event.target.value.trimStart() }))} /></label><small>Use a path relative to this workspace. A filename with no folder, such as <code>requirements.pdf</code>, is in the workspace root.</small></> : <><label>Provided as<select data-testid={`workflow-recovery-input-mode-${block.id}`} disabled={readOnly} value={inputMode} onChange={(event) => setConfig("input_mode", event.target.value)}><option value="text">Typed text</option><option value="workspace-file">Workspace file</option></select></label>{inputMode === "text" ? <label>Text or design instructions<textarea rows={5} data-testid={`workflow-recovery-input-text-${block.id}`} value={String(configuration.input_text ?? "")} readOnly={readOnly} placeholder="Describe the design intent, requirements, or context…" onChange={(event) => setConfiguration((current) => ({ ...current, input_mode: "text", input_text: event.target.value }))} /></label> : null}</>}
    </fieldset>}
    {!direct && !isPromptFirst && instructionsField}
    {!direct && isPromptFirst && stepNameField}
    {reviewCriteria && <details open={direct || undefined} className="recovery-inspector__field"><summary data-testid={`workflow-recovery-block-review-toggle-${block.id}`}>Review criteria</summary><textarea aria-label="Engineer approval checklist" data-testid={`workflow-recovery-block-review-${block.id}`} readOnly value={reviewCriteria} /><small>These criteria come from this step&apos;s accept and revise paths; edit the corresponding connection to change them.</small></details>}
    {!isReview && <details className="recovery-technical-details"><summary data-testid={`workflow-recovery-settings-advanced-${block.id}`}>Parameters and advanced settings</summary>
    ...
    {readOnly ? <p data-testid="workflow-recovery-candidate-readonly">Editing is unavailable while a run, save, or suggestion review is in progress.</p> : <div className="recovery-inspector__actions">{!direct && <button type="button" className="recovery-button recovery-button--primary" data-testid="workflow-recovery-config-apply" onClick={save}>Apply settings</button>}<button type="button" className="recovery-button recovery-button--danger" data-testid="workflow-recovery-delete" onClick={onDelete}>Delete block…</button></div>}
  </section>;
}
