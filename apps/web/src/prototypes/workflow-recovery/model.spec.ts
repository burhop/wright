...
describe("recovery renderer projection", () => {
  ...
  it("derives approval gates and feedback paths from canonical relationships", () => {
    ...
    const specification = projection.phases
      .flatMap((phase) => phase.blocks)
      .find(
        (block) => block.semanticId === "block.create-design-specification",
      );
    const review = projection.phases
      .flatMap((phase) => phase.blocks)
      .find((block) => block.semanticId === "block.review-design");

    expect(specification?.gates).toEqual([
  ...
  it("resolves the golden review component to one stable diagnostic and run lineage scope", () => {
    const instance = initialWorkflow.blocks.find(
    ...
    expect(
      { code: "WFR-REVIEW-INPUT", componentScope: scope }.componentScope,
    ).toBe(scope);
    expect({ state: "queued", componentScope: scope }.componentScope).toBe(
      scope,
    );
  });
  ...
  it("does not invent a gate for an approval block without decision and feedback relationships", () => {
    ...
    const drawingReview = projection.phases
      .flatMap((phase) => phase.blocks)
      .find((block) => block.semanticId === "block.review-inspection-drawing");
  ...
  it("projects every valid feedback outcome from one approval without collapsing identities", () => {
    ...
    expect(
      projection.feedbackPaths.map((feedback) => feedback.semanticId),
    ).toEqual([
      "rel.specification-revise",
      "rel.review-revise",
      "rel.review-restart",
    ]);
    expect(projection.feedbackPaths[2]).toEqual(
      expect.objectContaining({
        from_gate_id: "gate.review-decision",
        to_block_id: "block.design-intent",
        label: "Restart from design intent",
  ...
  it("projects decision-origin feedback directly when no approval gate adapter exists", () => {
    ...
    workflow.phases
      .find((phase) => phase.id === "phase.verify")!
      .blockIds.push("block.route-rework");
    workflow.relationships.push({
    ...
    expect(projection.feedbackPaths).toContainEqual(
      expect.objectContaining({
        id: "rel.route-rework-to-design-intent",
  ...
  it("keeps canonical unphased component blocks visible through an explicit projection-only group", () => {
    ...
    const projection = toDraftProjection(workflow, layout);
    const components = projection.phases.find(
      (phase) => phase.id === "phase.unassigned-component",
    );
    expect(components?.blocks.map((block) => block.semanticId)).toEqual([
      "block.reusable-fixture",
    ]);
    const internalAddress = workflow.components[0]!.internalAddresses.find(
      (address) => address.conceptKind === "relationship",
    )!;
    const scopedAddress = resolveRecoveryComponentScope(
      workflow,
      "block.reusable-fixture",
      internalAddress.semanticId,
    );
  ...
  it("replaces renderer implementations without changing projection facts or emitted host intents", () => {
    ...
    const fake =
      (index: number): DraftCanvasRenderer =>
      ({ projection: value, onIntent }) => {
        observed[index] = value.phases.flatMap((phase) =>
          phase.blocks.map((block) => block.semanticId),
        );
        const intent: DraftCanvasIntent = {
          type: "select",
    ...
    fake(0)({
      ...
      onIntent: (intent) => hostIntents[0]!.push(intent),
    });
    fake(1)({
      projection,
      selectedSemanticId: null,
      onIntent: (intent) => hostIntents[1]!.push(intent),
    });
  ...
  it("rejects an unknown run version without changing the immutable record", () => {
    ...
    expect(validateRecoveryRunProjection(unknown)[0]?.code).toBe(
      "WFR-RUN-VERSION-UNSUPPORTED",
    );
