...
export interface CanonicalWorkflowWire {
  ...
  relationships: {
    id: string;
    kind: "data" | "control" | "decision" | "feedback";
    source_id: string;
    target_id: string;
    label: string;
  ...
  artifact_contracts: {
    ...
    allowed_actions: (
      "inspect" | "preview" | "open" | "download" | "replace"
    )[];
  }[];
  bindings: {
    id: string;
...
export function canonicalDefinitionBytes(
  workflow: RecoveryWorkflow,
  excludeAuthority = false,
): string {
  ...
  wire.artifact_contracts.sort((left, right) =>
...
export function toCanonicalWire(
  ...
  return {
    ...
    ports: workflow.ports.map((port) => ({
      id: port.id,
      owner_block_id: port.ownerBlockId,
      direction: port.direction,
      name: port.name,
      type_id: port.typeId,
      required: port.required,
      cardinality: port.cardinality,
      artifact_contract_id: port.artifactContractId,
      description: port.description,
    })),
    relationships: workflow.relationships.map((relationship) => ({
      id: relationship.id,
    ...
    artifact_contracts: workflow.artifactContracts.map((artifact) => ({
      id: artifact.id,
      name: artifact.name,
      type_id: artifact.typeId,
      media_type: artifact.mediaType,
      description: artifact.description,
      producer_block_id: artifact.producerBlockId,
      required_for_block_ids: [...artifact.requiredForBlockIds],
      preview_policy: artifact.previewPolicy,
      allowed_actions: [...artifact.allowedActions],
    })),
    ...
    components: workflow.components.map((component) => ({
      ...
      internal_addresses: component.internalAddresses.map((address) => ({
...
export function fromCanonicalWire(
  wire: CanonicalWorkflowWire,
): RecoveryWorkflow {
  return {
    ...
    metadata: {
      ...
      purpose: wire.metadata.purpose,
      engineeringDomain: wire.metadata.engineering_domain,
      authorship: wire.metadata.authorship,
    },
    ...
    blocks: wire.blocks.map((block) => ({
      ...
      componentRef:
        block.component_ref === null
          ? null
          : {
              componentId: block.component_ref.component_id,
              versionRange: block.component_ref.version_range,
            },
    })),
    ports: wire.ports.map((port) => ({
      id: port.id,
      ownerBlockId: port.owner_block_id,
      direction: port.direction,
      name: port.name,
      typeId: port.type_id,
      required: port.required,
      cardinality: port.cardinality,
      artifactContractId: port.artifact_contract_id,
      description: port.description,
    })),
    ...
    artifactContracts: wire.artifact_contracts.map((artifact) => ({
      id: artifact.id,
      name: artifact.name,
      typeId: artifact.type_id,
      mediaType: artifact.media_type,
      description: artifact.description,
      producerBlockId: artifact.producer_block_id,
      requiredForBlockIds: [...artifact.required_for_block_ids],
      previewPolicy: artifact.preview_policy,
      allowedActions: [...artifact.allowed_actions],
    })),
    bindings: wire.bindings.map((binding) => ({
      ...
      argumentMap: binding.argument_map.map((item) => ({
        semanticSource: item.semantic_source,
        implementationTarget: item.implementation_target,
      })),
      resultMap: binding.result_map.map((item) => ({
        semanticSource: item.semantic_source,
        implementationTarget: item.implementation_target,
      })),
    ...
    components: wire.components.map((component) => ({
      ...
      internalAddresses: component.internal_addresses.map((address) => ({
        semanticId: address.semantic_id,
        conceptKind: address.concept_kind,
