...
vi.mock("../../services/workspace-service", () => ({
  workspaceService: {
    getWorkspaceWorkflowRuns: vi.fn(),
    runWorkspaceWorkflowSource: vi.fn(),
  },
}));
...
const row = (
  ...
  results: [],
  execution: {
    active_task_id: status === "running" ? "design" : null,
    completed_task_ids: [],
    event_count: 1,
    model_call_count: 1,
    tool_call_count: 0,
...
describe("scoped run observer", () => {
  ...
  it("preserves last known status on fetch failure and refreshes on focus", async () => {
    const { result } = renderHook(() =>
      useWorkflowRunObserver("s", "w", "path", "0"),
    );
    await flush();
    ...
    get.mockResolvedValue([row("failed")]);
    await act(async () => {
      window.dispatchEvent(new Event("focus"));
    });
    expect(result.current.record?.status).toBe("failed");
  ...
  it("ignores stale in-flight replies after changing workspace and path", async () => {
    ...
    const { result, rerender } = renderHook(
      ({ w, p }) => useWorkflowRunObserver("s", w, p, "0"),
      { initialProps: { w: "old", p: "old-path" } },
    );
    const signal = get.mock.calls[0]![2]!.signal!;
    get.mockResolvedValue([]);
    rerender({ w: "new", p: "new-path" });
    await flush();
    await act(async () => {
      finish([row()]);
    });
    ...
    expect(get).toHaveBeenLastCalledWith(
      "s",
      "new-path",
      expect.objectContaining({ workspaceId: "new", latestOnly: true }),
    );
  });
  it("stops hidden polling and refreshes when shown or reopened", async () => {
    ...
    const { rerender } = renderHook(
      ({ key }) => useWorkflowRunObserver("s", "w", "path", key),
      { initialProps: { key: "0" } },
    );
    await flush();
  ...
  it("never takes precedence over a pending POST or an older owned identity", () => {
    ...
    expect(
      preferObservedRun(
        { ...row(), started_at: "2026-09-08T12:00:00Z" },
        { ...owned, pending: false },
      ),
    ).toBe(true);
    expect(
      preferObservedRun(
        { ...row(), path: owned.logPath },
        { ...owned, pending: false, hasResult: true },
      ),
    ).toBe(false);
    expect(
      preferObservedRun(
        { ...row(), path: owned.logPath },
        { ...owned, pending: false, hasResult: false },
      ),
    ).toBe(true);
  });
});
