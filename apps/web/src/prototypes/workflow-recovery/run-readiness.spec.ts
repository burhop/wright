...
function graph(count: number, edges: [number, number][] = []) {
  ...
  workflow.blocks = Array.from({ length: count }, (_, i) => ({
    ...workflow.blocks[0]!,
    id: `b${i}`,
    title: `Task ${i}`,
    inputPortIds: [`in${i}`],
  ...
  workflow.relationships = edges.map(([a, b], i) => ({
    id: `e${i}`,
    kind: "data",
    sourceId: `out${a}`,
    targetId: `in${b}`,
...
describe("single-process run readiness", () => {
  ...
  it("identifies two independent chains and an isolated block, even in one visual group", () => {
    const issue = disconnectedProcessDiagnostic(
      graph(5, [
        [0, 1],
        [2, 3],
      ]),
  ...
  it.each(["control", "decision", "feedback"] as const)(
    "recognizes %s paths between tasks",
    (kind) => {
