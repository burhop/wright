...
import type {
  ...
  DraftPortProjection,
} from "../../components/workflow-composer/draft-projection";
...
function blockIcon(block: DraftBlockProjection): string {
  const types = [...block.inputs, ...block.outputs]
    .map((port) => port.value_type_id)
    .join(" ");
  if (block.role === "input")
    return types.includes("image")
      ? "image"
      : types.includes("context")
        ? "context"
        : types.includes("file.workspace")
          ? "file"
          : "text";
  if (block.role === "release") return "output";
  if (block.role === "review")
    return types.includes("geometry") ? "review" : "llm-document";
  if (types.includes("drawing")) return "drawing";
  if (types.includes("toolpath")) return "fdm";
  if (block.outputs.some((port) => port.value_type_id.includes("report")))
    return "3d-check";
  if (block.outputs.some((port) => port.value_type_id.includes("geometry")))
    return "model";
...
function componentAddressLabel(
  semanticId: string,
  conceptKind: string,
): string {
  if (semanticId.endsWith(".block.evaluate"))
    return "Evaluate the design review";
  if (semanticId.endsWith(".relationship.accept"))
    return "Accept the reviewed design";
  if (semanticId.endsWith(".artifact.approved"))
    return "Approved design record";
...
function RelationshipRoutingHandles({ blockId }: { readonly blockId: string }) {
  return (
    <>
      {(["flow", "feedback"] as const).flatMap((kind) =>
        (["source", "target"] as const).map((direction) => {
...
function PortRow({
  ...
  return (
    <div
      className={`recovery-port recovery-port--${side}`}
      data-semantic-id={port.semanticId}
      title={port.name}
    >
      {side === "input" && handle}
      <span className="recovery-port__tooltip" aria-hidden="true">
        {port.name}
      </span>
      {side === "output" && handle}
    </div>
...
const RecoveryBlockNode = memo(function RecoveryBlockNode({
  ...
  const portIdentity = [...block.inputs, ...block.outputs]
    .map((port) => port.semanticId)
    .join("|");
  useEffect(() => {
    updateInternals(block.semanticId);
  }, [block.semanticId, portIdentity, updateInternals]);
  ...
  return (
    <article
      ...
      aria-description={
        data.componentState === null
          ? undefined
          : "Use Right Arrow to show grouped review details and Left Arrow to hide them."
      }
      tabIndex={0}
    ...
    >
      ...
      {data.active && (
        <div className="recovery-block__active">▶ ACTIVE TASK</div>
      )}
      {data.proposed && (
        <div className="recovery-block__proposal">
          AI SUGGESTION · REVIEW BEFORE ADDING
        </div>
      )}
      <header className="recovery-block__heading">
        <span className="recovery-block__icon">
          <WorkflowObjectIcon kind={data.iconKind} />
        </span>
        <div className="recovery-block__identity">
          <span className="recovery-block__kind">{roleLabel[block.role]}</span>
          {renaming && !data.readOnly ? (
            <input
              className="recovery-block__title-editor nodrag nopan"
              data-testid={`workflow-recovery-rename-${block.semanticId}`}
            ...
            <h3
              title={data.readOnly ? undefined : "Double-click to rename · F2"}
              onDoubleClick={(event) => {
                event.stopPropagation();
                startRename();
              }}
        ...
        <span
          className={`recovery-state recovery-state--${data.runState}`}
          aria-label={`Run state ${data.runState}`}
        >
          {stateGlyph[data.runState]}
          {data.runState === "idle"
            ? ""
            : ` ${data.runState.replace("-", " ")}`}
        </span>
      </header>
      <div
        className="recovery-block__io-summary"
        aria-label={`${block.inputs.length} inputs and ${block.outputs.length} outputs`}
      >
      ...
      {data.componentState !== null && (
        <section
          ...
          aria-label={`${data.componentState.component.title} grouped review details`}
        >
          <header>
          ...
          {data.componentState.targetedInternalSemanticIds.length > 0 && (
            <div className="recovery-component__targets" role="note">
              <b>
                {data.componentState.targetedInternalSemanticIds.length} issue
                {data.componentState.targetedInternalSemanticIds.length === 1
                  ? ""
                  : "s"}
              </b>
              {!data.componentState.collapsed &&
                data.componentState.targetedInternalSemanticIds.map((id) => {
                  const address =
                    data.componentState?.component.internalAddresses.find(
                      (item) => item.semanticId === id,
                    );
                  return (
                    <span key={id}>
                      {componentAddressLabel(
                        id,
                        address?.conceptKind ?? "review item",
                      )}
                    </span>
                  );
                })}
          ...
          {!data.componentState.collapsed && (
            <div className="recovery-component__details">
              <small>
                {data.componentState.internalAddressCount} technical review
                items
              </small>
              <ul
                data-testid={`workflow-recovery-component-addresses-${block.semanticId}`}
              >
                {data.componentState.component.internalAddresses.map(
                  (address) => (
                    <li
                      data-semantic-id={address.semanticId}
                      key={address.semanticId}
                    >
                      <span>
                        {componentAddressLabel(
                          address.semanticId,
      ...
      <div className="recovery-block__ports recovery-block__ports--inputs">
        {block.inputs.map((port) => (
          <PortRow
            ...
            port={port}
            side="input"
            keyboardSource={data.keyboardSource}
            onPortKey={data.onPortKey}
          />
        ))}
      </div>
      <div className="recovery-block__ports recovery-block__ports--outputs">
        {block.outputs.map((port, index) => (
          <div key={port.semanticId}>
            {data.outputGroups?.[port.semanticId] &&
              (index === 0 ||
                data.outputGroups[port.semanticId] !==
                  data.outputGroups[block.outputs[index - 1].semanticId]) && (
                <small className="recovery-output-group">
                  {data.outputGroups[port.semanticId]}
                </small>
              )}
            <PortRow
      ...
      {block.gates.length > 0 && (
        <div className="recovery-block__gate">◇ Approval required</div>
      )}
    </article>
  );
});
...
function RecoveryEdge({
  ...
  const [path, labelX, labelY] = getBezierPath({
    ...
    sourcePosition,
    targetPosition,
    curvature: data?.kind === "feedback" ? 0.5 : 0.24,
  });
  const active = Boolean(data?.active);
  const stableLabelLane =
    ([...id].reduce((sum, character) => sum + character.charCodeAt(0), 0) % 5) -
    2;
  const labelOffsetY =
    (data?.kind === "decision" ? -20 : data?.kind === "data" ? 10 : 0) +
    stableLabelLane * 14;
  return (
    <g
...
function RecoveryCanvasControls() {
  ...
  return (
    <Controls
      ...
      data-testid="workflow-recovery-canvas-controls"
    >
      <ControlButton
        data-testid="workflow-recovery-canvas-zoom-in"
        aria-label="Zoom in"
        title="Zoom in"
        onClick={() => void zoomIn()}
      >
      ...
      <ControlButton
        data-testid="workflow-recovery-canvas-zoom-out"
        aria-label="Zoom out"
        title="Zoom out"
      ...
      <ControlButton
        data-testid="workflow-recovery-canvas-fit"
...
function RecoveryCanvasViewport({
  selectedNodeId,
}: {
  readonly selectedNodeId: string | null;
}) {
  const { getNodes, getNode, getViewport, setViewport, fitView } =
    useReactFlow();
  ...
  useEffect(() => {
    ...
    const right = Math.max(
      ...nodes.map((node) => node.position.x + (node.measured?.width ?? 220)),
    );
    const bottom = Math.max(
      ...nodes.map((node) => node.position.y + (node.measured?.height ?? 110)),
    );
    if (
      (right - left) * 0.9 < width - 80 &&
      (bottom - top) * 0.9 < height - 120
    ) {
      void fitView({ padding: 0.12, minZoom: 0.9, maxZoom: 1 });
    } else {
      ...
      void setViewport({
        x: 40 - left * 0.9,
        y: Math.max(90, (height - (bottom - top) * 0.9) / 2) - top * 0.9,
        zoom: 0.9,
      });
    }
  ...
  useEffect(() => {
    ...
    const dx =
      left < 30 ? 30 - left : right > width - 30 ? width - 30 - right : 0;
    const dy =
      top < 70 ? 70 - top : bottom > height - 45 ? height - 45 - bottom : 0;
    if (dx || dy)
      void setViewport(
...
function RecoveryCanvasNavigator({
  ...
  const focus = () => {
    ...
    if (block === null) {
      setStatus(
        `No workflow step matches ${query.trim() || "the empty query"}.`,
      );
      return;
    }
  ...
  return (
    <>
      {showSearch && (
        <form
          className="recovery-canvas__find nodrag nopan"
          onSubmit={(event) => {
            event.preventDefault();
            focus();
          }}
        >
          ...
          />
          <button data-testid="workflow-recovery-find-submit" type="submit">
            Show
          </button>
          {status !== "" && (
...
export const ReactFlowRecoveryCanvas: DraftCanvasRenderer = ({
  projection,
  selectedSemanticId,
  onIntent,
}) => {
  const runtime = useContext(RuntimeContext);
  const runIssue = validateRecoveryRunProjection(
    runtime.run,
    runtime.runSubject,
  )[0];
  ...
  const [expandedComponentIds, setExpandedComponentIds] = useState<
    ReadonlySet<string>
  >(() => new Set());
  const detailLevel = graphDetailLevel(blocks.length);
  const componentTargets = useMemo(
    () =>
      Object.values(runtime.run.steps).flatMap((step) =>
        step.componentScope === undefined ||
        !["failed", "blocked", "needs-input"].includes(step.state)
          ? []
          : [step.componentScope],
      ),
    [runtime.run.steps],
        ...
        blocks
          .filter(
            (block) =>
              ...
              !expandedComponentIds.has(block.semanticId),
          )
    ...
    () =>
      new Map(
        projectComponentStates(
          ...
          componentTargets,
        ).map((state) => [state.instanceSemanticId, state]),
      ),
  ...
  const gateOwners = useMemo(
    () =>
      new Map(
        blocks.flatMap((block) =>
          block.gates.map(
            (gate) => [gate.semanticId, block.semanticId] as const,
          ),
        ),
      ),
    [blocks],
  ...
  const focusedPath = useMemo(() => {
    const relations = [
      ...
      ...projection.feedbackPaths.map((path) => ({
        id: path.semanticId,
        source: gateOwners.get(path.from_gate_id) ?? path.from_gate_id,
        target: path.to_block_id,
      })),
    ...
    const selectedRelation = relations.find(
      (item) => item.id === selectedSemanticId,
    );
    const seeds = selectedRelation
    ...
    return {
      ...
      edges: new Set(
        relations
          .filter(
            (item) => visited.has(item.source) && visited.has(item.target),
          )
          .map((item) => item.id),
      ),
    };
    ...
    projection.feedbackPaths,
    runtime.overlayRelationships,
    selectedSemanticId,
  ]);
  ...
  const portLookup = useMemo(
    () =>
      new Map(
        blocks
          .flatMap((block) => [...block.inputs, ...block.outputs])
          .map((port) => [port.semanticId, port]),
      ),
    [blocks],
  );
  const canConnect = useCallback(
    (sourceId: string, targetId: string) => {
      ...
      return canConnectAuthoringPorts(
        source && {
          ...source,
          typeId: source.value_type_id,
          ownerBlockId: source.owner_block_id,
        },
        target && {
          ...target,
          typeId: target.value_type_id,
          ownerBlockId: target.owner_block_id,
        },
      );
    },
  ...
  const blockIds = useMemo(
    () => new Set(blocks.map((block) => block.semanticId)),
    [blocks],
  );
  const requireBlockEndpoint = (
    semanticId: string,
    relationshipId: string,
    endpoint: "source" | "target",
  ) => {
    if (!blockIds.has(semanticId))
      throw new Error(
        `RECOVERY_EDGE_${endpoint.toUpperCase()}_BLOCK_MISSING:${relationshipId}:${semanticId}`,
      );
    return semanticId;
  };
  const onPortKey = useCallback(
    (port: DraftPortProjection) => {
      if (runtime.readOnly) return;
      if (port.direction === "output") {
        setConnectionMessage("");
        setKeyboardSource((current) =>
          current === port.semanticId ? null : port.semanticId,
        );
        return;
      }
      if (keyboardSource !== null) {
        if (!canConnect(keyboardSource, port.semanticId)) {
          setConnectionMessage(
            runtime.connectionIssue?.(keyboardSource, port.semanticId) ||
              (portLookup
                .get(keyboardSource)
                ?.value_type_id.startsWith("type.image.")
                ? "Connect the image to an input that accepts images."
                : "Choose an input that accepts this output type."),
          );
          return;
        }
        onIntent({
          type: "create-connection",
          sourcePortId: keyboardSource,
          targetPortId: port.semanticId,
        });
        setConnectionMessage("");
  ...
  const projectedNodes = useMemo<RecoveryFlowNode[]>(
    () =>
      blocks.map((block) => ({
        id: block.semanticId,
        type: "recovery",
        ...
        data: {
          block: {
            ...
            inputs: block.inputs.filter(
              (port) => !runtime.hiddenPorts?.has(port.semanticId),
            ),
            outputs: block.outputs
              .filter((port) => !runtime.hiddenPorts?.has(port.semanticId))
              .map((port) => ({
                ...port,
                name: runtime.outputLabels?.[port.semanticId] ?? port.name,
              })),
          },
          ...
          onDelete: () =>
            onIntent({ type: "delete-concept", semanticId: block.semanticId }),
  ...
  const [nodes, setNodes, applyPreviewNodeChanges] =
    useNodesState<RecoveryFlowNode>(projectedNodes);
  ...
  const previewNodeChanges = useCallback(
    (changes: NodeChange<RecoveryFlowNode>[]) => {
  ...
  const edges: RecoveryFlowEdge[] = [
    ...projection.connections.map((connection) => ({
      ...
      label:
        runtime.relationshipLabels[connection.semanticId] ??
      ...
      data: {
        ...
        kind: "data" as const,
        active: runtime.run.activeRelationshipId === connection.semanticId,
        onSelect: (semanticId: string) =>
          onIntent({ type: "select", semanticId }),
      },
    })),
    ...projection.feedbackPaths.map((feedback) => {
      const source = requireBlockEndpoint(
        gateOwners.get(feedback.from_gate_id) ?? feedback.from_gate_id,
        feedback.semanticId,
        "source",
      );
      const target = requireBlockEndpoint(
        feedback.to_block_id,
        feedback.semanticId,
        "target",
      );
      return {
        ...
        data: {
          ...
          active: runtime.run.activeRelationshipId === feedback.semanticId,
          onSelect: (semanticId: string) =>
            onIntent({ type: "select", semanticId }),
        },
      };
    ...
    ...runtime.overlayRelationships.map((relationship) => {
      ...
      return {
        ...
        data: {
          ...
          kind: relationship.kind as "control" | "decision",
          active: runtime.run.activeRelationshipId === relationship.id,
          onSelect: (semanticId: string) =>
            onIntent({ type: "select", semanticId }),
        },
      };
    ...
    data: {
      ...edge.data,
      traced: focusPath && focusedPath.edges.has(edge.id),
      muted: focusPath && !focusedPath.edges.has(edge.id),
    },
  }));
  ...
  const connect = (connection: Connection) => {
    ...
    if (!source || !target || !canConnect(source.semanticId, target.semanticId))
      return;
    onIntent({
      type: "create-connection",
      sourcePortId: source.semanticId,
      targetPortId: target.semanticId,
    });
  };

  return (
    <div
      className={`recovery-canvas recovery-canvas--${runtime.portTreatment}`}
      data-testid="workflow-recovery-canvas"
      data-port-treatment={runtime.portTreatment}
      data-detail-level={detailLevel}
      data-focus-path={focusPath}
      aria-label={`${projection.title} diagram`}
    >
      {(keyboardSource || connectionMessage) && (
        <div className="recovery-keyboard-connection" role="status">
          {connectionMessage ||
            "Connection started. Focus a compatible input and press Enter; Escape cancels."}
        </div>
      )}
      <ReactFlow
        ...
        defaultViewport={{ x: 40, y: 90, zoom: 0.9 }}
        ...
        onNodeClick={(_, node) =>
          onIntent({ type: "select", semanticId: node.id })
        }
        onEdgeClick={(_, edge) =>
          onIntent({ type: "select", semanticId: edge.id })
        }
        ...
        onEdgesDelete={(deleted) =>
          deleted.forEach((edge) =>
            onIntent({ type: "delete-connection", semanticId: edge.id }),
          )
        }
        onNodesDelete={(deleted) =>
          deleted.forEach((node) =>
            onIntent({ type: "delete-concept", semanticId: node.id }),
          )
        }
        onConnect={connect}
        onConnectStart={() => setConnectionMessage("")}
        onConnectEnd={(_, state) => {
          if (state.isValid || !state.fromHandle?.id || !state.toHandle?.id)
            return;
          const source =
          ...
          const target =
            state.fromHandle.type === "target"
              ...
              : state.toHandle.id;
          setConnectionMessage(
            runtime.connectionIssue?.(source, target) ||
              "Choose an input that accepts this output type.",
          );
        }}
        isValidConnection={(connection) =>
          Boolean(
            ...
            canConnect(connection.sourceHandle, connection.targetHandle),
          )
        }
        onKeyDown={(event) => {
          if (event.key === "Escape") {
          ...
          if (event.key === "ArrowRight" || event.key === "ArrowLeft") {
            ...
            const componentState =
              semanticId === undefined
                ? undefined
                : componentStates.get(semanticId);
            if (node && target === node && componentState !== undefined) {
              const shouldExpand =
                event.key === "ArrowRight" && componentState.collapsed;
              const shouldCollapse =
                event.key === "ArrowLeft" && !componentState.collapsed;
      ...
      >
        ...
        <button
          type="button"
          className={`recovery-minimap-toggle nodrag nopan${minimapVisible ? " is-open" : ""}`}
          data-testid="workflow-recovery-minimap-toggle"
          aria-label={minimapVisible ? "Hide minimap" : "Show minimap"}
          aria-expanded={minimapVisible}
          onClick={() => setMinimapVisible((visible) => !visible)}
        >
          {minimapVisible ? "⌄" : "Overview map"}
        </button>
        <div className="recovery-minimap-key nodrag nopan" aria-hidden="true">
          Blue frame = current view
        </div>
        ...
        <RecoveryCanvasViewport
          selectedNodeId={
            (selectedSemanticId &&
              (blockIds.has(selectedSemanticId)
                ? selectedSemanticId
                : (portLookup.get(selectedSemanticId)?.owner_block_id ??
                  edges.find((edge) => edge.id === selectedSemanticId)
                    ?.source))) ||
            null
          }
        />
