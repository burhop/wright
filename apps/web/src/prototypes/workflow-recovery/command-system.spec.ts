...
import goldenAuthoringSourceRaw from "../../../../../specs/080-canonical-workflow-recovery/fixtures/mounting-bracket.workflow.wflow?raw";
import goldenInternalDslRaw from "../../../../../specs/080-canonical-workflow-recovery/fixtures/mounting-bracket.workflow.internal-ir.wflow?raw";

const goldenAuthoringSource = goldenAuthoringSourceRaw.replace(/\r\n?/g, "\n");
const goldenInternalDsl = goldenInternalDslRaw.replace(/\r\n?/g, "\n");
...
describe("recovery command and source conformance", () => {
  it("is a lossless ergonomic view of the committed canonical wire document", () => {
    ...
    expect(
      fromCanonicalWire(goldenWire as unknown as CanonicalWorkflowWire),
    ).toEqual(initialWorkflow);
    expect(
      toCanonicalWire(
        fromCanonicalWire(goldenWire as unknown as CanonicalWorkflowWire),
      ),
  ...
  it("keeps the exact full-IR projection as an internal conformance treatment", () => {
    ...
    expect(
      toCanonicalWire(parseRecoveryDsl(goldenInternalDsl).workflow!),
    ).toEqual(goldenWire);
  });
  ...
  it("applies a semantic batch atomically and advances one revision", () => {
    const result = applyRecoveryBatch(
      cloneWorkflow(initialWorkflow),
      cloneLayout(initialLayout),
      recoveryCommandBatch(initialWorkflow.revision, "form", [
        {
    ...
    expect(
      accepted?.workflow.blocks.find(
        (block) => block.id === "block.generate-geometry",
      )?.configuration.thickness_mm,
    ).toBe(8);
  });
  ...
  it("keeps layout moves outside the semantic revision", () => {
    const result = applyRecoveryBatch(
      initialWorkflow,
      initialLayout,
      recoveryCommandBatch(initialWorkflow.revision, "graph", [
    ...
    expect(accepted?.layout.positions["block.export-step"]).toEqual({
      x: 900,
      y: 120,
    });
  });
    ...
    const result = applyRecoveryBatch(
      ...
      recoveryCommandBatch(initialWorkflow.revision, "graph", [
        { kind: "move_block", blockId: "block.export-step", x: 900, y: 120 },
        {
          ...
          blockId: "block.export-step",
          title: "Export reviewed STEP",
        },
      ]),
    );
  ...
  it("rejects a valid canonical candidate when the public source cannot reconstruct it", () => {
    ...
    snapshot.artifactContracts.find(
      (item) => item.id === "artifact.design-specification",
    )!.mediaType = "application/octet-stream";
    const result = applyRecoveryBatch(
      initialWorkflow,
      initialLayout,
      recoveryCommandBatch(initialWorkflow.revision, "history", [
        {
    ...
    expect(
      initialWorkflow.artifactContracts.find(
        (item) => item.id === "artifact.design-specification",
      )?.mediaType,
    ).toBe("text/markdown");
  });
  ...
  it("fails a stale or invalid batch without a partial mutation", () => {
    const stale = applyRecoveryBatch(
      initialWorkflow,
      initialLayout,
      recoveryCommandBatch(initialWorkflow.revision - 1, "ai_proposal", [
        {
          kind: "set_block_title",
          blockId: "block.generate-geometry",
          title: "Changed",
    ...
    expect(
      initialWorkflow.blocks.find(
        (block) => block.id === "block.generate-geometry",
      )?.title,
    ).toBe("Create bracket CAD model");
      ...
      recoveryCommandBatch(initialWorkflow.revision, "graph", [
        {
          ...
          relationship: {
            id: "rel.invalid",
            kind: "data",
            sourceId: "port.design-specification-check-in",
            targetId: "port.step-out",
            label: "invalid",
            condition: null,
          },
        },
    ...
    expect(
      initialWorkflow.relationships.some((item) => item.id === "rel.invalid"),
    ).toBe(false);
  });

  it.each([
    [
      "rel.duplicate-endpoints",
      "data",
      "port.design-intent-out",
      "port.design-intent-in",
      "WFR-RELATIONSHIP-DUPLICATE",
    ],
    [
      "rel.invalid-feedback-source",
      "feedback",
    ...
    "rejects invalid relationship topology %s",
    (id, kind, sourceId, targetId, code) => {
      const result = applyRecoveryBatch(
        initialWorkflow,
        initialLayout,
        recoveryCommandBatch(initialWorkflow.revision, "graph", [
  ...
  it("translates valid text edits into the same command protocol", () => {
    const editedSource = formatRecoveryAuthoringSource(
      initialWorkflow,
    ).text.replace(
      "Create bracket CAD model",
      "Create production bracket CAD model",
    );
    ...
    expect(commands).toEqual([
      {
        kind: "set_block_title",
        blockId: "block.generate-geometry",
        title: "Create production bracket CAD model",
      },
    ]);
  });
  ...
  it("previews an AI proposal against a base revision without mutating accepted state", () => {
    ...
    expect(result.ok).toBe(true);
    expect(
      result.diff.some((line) =>
        line.startsWith(
          "Add block · Create manufacturing drawing (block.create-inspection-drawing) ·",
        ),
      ),
    ).toBe(true);
    expect(
      result.diff.some((line) =>
        line.startsWith(
          "Add port · Manufacturing drawing (port.drawing-out) ·",
        ),
      ),
    ).toBe(true);
    expect(
      result.diff.some((line) =>
        line.startsWith(
          "Connect · manufacturing drawing (rel.drawing-to-review) ·",
        ),
      ),
    ).toBe(true);
  ...
  it("reports every fact changed by one reviewed batch", () => {
    const result = applyRecoveryBatch(
      initialWorkflow,
      initialLayout,
      recoveryCommandBatch(initialWorkflow.revision, "ai_proposal", [
        {
          kind: "set_block_title",
          blockId: "block.generate-geometry",
          title: "Generate production geometry",
        },
        {
          kind: "set_block_configuration",
          blockId: "block.generate-geometry",
  ...
  it("rejects unknown command and layout versions without rewriting either input", () => {
    const command = recoveryCommandBatch(initialWorkflow.revision, "form", [
      {
        kind: "set_block_title",
        blockId: "block.generate-geometry",
        title: "Changed",
    ...
    } as unknown as RecoveryCommandBatch;
    const commandBefore = structuredClone(unknownCommand);
    const commandResult = applyRecoveryBatch(
      initialWorkflow,
      initialLayout,
      unknownCommand,
    );
    expect(commandResult.ok).toBe(false);
    expect(commandResult.diagnostics[0]?.code).toBe(
  ...
  it("routes undo and redo snapshots through one atomic validated history batch", () => {
      ...
      recoveryCommandBatch(initialWorkflow.revision, "form", [
        {
          kind: "set_block_title",
          blockId: "block.generate-geometry",
          title: "Changed geometry",
        },
      ]),
    );
    const changed = acceptRecoveryResult(
      initialWorkflow,
      initialLayout,
      changedResult,
    )!;
    const historyResult = applyRecoveryBatch(
      changed.workflow,
      changed.layout,
      recoveryCommandBatch(changed.workflow.revision, "history", [
        {
          kind: "restore_snapshot",
          direction: "undo",
    ...
    const restored = acceptRecoveryResult(
      changed.workflow,
      changed.layout,
      historyResult,
    )!;
    ...
    expect(invalid.ok).toBe(false);
    expect(
      changed.workflow.blocks.find(
        (block) => block.id === "block.generate-geometry",
      )?.title,
    ).toBe("Changed geometry");
  });
  ...
  it("does not regress semantic identity when history restores layout only", () => {
    ...
    const result = applyRecoveryBatch(
      currentWorkflow,
      currentLayout,
      recoveryCommandBatch(11, "history", [
        {
          kind: "restore_snapshot",
          direction: "undo",
    ...
    expect(restored.layout.positions["block.generate-geometry"]).toEqual(
      initialLayout.positions["block.generate-geometry"],
    );
  });
        ...
        recoveryCommandBatch(initialWorkflow.revision, "form", [
          {
            kind: "set_block_title",
            blockId: "block.generate-geometry",
            title: `Geometry treatment ${index}`,
          },
        ]),
      );
      expect(result.ok).toBe(true);
