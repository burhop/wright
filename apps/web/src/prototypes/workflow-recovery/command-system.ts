...
export type RecoveryCommand =
  ...
  | {
      kind: "set_workflow_metadata";
      patch: Partial<
        Pick<
          RecoveryWorkflow["metadata"],
          "title" | "purpose" | "engineeringDomain"
        >
      >;
    }
  ...
  | {
      ...
      blockId: string;
      patch: Partial<Pick<RecoveryBlock, "purpose" | "instructions">>;
    }
  | {
  ...
  | {
      ...
      patch: Partial<
        Pick<
          ...
          "kind" | "sourceId" | "targetId" | "label" | "condition"
        >
      >;
    }
  | {
...
function failure(
  ...
  return {
    ...
    workflow: null,
    layout: null,
    diagnostics: [{ code, explanation, correction, semanticId, line: null }],
    diff: [],
...
function applyCommand(
  ...
  if (command.kind === "add_port") {
    ...
    if (workflow.ports.some((port) => port.id === command.port.id))
      throw new Error("Connection point already exists");
    workflow.ports.push(structuredClone(command.port));
    (command.port.direction === "input"
      ? owner.inputPortIds
      : owner.outputPortIds
    ).push(command.port.id);
    return;
  }
  ...
  if (command.kind === "set_workflow_metadata") {
    if (
      ...
      Object.values(command.patch).some(
    ...
    Object.assign(
      workflow.metadata,
      Object.fromEntries(
        Object.entries(command.patch).map(([key, value]) => [
          key,
          value!.trim(),
        ]),
      ),
  ...
  if (command.kind === "set_phase_name") {
    ...
    if (!phase || !command.name.trim())
      throw new Error(`WFR-COMMAND-TARGET-MISSING:${command.phaseId}`);
    phase.name = command.name.trim();
    return;
  }
  if (command.kind === "set_block_title") {
    if (!command.title.trim())
      throw new Error(`WFR-COMMAND-TITLE-INVALID:${command.blockId}`);
  ...
  if (command.kind === "set_block_definition") {
    if (
      ...
      Object.entries(command.patch).some(
        ([key, value]) =>
          typeof value !== "string" ||
          (key !== "instructions" && value.trim() === ""),
      )
    ) {
      throw new Error(`WFR-COMMAND-DEFINITION-INVALID:${command.blockId}`);
    }
    Object.assign(
      block(workflow, command.blockId),
      Object.fromEntries(
        Object.entries(command.patch).map(([key, value]) => [
          key,
          key === "instructions" ? value : value!.trim(),
        ]),
      ),
    );
    return;
  ...
  if (command.kind === "set_port_type" || command.kind === "delete_port") {
    ...
    if (
      ...
      workflow.relationships.some(
        (edge) => edge.sourceId === port.id || edge.targetId === port.id,
      )
    )
      throw new Error("Disconnect this connection point before changing it.");
  ...
  if (command.kind === "set_binding_tool") {
    const binding = workflow.bindings.find(
      (item) => item.id === command.bindingId,
    );
    if (!binding)
      throw new Error(`WFR-COMMAND-TARGET-MISSING:${command.bindingId}`);
  ...
  if (command.kind === "update_relationship") {
    ...
    if (!relationship || Object.keys(command.patch).length === 0)
      throw new Error(`WFR-COMMAND-TARGET-MISSING:${command.relationshipId}`);
    Object.assign(relationship, structuredClone(command.patch));
    return;
  }
  if (command.kind === "add_block") {
    if (
      workflow.blocks.some((item) => item.id === command.block.id) ||
      workflow.ports.some((item) =>
        command.ports.some((port) => port.id === item.id),
      )
    ) {
      throw new Error(`WFR-ID-DUPLICATE:${command.block.id}`);
    }
    const phase =
      command.block.phaseId === null
        ? null
        : workflow.phases.find((item) => item.id === command.block.phaseId);
    if (command.block.phaseId !== null && !phase)
      throw new Error(`WFR-REFERENCE-DANGLING:${command.block.phaseId}`);
  ...
  if (command.kind === "delete_block") {
    ...
    const dependencies = workflow.relationships.filter(
      (item) =>
        portIds.has(item.sourceId) ||
        portIds.has(item.targetId) ||
        item.sourceId === target.id ||
        item.targetId === target.id,
    );
    if (dependencies.length > 0)
      throw new Error(
        `WFR-COMMAND-DEPENDENCY:${dependencies[0]?.id ?? target.id}`,
      );
    ...
    for (const phase of workflow.phases)
      phase.blockIds = phase.blockIds.filter((id) => id !== target.id);
    delete layout.positions[target.id];
    return;
  }
  if (command.kind === "connect") {
    if (
      workflow.relationships.some((item) => item.id === command.relationship.id)
    )
      throw new Error(`WFR-ID-DUPLICATE:${command.relationship.id}`);
    workflow.relationships.push(structuredClone(command.relationship));
    return;
  }
  const count = workflow.relationships.length;
  workflow.relationships = workflow.relationships.filter(
    (item) => item.id !== command.relationshipId,
  );
  if (workflow.relationships.length === count)
    throw new Error(`WFR-COMMAND-TARGET-MISSING:${command.relationshipId}`);
}
...
function modelDiff(
  ...
  const diff: string[] = [];
  const rootBefore = {
    documentKind: before.documentKind,
    schemaVersion: before.schemaVersion,
    workflowId: before.workflowId,
    metadata: before.metadata,
  };
  const rootAfter = {
    documentKind: after.documentKind,
  ...
  for (const field of Object.keys(
    rootBefore,
  ).sort() as (keyof typeof rootBefore)[]) {
    if (stableValue(rootBefore[field]) !== stableValue(rootAfter[field]))
      diff.push(
        `Change workflow.${field} · ${stableValue(rootBefore[field])} → ${stableValue(rootAfter[field])}`,
      );
  }
  ...
  const compareCollection = <T extends { id: string }>(
    ...
    for (const id of [
      ...
      for (const field of fields) {
        ...
        if (kind === "block" && field === "title")
          diff.push(
            `Rename ${id} · ${stableValue(beforeValue)} → ${stableValue(afterValue)}`,
          );
        else if (kind === "block" && field === "configuration")
          diff.push(
            `Configure ${id} · ${stableValue(beforeValue)} → ${stableValue(afterValue)}`,
          );
        else
          diff.push(
            `Change ${kind} ${id}.${field} · ${stableValue(beforeValue)} → ${stableValue(afterValue)}`,
          );
      }
    }
  };
  ...
  compareCollection(
    "artifact",
    before.artifactContracts,
    after.artifactContracts,
    (item) => item.name,
  );
  compareCollection(
    "binding",
...
export function applyRecoveryBatch(
  ...
  if (
    batch.documentKind !== "workflow-command-batch" ||
    batch.schemaVersion !== "1.0.0-recovery.1"
  ) {
    return failure(
      "WFR-COMMAND-VERSION-UNSUPPORTED",
      `Unsupported command document ${String(batch.documentKind)} version ${String(batch.schemaVersion)}.`,
      "Preserve the original command bytes and use an explicitly compatible reader; never silently rewrite an unknown version.",
    );
  }
  const inputLayoutIssue = validateRecoveryLayoutDocument(
    workflowValue,
    layoutValue,
  )[0];
  if (inputLayoutIssue)
    return failure(
      inputLayoutIssue.code,
      inputLayoutIssue.explanation,
      inputLayoutIssue.correction,
      inputLayoutIssue.semanticId,
    );
  if (batch.baseRevision !== workflowValue.revision) {
    return failure(
      "WFR-COMMAND-STALE-BASE",
      `Revision ${batch.baseRevision} is stale; the current definition is revision ${workflowValue.revision}.`,
      "Refresh, rebase, and review the new semantic diff.",
    );
  }
  if (batch.commands.length === 0)
    return failure(
      "WFR-COMMAND-BATCH-EMPTY",
      "A command batch must contain at least one change.",
      "Add a command or reject the proposal.",
    );
  const historyCommands = batch.commands.filter(
    (command) => command.kind === "restore_snapshot",
  );
  if (
    (historyCommands.length > 0 &&
      (batch.origin !== "history" || batch.commands.length !== 1)) ||
    (batch.origin === "history" && historyCommands.length !== 1)
  ) {
    return failure(
      "WFR-HISTORY-BATCH-INVALID",
      "Undo/redo must be one isolated restore command in a history-origin batch.",
      "Submit exactly one validated restore snapshot command.",
    );
  }
  const hasLayoutOnlyCommand = batch.commands.some(
    (command) => command.kind === "move_block",
  );
  const hasSemanticCommand = batch.commands.some(
    (command) =>
      command.kind !== "move_block" && command.kind !== "restore_snapshot",
  );
  if (hasLayoutOnlyCommand && hasSemanticCommand) {
    return failure(
      "WFR-COMMAND-MIXED-CONTAINMENT",
      "Layout-only moves and semantic definition changes cannot share one atomic batch.",
      "Submit one semantic batch and one layout batch so revision ownership remains explicit.",
    );
  }
  ...
  try {
    for (const command of batch.commands)
      applyCommand(workflow, layout, command);
  } catch (error) {
    const message =
      error instanceof Error ? error.message : "WFR-COMMAND-UNKNOWN";
    const [code, identity] = message.split(":");
    return failure(
      code?.startsWith("WFR-") ? code : "WFR-COMMAND-UNKNOWN",
      message,
      "Correct or remove the invalid command; no change was applied.",
      identity ?? null,
    );
  }
  const inputDiagnostics = [
    ...workflow.blocks.flatMap(validateAuthoringConfiguration),
    ...validateAuthoringConnections(workflow),
    ...(batch.origin === "history"
      ? []
      : validateOutputChanges(workflowValue, workflow)),
  ];
  ...
  if (!parsed.ok || parsed.workflow === null)
    return {
      ok: false,
      workflow: null,
      layout: null,
  ...
  const candidateLayoutIssue = validateRecoveryLayoutDocument(
    parsed.workflow,
    layout,
  )[0];
  if (candidateLayoutIssue)
    return failure(
      candidateLayoutIssue.code,
      candidateLayoutIssue.explanation,
      candidateLayoutIssue.correction,
      candidateLayoutIssue.semanticId,
    );
  ...
  if (!sourceContainment.ok)
    return {
      ...
      workflow: null,
      layout: null,
      diagnostics: sourceContainment.diagnostics,
      diff: [],
      semanticChanged: false,
    };
...
export function acceptRecoveryResult(
  current: RecoveryWorkflow,
  currentLayout: RecoveryLayout,
  result: RecoveryApplyResult,
): { workflow: RecoveryWorkflow; layout: RecoveryLayout } | null {
  if (!result.ok || result.workflow === null || result.layout === null)
    return null;
  ...
  const layoutChanged =
    ...
    stableValue({ positions: layout.positions, viewport: layout.viewport });
  layout.workflowId = workflow.workflowId;
  layout.semanticRevision = workflow.revision;
  layout.layoutRevision =
    layoutChanged || result.semanticChanged
      ? currentLayout.layoutRevision + 1
      : currentLayout.layoutRevision;
...
function normalized(workflow: RecoveryWorkflow): string {
  ...
  value.artifact_contracts.sort((left, right) =>
...
export function textEditCommands(
  before: RecoveryWorkflow,
  after: RecoveryWorkflow,
  currentLayout?: RecoveryLayout,
): RecoveryCommand[] | RecoveryDiagnostic[] {
  ...
  if (before.metadata.title !== after.metadata.title)
    metadataPatch.title = after.metadata.title;
  if (before.metadata.purpose !== after.metadata.purpose)
    metadataPatch.purpose = after.metadata.purpose;
  if (before.metadata.engineeringDomain !== after.metadata.engineeringDomain)
    metadataPatch.engineeringDomain = after.metadata.engineeringDomain;
  if (Object.keys(metadataPatch).length > 0)
    edits.push({ kind: "set_workflow_metadata", patch: metadataPatch });
  ...
  for (const current of before.phases) {
    ...
    if (!edited)
      return [
        {
          code: "WFR-TEXT-STRUCTURE-UNSUPPORTED",
          semanticId: current.id,
          line: null,
          explanation:
            "Engineering source cannot remove an accepted optional group in this concept.",
          correction:
            "Omit group sections from the authoring view or regroup with a reviewed structural command.",
        },
      ];
    if (current.name !== edited.name)
      edits.push({
        kind: "set_phase_name",
        phaseId: current.id,
        name: edited.name,
  ...
  for (const current of before.blocks) {
    ...
    if (current.title !== edited.title)
      edits.push({
        kind: "set_block_title",
        blockId: current.id,
        title: edited.title,
      });
    const definitionPatch: Partial<
      Pick<RecoveryBlock, "purpose" | "instructions">
    > = {};
    if (current.purpose !== edited.purpose)
      definitionPatch.purpose = edited.purpose;
    if (current.instructions !== edited.instructions)
      definitionPatch.instructions = edited.instructions;
    ...
    const keys = new Set([
      ...
      ...Object.keys(edited.configuration),
    ]);
    for (const key of keys) {
      if (edited.configuration[key] === undefined)
        return [
          {
            code: "WFR-TEXT-STRUCTURE-UNSUPPORTED",
            semanticId: current.id,
            line: null,
            explanation:
              "Configuration key deletion is not promoted by this recovery parser.",
            correction:
              "Restore the key or configure the block in the inspector.",
          },
        ];
      if (current.configuration[key] !== edited.configuration[key])
        edits.push({
          kind: "set_block_configuration",
          blockId: current.id,
          key,
          value: edited.configuration[key] as string | number | boolean,
        });
    }
  }
      ...
      return [
        {
          ...
          line: null,
          explanation:
            "A new workflow step has an incomplete connection-point definition.",
          correction:
            "Declare every input and output connection point in the new task section.",
        },
      ];
  ...
  const afterArtifacts = new Map(
    after.artifactContracts.map((item) => [item.id, item]),
  );
      ...
      return [
        {
          ...
          line: null,
          explanation:
            "Engineering source cannot remove an accepted engineering item in this concept.",
          correction:
            "Restore the item or change structure through a reviewed canvas command.",
        },
      ];
  ...
  const deletedBlockIds = new Set(
    deletedBlocks.map((command) =>
      command.kind === "delete_block" ? command.blockId : "",
    ),
  );
  const addedPortIds = new Set(
    addedBlocks.flatMap((command) =>
      command.kind === "add_block" ? command.ports.map((port) => port.id) : [],
    ),
  );
      ...
      return [
        {
          ...
          line: null,
          explanation:
            "Engineering source cannot remove one connection point from an existing step.",
          correction:
            "Restore the connection point or remove the complete step.",
        },
      ];
    }
    if (
      current.required !== edited.required ||
      current.cardinality !== edited.cardinality
    )
      edits.push({
        kind: "set_port_contract",
        portId: current.id,
        required: edited.required,
        cardinality: edited.cardinality,
      });
  }
  const beforePortIds = new Set(before.ports.map((port) => port.id));
  for (const port of after.ports) {
    if (!beforePortIds.has(port.id) && !addedPortIds.has(port.id))
      edits.push({ kind: "add_port", port: structuredClone(port) });
  }
  const afterRelationships = new Map(
    after.relationships.map((item) => [item.id, item]),
  );
  const beforeRelationshipIds = new Set(
    before.relationships.map((item) => item.id),
  );
  for (const current of before.relationships) {
    ...
    if (current.condition !== edited.condition)
      patch.condition = edited.condition;
    if (Object.keys(patch).length > 0)
  ...
  for (const relationship of after.relationships)
    if (!beforeRelationshipIds.has(relationship.id))
      connections.push({
        kind: "connect",
      ...
      return [
        {
          ...
          line: null,
          explanation:
            "Engineering source cannot add or remove configured tool assignments in this editor.",
          correction:
            "Restore the assignment or change the configured tool in Technical details.",
        },
      ];
    if (current.toolId !== edited.toolId)
      edits.push({
        kind: "set_binding_tool",
        bindingId: current.id,
        toolId: edited.toolId,
      });
  }
  const commands = [
    ...disconnects,
    ...
    return [
      {
        ...
        line: null,
        explanation:
          "The source contains a structural or managed-authority change that this editor cannot apply.",
        correction:
          "Restore the last accepted source or make the structural change on the canvas.",
      },
    ];
  }
  return commands;
...
export function paletteBlock(
  ...
  return {
    ...
    block: {
      ...
      purpose: drawing
        ? "Create a reviewable manufacturing drawing from the approved CAD model."
        : "Compare critical dimensions and tolerances with the design requirements.",
      phaseId: null,
      executionKind: "deterministic",
      instructions: drawing
        ? "Create an A3 manufacturing drawing with dimensions and source revision."
        : "Report each checked dimension and tolerance with units and evidence.",
      configuration: drawing
        ? { sheet: "A3", standard: "ASME Y14.5" }
        : { criteria_source: "reviewed-design-specification" },
      inputPortIds: drawing ? [inputId] : [inputId, specificationInputId],
      outputPortIds: [outputId],
      bindingId: null,
      componentRef: null,
    },
    ports: [
      {
        ...
        direction: "input",
        name: drawing ? "Approved CAD model" : "Bracket CAD model",
        typeId: drawing ? "type.geometry.approved" : "type.geometry.brep",
        required: true,
        cardinality: "one",
        artifactContractId: drawing
          ? "artifact.approved-geometry"
          : "artifact.geometry",
        description: "Selected source CAD model.",
      },
      ...(!drawing
        ? [
            {
              id: specificationInputId,
              ownerBlockId: blockId,
              ...
              description:
                "Approved dimensions and tolerances to compare with the CAD model.",
            },
          ]
        : []),
      {
        ...
        name: drawing
          ...
          : "Dimension and tolerance report",
        typeId: drawing ? "type.file.drawing" : "type.report.tolerance",
        required: true,
        cardinality: "one",
        artifactContractId: null,
        description: "Reviewable engineering output.",
      },
    ],
    position: { x, y },
  };
...
export function aiDrawingProposal(
  workflow: RecoveryWorkflow,
): RecoveryCommandBatch {
  const first: RecoveryBlock = {
    id: "block.create-inspection-drawing",
    kind: "work",
    title: "Create manufacturing drawing",
    purpose:
      "Create a dimensioned drawing from the approved bracket CAD model.",
    phaseId: null,
    executionKind: "deterministic",
    instructions:
      "Create an A3 manufacturing drawing with critical dimensions, datums, tolerances, and exact source revision.",
    configuration: { sheet: "A3", standard: "ASME Y14.5" },
    inputPortIds: ["port.drawing-geometry-in"],
    outputPortIds: ["port.drawing-out"],
    bindingId: null,
    componentRef: null,
  };
  const second: RecoveryBlock = {
    id: "block.review-inspection-drawing",
    kind: "approval",
    title: "Review manufacturing drawing",
    purpose:
      "Confirm drawing completeness before it enters the design handoff package.",
    phaseId: null,
    executionKind: "human",
    instructions:
      "Accept only when dimensions, tolerances, revision, and the source CAD model agree.",
    configuration: { required_role: "drawing-checker" },
    inputPortIds: ["port.drawing-review-in"],
    outputPortIds: ["port.drawing-approved-out"],
    bindingId: null,
    componentRef: null,
  };
  const ports: RecoveryPort[] = [
    {
      id: "port.drawing-geometry-in",
      ownerBlockId: first.id,
      direction: "input",
      ...
      description: "Approved bracket CAD model.",
    },
    {
      id: "port.drawing-out",
      ownerBlockId: first.id,
      ...
      cardinality: "one",
      artifactContractId: null,
      description: "Dimensioned A3 manufacturing drawing.",
    },
    {
      id: "port.drawing-review-in",
      ownerBlockId: second.id,
      direction: "input",
      name: "Manufacturing drawing",
      typeId: "type.file.drawing",
      required: true,
      cardinality: "one",
      artifactContractId: null,
      description: "Manufacturing drawing presented for checking.",
    },
    {
      id: "port.drawing-approved-out",
      ownerBlockId: second.id,
      direction: "output",
      name: "Approved manufacturing drawing",
      typeId: "type.file.drawing.approved",
      required: true,
      cardinality: "one",
      artifactContractId: null,
      description: "Drawing plus checker decision.",
    },
  ];
  return recoveryCommandBatch(workflow.revision, "ai_proposal", [
    ...
    },
    {
      kind: "connect",
      relationship: {
        id: "rel.approved-to-drawing",
        kind: "data",
        sourceId: "port.approved-geometry-out",
        targetId: "port.drawing-geometry-in",
    ...
    },
    {
      kind: "connect",
      relationship: {
        id: "rel.drawing-to-review",
        kind: "data",
        sourceId: "port.drawing-out",
        targetId: "port.drawing-review-in",
        label: "manufacturing drawing",
        condition: null,
      },
    },
  ]);
