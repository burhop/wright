...
import { RECOVERY_AUTHORING_SECTION_CONFIGURATION_KEY, recoveryAuthoringSectionKind, type RecoveryBlock, type RecoveryDiagnostic, type RecoveryLayout, type RecoveryPort, type RecoveryRelationship, type RecoveryWorkflow } from "./model";
import { findAuthoringPosition, type AuthoringPoint } from "./authoring-positioning";

export { findAuthoringPosition, hydrateAuthoringLayout } from "./authoring-positioning";
...
const documentOut: TemplatePort = { key: "document-out", name: "Engineering document", typeId: "type.document.engineering", direction: "output" };
const modelIn: TemplatePort = { key: "model-in", name: "CAD model", typeId: "type.geometry.brep", direction: "input" };
const reportOut: TemplatePort = { key: "report-out", name: "Check report", typeId: "type.report.check", direction: "output" };
const verdictOut: TemplatePort = { key: "verdict-out", name: "Check result", typeId: "type.verdict.check", direction: "output" };
...
export function createAuthoringObject(templateId: string, workflow: RecoveryWorkflow, layout: RecoveryLayout, options: { selectedBlockId?: string | null; viewportCenter?: AuthoringPoint } = {}): Extract<RecoveryCommand, { kind: "add_block" }> {
  ...
  const ids = new Set([...workflow.blocks, ...workflow.ports, ...workflow.artifactContracts, ...workflow.bindings, ...workflow.relationships, ...workflow.components, ...workflow.phases].map((item) => item.id));
  let index = 1;
  while (ids.has(`block.${template.id}-${index}`) || template.ports.some((port) => ids.has(`port.${template.id}-${index}-${port.key}`))) index += 1;
  ...
  const selectedPosition = options.selectedBlockId ? layout.positions[options.selectedBlockId] : undefined;
  const preferred = template.group === "input" ? { x: 40, y: 100 } : selectedPosition ? { x: selectedPosition.x + 320, y: selectedPosition.y } : options.viewportCenter ?? { x: 380, y: 100 };
  return { kind: "add_block", block: { id: blockId, title: template.label, purpose: template.description, kind: "work", executionKind: template.executionKind, phaseId: null, instructions: template.instructions, configuration, inputPortIds: ports.filter((port) => port.direction === "input").map((port) => port.id), outputPortIds: ports.filter((port) => port.direction === "output").map((port) => port.id), bindingId: null, componentRef: null }, ports, position: findAuthoringPosition(workflow, layout, preferred) };
}

export interface AuthoringInputState { status: "missing" | "configured" | "unavailable"; summary: string; reason?: string }

export function isSafeWorkspaceInputPath(value: string): boolean {
  if (value.length === 0 || value.length > 1024 || value !== value.trim() || /[\\%<>:"|?*\u0000-\u001f\u007f]/.test(value) || value.startsWith("/") || /^[a-z]+:/i.test(value)) return false;
  return value.split("/").every((part) => part.length > 0 && part !== "." && part !== ".." && !/[. ]$/.test(part) && !/^\.(?:git|wright)$/i.test(part) && !/^(?:con|prn|aux|nul|com[1-9]|lpt[1-9])(?:\.|$)/i.test(part));
}

/** Shared by every command and source parse, not only the file-picker UI. */
export function validateAuthoringConfiguration(block: RecoveryBlock): RecoveryDiagnostic[] {
...
export function validateAuthoringConnections(workflow: RecoveryWorkflow): RecoveryDiagnostic[] {
  const ports = new Map(workflow.ports.map((port) => [port.id, port]));
  return workflow.relationships.flatMap((relationship): RecoveryDiagnostic[] => {
    if (relationship.kind !== "data") return [];
    const source = ports.get(relationship.sourceId);
    const target = ports.get(relationship.targetId);
    if (source?.cardinality !== "many" || !target || target.cardinality === "many") return [];
    return [{ code: "WFR-PORT-COLLECTION-MISMATCH", semanticId: relationship.id, line: null, explanation: `${source.name} supplies a collection but ${target.name} accepts one item.`, correction: "Use a collection input or an explicit step that selects an item from the collection." }];
  });
}

export function authoringInputState(block: RecoveryBlock, permittedFiles?: readonly string[]): AuthoringInputState {
  const diagnostics = validateAuthoringConfiguration(block);
  if (diagnostics.length > 0) return { status: "unavailable", summary: "Input needs correction", reason: diagnostics[0]!.explanation };
...
export function authoringReadiness(workflow: RecoveryWorkflow, permittedFiles?: readonly string[]) {
  const inputs = workflow.blocks.filter((block) => recoveryAuthoringSectionKind(block) === "input").map((block) => ({ blockId: block.id, title: block.title, ...authoringInputState(block, permittedFiles) }));
  return { inputs, configuredCount: inputs.filter((input) => input.status === "configured").length, totalCount: inputs.length };
}

export function authoringConfigurationCommands(block: RecoveryBlock, patch: Record<string, string | number | boolean>): RecoveryCommand[] {
  if (Object.hasOwn(patch, RECOVERY_AUTHORING_SECTION_CONFIGURATION_KEY) || Object.keys(patch).some((key) => ["__proto__", "prototype", "constructor"].includes(key))) throw new Error("Wright-managed settings cannot be changed from the input editor.");
  const diagnostics = validateAuthoringConfiguration({ ...block, configuration: { ...block.configuration, ...patch } });
  if (diagnostics.length) throw new Error(`${diagnostics[0]!.code}: ${diagnostics[0]!.explanation}`);
  return Object.entries(patch).filter(([key, value]) => block.configuration[key] !== value).map(([key, value]) => ({ kind: "set_block_configuration", blockId: block.id, key, value }));
}

export function authoringDeletionImpact(workflow: RecoveryWorkflow, blockId: string): { relationships: RecoveryRelationship[]; blockedReason?: string } {
  const block = workflow.blocks.find((item) => item.id === blockId);
  if (!block) return { relationships: [], blockedReason: "This step no longer exists." };
  const endpoints = new Set([block.id, ...block.inputPortIds, ...block.outputPortIds]);
  const relationships = workflow.relationships.filter((item) => endpoints.has(item.sourceId) || endpoints.has(item.targetId));
  const artifact = workflow.artifactContracts.find((item) => item.producerBlockId === blockId || item.requiredForBlockIds.includes(blockId));
  const binding = workflow.bindings.find((item) => [...item.argumentMap, ...item.resultMap].some((mapping) => endpoints.has(mapping.semanticSource) || mapping.semanticSource.startsWith(`${blockId}.configuration.`)));
  const component = workflow.components.find((item) => [...item.inputPortIds, ...item.outputPortIds].some((id) => endpoints.has(id)));
  const blockedReason = artifact ? `This step is referenced by the ${artifact.name} file contract. Review that contract before deleting the step.` : binding ? `This step has a configured assignment (${binding.capabilityName}). Remove the assignment before deleting the step.` : component ? `This step owns interfaces of ${component.title}. Review the grouped step before deleting it.` : undefined;
  return { relationships, ...(blockedReason ? { blockedReason } : {}) };
}

export function buildDeletionCommands(workflow: RecoveryWorkflow, blockId: string): RecoveryCommand[] {
  const impact = authoringDeletionImpact(workflow, blockId);
  if (impact.blockedReason) throw new Error(impact.blockedReason);
  return [...impact.relationships.map((relationship): RecoveryCommand => ({ kind: "disconnect", relationshipId: relationship.id })), { kind: "delete_block", blockId }];
}
