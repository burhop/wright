...
it('keeps legacy native and export edges while removing duplicate unused presentation',()=>{
  ...
  const b=w.blocks[0];
  const native={...w.ports[0],id:'port.native',ownerBlockId:b.id,direction:'output' as const};
  const exported={...native,id:'port.step'};
  w.ports.push(native,exported);
  b.configuration.cad=JSON.stringify({native_port:sourceKey(native.id),exports:[{port:sourceKey(exported.id),format:'step',path:'parts/bracket.step'}]});
  ...
  w.relationships.push({...w.relationships[0],id:'rel.legacy-native',sourceId:native.id});
