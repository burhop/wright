...
export function useWorkflowRunObserver(
  ...
  const [state, setState] = useState<{
    scope: string;
    record: WorkspaceWorkflowRunSummary | null;
    error: string;
    checked: boolean;
  }>({ scope, record: null, error: "", checked: false });
  ...
  useEffect(() => {
    ...
    let latest =
      lastKnown.current.scope === scope ? lastKnown.current.record : null;
    setState((previous) =>
    ...
    const refresh = async () => {
      ...
      const token = ++serial;
      try {
        const runs = await workspaceService.getWorkspaceWorkflowRuns(
          sessionId!,
          path,
          { workspaceId, latestOnly: true, signal: request.signal },
        );
        ...
        latest =
          latest &&
          incoming &&
          Date.parse(incoming.started_at) < Date.parse(latest.started_at)
            ? latest
            : (incoming ?? latest);
        ...
        if (current && token === serial && isVisible())
          timer = setTimeout(
            () => void refresh(),
            latest?.status === "running" ? 2000 : 10000,
          );
      }
    };
    const onVisible = () => {
  ...
  return {
    ...(state.scope === scope
      ? state
      : { record: null, error: "", checked: false }),
    enabled,
    refresh: () => setRetry((value) => value + 1),
  };
}

export function preferObservedRun(
  record: WorkspaceWorkflowRunSummary | null,
  owned: {
    pending: boolean;
    startedAt: string;
