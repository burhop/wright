...
export function disconnectedProcessDiagnostic(
  workflow: RecoveryWorkflow,
): RecoveryDiagnostic | null {
  const neighbors = new Map(
    workflow.blocks.map((block) => [block.id, new Set<string>()]),
  );
  const owners = new Map(
    workflow.ports.map((port) => [port.id, port.ownerBlockId]),
  );
  ...
  const names = new Map(
    workflow.blocks.map((block) => [block.id, block.title]),
  );
  const examples = groups
    .slice(0, 4)
    .map(
      (group) =>
        `“${names.get(group[0]!)}” (${group.length} ${group.length === 1 ? "block" : "blocks"})`,
    )
    .join("; ");
  return {
    code: "WFR-DISCONNECTED-PROCESS",
    semanticId: null,
    line: null,
