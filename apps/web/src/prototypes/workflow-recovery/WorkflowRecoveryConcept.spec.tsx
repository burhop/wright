...
import { WorkflowRecoveryConcept as Editor, type WorkflowRecoveryConceptProps, type WorkflowRecoveryPersistedSource } from "./WorkflowRecoveryConcept";
...
import { workspaceService, type WorkspaceWorkflowReview } from "../../services/workspace-service";
...
const WorkflowRecoveryConcept = (props: WorkflowRecoveryConceptProps) => <Editor {...props} simulationPreview />;

const reportPrompt = "Compare aluminum and steel for a mounting bracket. Include a table and a conclusion.";
...
it("rehydrates a pending review into the normal run drawer without running or pulsing", async () => {
  const review: WorkspaceWorkflowReview = {review_id:"review-1",package_digest:"package",state:"pending",run_id:"run-1",workflow_path:"workflows/review.wflow",source_digest:"source",task_id:"review",task_title:"Engineer review",instructions:"Check dimensions.",artifacts:[{output_path:"report-001.html",sha256:"sha",output_bytes:128,output_format:"html",task_id:"document_1",task_title:"Report"}],created_at:"2026-09-08T10:00:00Z",decided_at:null,actor:null,reason:null};
  const fetch = vi.spyOn(workspaceService,"getWorkspaceWorkflowReviews").mockResolvedValue([review]);
  const onRun = vi.fn(); const onOpenFile = vi.fn();
  render(<Editor workflowSource={reportWorkflowSource} workflowFilePath={review.workflow_path} definitionRevision={2} storageDigest={"b".repeat(64)} workspaceSessionId="session" onRun={onRun} onOpenFile={onOpenFile}/>);
  await waitFor(() => expect(screen.getByTestId("workflow-recovery-run-details-toggle")).toHaveTextContent("Awaiting your review"));
  fireEvent.click(screen.getByTestId("workflow-recovery-run-details-toggle"));
  expect(await screen.findByTestId("workflow-document-review")).toHaveTextContent("Check dimensions.");
  ...
  expect(screen.getByTestId("workflow-recovery-block-block.document-1")).toHaveAttribute("data-run-state","idle");
  fetch.mockRestore();
});
...
] as const)("rehydrates %s into the drawer and summary, preserving older reviews", async (state, label) => {
  const current: WorkspaceWorkflowReview = {review_id:"new-review",package_digest:"new-package",state,run_id:"run-new",workflow_path:"workflows/review.wflow",source_digest:"source",task_id:"review",task_title:"Latest engineer review",instructions:"Check the current synthetic package.",artifacts:[],created_at:"2026-09-08T10:00:00Z",decided_at:state === "pending" ? null : "2026-09-08T10:10:00Z",actor:null,reason:state === "changes_requested" ? "Add tolerances" : null};
  const older = {...current, review_id:"old-review", package_digest:"old-package", state:"pending" as const, task_title:"Older engineer review", created_at:"2026-09-07T10:00:00Z"};
  const fetch = vi.spyOn(workspaceService,"getWorkspaceWorkflowReviews").mockResolvedValue([current,older]);
  const onRun=vi.fn();
  render(<Editor workflowSource={reportWorkflowSource} workflowFilePath={current.workflow_path} definitionRevision={2} storageDigest={"b".repeat(64)} workspaceSessionId="session" onRun={onRun}/>);
  await waitFor(()=>expect(screen.getByTestId("workflow-recovery-run-details-toggle")).toHaveTextContent(label));
  fireEvent.click(screen.getByTestId("workflow-recovery-run-details-toggle"));
  expect(screen.getByTestId("workflow-native-run-summary")).toHaveTextContent(label);
  expect(screen.getByTestId("workflow-native-run-summary")).not.toHaveTextContent("Run the saved workflow");
  expect(screen.getByText("Awaiting your review · Older engineer review")).toBeVisible();
  expect(onRun).not.toHaveBeenCalled();
  expect(screen.getByTestId("workflow-recovery-block-block.document-1")).toHaveAttribute("data-run-state","idle");
  fetch.mockRestore();
});

it.each(["approved", "changes_requested"] as const)("updates summary immediately after %s on a reopened review", async state => {
  const pending: WorkspaceWorkflowReview={review_id:"review-decision",package_digest:"package",state:"pending",run_id:"run",workflow_path:"workflows/review.wflow",source_digest:"source",task_id:"review",task_title:"Engineer review",instructions:"Review the synthetic document.",artifacts:[],created_at:"2026-09-08T10:00:00Z",decided_at:null,actor:null,reason:null};
  const fetch=vi.spyOn(workspaceService,"getWorkspaceWorkflowReviews").mockResolvedValue([pending]);
  const decide=vi.spyOn(workspaceService,"decideWorkspaceWorkflowReview").mockResolvedValue({...pending,state,reason:"Synthetic review note"});
  const onRun=vi.fn();
  render(<Editor workflowSource={reportWorkflowSource} workflowFilePath={pending.workflow_path} definitionRevision={2} storageDigest={"b".repeat(64)} workspaceSessionId="session" onRun={onRun}/>);
  await waitFor(()=>expect(screen.getByTestId("workflow-recovery-run-details-toggle")).toHaveTextContent("Awaiting your review"));
  fireEvent.click(screen.getByTestId("workflow-recovery-run-details-toggle"));
  if(state==="changes_requested") fireEvent.change(screen.getByLabelText("Review notes (required for changes)"),{target:{value:"Synthetic review note"}});
  fireEvent.click(screen.getByRole("button",{name:state==="approved"?"Approve":"Request changes"}));
  const label=state==="approved"?"Review approved":"Changes requested";
  await waitFor(()=>expect(screen.getByTestId("workflow-recovery-run-details-toggle")).toHaveTextContent(label));
  expect(screen.getByTestId("workflow-native-run-summary")).toHaveTextContent(label);
  expect(screen.getByTestId("workflow-native-run-summary")).not.toHaveTextContent("Ready to run");
  expect(onRun).not.toHaveBeenCalled();
  expect(decide).toHaveBeenCalledOnce();
  fetch.mockRestore();decide.mockRestore();
});
...
function renameBlock(id: string, title: string) {
  fireEvent.doubleClick(screen.getByTestId(`workflow-recovery-block-${id}`).querySelector("h3")!);
...
describe("WorkflowRecoveryConcept component states", () => {
  it("renames a block from its inspector heading through the normal undo and save commands", async () => {
    const onSave = vi.fn(async (source: string) => ({ source, definition_revision: 3, storage_digest: "c".repeat(64) }));
    render(<Editor workflowSource={reportWorkflowSource} definitionRevision={2} storageDigest={"b".repeat(64)} onSave={onSave} />);
    fireEvent.click(screen.getByTestId("workflow-recovery-block-block.document-1"));
    const name = screen.getByRole("textbox", { name: "Block name" });
    await userEvent.click(name);
    await userEvent.keyboard("Product design report{Enter}");
    expect(screen.getByTestId("workflow-recovery-block-block.document-1")).toHaveTextContent("Product design report");
  ...
  it("edits legacy review, tool and output blocks directly, preserving parameters and Undo", async () => {
    const onSave = vi.fn(async (source: string) => ({ source, definition_revision: 3, storage_digest: "c".repeat(64) }));
    render(<Editor workflowSource={savedWorkflowSource} definitionRevision={2} storageDigest={"b".repeat(64)} onSave={onSave} />);
    for (const id of ["block.create-design-specification", "block.generate-geometry", "block.export-step"]) {
      fireEvent.click(screen.getByTestId(`workflow-recovery-block-${id}`));
      expect(screen.queryByRole("tablist", { name: "Step detail sections" })).not.toBeInTheDocument();
      expect(screen.queryByTestId("workflow-recovery-config-apply")).not.toBeInTheDocument();
      const instructions = screen.getByTestId(`workflow-recovery-block-instructions-${id}`);
      expect(instructions).toBeVisible();
      const original = (instructions as HTMLTextAreaElement).value;
      fireEvent.change(instructions, { target: { value: "Use the connected design requirements." } });
      await userEvent.click(screen.getByTestId("workflow-recovery-undo"));
      expect(instructions).toHaveValue(original);
      fireEvent.change(instructions, { target: { value: "Use the connected design requirements." } });
      await userEvent.click(screen.getByTestId("workflow-recovery-block-connections"));
      expect(screen.getByRole("heading", { name: "Inputs", exact: true })).toBeVisible();
      expect(screen.getByRole("heading", { name: "Outputs", exact: true })).toBeVisible();
    }
    fireEvent.click(screen.getByTestId("workflow-recovery-block-block.generate-geometry"));
    await userEvent.click(screen.getByTestId("workflow-recovery-settings-advanced-block.generate-geometry"));
    const thickness = screen.getByTestId("workflow-recovery-block-thickness-block.generate-geometry");
    fireEvent.change(thickness, { target: { value: "3" } });
    expect(thickness).toHaveValue(3);
    await userEvent.click(screen.getByTestId("workflow-recovery-save"));
    await waitFor(() => expect(onSave).toHaveBeenCalledOnce());
    expect(onSave.mock.calls[0]![0]).toContain("Use the connected design requirements.");
  ...
  it("blocks disconnected processes before saving or running, but permits draft saves and repair", async () => {
    const onSave = vi.fn(async (source: string) => ({ source, definition_revision: 3, storage_digest: "c".repeat(64) }));
    const onRun = vi.fn(async () => ({ outputPath: "report.html", outputBytes: 100, taskId: "document_1", taskTitle: "Create HTML report" }));
    render(<Editor workflowSource={reportWorkflowSource} definitionRevision={2} storageDigest={"b".repeat(64)} onSave={onSave} onRun={onRun} />);
    fireEvent.click(screen.getByTestId("workflow-recovery-create-group-document"));
    fireEvent.click(screen.getByTestId("workflow-recovery-create-template-document"));
    fireEvent.click(screen.getByTestId("workflow-recovery-validate"));
    expect(screen.getByRole("alert")).toHaveTextContent("2 disconnected groups");
    expect(screen.getByRole("alert")).toHaveTextContent("move each independent process to its own workflow");
    const dismiss = screen.getByRole("button", { name: "Dismiss workflow messages" });
    dismiss.focus();
    ...
    fireEvent.click(screen.getByTestId("workflow-recovery-validate"));
    expect(screen.getByRole("alert")).toHaveTextContent("2 disconnected groups");
    fireEvent.click(screen.getByRole("button", { name: "Dismiss workflow messages" }));

    fireEvent.click(screen.getByTestId("workflow-recovery-run-start"));
    expect(screen.getByRole("alert")).toHaveTextContent("2 disconnected groups");
    expect(onRun).not.toHaveBeenCalled();
    expect(onSave).not.toHaveBeenCalled();
    expect(screen.getByTestId("workflow-recovery-run-start")).not.toHaveTextContent("Running");
    fireEvent.click(screen.getByTestId("workflow-recovery-save"));
    await waitFor(() => expect(onSave).toHaveBeenCalledOnce());
    fireEvent.click(screen.getByTestId("workflow-recovery-run-start"));
    expect(onRun).not.toHaveBeenCalled();
    const added = screen.getByTestId("workflow-recovery-block-block.document-2");
    added.focus();
    await userEvent.keyboard("{Delete}");
    fireEvent.click(screen.getByTestId("workflow-recovery-delete-confirm"));
    expect(screen.queryByTestId("workflow-recovery-diagnostic-WFR-DISCONNECTED-PROCESS")).not.toBeInTheDocument();
  ...
  it("corrects the old single-file image template on save while preserving its file and identity", async () => {
    ...
    const onSave = vi.fn(async (source: string) => ({ source, definition_revision: 3, storage_digest: "a".repeat(64) }));
    render(<Editor workflowSource={formatRecoveryAuthoringSource(workflow).text} definitionRevision={2} storageDigest={"b".repeat(64)} onSave={onSave} />);
    const block = screen.getByTestId(`workflow-recovery-block-${image.block.id}`);
    fireEvent.doubleClick(block.querySelector("h3")!);
    const name = screen.getByTestId(`workflow-recovery-rename-${image.block.id}`);
    fireEvent.change(name, { target: { value: "Selected reference" } });
    fireEvent.keyDown(name, { key: "Enter" });
    await userEvent.click(screen.getByTestId("workflow-recovery-save"));
    await waitFor(() => expect(screen.getByTestId("workflow-recovery-save-status")).toHaveTextContent("Saved"));
    expect(onSave.mock.calls[0]![0]).toContain("inputs/reference.png");
    expect(screen.getByTestId(`workflow-recovery-block-${image.block.id}`)).toHaveTextContent("Selected reference");
    expect(screen.getByTestId(`workflow-recovery-handle-${image.ports[0]!.id}`)).toHaveAttribute("data-cardinality", "one");
  });
  it("blocks a workspace run with actionable missing inputs and no manual simulation", async () => {
    render(<Editor workflowSource={savedWorkflowSource} definitionRevision={2} storageDigest={"b".repeat(64)} />);
    ...
    expect(dialog).toHaveTextContent("configuring inputs alone will not enable execution");
    expect(screen.queryByTestId("workflow-recovery-run-mode")).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Advance simulation" })).not.toBeInTheDocument();
    await userEvent.click(screen.getByRole("button", { name: "Configure Design intent" }));
    expect(screen.queryByRole("dialog")).not.toBeInTheDocument();
    expect(screen.getByTestId("workflow-recovery-input-text-block.design-intent")).toBeVisible();
    expect(screen.queryByTestId("workflow-recovery-config-apply")).not.toBeInTheDocument();
  });
  ...
  it("does not report configured inputs as missing or allow a run after configuring them", async () => {
    ...
    await userEvent.click(screen.getByRole("button", { name: "Configure Design intent" }));
    fireEvent.change(screen.getByTestId("workflow-recovery-input-text-block.design-intent"), { target: { value: "Support the pump load." } });
    await userEvent.click(screen.getByTestId("workflow-recovery-run-start"));
    expect(screen.getByRole("dialog")).toHaveTextContent("2 inputs need configuration");
    expect(screen.queryByRole("button", { name: "Configure Design intent" })).not.toBeInTheDocument();
    expect(screen.getByTestId("workflow-recovery-execution-unavailable")).toBeVisible();
    expect(screen.queryByTestId("workflow-recovery-run-mode")).not.toBeInTheDocument();
  });

  it("runs the saved workspace source and reports the declared file without adding a viewer", async () => {
    let resolveRun!: (result: { outputPath: string; outputBytes: number; taskTitle: string; taskId: string }) => void;
    const onRun = vi.fn().mockImplementation(() => new Promise<{ outputPath: string; outputBytes: number; taskTitle: string; taskId: string }>((resolve) => { resolveRun = resolve; }));
    render(<Editor workflowSource={reportWorkflowSource} definitionRevision={2} storageDigest={"b".repeat(64)} workspaceSessionId="session-1" onRun={onRun} />);

    expect(screen.queryByTestId("workflow-recovery-native-run-input")).not.toBeInTheDocument();

    await userEvent.click(screen.getByTestId("workflow-recovery-run-start"));

    await waitFor(() => expect(onRun).toHaveBeenCalledOnce());
    expect(screen.getByTestId("workflow-recovery-native-run-mode")).toHaveTextContent("Workflow running");
    expect(screen.getByTestId("workflow-recovery-native-run-input")).toHaveTextContent("Prompt sent to");
    expect(screen.getByTestId("workflow-recovery-native-run-input")).toHaveTextContent(reportPrompt);
    expect(screen.getByTestId("workflow-recovery-native-run-log")).toHaveTextContent("Run requested");
    expect(document.querySelector('[data-testid^="workflow-recovery-block-"][data-run-state="running"][data-active="true"]')).not.toBeNull();
    await act(async () => { resolveRun({ outputPath: "report.html", outputBytes: 321, taskTitle: "Create HTML report", taskId: "document_1" }); });
    expect(screen.getByTestId("workflow-recovery-native-run-mode")).toHaveTextContent("Workflow completed");
    expect(screen.getByTestId("workflow-recovery-native-run-mode")).toHaveTextContent("report.html");
    expect(screen.getByTestId("workflow-recovery-native-run-output-link")).toHaveAttribute("href", expect.stringContaining("path=report.html"));
    expect(screen.getByTestId("workflow-recovery-block-block.document-1")).toHaveAttribute("data-run-state", "idle");
    expect(screen.getByTestId("workflow-recovery-native-run-input")).toHaveTextContent(reportPrompt);
    expect(screen.queryByText("Automatic execution is not available in this build.")).not.toBeInTheDocument();

    // Editing the next run must not rewrite the prompt shown for the completed run.
    fireEvent.click(screen.getByTestId("workflow-recovery-block-block.document-1"));
    fireEvent.change(screen.getByTestId("workflow-recovery-block-instructions-block.document-1"), { target: { value: "Write a different report about titanium." } });
    expect(screen.queryByTestId("workflow-recovery-config-apply")).not.toBeInTheDocument();
    expect(screen.getByTestId("workflow-recovery-native-run-input")).toHaveTextContent(reportPrompt);
    expect(screen.getByTestId("workflow-recovery-native-run-input")).not.toHaveTextContent("titanium");
  });

  it("projects completed steps, correction invalidation and transition gaps only during the active run", async () => {
    const secondTask = reportWorkflowSource.slice(reportWorkflowSource.indexOf("task document_1"))
      .replaceAll("document_1", "document_2").replaceAll("Create HTML report", "Create second report")
      .replace("inputs: []", 'inputs: [{"description":"First report","item":null,"key":"document_2_input","kind":"engineering_document","name":"First report","quantity":"one","required":true}]');
...
connection reports
  ...
  when: null
    ...
    let finish!: (result: { outputPath: string; outputBytes: number; taskTitle: string }) => void;
    const onRun = vi.fn((value?: WorkflowRunOptions) => {
      options = value!;
      return new Promise<{ outputPath: string; outputBytes: number; taskTitle: string }>(resolve => { finish = resolve; });
    });
    render(<Editor workflowSource={source} definitionRevision={2} storageDigest={"b".repeat(64)} onRun={onRun} />);
    await userEvent.click(screen.getByTestId("workflow-recovery-run-start"));
    await waitFor(() => expect(onRun).toHaveBeenCalledOnce());
    const first = screen.getByTestId("workflow-recovery-block-block.document-1");
    const second = screen.getByTestId("workflow-recovery-block-block.document-2");
    const send = async (kind: "step_started" | "step_completed" | "design_revision", taskId: string, invalidated?: string[]) => {
      await act(async () => { options.onEvent?.({ kind, at: new Date().toISOString(), task_id: taskId, task_title: taskId, invalidated_task_ids: invalidated }); });
    };
    ...
    await send("step_started", "document_2");
    await send("step_completed", "document_2");
    await act(async () => { finish({ outputPath: "report.html", outputBytes: 100, taskTitle: "Create second report" }); });
...
connection brief
  ...
  when: null
    ...
    let finish!: (result: { outputPath: string; outputBytes: number; taskTitle: string }) => void;
    const onRun = vi.fn(() => new Promise<{ outputPath: string; outputBytes: number; taskTitle: string }>(resolve => { finish = resolve; }));
    render(<Editor workflowSource={source} definitionRevision={2} storageDigest={"b".repeat(64)} onRun={onRun} />);
    await userEvent.click(screen.getByTestId("workflow-recovery-run-start"));
    await waitFor(() => expect(onRun).toHaveBeenCalledOnce());
    const input = screen.getByTestId("workflow-recovery-block-block.design-brief");
    ...
    await act(async () => { finish({ outputPath: "report.html", outputBytes: 100, taskTitle: "Create HTML report" }); });
    expect(input).toHaveAttribute("data-run-state", "idle");
    expect(screen.getByTestId("workflow-recovery-block-block.document-1")).toHaveAttribute("data-run-state", "idle");
  });

  it("keeps verified partial results and the durable log accessible after a later step fails", async () => {
    const produced: WorkspaceEngineeringResult = {schema_version:1,id:"run:first:model",kind:"cad_model",name:"Bracket",exports:[],
      provenance:{run_id:"run",task_id:"first",output_port:"model",input_revisions:[]},
      representations:[{kind:"workspace_file",location:"bracket.psm",format:"psm",provider_id:"",resource_id:"",revision:null,durability:"persistent",sha256:null,size_bytes:42}]};
    const onOpenFile = vi.fn();
    const onRun = vi.fn(async (options?: WorkflowRunOptions) => {
      options?.onEvent?.({kind:"run_started",at:new Date().toISOString(),task_id:"",task_title:"Workflow",run_log_path:"runs/failed.json"});
      options?.onEvent?.({kind:"result_ready",at:new Date().toISOString(),task_id:"first",task_title:"Create bracket",engineering_result:produced});
      throw new Error("Analysis service disconnected");
    });
    render(<Editor workflowSource={reportWorkflowSource} definitionRevision={2} storageDigest={"b".repeat(64)} onRun={onRun} onOpenFile={onOpenFile}/>);
    await userEvent.click(screen.getByTestId("workflow-recovery-run-start"));
    await waitFor(() => expect(screen.getByTestId("workflow-recovery-native-run-mode")).toHaveTextContent("Workflow failed"));
    expect(screen.getByTestId("workflow-recovery-partial-results")).toHaveTextContent("Results produced so far");
    await userEvent.click(screen.getByRole("button",{name:"Open bracket.psm"}));
    expect(onOpenFile).toHaveBeenCalledWith("bracket.psm");
    await userEvent.click(screen.getByText("Technical log and telemetry"));
    await userEvent.click(screen.getByTestId("workflow-recovery-open-run-record"));
  ...
  it("aborts a submitted run without resubmission and preserves its verified result", async () => {
    const produced: WorkspaceEngineeringResult = {schema_version:1,id:"run:first:model",kind:"cad_model",name:"Bracket",exports:[],
      provenance:{run_id:"run",task_id:"first",output_port:"model",input_revisions:[]},
      representations:[{kind:"workspace_file",location:"bracket.psm",format:"psm",provider_id:"",resource_id:"",revision:null,durability:"persistent",sha256:null,size_bytes:42}]};
    const onRun = vi.fn((options?: WorkflowRunOptions) => new Promise<never>((_resolve, reject) => {
      options?.onEvent?.({kind:"result_ready",at:new Date().toISOString(),task_id:"first",task_title:"Create bracket",engineering_result:produced});
    ...
    await waitFor(() => expect(screen.getByTestId("workflow-recovery-native-run-mode")).toHaveTextContent("may still be running"));
  ...
  it("clears a failed run's canvas badge while keeping the error and submitted prompt in run details", async () => {
    const onRun = vi.fn().mockRejectedValue(new Error("Model connection lost. Reconnect the model and run again."));
    render(<Editor workflowSource={reportWorkflowSource} definitionRevision={2} storageDigest={"b".repeat(64)} onRun={onRun} />);
    await userEvent.click(screen.getByTestId("workflow-recovery-run-start"));
    await waitFor(() => expect(screen.getByTestId("workflow-recovery-native-run-mode")).toHaveTextContent("Model connection lost"));
    expect(screen.getByTestId("workflow-recovery-native-run-input")).toHaveTextContent(reportPrompt);
    expect(screen.getByTestId("workflow-recovery-block-block.document-1")).toHaveAttribute("data-run-state", "idle");
  });
  ...
  it("saves the latest prompt and output options before running that exact saved digest", async () => {
    const onSave = vi.fn(async (source: string) => ({ source, definition_revision: 3, storage_digest: "c".repeat(64) }));
    const onRun = vi.fn(async () => ({ outputPath: "report-001.json", outputBytes: 10, taskTitle: "Create HTML report" }));
    render(<Editor workflowSource={reportWorkflowSource} definitionRevision={2} storageDigest={"b".repeat(64)} onSave={onSave} onRun={onRun} />);
    fireEvent.click(screen.getByTestId("workflow-recovery-block-block.document-1"));
    expect(screen.queryByRole("tab", { name: "Settings" })).not.toBeInTheDocument();
    expect(screen.getByTestId("workflow-recovery-save-response")).toBeChecked();
    expect(screen.getByTestId("workflow-recovery-save-response")).toBeDisabled();
    await userEvent.clear(screen.getByTestId("workflow-recovery-block-instructions-block.document-1"));
    await userEvent.type(screen.getByTestId("workflow-recovery-block-instructions-block.document-1"), "Return a JSON answer.");
    fireEvent.change(screen.getByTestId("workflow-recovery-response-format"), { target: { value: "json" } });
    fireEvent.change(screen.getByTestId("workflow-recovery-file-policy"), { target: { value: "overwrite" } });
    await userEvent.click(screen.getByRole("button", { name: "Save & run" }));
    await waitFor(() => expect(onRun).toHaveBeenCalledOnce());
    expect(onSave.mock.calls[0]![0]).toContain('prompt: "Return a JSON answer."');
    expect(onSave.mock.calls[0]![0]).toContain('"output_format":"json"');
    expect(onSave.mock.calls[0]![0]).toContain('"file_policy":"overwrite"');
    expect(onRun).toHaveBeenCalledWith(expect.objectContaining({ expectedStorageDigest: "c".repeat(64) }));
  });
  ...
  it("does not run after a failed save and keeps the prompt draft", async () => {
    ...
    render(<Editor workflowSource={reportWorkflowSource} definitionRevision={2} storageDigest={"b".repeat(64)} onSave={onSave} onRun={onRun} />);
    fireEvent.click(screen.getByTestId("workflow-recovery-block-block.document-1"));
    fireEvent.change(screen.getByTestId("workflow-recovery-block-instructions-block.document-1"), { target: { value: "Keep this edit." } });
    await userEvent.click(screen.getByRole("button", { name: "Save & run" }));
    await waitFor(() => expect(onSave).toHaveBeenCalledOnce());
    expect(onRun).not.toHaveBeenCalled();
    expect(screen.getByTestId("workflow-recovery-block-instructions-block.document-1")).toHaveValue("Keep this edit.");
  });

  it("connects another response as the prompt and derives terminal saving from that connection", async () => {
    render(<Editor workflowSource={reportWorkflowSource} definitionRevision={2} storageDigest={"b".repeat(64)} />);
    await userEvent.click(screen.getByTestId("workflow-recovery-create-group-document"));
    await userEvent.click(screen.getByTestId("workflow-recovery-create-template-ai-prompt"));
    fireEvent.change(screen.getByTestId("workflow-recovery-prompt-source"), { target: { value: "connection" } });
    fireEvent.change(screen.getByTestId("workflow-recovery-prompt-from"), { target: { value: "port.document-1-document-out" } });
    expect(screen.getByTestId("workflow-recovery-prompt-from")).toHaveValue("port.document-1-document-out");
    expect(screen.getByTestId("workflow-recovery-save-response")).toBeDisabled();
    fireEvent.click(screen.getByTestId("workflow-recovery-block-block.document-1"));
    expect(screen.getByTestId("workflow-recovery-save-response")).not.toBeDisabled();
    expect(screen.getByTestId("workflow-recovery-save-response")).not.toBeChecked();
    await userEvent.click(screen.getByTestId("workflow-recovery-view-code"));
    const code = screen.getByTestId<HTMLTextAreaElement>("workflow-recovery-source-editor").value;
  ...
  it("creates independent text inputs by keyboard, persists real text, and cold-reopens authored input", async () => {
    ...
    expect(screen.getByTestId("workflow-recovery-input-text-block.text-input-1")).toHaveValue("Support a 400 N load. Use stainless steel. Review the mounting interface.");
  });
  ...
  it("stores a workspace-relative file path and treats a bare filename as the workspace root", async () => {
    ...
    expect(screen.queryByTestId("workflow-recovery-block-title-block.file-input-1")).not.toBeInTheDocument();
    fireEvent.change(screen.getByTestId("workflow-recovery-input-file-block.file-input-1"), { target: { value: "requirements.docx" } });
    expect(screen.getByTestId("workflow-recovery-inputs-toggle")).toHaveTextContent("1/4 configured");
    expect(screen.getByTestId("workflow-recovery-input-file-block.file-input-1")).toHaveValue("requirements.docx");
    await userEvent.click(screen.getByTestId("workflow-recovery-view-code"));
    expect(screen.getByTestId<HTMLTextAreaElement>("workflow-recovery-source-editor").value).toContain("requirements.docx");
  });
  ...
  it("edits unbound prompts and parameters, confirms deletion, and restores the full object with undo", async () => {
    ...
    fireEvent.change(screen.getByTestId("workflow-recovery-response-format"), { target: { value: "markdown" } });
    ...
    await userEvent.click(screen.getByTestId("workflow-recovery-undo"));
    fireEvent.click(screen.getByTestId(`workflow-recovery-block-${id}`));
    expect(screen.queryByTestId("workflow-recovery-inspector-tab-definition")).not.toBeInTheDocument();
    ...
    expect(screen.getByTestId("workflow-recovery-response-format")).toHaveValue("markdown");
  });
  ...
  it("renders the compact Create rail, transient Inputs and contextual Inspector with stable controls", async () => {
    ...
    expect(screen.getByTestId("workflow-recovery-filebar").querySelector("h1")).toHaveAttribute("title", "mounting-bracket.workflow.wflow · Local preview · not saved");
    expect(screen.queryByRole("heading", { name: "Three source inputs" })).not.toBeInTheDocument();
    expect(screen.getAllByTestId(/workflow-recovery-create-group-/)).toHaveLength(4);
    ...
    expect(screen.queryByRole("tablist", { name: "Step detail sections" })).not.toBeInTheDocument();
    expect(screen.getByLabelText(/^AI prompt/, { selector: "textarea" })).toBeVisible();
    expect(screen.getByTestId("workflow-recovery-block-review-toggle-block.create-design-specification").parentElement).toHaveAttribute("open");
  ...
  it("loads a workspace source, marks semantic edits unsaved, and marks them saved only after success", async () => {
    ...
    expect(screen.getByTestId("workflow-recovery-filebar").querySelector("h1")).toHaveAttribute("title", "bracket.workflow.wflow · Saved in workspace");
    ...
    expect(screen.getByTestId("workflow-recovery-filebar").querySelector("h1")).toHaveAttribute("title", expect.stringContaining("Unsaved changes"));
    expect(screen.getByTestId("workflow-recovery-save-status")).toHaveTextContent("Unsaved");
    expect(screen.getByTestId("workflow-recovery-run-start")).toBeDisabled();
    ...
    expect(screen.getByTestId("workflow-recovery-save-status")).toHaveTextContent("Saved");
    expect(screen.getByTestId("workflow-recovery-filebar")).toHaveTextContent("Saved");
  ...
  it("locks semantic edits and candidate previews to an active run subject while leaving layout interaction available", async () => {
    ...
    fireEvent.doubleClick(screen.getByTestId("workflow-recovery-block-block.design-intent").querySelector("h3")!);
    expect(screen.queryByTestId("workflow-recovery-rename-block.design-intent")).not.toBeInTheDocument();
  ...
  it("protects local source on stale saves until the engineer explicitly compares, copies, or reloads", async () => {
    ...
    renameBlock("block.design-intent", "Local requirements retained after conflict");
    expect(screen.getByTestId("workflow-recovery-save")).toBeDisabled();
