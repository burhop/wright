...
type AuthoringSectionKind =
  "workflow" | "item" | "input" | "task" | "group" | "connection";
...
const TYPE_ID_BY_PUBLIC_PORT_KIND = new Map<PublicPortKind, string>(
  [...PUBLIC_PORT_KIND_BY_TYPE_ID].map(
    ([typeId, kind]) => [kind, typeId] as const,
  ),
);
...
function itemType(artifact: RecoveryArtifactContract): string {
  ...
  if (artifact.mediaType === "application/vnd.wright.context+json")
    return "company_knowledge";
  if (artifact.mediaType === "model/step") return "step_file";
  if (artifact.mediaType.startsWith("model/"))
    return artifact.id.includes("approved")
      ? "approved_cad_model"
      : "cad_model";
...
function performedBy(block: RecoveryBlock): string {
  if (block.kind === "approval" && block.executionKind === "ai_capable")
...
function recoveryConnectionType(
  value: string,
): RecoveryRelationship["kind"] | null {
  ...
  return {
    ...
    item:
      port.artifactContractId === null
        ? null
        : stableKey(port.artifactContractId),
...
function formattedSections(workflow: RecoveryWorkflow): FormattedSection[] {
  ...
  const sections: FormattedSection[] = [
    {
      ...
      fields: [
        ...
        [
          ...
          workflow.metadata.authorship === "human_with_ai_proposal",
        ],
      ],
  ...
  for (const group of projectGroups
    ? [...workflow.phases].sort((left, right) => left.order - right.order)
    : []) {
    sections.push({
      kind: "group",
  ...
  for (const block of workflow.blocks) {
    ...
    const binding =
      block.bindingId === null
        ? null
        : (workflow.bindings.find((entry) => entry.id === block.bindingId) ??
          null);
    const fields: [string, unknown][] = [
      ...
      [
        "group",
        projectGroups && block.phaseId !== null
          ? stableKey(block.phaseId)
          : null,
      ],
      ...(isInput
        ? [["provided_by", providedBy(block)] as [string, unknown]]
        : [["performed_by", performedBy(block)] as [string, unknown]]),
      [
        "inputs",
        block.inputPortIds.map((portId) => sourcePort(workflow, portId)),
      ],
      [
        "outputs",
      ...
      [
        ...
        binding === null
          ? null
          : { assignment: stableKey(binding.id), action: binding.toolId },
      ],
      [
        "reusable_step",
        block.componentRef === null
          ? null
          : {
              key: stableKey(block.componentRef.componentId),
              version: block.componentRef.versionRange,
            },
      ],
    ];
    sections.push({
      ...
      canonicalIds: [
        ...
        ...block.inputPortIds,
        ...block.outputPortIds,
        ...(block.bindingId ? [block.bindingId] : []),
      ],
    });
  }
...
export function formatRecoveryAuthoringSource(workflow: RecoveryWorkflow): {
  text: string;
  ...
  for (const entry of formattedSections(workflow)) {
    ...
    const sectionLines = [
      ...
      ...entry.fields.map(([name, value]) => `  ${name}: ${scalar(value)}`),
      "end",
      "",
    ];
    ...
    const span = {
      startLine,
      endLine: startLine + sectionLines.length - 2,
      startOffset,
      endOffset: offset - 2,
    };
...
function parseSections(text: string): {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (current === null) {
      ...
      const header =
        /^(workflow|item|input|task|group|connection)\s+([a-z][a-z0-9_]*)$/.exec(
          trimmed,
        );
      if (!header) {
        diagnostics.push(
          diagnostic(
            "WFR-SOURCE-SECTION-INVALID",
            `Line ${lineNumber} is not a workflow, item, input, task, optional group, or connection declaration.`,
            "Use '<kind> <stable_name>', indented fields, and 'end'.",
            null,
            lineNumber,
          ),
        );
      ...
      if (keys.has(sectionKey))
        diagnostics.push(
          diagnostic(
            "WFR-SOURCE-KEY-DUPLICATE",
            `The stable name ${header[2]} is declared twice as ${header[1]}.`,
            "Keep one declaration for each stable name.",
            null,
            lineNumber,
          ),
        );
      keys.add(sectionKey);
      current = {
        kind: header[1] as AuthoringSectionKind,
        key: header[2]!,
        fields: {},
    ...
    if (!field) {
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-FIELD-INVALID",
          `Line ${lineNumber} is not an indented name/value field.`,
          "Indent two spaces and write 'field: value', or close the section with 'end'.",
          null,
          lineNumber,
        ),
      );
    ...
    if (RESERVED_HOST_FIELDS.has(field[1]!)) {
      diagnostics.push(
        diagnostic(
          ...
          `${field[1]} is managed by Wright and cannot be assigned in engineering source.`,
          "Remove the field; Wright records accepted version, ancestry, and integrity after validation.",
          null,
          lineNumber,
        ),
      );
    ...
    if (Object.hasOwn(current.fields, field[1]!)) {
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-FIELD-DUPLICATE",
          `${field[1]} is repeated in ${current.kind} ${current.key}.`,
          "Keep one value for each field.",
          null,
          lineNumber,
        ),
      );
    ...
    try {
      ...
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-VALUE-INVALID",
          `Line ${lineNumber} has an invalid value.`,
          "Use a quoted string, JSON array/object, number, boolean, null, or lower_snake_case name.",
          null,
          lineNumber,
        ),
      );
  ...
  if (current !== null)
    diagnostics.push(
      diagnostic(
        "WFR-SOURCE-SECTION-UNTERMINATED",
        `${current.kind} ${current.key} is missing 'end'.`,
        "Add 'end' before the next declaration.",
        null,
        current.startLine,
      ),
    );
...
function requireFields(
  ...
  for (const field of required) {
    if (!Object.hasOwn(section.fields, field))
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-FIELD-MISSING",
          `${section.kind} ${section.key} is missing '${field}'.`,
  ...
  for (const field of Object.keys(section.fields)) {
    if (!allowed.has(field))
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-FIELD-UNKNOWN",
          `${section.kind} ${section.key} contains unknown field '${field}'.`,
          "Use only documented engineering-source fields.",
          null,
          section.startLine,
        ),
      );
  ...
  if (typeof value !== "string" || value.trim() === "") {
    diagnostics.push(
      diagnostic(
        ...
        `${section.kind} ${section.key}.${field} must be a non-empty string or stable name.`,
        "Provide a quoted string or lower_snake_case name.",
        null,
        section.startLine,
      ),
    );
...
function nullableStringField(
  ...
  if (typeof value !== "string" || value.trim() === "") {
    diagnostics.push(
      diagnostic(
        "WFR-SOURCE-FIELD-TYPE",
        `${section.kind} ${section.key}.${field} must be a string or null.`,
        "Provide a quoted string, lower_snake_case name, or null.",
        null,
        section.startLine,
      ),
    );
...
function stringArrayField(
  ...
  if (!Array.isArray(value) || value.some((item) => typeof item !== "string")) {
    diagnostics.push(
      diagnostic(
        "WFR-SOURCE-FIELD-TYPE",
        `${section.kind} ${section.key}.${field} must be an array of stable names.`,
...
function booleanField(
  ...
  if (typeof value !== "boolean") {
    diagnostics.push(
      diagnostic(
        "WFR-SOURCE-FIELD-TYPE",
        `${section.kind} ${section.key}.${field} must be true or false.`,
        "Use an unquoted boolean.",
        null,
        section.startLine,
      ),
    );
...
function numberField(
  ...
  if (typeof value !== "number" || !Number.isInteger(value) || value < 0) {
    diagnostics.push(
      diagnostic(
        "WFR-SOURCE-FIELD-TYPE",
        `${section.kind} ${section.key}.${field} must be a non-negative integer.`,
        "Use an unquoted whole number.",
        null,
        section.startLine,
      ),
    );
...
function objectOrNullField(
  ...
  if (
    ...
    diagnostics.push(
      diagnostic(
        "WFR-SOURCE-FIELD-TYPE",
        `${section.kind} ${section.key}.${field} must be an object or null.`,
        "Use a JSON object or null.",
        null,
        section.startLine,
      ),
    );
...
function sourceMapForResolved(
  section: AuthoringSection,
  canonicalIds: readonly string[],
  map: Record<string, RecoverySourceMapEntry>,
): void {
  const span = {
...
function parsePortList(
  ...
  if (!Array.isArray(value)) {
    diagnostics.push(
      diagnostic(
        "WFR-SOURCE-FIELD-TYPE",
        `${section.kind} ${section.key}.${field} must be an array of connection-point objects.`,
        "Use the connection points emitted by Diagram or add a complete object with key, name, kind, item, required, quantity, and description.",
        ownerBlockId,
        section.startLine,
      ),
    );
  ...
  const direction: RecoveryPort["direction"] =
    field === "inputs" ? "input" : "output";
  const basePortsByKey = new Map(
    base.ports.map((port) => [stableKey(port.id), port]),
  );
  ...
  for (const raw of value) {
    if (raw === null || typeof raw !== "object" || Array.isArray(raw)) {
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-PORT-INVALID",
          `${section.kind} ${section.key}.${field} contains an invalid connection point.`,
          "Use a JSON object for each connection point.",
          ownerBlockId,
          section.startLine,
        ),
      );
    ...
    if (
      requiredFields.some((key) => !Object.hasOwn(port, key)) ||
      Object.keys(port).some((key) => !requiredFields.includes(key))
    ) {
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-PORT-FIELDS",
          `${section.kind} ${section.key}.${field} has incomplete or unknown connection-point fields.`,
          "Use exactly key, name, kind, item, required, quantity, and description.",
          ownerBlockId,
          section.startLine,
        ),
      );
    ...
    if (
      ...
      !["one", "optional", "many"].includes(String(quantity)) ||
      (item !== null &&
      ...
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-PORT-INVALID",
          `${section.kind} ${section.key}.${field} contains an invalid connection-point value.`,
          "Use a stable key, clear names/types/descriptions, true or false, one/optional/many, and an item stable name or null.",
          ownerBlockId,
          section.startLine,
        ),
      );
    ...
    const mappedTypeId = TYPE_ID_BY_PUBLIC_PORT_KIND.get(
      kind as PublicPortKind,
    );
    if (!mappedTypeId) {
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-PORT-KIND",
          `Connection point '${key}' uses unknown engineering kind '${kind}'.`,
          `Use a supported kind: ${[...TYPE_ID_BY_PUBLIC_PORT_KIND.keys()].join(", ")}.`,
          ownerBlockId,
          section.startLine,
        ),
      );
    ...
    if (seen.has(key)) {
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-PORT-DUPLICATE",
          `${section.kind} ${section.key} repeats connection point '${key}'.`,
          "Keep each connection point once.",
          ownerBlockId,
          section.startLine,
        ),
      );
    ...
    if (item !== null && !artifact)
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-ITEM-UNKNOWN",
          `Connection point '${key}' refers to unknown engineering item '${item}'.`,
          "Declare the item or select an existing item stable name.",
          ownerBlockId,
          section.startLine,
        ),
      );
    ...
    if (basePort && basePort.typeId !== mappedTypeId) {
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-PORT-KIND",
          `Connection point '${key}' is '${PUBLIC_PORT_KIND_BY_TYPE_ID.get(basePort.typeId) ?? "an unsupported internal kind"}', not '${kind}'.`,
          "Restore the accepted engineering kind; changing a connection-point kind requires a reviewed structural operation.",
          ownerBlockId,
          section.startLine,
        ),
      );
    ...
    if (artifact && artifact.typeId !== typeId) {
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-PORT-ITEM-TYPE",
          `Connection point '${key}' kind '${kind}' does not match engineering item '${item}'.`,
          "Select an item with the same engineering kind, or correct the connection-point kind.",
          ownerBlockId,
          section.startLine,
        ),
      );
...
function executionKindForActor(
  actor: string,
): RecoveryBlock["executionKind"] | null {
  if (actor === "engineer") return "human";
  if (actor === "configured_tool" || actor === "company_library")
    return "deterministic";
  if (actor === "ai_assisted" || actor === "ai_then_engineer")
    return "ai_capable";
  return null;
}

export function parseRecoveryAuthoringSource(
  text: string,
  base: RecoveryWorkflow,
): RecoveryParseResult {
  const parsed = parseSections(text.replace(/\r\n?/g, "\n"));
  const diagnostics = [...parsed.diagnostics];
  const sourceMap: Record<string, RecoverySourceMapEntry> = {};
  const candidate = cloneWorkflow(base);
  const sectionsByKind = (kind: AuthoringSectionKind) =>
    parsed.sections.filter((section) => section.kind === kind);

  const workflowSections = sectionsByKind("workflow");
  if (workflowSections.length !== 1)
    diagnostics.push(
      diagnostic(
        "WFR-SOURCE-WORKFLOW-COUNT",
        `Engineering source must contain exactly one workflow section; found ${workflowSections.length}.`,
        "Keep one workflow declaration.",
      ),
    );
  const workflowSection = workflowSections[0];
  if (workflowSection) {
    diagnostics.push(
      ...requireFields(workflowSection, [
        "name",
        "purpose",
        "discipline",
        "reviewed_ai_suggestions",
    ...
    if (workflowSection.key !== stableKey(base.workflowId))
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-IDENTITY-UNKNOWN",
          `Workflow stable name '${workflowSection.key}' does not match this workflow.`,
          `Keep '${stableKey(base.workflowId)}'; rename the displayed name instead.`,
          base.workflowId,
          workflowSection.startLine,
        ),
      );
    ...
    if (reviewedAi !== null)
      candidate.metadata.authorship = reviewedAi
        ? "human_with_ai_proposal"
        : "human";
  ...
  const groupsByKey = new Map(
    candidate.phases.map((phase) => [stableKey(phase.id), phase]),
  );
  for (const group of groupSections) {
    ...
    if (!phase) {
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-GROUP-UNKNOWN",
          `Optional group '${group.key}' is not part of this workflow.`,
          "Remove it; group creation is a separate reviewed structural operation.",
          null,
          group.startLine,
        ),
      );
  ...
  const itemSections = new Map(
    sectionsByKind("item").map((section) => [section.key, section]),
  );
  ...
  const artifactsByKey = new Map(
    candidate.artifactContracts.map((artifact) => [
  ...
  for (const artifact of candidate.artifactContracts) {
    ...
    if (!section) continue;
    diagnostics.push(
      ...requireFields(section, ["name", "type", "formats", "description"]),
    );
    ...
    if (type && type !== itemType(artifact))
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-TYPE-CHANGE-UNSUPPORTED",
          `Item '${key}' is '${itemType(artifact)}', not '${type}'.`,
          "Restore the accepted engineering item type.",
          artifact.id,
          section.startLine,
        ),
      );
    if (formats && !same(formats, itemFormats(artifact)))
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-FORMAT-CHANGE-UNSUPPORTED",
          `Item '${key}' formats do not match its accepted file contract.`,
          `Restore ${stableJson(itemFormats(artifact))}.`,
          artifact.id,
          section.startLine,
        ),
      );
  ...
  for (const section of itemSections.values())
    diagnostics.push(
      diagnostic(
        "WFR-SOURCE-IDENTITY-UNKNOWN",
        `Unknown engineering item '${section.key}'.`,
        "Create new item contracts through a reviewed structural command.",
        null,
        section.startLine,
      ),
    );

  const baseBlocksByKey = new Map(
    base.blocks.map((block) => [stableKey(block.id), block]),
  );
  const blockSections = parsed.sections.filter(
    (section) => section.kind === "input" || section.kind === "task",
  );
  const blockKeys = new Set<string>();
  const nextBlocks: RecoveryBlock[] = [];
  const nextPorts: RecoveryPort[] = [];
  const bindingByKey = new Map(
    candidate.bindings.map((binding) => [stableKey(binding.id), binding]),
  );
  ...
  for (const section of blockSections) {
    if (blockKeys.has(section.key)) {
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-KEY-DUPLICATE",
          `Workflow step '${section.key}' is declared more than once.`,
          "Keep exactly one input or task section for each stable step name.",
          null,
          section.startLine,
        ),
      );
    ...
    const expectedKind: AuthoringSectionKind = baseBlock
      ? recoveryAuthoringSectionKind(baseBlock)
      : section.kind;
    if (baseBlock && section.kind !== expectedKind)
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-STEP-KIND",
          `'${section.key}' must be declared as ${expectedKind}, not ${section.kind}.`,
          `Change the section keyword to '${expectedKind}'.`,
          baseBlock.id,
          section.startLine,
        ),
      );
    ...
    if (stepTypeValue && !stepType)
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-STEP-TYPE",
          `Workflow step '${section.key}' has unknown step_type '${stepTypeValue}'.`,
          "Use work, review, decision, or reusable_step.",
          baseBlock?.id ?? null,
          section.startLine,
        ),
      );
    if (actor && !executionKind)
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-ACTOR",
          `Workflow step '${section.key}' has unknown performer '${actor}'.`,
    ...
    if (groupKey && !phase)
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-GROUP-UNKNOWN",
          `Workflow step '${section.key}' refers to unknown group '${groupKey}'.`,
          "Use an existing optional group stable name or null.",
          baseBlock?.id ?? null,
          section.startLine,
        ),
      );
    ...
    if (tool) {
      if (
        Object.keys(tool).some(
          (key) => !["assignment", "action"].includes(key),
        ) ||
        typeof tool.assignment !== "string" ||
        (tool.action !== null && typeof tool.action !== "string")
      )
        diagnostics.push(
          diagnostic(
            "WFR-SOURCE-TOOL-INVALID",
            `Workflow step '${section.key}' has an invalid tool assignment.`,
            'Use {"assignment":"stable_name","action":"tool.action"} or null.',
            blockId,
            section.startLine,
          ),
        );
        ...
        if (!binding)
          diagnostics.push(
            diagnostic(
              ...
              `Workflow step '${section.key}' refers to unknown tool assignment '${tool.assignment}'.`,
              "Select a configured assignment from Technical details.",
              blockId,
              section.startLine,
            ),
          );
    ...
    if (reusable) {
      if (
        Object.keys(reusable).some(
          (key) => !["key", "version"].includes(key),
        ) ||
        typeof reusable.key !== "string" ||
        typeof reusable.version !== "string"
      )
        diagnostics.push(
          diagnostic(
            "WFR-SOURCE-REUSABLE-INVALID",
            `Workflow step '${section.key}' has an invalid reusable_step reference.`,
            'Use {"key":"stable_name","version":"range"} or null.',
            blockId,
            section.startLine,
          ),
        );
        ...
        if (!component)
          diagnostics.push(
            diagnostic(
              "WFR-SOURCE-REUSABLE-UNKNOWN",
              `Workflow step '${section.key}' refers to unknown reusable step '${reusable.key}'.`,
              "Select an installed reusable step.",
              blockId,
              section.startLine,
            ),
          );
    ...
    const block: RecoveryBlock = {
      ...
      phaseId:
        phase?.id ??
        (groupSections.length === 0 && baseBlock ? baseBlock.phaseId : null),
    ...
    diagnostics.push(
      ...validateAuthoringConfiguration(block).map((item) => ({
        ...item,
  ...
  const retainedBindings = new Set(
    nextBlocks.flatMap((block) => (block.bindingId ? [block.bindingId] : [])),
  );
  const retainedComponents = new Set(
    nextBlocks.flatMap((block) =>
      block.componentRef ? [block.componentRef.componentId] : [],
    ),
  );
  candidate.bindings = candidate.bindings.filter((binding) =>
    retainedBindings.has(binding.id),
  );
  candidate.components = candidate.components.filter((component) =>
    retainedComponents.has(component.id),
  );
  for (const phase of candidate.phases)
    phase.blockIds = nextBlocks
      .filter((block) => block.phaseId === phase.id)
      .map((block) => block.id);

  const blocksByKey = new Map(
    candidate.blocks.map((block) => [stableKey(block.id), block]),
  );
  const portsByOwnerAndKey = new Map(
    candidate.ports.map((port) => [
      `${stableKey(port.ownerBlockId)}.${stableKey(port.id)}`,
      port,
    ]),
  );
  const baseRelationshipsByKey = new Map(
    base.relationships.map((relationship) => [
      stableKey(relationship.id),
      relationship,
    ]),
  );
  const resolveEndpoint = (
    ...
    diagnostics.push(
      diagnostic(
        "WFR-SOURCE-ENDPOINT-UNKNOWN",
        `Connection '${section.key}' refers to unknown endpoint '${value}'.`,
        "Use task_name.connection_point for items or task_name for approval/order/revision paths.",
        null,
        section.startLine,
      ),
    );
  ...
  for (const section of sectionsByKind("connection")) {
    diagnostics.push(
      ...requireFields(section, ["type", "from", "to", "label", "when"]),
    );
    ...
    if (typeValue && !kind)
      diagnostics.push(
        diagnostic(
          "WFR-SOURCE-CONNECTION-TYPE",
          `Connection '${section.key}' has unknown type '${typeValue}'.`,
          "Use item, order, approval, or revision.",
          null,
          section.startLine,
        ),
      );
    ...
    if (kind && sourceId && targetId && label && when !== undefined)
  ...
  if (diagnostics.length > 0)
    return { ok: false, workflow: null, diagnostics, sourceMap };
  const validated = parseRecoveryDsl(formatRecoveryDsl(candidate).text);
  if (!validated.ok || validated.workflow === null)
    return {
      ok: false,
      workflow: null,
...
export function validateRecoveryAuthoringRoundTrip(
  ...
  if (
    canonicalDefinitionBytes(parsed.workflow, true) !==
    canonicalDefinitionBytes(workflow, true)
  ) {
    return {
      ok: false,
      workflow: null,
      diagnostics: [
        diagnostic(
          "WFR-SOURCE-ROUNDTRIP-LOSS",
          "The engineering source cannot reconstruct every semantic fact in this candidate.",
          "The accepted workflow was kept unchanged. Add a lossless source representation before accepting or saving this change.",
        ),
      ],
...
export function recoveryAuthoringSemanticIdAtOffset(
  ...
  const matches = Object.entries(sourceMap)
    .filter(
      ([, span]) => span.startOffset <= offset && offset <= span.endOffset,
    )
    .sort(([leftId, leftSpan], [rightId, rightSpan]) => {
      const spanLength =
        leftSpan.endOffset -
        leftSpan.startOffset -
        (rightSpan.endOffset - rightSpan.startOffset);
      if (spanLength !== 0) return spanLength;
      const priority =
        sourceSelectionPriority(leftId) - sourceSelectionPriority(rightId);
