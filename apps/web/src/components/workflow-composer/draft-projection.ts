...
export interface DraftPortProjection extends DraftPort {
  readonly semanticId: string;
}
export interface DraftGateProjection extends DraftGate {
  readonly semanticId: string;
}
export interface DraftArtifactProjection extends DraftArtifact {
  readonly semanticId: string;
}
export type DraftComponentConceptKind =
  | "block"
  | "port"
  | "relationship"
  | "artifact_contract"
...
function required<T>(registry: ReadonlyMap<string, T>, id: string): T {
  ...
  if (value === undefined)
    throw new Error(`WORKFLOW_DRAFT_REFERENCE_UNRESOLVED:${id}`);
  return value;
}
...
export function buildDraftProjection(draft: WorkflowDraft): DraftProjection {
  ...
  const artifacts = new Map(
    source.semantic.intended_artifacts.map((item) => [item.id, item]),
  );
  const positions = new Map(
    source.layout.positions.map((item) => [item.semantic_id, item]),
  );
  if (
    positions.size !== source.layout.positions.length ||
    positions.size !== blocks.size
  ) {
    throw new Error("WORKFLOW_DRAFT_LAYOUT_POSITION_IDENTITY_INVALID");
  }
  const portProjection = (id: string): DraftPortProjection => ({
    ...required(ports, id),
    semanticId: id,
  });
  const gateProjection = (id: string): DraftGateProjection => ({
    ...required(gates, id),
    semanticId: id,
  });
  const artifactProjection = (id: string): DraftArtifactProjection => ({
  ...
  const portOwners = new Map(
    source.semantic.ports.map((item) => [item.id, item.owner_block_id]),
  );
  const projection: DraftProjection = {
    ...
    phases: source.semantic.phases.map((phase, index) => {
      if (phase.order !== index)
        throw new Error("WORKFLOW_DRAFT_PHASE_ORDER_INVALID");
      return {
        ...phase,
        semanticId: phase.id,
...
export function draftSemanticIds(draft: WorkflowDraft): readonly string[] {
  return requireUniqueSemanticIds(
    draftSemanticReferences(draft).map((item) => item.semanticId),
  );
}

export function projectionSemanticIds(
  projection: DraftProjection,
): readonly string[] {
  const ids = [
    ...projection.phases.map((item) => item.semanticId),
    ...projection.phases.flatMap((phase) =>
      phase.blocks.map((item) => item.semanticId),
    ),
    ...projection.phases.flatMap((phase) =>
      phase.blocks.flatMap((block) =>
        [...block.inputs, ...block.outputs].map((item) => item.semanticId),
      ),
    ),
    ...projection.connections.map((item) => item.semanticId),
    ...projection.phases.flatMap((phase) =>
      phase.blocks.flatMap((block) =>
        block.gates.map((item) => item.semanticId),
      ),
    ),
    ...projection.feedbackPaths.map((item) => item.semanticId),
    ...projection.phases.flatMap((phase) =>
      phase.blocks.flatMap((block) =>
        block.artifacts.map((item) => item.semanticId),
      ),
    ),
  ];
