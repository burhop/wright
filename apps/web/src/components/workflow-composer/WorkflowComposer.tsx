...
export function WorkflowComposer({
  initialDraft,
  initialEtag,
  renderer: Renderer = FirstPartyDraftCanvas,
}: WorkflowComposerProps): React.ReactNode {
  ...
  const [busyAction, setBusyAction] = useState<
    "validate" | "save" | "reopen" | null
  >(null);
  const [paletteBusy, setPaletteBusy] = useState(false);
  const [validation, setValidation] = useState<WorkflowDraftValidation | null>(
    null,
  );
  const [localDiagnostics, setLocalDiagnostics] = useState<
    readonly DraftDiagnostic[]
  >([]);
  const [staleCurrent, setStaleCurrent] = useState<WorkflowDraftResult | null>(
    null,
  );
  ...
  const handleIntent = useCallback(
    (intent: DraftCanvasIntent) => {
      void reduceDraftCanvasIntent(
        ...
        .catch(() => {
          setNotice(
            "Edit could not be applied. The last valid working draft remains active.",
          );
        });
    },
  ...
  const addBoundedBlock = async (role: RepresentativeRole): Promise<void> => {
    ...
    try {
      ...
      setNotice(
        "That block cannot be added in the bounded composition order. The working draft was not changed.",
      );
    } finally {
  ...
  const validate = async (): Promise<void> => {
    ...
    try {
      ...
      setNotice(
        "Validation could not complete. The current working draft was not changed.",
      );
    } finally {
      setBusyAction(null);
    }
  ...
  const save = async (): Promise<void> => {
    if (etag === null) {
      setNotice(
        "This local projection has no save identity. Reopen it from the draft service before saving.",
      );
      return;
    }
    setBusyAction("save");
    ...
    try {
      ...
      if (!result.valid) {
        setNotice(
          `Save stopped: resolve ${result.diagnostics.length} validation diagnostic${result.diagnostics.length === 1 ? "" : "s"}.`,
        );
        return;
      }
      ...
      if (
        error instanceof WorkflowDraftClientError &&
        error.errorCode === "WORKFLOW_DRAFT_STALE_REVISION"
      ) {
        try {
          const current = await readWorkflowDraft(draft.draft_id);
          setStaleCurrent(current);
          setNotice(
            `Save stopped because the draft changed. Your local candidate is preserved; current saved revision ${current.draft.revision} is available to load explicitly.`,
          );
        } catch {
          setNotice(
            "Save stopped because the draft changed. Reopen the current draft before retrying.",
          );
        }
      } else {
  ...
  const reopen = async (): Promise<void> => {
    ...
    try {
      ...
      setSelectedSemanticId(null);
      setNotice(
        `Reopened revision ${current.draft.revision} with its saved semantic and layout identities.`,
      );
    } catch {
      setNotice(
        "The saved draft could not be reopened. No local draft bytes were replaced.",
      );
    } finally {
      setBusyAction(null);
    }
  ...
  if (closed) {
    return (
      <section
        className="workflow-composer workflow-composer__closed"
        data-testid="workflow-composer-closed"
      >
        <p className="workflow-composer__eyebrow">Working draft closed</p>
        <h1>{draft.semantic.title}</h1>
        <p>
          The editor session is closed. Saved revision {draft.revision} remains
          in Wright's local draft store.
        </p>
        <dl>
          <dt>Draft</dt>
          <dd>
          ...
          <dd>
            <code>{draft.semantic_sha256}</code>
          </dd>
          <dt>Layout digest</dt>
          <dd>
            <code>{draft.layout_sha256}</code>
          </dd>
        </dl>
        <button
          data-testid="workflow-composer-reopen"
          type="button"
        ...
        {notice !== null && (
          <p className="workflow-composer__notice" role="status">
            {notice}
          </p>
        )}
      </section>
  ...
  return (
    <section className="workflow-composer" data-testid="workflow-composer">
      <header className="workflow-composer__header">
        <div>
          <p className="workflow-composer__eyebrow">
            Workflow Composer · provisional
          </p>
          <h1>{projection.title}</h1>
          <p>{projection.purpose}</p>
        </div>
        <div
          className="workflow-composer__authority"
          role="note"
          aria-label="Working draft authority"
        >
      ...
      <nav
        className="workflow-composer__commandbar"
      ...
      >
        <div className="workflow-composer__palette" aria-label="Block palette">
          ...
          {PALETTE.map((item) =>
            (() => {
              const added = draft.semantic.blocks.some(
                (block) => block.role === item.role,
              );
              const enabled =
                nextRole === item.role && busyAction === null && !paletteBusy;
              return (
                <button
                ...
                >
                  <b aria-hidden="true">{added ? "✓" : item.symbol}</b>{" "}
                  {item.label}
                  {added ? " · added" : ""}
                </button>
              );
        ...
        <div
          className="workflow-composer__view-controls"
        ...
        >
          ...
          <button
            data-testid="workflow-composer-validate"
            type="button"
            disabled={busyAction !== null}
          ...
          <button
            data-testid="workflow-composer-save"
            type="button"
            disabled={
              busyAction !== null || etag === null || staleCurrent !== null
            }
            onClick={() => void save()}
          >
          ...
          <button
            data-testid="workflow-composer-close"
            type="button"
            disabled={busyAction !== null}
            onClick={() => {
              setClosed(true);
      ...
      {staleCurrent !== null && (
        <div className="workflow-composer__stale" role="alert">
          ...
          <p>
            Your local candidate remains visible. Loading revision{" "}
            {staleCurrent.draft.revision} will replace it.
          </p>
          <button
            ...
            onClick={() => {
              ...
              setLocalDiagnostics([]);
              setNotice(
                `Loaded current saved revision ${staleCurrent.draft.revision}.`,
              );
            }}
          >
      ...
      <DraftDiagnosticProvider diagnostics={diagnostics}>
        <div
          className={`workflow-composer__workspace workflow-composer__workspace--${view}`}
        >
          {view !== "text" && (
          ...
          {view !== "diagram" && (
            <DraftTextProjection projection={projection} />
          )}
          <DraftInspector
            projection={projection}
            selectedSemanticId={selectedSemanticId}
