...
function allBlocks(projection: DraftProjection): readonly DraftBlockProjection[] {
  return projection.phases.flatMap((phase) => phase.blocks);
}

export function validateComponentProjection(projection: DraftProjection): readonly string[] {
  const errors: string[] = [];
  ...
  for (const component of projection.components) {
    ...
    if (component.internalAddresses.length === 0) {
      errors.push(`WORKFLOW_COMPONENT_INTERNAL_ADDRESS_EMPTY:${component.semanticId}`);
    }
    const semanticIds = new Set<string>();
    const paths = new Set<string>();
    for (const address of component.internalAddresses) {
      if (!address.semanticId.startsWith(`${component.semanticId}.`)) {
        errors.push(`WORKFLOW_COMPONENT_INTERNAL_ADDRESS_SCOPE_INVALID:${address.semanticId}`);
      }
      if (
        !address.relativePath.startsWith(PATH_ROOTS[address.conceptKind])
        || address.relativePath.split("/").some((segment) => segment === "." || segment === "..")
      ) {
        errors.push(`WORKFLOW_COMPONENT_INTERNAL_ADDRESS_PATH_INVALID:${address.relativePath}`);
      }
      if (semanticIds.has(address.semanticId) || paths.has(address.relativePath)) {
        errors.push(`WORKFLOW_COMPONENT_INTERNAL_ADDRESS_DUPLICATE:${component.semanticId}`);
      }
  ...
  for (const block of allBlocks(projection)) {
    if (block.componentRef !== null && block.componentRef !== undefined && !componentIds.has(block.componentRef.componentId)) {
      errors.push(`WORKFLOW_COMPONENT_REFERENCE_UNRESOLVED:${block.semanticId}`);
    }
  }
...
export function resolveComponentScope(
  ...
  const instance = allBlocks(projection).find((block) => block.semanticId === componentInstanceId);
  if (instance?.componentRef === null || instance?.componentRef === undefined) {
    throw new Error(`WORKFLOW_COMPONENT_INSTANCE_MISSING:${componentInstanceId}`);
  }
  const component = projection.components.find((item) => item.semanticId === instance.componentRef?.componentId);
  if (component === undefined) throw new Error(`WORKFLOW_COMPONENT_REFERENCE_MISSING:${instance.componentRef.componentId}`);
  if (!component.internalAddresses.some((address) => address.semanticId === internalSemanticId)) {
    throw new Error(`WORKFLOW_COMPONENT_INTERNAL_ADDRESS_MISSING:${internalSemanticId}`);
  }
...
export function projectComponentStates(
  ...
  return Object.freeze(allBlocks(projection).flatMap((block) => {
    if (block.componentRef === null || block.componentRef === undefined) return [];
    const component = projection.components.find((item) => item.semanticId === block.componentRef?.componentId);
    ...
    return [Object.freeze({
      ...
      targetedInternalSemanticIds: Object.freeze(targetScopes
        .filter((scope) => scope.componentInstanceId === block.semanticId && scope.componentId === component.semanticId)
...
export function findBlockByIdentity(
  ...
  const blocks = [...allBlocks(projection)].sort((left, right) => left.semanticId.localeCompare(right.semanticId));
  return blocks.find((block) => block.semanticId.toLocaleLowerCase() === normalized)
    ?? blocks.find((block) => block.title.toLocaleLowerCase() === normalized)
    ?? blocks.find((block) => block.semanticId.toLocaleLowerCase().includes(normalized) || block.title.toLocaleLowerCase().includes(normalized))
    ?? null;
}
