...
function state(draft: WorkflowDraft = decodeWorkflowDraft(fixture)): DraftIntentState {
...
describe("reduceDraftCanvasIntent", () => {
  ...
  it("creates a host-identified block atomically and preserves revision authority", async () => {
    ...
    expect(result.lastValidDraft.semantic.phases.find((phase) => phase.id === "phase.release")?.block_ids)
      .toContain(added?.id);
  ...
  it("moves and edits only the selected block while retaining unrelated identities", async () => {
    ...
    expect(moved.lastValidDraft.semantic_sha256).toBe(source.lastValidDraft.semantic_sha256);
    expect(moved.lastValidDraft.layout_sha256).not.toBe(source.lastValidDraft.layout_sha256);
    expect(edited.lastValidDraft.layout_sha256).toBe(moved.lastValidDraft.layout_sha256);
    expect(edited.lastValidDraft.semantic.blocks.map((block) => block.id))
      .toEqual(source.lastValidDraft.semantic.blocks.map((block) => block.id));
    expect(edited.lastValidDraft.semantic.blocks.find((block) => block.id === "block.review-product-definition")?.title)
  ...
  it("creates and deletes one compatible connection without mutating ports", async () => {
    ...
    expect(restored.lastValidDraft.semantic.connections.at(-1)?.id)
      .toMatch(/^connection\.draft-[0-9]+$/);
    expect(restored.lastValidDraft.semantic.ports).toEqual(source.lastValidDraft.semantic.ports);
  ...
  it("creates and edits gate, feedback, and intended artifact relationships atomically", async () => {
    ...
    expect(feedbackEdited.lastValidDraft.semantic.gates.at(-1)?.revise_target_block_id)
    ...
    const artifact = artifactCreated.lastValidDraft.semantic.intended_artifacts.at(-1);
    expect(artifact?.id).toMatch(/^artifact\.draft-[0-9]+$/);
    expect(artifactCreated.lastValidDraft.semantic.blocks.find(
      (block) => block.id === "block.review-product-definition",
    )?.intended_artifact_ids).toContain(artifact?.id);
  ...
  it("rejects dangling block deletion and recovers with a corrected intent", async () => {
    ...
    expect(rejected.diagnostics[0]).toEqual(expect.objectContaining({
    ...
    expect(corrected.lastValidDraft.layout.positions.find(
      (position) => position.semantic_id === "block.review-product-definition",
