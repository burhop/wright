  ...
  return (
    ...
    >
      <h2 id="workflow-composer-diagnostics-heading">
        Edit not applied · {diagnostics.length} diagnostic
        {diagnostics.length === 1 ? "" : "s"}
      </h2>
      <p>
        The last valid working draft and current saved revision are unchanged.
      </p>
      <ol>
        {diagnostics.map((item) => (
          ...
          >
            ...
            <span>
              <b>Affected:</b>{" "}
              {item.affected_semantic_ids.join(", ") || "Complete draft"}
            </span>
            <span>
              <b>Correction:</b> {item.correction}
            </span>
          </li>
        ))}
      </ol>
