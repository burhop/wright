...
describe("component and large-graph projection behavior", () => {
  it("keeps a stable scoped internal identity addressable while its instance is collapsed", () => {
    ...
    const state = projectComponentStates(projection, new Set(["block.review-design"]), [scope])[0];
    expect(state).toMatchObject({
      instanceSemanticId: "block.review-design",
      collapsed: true,
      internalAddressCount: 2,
      targetedInternalSemanticIds: ["component.review-cell.relationship.accept"],
    });
  });
  ...
  it("fails closed for duplicate, escaping, or out-of-scope internal addresses", () => {
    ...
    expect(() => resolveComponentScope(invalid, "block.review-design", "component.other.block.escape"))
  ...
  it("uses deterministic detail levels and stable identity-first graph navigation", () => {
    ...
    expect(findBlockByIdentity(projection, "block.review-design")?.semanticId).toBe("block.review-design");
    expect(findBlockByIdentity(projection, "review design")?.semanticId).toBe("block.review-design");
