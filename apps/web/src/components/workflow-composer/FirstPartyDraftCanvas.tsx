...
export function FirstPartyDraftCanvas({
  ...
  const fit = (): void => {
    ...
    setZoom(
      Math.max(
        MIN_ZOOM,
        Math.min(MAX_ZOOM, Math.floor((viewportWidth / boardWidth) * 100)),
      ),
    );
  };
  ...
  return (
    <section
      ...
      data-testid="workflow-composer-canvas"
      aria-label="Workflow draft diagram"
    >
      <header className="workflow-canvas__toolbar">
        <div>
          <p className="workflow-composer__eyebrow">First-party diagram</p>
          <strong>
            {blocks.length} blocks · {projection.phases.length} phases
          </strong>
        </div>
        <div
          className="workflow-canvas__zoom"
        ...
        >
          ...
          <button data-testid="workflow-canvas-fit" type="button" onClick={fit}>
            Fit
          </button>
        </div>
      </header>
      ...
      <div className="workflow-canvas__viewport" ref={viewportRef}>
        <div
          className="workflow-canvas__board"
          ref={boardRef}
          style={
            { "--workflow-canvas-zoom": zoom / 100 } as React.CSSProperties
          }
        >
                ...
                <div className="workflow-canvas__blocks">
                  {phase.blocks.map((block) => (
                    <article
                      ...
                      data-selected={
                        selectedSemanticId === block.semanticId
                          ? "true"
                          : "false"
                      }
                      ...
                      data-diagnostic={
                        diagnosticIds.has(block.semanticId) ? "true" : "false"
                      }
                    ...
                    >
                      ...
                      >
                        ...
                        <small>
                          Layout {block.position.x}, {block.position.y}
                          {selectedSemanticId === block.semanticId
                            ? " · Selected"
                            : ""}
                        </small>
                        ...
                        <div>
                          ...
                          {block.inputs.length > 0 ? (
                            block.inputs.map((port) => (
                              <span
                                className="workflow-canvas__port"
                                data-direction="input"
                                data-semantic-id={port.semanticId}
                                data-diagnostic={
                                  diagnosticIds.has(port.semanticId)
                                    ? "true"
                                    : "false"
                                }
                                key={port.semanticId}
                      ...
                      {block.gates.map((gate) => (
                        <section
                          className="workflow-canvas__gate"
                          data-semantic-id={gate.semanticId}
                          data-diagnostic={
                            diagnosticIds.has(gate.semanticId)
                              ? "true"
                              : "false"
                          }
                          key={gate.semanticId}
                      ...
                      {block.artifacts.map((artifact) => (
                        <section
                          className="workflow-canvas__artifact"
                          data-semantic-id={artifact.semanticId}
                          data-diagnostic={
                            diagnosticIds.has(artifact.semanticId)
                              ? "true"
                              : "false"
                          }
                  ...
                  {phase.blocks.length === 0 && (
                    <p className="workflow-canvas__empty">
                      Choose a block from the palette to begin this provisional
                      phase.
                    </p>
                  )}
                </div>
          ...
          <section
            className="workflow-canvas__relationships"
            aria-label="Declared relationships"
          >
            <div>
              ...
              {projection.connections.map((connection) => (
                <p
                  ...
                  data-diagnostic={
                    diagnosticIds.has(connection.semanticId) ? "true" : "false"
                  }
            ...
            <div>
              ...
              {projection.feedbackPaths.map((feedback) => (
                <p
                  ...
                  data-diagnostic={
                    diagnosticIds.has(feedback.semanticId) ? "true" : "false"
                  }
                  key={feedback.semanticId}
                >
                  <strong>↩</strong> <code>{feedback.semanticId}</code>
                  <span>
                    {feedback.from_gate_id} → {feedback.to_block_id}
                  </span>
