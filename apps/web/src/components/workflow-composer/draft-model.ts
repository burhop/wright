...
export type DraftFeedbackPath =
  WorkflowDraft["semantic"]["feedback_paths"][number];
export type DraftArtifact =
  WorkflowDraft["semantic"]["intended_artifacts"][number];
...
export function draftSemanticReferences(
  ...
  return [
    ...draft.semantic.phases.map((item) => ({
      semanticId: item.id,
      kind: "phase" as const,
    })),
    ...draft.semantic.blocks.map((item) => ({
      semanticId: item.id,
      kind: "block" as const,
    })),
    ...draft.semantic.ports.map((item) => ({
      semanticId: item.id,
      kind: "port" as const,
    })),
    ...draft.semantic.connections.map((item) => ({
      semanticId: item.id,
      kind: "connection" as const,
    })),
    ...draft.semantic.gates.map((item) => ({
      semanticId: item.id,
      kind: "gate" as const,
    })),
    ...draft.semantic.feedback_paths.map((item) => ({
      semanticId: item.id,
      kind: "feedback_path" as const,
    })),
    ...draft.semantic.intended_artifacts.map((item) => ({
      semanticId: item.id,
      kind: "intended_artifact" as const,
    })),
  ];
}

export function requireUniqueSemanticIds(
  ids: readonly string[],
): readonly string[] {
