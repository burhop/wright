...
export function RivetWorkflowsPanel({ sessionId, workspaceId }: { sessionId: string | null; workspaceId: string | null }) {
  ...
  const [message, setMessage] = useState("Workflows remain inside this workspace.");

  const refresh = useCallback(async () => {
    if (!sessionId) return;
    try {
      setWorkflows(await workspaceService.listRivetWorkflowOperations(sessionId));
      setMessage("Workflow catalog is up to date.");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Rivet workflows are unavailable.");
    }
  }, [sessionId]);
  ...
  const approve = async (workflow: RivetWorkflowOperation) => {
    if (!sessionId) return;
    await workspaceService.reviewRivetWorkflow(sessionId, workflow.slug, "approved", "local-user");
    await refresh();
  };
  const run = async (workflow: RivetWorkflowOperation) => {
    if (!sessionId) return;
    try {
      const result = await workspaceService.runRivetWorkflow(sessionId, workflow.slug);
      setRuns((current) => ({ ...current, [workflow.workflow_id]: result }));
      setMessage(`Run ${result.run_id} is ${result.state}.`);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Workflow could not start.");
    }
  };
  const showHistory = async (workflowId: string) => {
    if (!sessionId || !runs[workflowId]) return;
    try {
      const events = await workspaceService.getRivetWorkflowHistory(sessionId, runs[workflowId].run_id);
      setHistory((current) => ({ ...current, [workflowId]: events.map((event) => event.kind).join(" → ") || "No events yet" }));
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Workflow history is unavailable.");
    }
  };
  const cancel = async (workflowId: string) => {
    if (!sessionId || !runs[workflowId]) return;
    try {
      const result = await workspaceService.cancelRivetWorkflow(sessionId, runs[workflowId]);
    ...
    <section data-testid="rivet-workflows-tab" style={{ padding: "var(--space-md)" }}>
      ...
      {workflows.map((workflow) => (
        <div key={workflow.workflow_id} style={{ borderTop: "1px solid var(--color-border)", marginTop: "var(--space-sm)", paddingTop: "var(--space-sm)" }}>
          ...
          <small>Revision {workflow.revision} · {workflow.review_state || "needs review"}</small><br />
          <button data-testid={`rivet-workflow-approve-${workflow.slug}`} type="button" onClick={() => void approve(workflow)}>Approve revision</button>{" "}
          <button data-testid={`rivet-workflow-run-${workflow.slug}`} type="button" disabled={workflow.review_state !== "approved"} onClick={() => void run(workflow)}>Run</button>
          {runs[workflow.workflow_id] && <>
            <small> {runs[workflow.workflow_id].state}</small>{" "}
            <button data-testid={`rivet-workflow-history-${workflow.slug}`} type="button" onClick={() => void showHistory(workflow.workflow_id)}>History</button>{" "}
            <button data-testid={`rivet-workflow-cancel-${workflow.slug}`} type="button" onClick={() => void cancel(workflow.workflow_id)}>Cancel</button>
            {history[workflow.workflow_id] && <small> {history[workflow.workflow_id]}</small>}
          </>}
        </div>
      ))}
