...
export function ToolCard({
  ...
  const formatBillingPrice = (product: BillingProduct) => {
    const currency = product.currency.toUpperCase();
    const amount = new Intl.NumberFormat(undefined, {
      style: "currency",
      currency,
    }).format(product.price_cents / 100);
    return product.interval ? `${amount} / ${product.interval}` : amount;
  };

  const checkoutUrl = billingProduct
    ? mcpService.buildMcpCheckoutUrl(server.server_id, activeSessionId)
    : null;
  ...
  return (
    ...
    >
      ...
      {billingProduct && (
        <div
          data-testid={`server-card-billing-${server.server_id}`}
          style={{
            display: "flex",
            justifyContent: "space-between",
            alignItems: "center",
            ...
            borderRadius: "var(--radius-md)",
            backgroundColor: billingProduct.purchased
          ...
          >
            <span
              style={{
                fontSize: "0.68rem",
                fontWeight: 800,
                color: billingProduct.purchased
                  ? "var(--color-success)"
                  : "var(--color-secondary)",
                textTransform: "uppercase",
            ...
            >
              {billingProduct.purchased
                ? "Stripe subscription active"
                : "Stripe Billing MCP"}
            </span>
            <span
              data-testid={`server-card-price-${server.server_id}`}
              style={{
                fontSize: "0.95rem",
                fontWeight: 800,
                color: "var(--color-primary)",
            ...
            >
              {formatBillingPrice(billingProduct)}
            </span>
          </div>
          {!billingProduct.purchased && checkoutUrl && (
            <a
              href={checkoutUrl}
              target="_blank"
              rel="noopener noreferrer"
              data-testid={`server-card-purchase-link-${server.server_id}`}
              style={{
                flexShrink: 0,
                padding: "var(--space-xs) var(--space-md)",
                borderRadius: "var(--radius-md)",
