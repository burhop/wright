...
describe("catalog facts and filters", () => {
  ...
  it("matches any selected license category and excludes unknown licensing", () => {
    ...
    expect(matchesCapabilityFilters(fixture({ license: "MIT" }), filters)).toBe(
      true,
    );
    expect(
      matchesCapabilityFilters(fixture({ license: "proprietary" }), filters),
    ).toBe(true);
    expect(matchesCapabilityFilters(fixture(), filters)).toBe(false);
  });
  it("uses declared OS support instead of the current computer's observation", () => {
    const item = fixture({
      supported_platforms: {
        linux_x64: { status: "yes", tested: false },
        windows_11_x64: { status: "no", tested: false },
      },
    });
    expect(
      matchesCapabilityFilters(item, {
  ...
  it("discards retired filters when reading a saved URL", () => {
    expect(
      readCapabilityFilters(
        "?domain=cad&evidence_class=official_preview&platform=windows_11_x64",
      ),
    ).toEqual({ ...emptyCapabilityFilters, platform: "windows" });
  });
});
