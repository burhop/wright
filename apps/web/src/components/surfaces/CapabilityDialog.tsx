...
export function CapabilityDialog({
  ...
  const administratorBlocked = request.administratorOnly && actorRole !== 'admin'
    ...
    <div className="surface-capability-backdrop">
      ...
      >
        <header>
          ...
          <p id={descriptionId}>
            <strong>{request.sourceId} · version {request.sourceVersion}</strong>
        ...
        <dl className="surface-capability-details">
          <div><dt>Operation</dt><dd>{request.operation}</dd></div>
          <div><dt>Data shared</dt><dd>{request.dataDescription}</dd></div>
          <div><dt>Risk</dt><dd className={`risk-${request.risk}`}>{riskLabels[request.risk]}</dd></div>
          <div><dt>Why it is needed</dt><dd>{request.reason}</dd></div>
          <div><dt>Effective policy</dt><dd>{request.effectivePolicy}</dd></div>
          <div><dt>Duration</dt><dd>{request.duration}</dd></div>
          <div><dt>Expires</dt><dd><time dateTime={request.expiresAt}>{request.expiresAt}</time></dd></div>
          <div><dt>Persistence</dt><dd>{request.persistence}</dd></div>
        </dl>
        ...
        {administratorBlocked && (
          <p role="status" className="surface-capability-admin">
            Administrator approval is required. You can deny or cancel this request,
            but an engineer cannot broaden the effective policy.
          </p>
        )}
