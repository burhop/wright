...
describe('ExternalUrlSurface', () => {
  it('is browser-only and discloses the missing Wright authority', async () => {
    ...
    expect(screen.getByText(/not sent through the Wright proxy/i)).toBeVisible()
    expect(screen.getByText(/no Wright credentials, tool bridge, or managed lifecycle/i)).toBeVisible()
