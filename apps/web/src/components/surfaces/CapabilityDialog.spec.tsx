...
import { CapabilityDialog, type CapabilityConsentRequest } from './CapabilityDialog'
...
const request: CapabilityConsentRequest = {
  ...
  denialConsequence: 'BREP stays open, but the selected geometry will not be imported.',
  administratorOnly: false,
}

describe('CapabilityDialog', () => {
  it('discloses exact source, bounded data, risk, policy and persistence', () => {
    render(<CapabilityDialog request={request} actorRole="engineer" onDecision={vi.fn()} />)
    const dialog = screen.getByRole('dialog', { name: /permission requested/i })
    ...
    expect(dialog).toHaveTextContent('One operation; it will not be remembered')
  ...
  it('keeps allow, deny and cancel as distinct decisions', async () => {
    ...
    const { rerender } = render(
      <CapabilityDialog request={request} actorRole="engineer" onDecision={onDecision} />,
    )
  ...
  it('shows administrator-only policy and prevents engineer broadening', () => {
    ...
    expect(screen.getByText(/administrator approval is required/i)).toBeVisible()
