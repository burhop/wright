...
export function DashboardPage() {
  ...
  const [workspaceIndexLoadFailed, setWorkspaceIndexLoadFailed] = useState(false);
  ...
  const fetchWorkspaces = useCallback(async () => {
    ...
    const completeIndex = allResult.status === "fulfilled" ? allResult.value : null;
    setAllWorkspaces(completeIndex ?? []);
    setRecentWorkspaces(recent.length > 0 ? recent : (completeIndex ?? []).slice(0, 5));
    setWorkspacesLoadFailed(recentResult.status === "rejected" && allResult.status === "rejected");
    setWorkspaceIndexLoadFailed(allResult.status === "rejected");
    setWorkspacesLoading(false);
    if (recentResult.status === "rejected") {
      logger.error("Failed to load recent workspaces", { err: recentResult.reason });
    }
    if (allResult.status === "rejected") {
      logger.error("Failed to load the workspace index", { err: allResult.reason });
    }
  }, [logger]);
  ...
  const handleSelectWorkspace = async (w: WorkspaceInfo) => {
    try {
      ...
      if (!activated) throw new Error("Wright could not activate this workspace session.");
  ...
  const handleWorkspaceCreated = async (workspace: WorkspaceInfo) => {
    ...
    try {
      const activated = await workspaceService.activateWorkspace(workspace.session_id);
      if (!activated) throw new Error("Wright could not activate the new workspace session.");
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {recentWorkspaces.length === 0 ? (
              ...
              >
                ...
                {!workspacesLoading && (workspacesLoadFailed || workspaceIndexLoadFailed) && (
                  <button
                    type="button"
          ...
          {!workspacesLoading && workspaceIndexLoadFailed && recentWorkspaces.length > 0 && (
            <div data-testid="workspace-index-partial" role="status" style={{ color: "var(--color-warning)", fontSize: "0.72rem" }}>
              Showing recent workspaces. The complete workspace list is temporarily unavailable.
            </div>
          )}
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <span>
                <strong>{workspacesLoading ? "Loading…" : workspaceIndexLoadFailed ? "Unavailable" : allWorkspaces.length}</strong>
              </span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span style={{ color: "var(--color-secondary)" }}>
