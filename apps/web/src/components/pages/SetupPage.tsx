...
const SetupPage: React.FC<SetupPageProps> = ({ onConfigured }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const response = await fetch(`/api/setup/health?url=${encodeURIComponent(llmUrl)}`);
        if (!response.ok) {
        ...
        setHealthStatus('unhealthy');
        setErrorMsg(err.message || 'Failed to connect to health check endpoint.');
        setLatency(null);
      }
    }, 600);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitError(err.message || 'An error occurred while saving the configuration.');
    } finally {
    ...
    <div style={styles.container}>
      ...
      <div style={styles.card} className="glass-panel animate-fade-in-up">
        <div style={styles.header}>
          ...
          <p style={styles.subtitle}>Configure your LLM connection and activate your workspace agent.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={styles.form}>
              ...
              <div style={styles.indicatorContainer}>
                ...
                {healthStatus === 'healthy' && (
                  <span style={styles.healthyBadge}>
                    <span style={styles.successDot} className="pulse-success-glow" />
            ...
            {errorMsg && (
              <div style={styles.errorText}>
                ...
                <div style={styles.helpText}>
                  Ensure the LLM API is running, accepts CORS, or uses the correct port (e.g. 8000/v1).
                </div>
              </div>
            )}
          </div>
          ...
          <div style={styles.formGroup}>
            ...
            <div style={styles.agentGrid}>
              ...
              <div 
                style={{
                  ...styles.agentCard,
                  borderColor: selectedAgent === 'hermes' ? 'var(--color-secondary)' : 'var(--color-border)',
                  backgroundColor: selectedAgent === 'hermes' ? 'rgba(56, 189, 248, 0.05)' : 'var(--color-surface-subtle)',
                  cursor: 'pointer'
                }}
              ...
              >
                ...
                <p style={styles.agentDesc}>Fully-integrated workspace agent supporting files, Git tools, and full-loop CAD operations.</p>
              </div>
              ...
              >
                ...
                <p style={styles.agentDesc}>Agent scaffolding for autonomous tool use and external platform bridges.</p>
              </div>
              ...
              >
                ...
                <p style={styles.agentDesc}>Optimized micro-agent framework for light-weight local code search & execution.</p>
              </div>
          ...
          <div style={styles.actionContainer}>
            <button
              ...
              style={{
                ...styles.button,
                backgroundColor: healthStatus === 'healthy' ? 'var(--color-secondary)' : '#1e293b',
                color: healthStatus === 'healthy' ? '#090D16' : '#64748b',
                cursor: healthStatus === 'healthy' ? 'pointer' : 'not-allowed',
                boxShadow: healthStatus === 'healthy' ? '0 0 15px rgba(56, 189, 248, 0.3)' : 'none'
              }}
            >
