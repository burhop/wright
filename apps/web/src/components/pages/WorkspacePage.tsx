...
export function WorkspacePage() {
    ...
    const loadWorkspace = async () => {
      try {
        ...
        if (ws.workspace_id !== workspaceId) {
          throw new Error(
            "Wright returned a different workspace than the one requested.",
          );
        }
  ...
  const currentWorkspace =
    workspace?.workspace_id === workspaceId ? workspace : null;
  const workspaceRouteIsChanging =
    workspace !== null && currentWorkspace === null;
  ...
  const handleSessionChange = (newSessionId: string) => {
    setWorkspace((prev) =>
      prev ? { ...prev, session_id: newSessionId } : null,
    );
  };
