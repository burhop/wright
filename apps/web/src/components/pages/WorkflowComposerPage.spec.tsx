...
describe("WorkflowComposerPage", () => {
  ...
  it("creates a separate working draft and enters the authority-labeled composer", async () => {
    ...
    expect(screen.getByRole("heading", { name: "Create an empty draft" })).toBeInTheDocument();
    expect(screen.getByText(/released Process Definition remains read-only/i)).toBeInTheDocument();
    await user.click(screen.getByTestId("workflow-composer-create"));

    expect(await screen.findByTestId("workflow-composer-canvas")).toBeInTheDocument();
    expect(createDraft).toHaveBeenCalledWith(
      "Product definition draft",
      "Capture, define, review, and release one product definition.",
    );
    expect(screen.getByLabelText("Working draft authority")).toHaveTextContent("Not released · Not executable");
    expect(new URLSearchParams(window.location.search).get("draft")).toBe(draft.draft_id);
  });

  it("reopens a draft named in the URL instead of creating another", async () => {
    const draft = decodeWorkflowDraft(fixture);
    readDraft.mockResolvedValue({ draft, etag: '"draft-etag"' });
    window.history.replaceState({}, "", `/workflow-composer?draft=${draft.draft_id}`);
