...
export default function WorkflowComposerPage(): React.ReactNode {
    ...
    void readWorkflowDraft(draftId)
      ...
      .catch(() => {
        if (!controller.signal.aborted)
          setState({
            kind: "error",
            message: "The requested working draft could not be reopened.",
          });
      });
  ...
  const create = async (event: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      setState({
        kind: "error",
        message:
          "Workflow Composer could not create a draft. Confirm that both composer feature flags are enabled.",
      });
    }
  };
  ...
  return (
    <main
      className="workflow-composer workflow-composer__start"
      data-testid="page-workflow-composer"
    >
      <header>
        ...
        <p>
          Create a separate working draft. The released Process Definition
      ...
      {state.kind === "error" && (
        <div className="workflow-composer__notice" role="alert">
          ...
          <p>{state.message}</p>
          <button type="button" onClick={() => setState({ kind: "start" })}>
            Try again
          </button>
        </div>
      )}
      {state.kind === "start" && (
        <form
          className="workflow-composer__create"
        ...
        >
          ...
          <label>
            Draft title
            <input
              data-testid="workflow-composer-create-title"
              required
              maxLength={500}
              value={title}
          ...
          <label>
            Purpose
            <textarea
              data-testid="workflow-composer-create-purpose"
              required
              maxLength={500}
              rows={3}
              value={purpose}
          ...
          <p className="workflow-composer__boundary">
            <strong>Draft authority only.</strong> This creates no run, release,
            approval, or published artifact.
          </p>
          <button data-testid="workflow-composer-create" type="submit">
