...
const mocks = vi.hoisted(() => ({ get: vi.fn(), create: vi.fn(), update: vi.fn(), files: vi.fn() }));
...
vi.mock("../../prototypes/workflow-recovery/WorkflowRecoveryConcept", async () => {
  ...
  function MockWorkflowRecoveryConcept(props: {
    ...
    onSave?: (source: string, layout?: import("../../prototypes/workflow-recovery/model").RecoveryLayout) => Promise<unknown>;
    onListWorkspaceFiles?: () => Promise<Array<{ path: string; name: string }>>;
...
import { initialWorkflow, initialLayout } from "../../prototypes/workflow-recovery/model";
...
function renderPage({ strict = false, onOpenWorkflow }: { strict?: boolean; onOpenWorkflow?: (path: string) => void } = {}) {
  const page = <WorkflowRecoveryPage workspaceId="workspace-1" sessionId="session-1" workspaceName="Bracket Program" workflowFilePath="/workflows/mounting-bracket.workflow.wflow" onOpenWorkflow={onOpenWorkflow} />;
...
describe("workspace workflow page", () => {
  ...
  it("passes persisted layout and loads real file choices on demand in the current workspace", async () => {
    ...
    mocks.get.mockResolvedValue({ ...document, layout, layout_revision: 3, layout_status: "current" });
    mocks.files.mockResolvedValue([{ path: "requirements.txt", name: "requirements.txt" }]);
    mocks.update.mockResolvedValue({ ...document, layout, layout_revision: 4 });
    renderPage();
    await screen.findByTestId("fixture-files");
    expect(screen.getByTestId("workflow-editor-fixture")).toHaveAttribute("data-layout", JSON.stringify(layout));
    expect(mocks.files).not.toHaveBeenCalled();
    await userEvent.click(screen.getByTestId("fixture-files"));
    await waitFor(() => expect(mocks.files).toHaveBeenCalledWith("session-1", "workspace-1"));
    expect(screen.getByTestId("workflow-editor-fixture")).toHaveAttribute("data-files", JSON.stringify([{ path: "requirements.txt", name: "requirements.txt" }]));
    await userEvent.click(screen.getByTestId("fixture-save"));
    expect(mocks.update).toHaveBeenCalledWith("session-1", document.path, "updated workflow source", 4, document.storage_digest, true, layout, 3);
  });
  ...
  it("leaves both files untouched when the chosen new name already exists", async () => {
    mocks.get.mockResolvedValue(document);
    mocks.create.mockRejectedValue(new Error("A workflow already exists with that name."));
    const onOpen = vi.fn();
    renderPage({ onOpenWorkflow: onOpen });
    await screen.findByTestId("fixture-save");
    await userEvent.click(screen.getByRole("button", { name: "New workflow" }));
    ...
    await userEvent.click(screen.getByRole("button", { name: "Create workflow" }));
    expect(await screen.findByTestId("workflow-files-error")).toHaveTextContent("already exists");
  ...
  it("gives separately created files distinct public workflow identities", async () => {
    mocks.get.mockResolvedValue(document);
    mocks.create.mockImplementation(async (_session, path, source) => ({ ...document, path, source }));
    ...
    const firstHeader = firstSource.match(/^workflow authored_[a-z0-9_]+$/m)?.[0];
    const secondHeader = secondSource.match(/^workflow authored_[a-z0-9_]+$/m)?.[0];
  ...
  it("opens an existing workflow from a fresh workspace file list without creating or saving", async () => {
    const onOpen = vi.fn();
    mocks.get.mockResolvedValue(document);
    mocks.files.mockResolvedValue([{ path: "workflows/inspection.workflow.wflow", name: "inspection.workflow.wflow" }, { path: "notes.md", name: "notes.md" }]);
    renderPage({ onOpenWorkflow: onOpen });
    await screen.findByTestId("fixture-save");
    await userEvent.click(screen.getByRole("button", { name: "Open workflow" }));
    await userEvent.click(await screen.findByRole("button", { name: "inspection.workflow.wflow" }));
