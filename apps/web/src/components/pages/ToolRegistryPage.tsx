...
export function ToolRegistryPage() {
  ...
  const categories = ['all', 'local', 'network', 'cad', 'simulation', 'manufacturing', 'plm', 'utilities'];
  ...
  const filteredServers = servers.filter((server) => {
    ...
    if (selectedCategory === 'all') {
      ...
      matchesCategory = server.category.toLowerCase() === selectedCategory.toLowerCase();
    }
  ...
  const getCategoryCount = (cat: string) => {
    ...
    if (cat === 'local') return servers.filter(s => s.type === 'stdio').length;
    if (cat === 'network') return servers.filter(s => ['sse', 'webmcp'].includes(s.type)).length;
    ...
    >
      ...
      <div
        style={{
          ...
          background: 'linear-gradient(180deg, var(--color-surface-subtle) 0%, var(--color-neutral) 100%)',
        }}
      >
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-md)' }}>
            <h1 style={{ fontSize: '1.8rem', fontFamily: 'var(--font-ui)', fontWeight: 700, color: 'var(--color-primary)', margin: 0 }}>
          ...
          <p style={{ fontSize: '0.9rem', color: 'var(--color-secondary)', marginTop: 'var(--space-xs)', margin: 0 }}>
            Discover, install, and manage MCP servers for CAD, FEA, CAM, and PLM workflows.
          </p>
        </div>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        >
          <span style={{ fontSize: '0.75rem', textTransform: 'uppercase', fontWeight: 700, color: 'var(--color-secondary)', letterSpacing: '1.5px', paddingLeft: 'var(--space-md)' }}>
            Categories
          </span>
              ...
              return (
                ...
                >
                  ...
                  <span style={{
                    ...
                    padding: '2px 6px',
                    borderRadius: '10px',
                    backgroundColor: selectedCategory === cat ? 'rgba(56, 189, 248, 0.15)' : 'rgba(255, 255, 255, 0.05)',
                    color: selectedCategory === cat ? 'var(--color-secondary)' : 'rgba(255, 255, 255, 0.4)',
                  }}>
                    {count}
                  </span>
                </button>
          ...
          <div style={{ marginTop: 'auto', padding: 'var(--space-md)', borderTop: '1px solid var(--color-border)' }}>
            <div style={{ fontSize: '0.75rem', color: 'rgba(255, 255, 255, 0.4)', lineHeight: 1.8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>Total Servers</span>
                <span style={{ fontWeight: 600, color: 'var(--color-primary)' }}>{servers.length}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>Installed</span>
                <span style={{ fontWeight: 600, color: 'var(--color-success)' }}>{installedCount}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>Available</span>
                <span style={{ fontWeight: 600, color: 'var(--color-secondary)' }}>{servers.length - installedCount}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', padding: 'var(--space-xl)' }}>
          ...
          {!isLoading && (
            <div style={{
              ...
              <span>
                Showing <strong style={{ color: 'var(--color-primary)' }}>{sortedServers.length}</strong> of {servers.length} servers
            ...
            <div style={{ flex: 1, overflowY: 'auto', paddingRight: '4px' }}>
              {sortedServers.length === 0 ? (
                <div style={{
                  ...
                  <h3 style={{ fontFamily: 'var(--font-ui)', fontWeight: 600, fontSize: '1.2rem', color: 'var(--color-primary)', marginBottom: 'var(--space-sm)' }}>
