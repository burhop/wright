...
interface FileTreeProps {
  ...
  onCreate?: (parentPath: string, name: string, type: 'file' | 'directory') => Promise<void>;
...
export function FileTree({
  ...
  const handleRenameSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (onRename) {
        ...
        const newPath = parentDir === '' ? `/${inputValue.trim()}` : `${parentDir}/${inputValue.trim()}`;
  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    if (sourcePath && sourcePath !== node.path) {
      ...
      const destPath = node.path === '/' ? `/${fileName}` : `${node.path}/${fileName}`;
  ...
  return (
    ...
    >
      ...
      >
        {isEditing ? (
          <form onSubmit={handleRenameSubmit} style={{ display: 'flex', width: '100%' }}>
            <input
              autoFocus
              type="text"
              value={inputValue}
      ...
      {errorMsg && (
        <div style={{ color: 'var(--color-error, #ef4444)', fontSize: '0.7rem', paddingLeft: 'var(--space-md)' }}>
          {errorMsg}
        </div>
      )}
      ...
      {isCreating && (
        <div style={{ paddingLeft: 'var(--space-lg)' }}>
          <form onSubmit={handleCreateSubmit} style={{ display: 'flex', padding: 'var(--space-xs) 0' }}>
            <span style={{ fontSize: '0.8rem', marginRight: 'var(--space-xs)' }}>
