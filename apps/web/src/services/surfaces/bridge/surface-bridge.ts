...
export type SurfaceBridgeMessageKind =
...
function isEnvelope(value: unknown): value is SurfaceBridgeEnvelope {
  ...
  return (
    kinds.has(value.kind as SurfaceBridgeMessageKind) &&
...
function sameBinding(
  left: SurfaceBridgeBinding,
  right: SurfaceBridgeBinding,
): boolean {
  return (
...
export class SurfaceBridge {
  ...
  send(
    ...
    if (!operationPattern.test(operation))
      throw new Error("Surface bridge operation is invalid");
    ...
    const message: SurfaceBridgeEnvelope = {
      ...
      deadlineAt: new Date(
        now.getTime() + this.options.deadlineMilliseconds,
      ).toISOString(),
      payload,
    };
