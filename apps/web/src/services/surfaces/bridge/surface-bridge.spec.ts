...
class FakeHostWindow {
  ...
  removeEventListener(_kind: 'message', listener: (event: MessageEvent) => void) {
...
function envelope(overrides: Partial<SurfaceBridgeEnvelope> = {}): SurfaceBridgeEnvelope {
  return {
    protocolVersion: '1.0',
    kind: 'event',
...
describe('SurfaceBridge', () => {
  it('accepts only the exact event origin and iframe window source', () => {
    ...
    host.dispatch(new MessageEvent('message', {
      origin: 'https://sibling.preview.test', source: target as unknown as Window,
      data: envelope(),
    }))
    host.dispatch(new MessageEvent('message', {
      origin: binding.documentOrigin, source: {} as Window, data: envelope(),
    }))
    ...
    host.dispatch(new MessageEvent('message', {
      origin: binding.documentOrigin, source: target as unknown as Window,
      data: envelope(),
    }))
  ...
  it('never permits wildcard outbound authority', () => {
    ...
    expect(() => new SurfaceBridge({
      hostWindow: new FakeHostWindow(), targetWindow: target, targetOrigin: '*',
      binding, onMessage: vi.fn(),
  ...
  it('rejects stale generation, malformed and oversized messages', () => {
    ...
    for (const data of [
      ...
      host.dispatch(new MessageEvent('message', {
        origin: binding.documentOrigin, source: target as unknown as Window, data,
      }))
    }
    expect(received).not.toHaveBeenCalled()
    expect(rejected.mock.calls.map(([code]) => code)).toEqual([
  ...
  it('removes authority before teardown races can deliver messages', () => {
    ...
    bridge.dispose()
    queued?.(new MessageEvent('message', {
      origin: binding.documentOrigin, source: target as unknown as Window,
      data: envelope(),
    }))
    expect(received).not.toHaveBeenCalled()
