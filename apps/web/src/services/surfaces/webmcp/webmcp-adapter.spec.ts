...
function tool() {
  return {
    ...
    handler: vi.fn(
      async (_args: unknown, _context: { signal: AbortSignal }) => ({
        ok: true,
      }),
    ),
...
describe("WebMCP native adapter", () => {
  it("disposes native registration by abort signal even when the browser returns no handle", async () => {
    const registerTool = vi.fn(
      async (_definition: unknown, _options: unknown) => undefined,
    );
    ...
    const nativeOptions = registerTool.mock.calls[0]?.[1] as unknown as {
      signal: AbortSignal;
    };
  ...
  it("passes native call cancellation to the handler and rejects calls after disposal", async () => {
    ...
    const definition = registerTool.mock.calls[0][0] as {
      execute: (args: object, options: { signal: AbortSignal }) => unknown;
    };
    const call = new AbortController();
    await definition.execute({}, { signal: call.signal });
    const context = original.handler.mock.calls[0]?.[1] as unknown as {
      signal: AbortSignal;
    };
    call.abort();
