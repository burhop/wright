...
it.each([
  [
    "project-inventory-fixtures/exceptions/package/assembly.csv",
    "/project-inventory-fixtures/exceptions/package/assembly.csv",
  ],
...
const unsafePaths = [
  ...
  "x/CON.txt",
  "x\u0000.txt",
  "file.txt?session_id=other",
];
it.each(unsafePaths)("refuses unsafe or non-workspace path %s", (href) =>
  expect(workspaceHtmlLinkPath("reports/review.html", href)).toBeNull(),
);

async function fixture(
  html = '<a href="project-inventory-fixtures/exceptions/package/assembly.csv">Assembly</a>',
) {
  ...
  const model = await provider.openDocument(
    {
      id: "reports/review.html",
      uri: "reports/review.html",
      name: "review.html",
      extension: "html",
      mimeType: "text/html",
    },
  ...
  const send = (
    ...
    window.dispatchEvent(
      new MessageEvent("message", {
        source,
        data: { type: "wright-open-workspace-file", id, ...extra },
      }),
    );
...
it("uses only mapped file identity and pinned session from the current frame", async () => {
  ...
  f.send(id, f.iframe().contentWindow, {
    path: "/secret",
  ...
  expect(f.events).toEqual([
    {
      ...
      path: "/project-inventory-fixtures/exceptions/package/assembly.csv",
      sessionId: "owned-session",
    },
  ]);
  expect(f.iframe().sandbox?.value ?? f.iframe().getAttribute("sandbox")).toBe(
    "allow-scripts",
  );
...
it("preserves local anchors and authored external base semantics while blocking prohibited schemes", async () => {
  const f = await fixture(
    '<a href="#source">Source</a><h2 id="source">Sources</h2><a href="file:///secret">Bad</a><a href="https://example.org/info">External</a>',
  );
  const p = f.preview();
  expect(p.querySelector("a")!.href).toBe("about:srcdoc#source");
  expect(p.querySelector("a")!.hasAttribute("data-wright-file-link")).toBe(
    false,
  );
  expect(
    p
      .querySelector('a[href^="file:"]')!
      .hasAttribute("data-wright-blocked-link"),
  ).toBe(true);
  expect(
    p.querySelector('a[href^="https:"]')!.hasAttribute("data-wright-file-link"),
  ).toBe(false);
  vi.mocked(workspaceService.getFileContentText).mockResolvedValue(
    '<base href="https://example.org/manual/"><a href="chapter.html">Chapter</a>',
  );
  await f.provider.resolveViewer(f.model, f.host, "preview", token);
  expect(f.preview().querySelector("a")!.href).toBe(
    "https://example.org/manual/chapter.html",
  );
  expect(
    f.preview().querySelector("a")!.hasAttribute("data-wright-file-link"),
  ).toBe(false);
});
...
it("does not mount a response arriving after host disposal", async () => {
  ...
  vi.mocked(workspaceService.getFileContentText).mockReturnValue(
    new Promise((resolve) => {
      complete = resolve;
    }),
  );
  ...
  complete("<h1>Stale workspace</h1>");
  await pending;
