...
describe("workspace viewer providers", () => {
  ...
  it("opens STEP text with its workspace session and surfaces read failures", async () => {
    const read = vi.spyOn(workspaceService, "getFileContentText").mockResolvedValue("ISO-10303-21;");
    const provider = new TextProvider();
    const model = await provider.openDocument(file("/cad/bracket.step", "step", "application/step"), {sessionId:"cad-session"});
    ...
    await expect(provider.openDocument(file("missing.step", "step", "application/step"), {sessionId:"cad-session"})).rejects.toThrow("File unavailable");
  });
  it("renders workspace HTML content inside the existing isolated iframe", async () => {
    ...
    const read = vi.spyOn(workspaceService, "getFileContentText").mockResolvedValue(html);
    ...
    expect(read).toHaveBeenCalledWith(documentModel.sessionId, documentModel.uri);
    ...
    expect(rendered.querySelector("base")?.getAttribute("href")).toBe("about:srcdoc");
  ...
  it("keeps report fragments local while preserving an authored external base", async () => {
    ...
    const provider = new IframeProvider();
    const model = await provider.openDocument(file("report.html", "html", "text/html"), { sessionId: "session" });
    for (const explicitBase of ["", '<base href="https://example.org/manual/">']) {
      read.mockResolvedValueOnce(`<!doctype html><html><head>${explicitBase}</head><body><a href="#source">Source</a><h2 id="source">Original quote</h2></body></html>`);
      const host = panel();
      await provider.resolveViewer(model, host, "preview", token);
      const preview = new DOMParser().parseFromString(host.container.querySelector("iframe")!.srcdoc, "text/html");
      expect(preview.querySelector("a")?.href).toBe(explicitBase ? "https://example.org/manual/#source" : "about:srcdoc#source");
      expect(preview.querySelector("#source")?.textContent).toBe("Original quote");
