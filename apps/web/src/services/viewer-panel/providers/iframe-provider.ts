...
export class IframeProvider implements ViewerProvider<IframeDocument> {
  ...
  private bindings = new Map<
    PanelHost,
    { document: IframeDocument; dispose: () => void }
  >();
  ...
  disposeDocument(doc: IframeDocument) {
    for (const binding of this.bindings.values())
      if (binding.document === doc) binding.dispose();
    doc.dispose();
  }
  ...
  async resolveViewer(
    ...
    const dispose = () => {
      ...
      disposed.dispose();
      cancelled.dispose();
      if (this.bindings.get(panel)?.dispose === dispose)
        this.bindings.delete(panel);
    };
    ...
    let html: string;
    try {
      html = await workspaceService.getFileContentText(
        document.sessionId,
        document.uri,
      );
    } catch (error) {
      if (active && !_token.isCancellationRequested) {
        ...
        loading.textContent =
          "Could not load HTML preview. Reopen the file to try again.";
      }
      dispose();
      throw error;
    }
    ...
    removeLinks = bindWorkspaceHtmlLinks(
      preview,
      document.uri,
      document.sessionId,
      panel,
