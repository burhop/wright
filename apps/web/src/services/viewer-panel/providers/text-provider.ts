...
export class TextProvider implements ViewerProvider<TextDocument> {
  ...
  async openDocument(file: FileDescriptor, context: OpenContext): Promise<TextDocument> {
    if (!context.sessionId) throw new Error("No workspace session was provided for this file.");
    const text = await workspaceService.getFileContentText(context.sessionId, file.uri);
