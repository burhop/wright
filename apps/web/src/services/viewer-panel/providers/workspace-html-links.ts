...
export function workspaceHtmlLinkPath(documentUri: string, href: string): string | null {
  if (!href || href !== href.trim() || href.length > 2048 || href.startsWith("#") ||
      href.startsWith("//") || /^[a-z][a-z0-9+.-]*:/i.test(href) || href.includes("?")) return null;
  let path: string;
  try { path = decodeURIComponent(href.split("#", 1)[0]); } catch { return null; }
  if (!path || /[\\%<>:"|?*\u0000-\u001f\u007f]/.test(path) || path.startsWith("//")) return null;
  const parts = path.startsWith("./") || path.startsWith("../") ? documentUri.replace(/^\//, "").split("/").slice(0, -1) : [];
  for (const part of path.split("/")) {
    if (!part || part === ".") continue;
    if (part === "..") { if (!parts.length) return null; parts.pop(); }
    else parts.push(part);
  }
  if (!parts.length || parts.some(part => /[. ]$/.test(part) || /^(?:\.git|\.wright|con|prn|aux|nul|com[1-9]|lpt[1-9])(?:\.|$)/i.test(part) ||
      /[\\%<>:"|?*\u0000-\u001f\u007f]/.test(part))) return null;
  return "/" + parts.join("/");
}

export function bindWorkspaceHtmlLinks(preview: Document, documentUri: string, sessionId: string, panel: PanelHost): () => void {
  const paths = new Map<string, string>();
  const renderId = crypto.randomUUID();
  const authoredBase = preview.querySelector("base[href]")?.getAttribute("href");
  // Preserve authored bases, especially external manuals with their own URL semantics.
  const hasAuthoredBase = Boolean(authoredBase && authoredBase !== "about:srcdoc");
  for (const anchor of preview.querySelectorAll<HTMLAnchorElement>("a[href]")) {
    ...
    if (href.startsWith("#") || /^(?:https?:|mailto:|tel:)/i.test(href) || href.startsWith("//")) continue;
  ...
  const subscription = panel.onDidReceiveMessage(value => {
    ...
    if (message.type !== "wright-open-workspace-file" || typeof message.id !== "string") return;
    const path = paths.get(message.id);
    if (path) panel.container.dispatchEvent(new CustomEvent("viewer-message", {
      detail: {type:"open-workspace-file", path, sessionId}, bubbles:true,
    }));
  });
  return () => { paths.clear(); subscription.dispose(); };
}
