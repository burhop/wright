...
export class WebMcpService {
  ...
  private wsUrl = 'ws://127.0.0.1:8000/api/webmcp/ws';
  ...
  constructor() {
    ...
    window.addEventListener('webmcp:response', (e: Event) => {
      ...
      webMcpLogger.info('Received local response event, sending to agent', { callId });
      ...
      if (this.socket && this.socket.readyState === WebSocket.OPEN) {
        ...
        webMcpLogger.warn('WebSocket not open, cannot send response', { callId });
      }
    });
  }
    ...
    try {
      ...
      this.socket.onmessage = (event) => {
        try {
          ...
          if (callId && method) {
            webMcpLogger.info('Forwarding WebMCP request event to window', { callId, method });
          ...
          webMcpLogger.error('Failed to parse WebMCP message', { data: event.data });
        }
      };

      this.socket.onclose = () => {
        webMcpLogger.warn('WebMCP host socket closed. Attempting reconnect in 5 seconds.');
        this.socket = null;
        this.startReconnect();
      };
