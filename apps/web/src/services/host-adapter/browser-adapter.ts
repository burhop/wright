...
export class BrowserHostAdapter implements HostAdapter {
  ...
  async readFile(path: string, options?: { sessionId?: string }): Promise<string> {
  ...
  async writeFile(path: string, content: string, options?: { sessionId?: string }): Promise<void> {
    const sessionId = options?.sessionId || '';
  ...
  async listDirectory(_path: string, _options?: { sessionId?: string }): Promise<FileEntry[]> {
