...
const identifier = z.string().regex(/^[a-z0-9][a-z0-9._-]*$/).max(96);
...
const feedbackPath = z.object({
  id: identifier,
  from_gate_id: identifier,
  to_block_id: identifier,
  reason: text,
}).strict();
const intendedArtifact = z.object({
  id: identifier,
  title: text,
...
const diagnostic = z.object({
  code: z.string().regex(/^[A-Z][A-Z0-9_]*$/).max(96),
...
const errorResult = z.object({
  error_code: z.string().regex(/^[A-Z][A-Z0-9_]*$/).max(96),
  message: text,
...
export async function verifyWorkflowDraftIdentity(draft: WorkflowDraft): Promise<void> {
...
async function decodeResponse(response: Response): Promise<WorkflowDraftResult> {
  if (!response.ok) {
    let code = "WORKFLOW_DRAFT_STORAGE_FAILED";
    try {
      const body = await response.text();
      if (new TextEncoder().encode(body).byteLength > 16 * 1024) throw new Error();
      const error = errorResult.parse(JSON.parse(body));
      code = error.error_code;
    } catch { /* use support-safe fallback */ }
  ...
  if (etag !== `"${await sha256(draft)}"`) throw new Error("WORKFLOW_DRAFT_ETAG_MISMATCH");
  return { draft, etag };
}
...
export async function createWorkflowDraft(title: string, purpose: string): Promise<WorkflowDraftResult> {
  return decodeResponse(await hostAdapter.fetch(url(), {
    method: "POST",
...
export async function readWorkflowDraft(draftId: string): Promise<WorkflowDraftResult> {
...
export async function validateWorkflowDraft(draft: WorkflowDraft): Promise<unknown> {
  ...
  if (!response.ok) throw new WorkflowDraftClientError(response.status, "WORKFLOW_DRAFT_INVALID");
  const body = await response.text();
  if (new TextEncoder().encode(body).byteLength > MAX_WORKFLOW_DRAFT_BYTES) {
    throw new Error("WORKFLOW_DRAFT_RESPONSE_TOO_LARGE");
  }
...
export async function saveWorkflowDraft(draft: WorkflowDraft, etag: string): Promise<WorkflowDraftResult> {
  return decodeResponse(await hostAdapter.fetch(url(`/${encodeURIComponent(draft.draft_id)}`), {
    method: "PUT",
