...
vi.mock("../src/services/workspace-service", () => ({
  workspaceService: {
    ...
    activateWorkspace: (...args: unknown[]) => {
      ...
      activationQueue = request.then(() => undefined, () => undefined);
      return request;
    },
  },
}));
...
vi.mock("../src/components/chat/WorkspacePanel", () => ({
  default: ({ workspace }: { workspace: { workspace_id: string; local_path: string } }) => (
    <div data-testid="workspace-panel-stub" data-workspace-id={workspace.workspace_id}>{workspace.local_path}</div>
  ),
}));
...
function WorkspaceRouteHarness() {
  ...
  return <>
    <button type="button" data-testid="go-workspace-b" onClick={() => navigate("/workspace/workspace-b")}>Open B</button>
    <Routes>
...
function WorkspaceUnmountHarness() {
  const navigate = useNavigate();
  return <>
    <button type="button" data-testid="leave-workspace" onClick={() => navigate("/")}>Leave workspace</button>
    <button type="button" data-testid="open-workspace-b" onClick={() => navigate("/workspace/workspace-b")}>Open B</button>
    <Routes>
...
describe("WorkspacePage", () => {
  ...
  it("never exposes the prior workspace while a reused route loads a missing workspace", async () => {
    ...
    getWorkspace.mockImplementation((workspaceId: string) => workspaceId === "workspace-a"
      ? Promise.resolve({ workspace_id: "workspace-a", session_id: "session-a", local_path: "C:/workspaces/a" })
      : workspaceB.promise);
    activateWorkspace.mockResolvedValue(true);
    ...
    expect(await screen.findByTestId("workspace-panel-stub")).toHaveAttribute("data-workspace-id", "workspace-a");
  ...
  it("ignores a late response from the prior workspace after a valid route transition", async () => {
    const workspaceA = deferred<{ workspace_id: string; session_id: string; local_path: string }>();
    const workspaceB = deferred<{ workspace_id: string; session_id: string; local_path: string }>();
    getWorkspace.mockImplementation((workspaceId: string) => workspaceId === "workspace-a" ? workspaceA.promise : workspaceB.promise);
    activateWorkspace.mockResolvedValue(true);
    ...
    await waitFor(() => expect(getWorkspace).toHaveBeenCalledWith("workspace-a"));
    fireEvent.click(screen.getByTestId("go-workspace-b"));
    await waitFor(() => expect(getWorkspace).toHaveBeenCalledWith("workspace-b"));

    await act(async () => {
      workspaceB.resolve({ workspace_id: "workspace-b", session_id: "session-b", local_path: "C:/workspaces/b" });
    });
    expect(await screen.findByTestId("workspace-panel-stub")).toHaveAttribute("data-workspace-id", "workspace-b");

    await act(async () => {
      workspaceA.resolve({ workspace_id: "workspace-a", session_id: "session-a", local_path: "C:/workspaces/a" });
    });
    expect(screen.getByTestId("workspace-panel-stub")).toHaveAttribute("data-workspace-id", "workspace-b");
  ...
  it("serializes in-flight activation so the latest workspace owns runtime context", async () => {
    ...
    getWorkspace.mockImplementation((workspaceId: string) => Promise.resolve(workspaceId === "workspace-a"
      ? { workspace_id: "workspace-a", session_id: "session-a", local_path: "C:/workspaces/a" }
      : { workspace_id: "workspace-b", session_id: "session-b", local_path: "C:/workspaces/b" }));
    activateWorkspace.mockImplementation((sessionId: string) => {
      const pending = sessionId === "session-a" ? activationA.promise : activationB.promise;
    ...
    await waitFor(() => expect(activateWorkspace).toHaveBeenCalledWith("session-a"));
    ...
    await act(async () => { activationA.resolve(true); });
    await waitFor(() => expect(activateWorkspace).toHaveBeenCalledWith("session-b"));
    expect(completedActivations).toEqual(["session-a"]);
    expect(screen.queryByTestId("workspace-panel-stub")).not.toBeInTheDocument();

    await act(async () => { activationB.resolve(true); });
    expect(await screen.findByTestId("workspace-panel-stub")).toHaveAttribute("data-workspace-id", "workspace-b");
  ...
  it("rejects a mismatched workspace identity before activation", async () => {
    ...
    expect(await screen.findByText("Workspace Not Found")).toBeVisible();
    expect(screen.getByText(/different workspace than the one requested/i)).toBeVisible();
    expect(activateWorkspace).not.toHaveBeenCalled();
  });
  ...
  it("fails closed when the backend declines workspace activation", async () => {
    ...
    expect(await screen.findByText("Workspace Not Found")).toBeVisible();
    expect(screen.getByText(/could not activate this workspace session/i)).toBeVisible();
    expect(screen.queryByTestId("workspace-panel-stub")).not.toBeInTheDocument();
  });
});
