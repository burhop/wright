...
describe('AddToolModal', () => {
  ...
  it('does not render when isOpen is false', () => {
    render(<AddToolModal isOpen={false} onClose={mockOnClose} onSave={mockOnSave} />);
    expect(screen.queryByText('Register Custom MCP Server')).not.toBeInTheDocument();
  });

  it('renders correctly when isOpen is true', () => {
    render(<AddToolModal isOpen={true} onClose={mockOnClose} onSave={mockOnSave} />);
  ...
  it('requires command input validation for stdio transport', async () => {
    ...
    expect(await screen.findByText('CLI command is required for stdio transport.')).toBeInTheDocument();
    expect(mockOnSave).not.toHaveBeenCalled();
  });
  ...
  it('splits stdio commands into arrays and invokes onSave', async () => {
    ...
    // Fill Name
    fireEvent.change(screen.getByLabelText('Server Name'), { target: { value: 'Stdio Solver' } });
  ...
  it('validates URL protocol and calls onSave for SSE transport', async () => {
    ...
    // Fill Name
    fireEvent.change(screen.getByLabelText('Server Name'), { target: { value: 'SSE Solver' } });
    
    // Select SSE transport
    fireEvent.change(screen.getByLabelText('Transport Type'), { target: { value: 'sse' } });
