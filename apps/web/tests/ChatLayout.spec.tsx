...
describe('ChatLayout', () => {
  beforeEach(() => {
    ...
    mockUseHealthStatus.mockReturnValue([
      ...
      { serviceId: 'hermes-agent', name: 'Hermes Agent', endpoint: '/api/agent/health', state: 'connected', lastChecked: Date.now() },
      { serviceId: 'inference', name: 'LLM Inference', endpoint: '/api/inference/health', state: 'connected', lastChecked: Date.now() },
    ]);
  });
  ...
  it('renders health banner when Hermes Agent is disconnected', () => {
    ...
    expect(screen.getByText(/Hermes agent is not available/i)).toBeInTheDocument();
  });
