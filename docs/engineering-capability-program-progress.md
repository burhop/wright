...
- [ ] `speckit-implement`: T001-T058 are complete. Public schemas, immutable domain/policy values, migration 16, transaction-safe persistence, content-addressed artifact storage/recovery, generated fixtures, boundary observability, offline catalog/UI, exact effect plans, safe HTTPS/resume and offline-import acquisition, durable atomic installation, thin authenticated API/SSE, and explicit review/progress/recovery UI are implemented; runtime/gateway and maintenance work remains.
...
Commit the verified T041-T058 acquisition/install slice, then implement T059-T078 test-first: supervised deterministic runtime, mandatory vectors/evidence, generic gateway capability providers, workspace-scoped model tools, real Rivet-worker coverage, and test/enablement UI. Keep the authoritative dev gate deferred to program closeout.
