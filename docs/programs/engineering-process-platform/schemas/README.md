...
`checkpoint-evidence-correction.schema.json` is independently closed to `COR-EPP-F01-T072-CHECKPOINT-EVIDENCE-001`: exactly the two TR-0047 output-digest pointers and TR-0050 event-domain claim. It requires exact Git-blob recomputation, fixed container identities, `3/3` equality, exact V8 approval, no policy widening, and zero readiness/benchmark/candidate/delivery/release effect. It does not govern the ordinary current-source catalog rebind or walkthrough repairs.

`preflight-evidence-correction.schema.json` is independently closed to `COR-EPP-F01-V9-PREFLIGHT-EVIDENCE-001`: exactly one external-schema binding for the immutable V8 discovery blob and one exact TR-0051 complete-set/self-path-order disposition. `v8-discovery-evidence.schema.json` is an exact-value external contract for that one already-committed schema-less blob. Neither schema permits schema inference, another schema-less artifact, generic manifest sorting, a changed path set, a future record, or any lifecycle/authority/readiness/benchmark/candidate/delivery/release effect. Unsupported consumers fail closed.
