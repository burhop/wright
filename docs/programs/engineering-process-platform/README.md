...
**Status:** EPP-F01 is blocked after T069. T001–T023 and T069 pass, including independently accepted `37/37` committed-identity disposition. One separate immutable defect remains: TR-0027 `/inputs/3` names an approval that Git proves was introduced with the transition rather than present at its source. DEC-P0-017 and exact V5 approvals are required before T024. EPP-F01B remains proposed and unauthorized.
...
16. [`evidence/corrections/COR-EPP-F01-US1-TR0027-INPUT-ORIGIN-001.json`](evidence/corrections/COR-EPP-F01-US1-TR0027-INPUT-ORIGIN-001.json), [`decisions/0017-closed-tr0027-input-origin-correction.md`](decisions/0017-closed-tr0027-input-origin-correction.md), and [`evidence/transitions/TR-0034.json`](evidence/transitions/TR-0034.json) — the exact one-claim proposal and V5 approval checkpoint. They do not authorize implementation.
...
Stop and request exact same-subject V5 `material_change` and `feature_implementation` approval accepting DEC-P0-017 and binding TR-0034. No mutating lease is active. After approval, execute only T024–T041, then stop at `REVIEW_EPP_F01_T041_VALUE_CHECKPOINT` with the required runnable demonstration. T042–T068, EPP-F01B, dependencies, benchmark execution, external change, push/PR/merge/dev integration, publication, and release remain unauthorized.
