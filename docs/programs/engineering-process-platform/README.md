...
**Status:** BLOCKED with no mutating lease. V8 authority is historical and interrupted before T073 completed. V9 planning covers only the exact schema-reference failure in immutable `EPP-F01-V8-discovery.json` and the exact complete-but-self-last TR-0051 changed-path manifest. The sole next action is fresh exact V9 `material_change` and `feature_implementation` approval for T077–T080.
...
Approve or reject the exact V9 subject frozen by TR-0053. Approval may authorize only local T077–T080 under a new bounded lease. Do not resume T073–T076, apply the preserved T073 stash, repair the roadmap-policy defect, execute T066–T068, implement EPP-F01B, add dependencies, generate or execute benchmarks, make external changes, push/open/merge/integrate, publish, or release without later explicit authority.
