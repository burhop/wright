...
**Boundary:** planning/program tooling only; no process-runtime or benchmark collection. Reject cycles, missing references, digest mismatch, stale approvals, impossible transitions, WIP/lease conflicts, hand-set aggregate green, and unknown schema majors. Historical disposition is limited to the exact V4-approved 37-claim committed-identity profile, the V5-approved one-claim TR-0027 input-origin profile, and—only after separate V7 approval—the exact two-claim repair-evidence profile. All preserve original bytes/findings and cannot alter lifecycle, readiness, benchmark, authority, freshness, candidate, delivery, or release results.
