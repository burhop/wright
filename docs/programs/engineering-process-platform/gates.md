...
| ID | Required capability | Passing evidence |
|---|---|---|
| `PROG-01` | Valid control plane | Schemas parse, references/digests match, roadmap is acyclic, state revision is monotonic, eligibility is derived, and dashboard is generated from source evidence. The committed-identity profile requires exact V4 binding and `37/37` proof; the independent TR-0027 input-origin profile requires exact V5 binding and `1/1` source-absence/container-introduction proof. Original findings remain visible; unsupported, partial, extra, substituted, or authority-affecting profiles block. |
...
| `PROG-05` | Risk/decision/change health | No hidden or overdue P0 question/risk/exception; material changes invalidate affected evidence and approvals; decision register is current. DEC-P0-016/RISK-018 govern the 37-claim profile; DEC-P0-017/RISK-019 govern the separate one-claim input-origin profile; DEC-P0-018/RISK-020 govern the separate two-claim repair-evidence profile. A correction can never act as a lifecycle, readiness, benchmark, authority, candidate, delivery, or release waiver. |
