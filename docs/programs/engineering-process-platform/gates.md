...
| `BENCH-05` | Holdout and anti-overfitting | 60 development, 20 validation, and 20 sealed holdout cases obey access rules; candidate freezes before holdout; case/domain dispatch review and perturbation/metamorphic checks pass. |
...
| ID | Required capability | Passing evidence |
|---|---|---|
| `PROG-01` | Valid control plane | Schemas parse, references/digests match, roadmap is acyclic, state revision is monotonic, eligibility is derived, and dashboard is generated from source evidence. |
...
| `PROG-05` | Risk/decision/change health | No hidden or overdue P0 question/risk/exception; material changes invalidate affected evidence and approvals; decision register is current. |
