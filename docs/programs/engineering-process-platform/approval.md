...
**Approved for bounded EPP-F01 implementation.** [`APR-EPP-F01-MC-003`](evidence/approvals/APR-EPP-F01-MC-003.json) and [`APR-EPP-F01-IMPL-003`](evidence/approvals/APR-EPP-F01-IMPL-003.json) record distinct approved scopes for commit `d308d03bc59bf83afc260cf60a9b9bd1729f9b96`, repository tree `e6a04a4de8c52148331b5d3a75209ef58122ad79`, program tree `e3ffc793a3dea088b27d9067eaf219f668c68c2e`, and all 43 artifact digests in `TR-0022`. Authority is limited to the existing 68 EPP-F01 validator/snapshot/provenance/CLI tasks and lease; DEC-P0-015 is accepted, while EPP-F01B and every excluded action remain unauthorized.

`TR-0019` and `TR-0020` remain valid historical authorization/start evidence. `TR-0021` stopped EPP-F01 and preserved incomplete WIP; `TR-0022` froze the corrected subject. `TR-0023` validates the v3 bundle, reconciles the approved prose with the missing machine recovery edge as bounded repair attempt 1, and atomically reactivates the same bounded lease. EPP-F01B requires its own future Spec Kit lifecycle and implementation approval.
...
The v3 approval bundle is current. Feature-owner mutation remains stopped until the separate `IMPLEMENTATION_AUTHORIZED` → `IMPLEMENTING` transition passes preflight and names `EXECUTE_EPP_F01_TASKS` as the sole action. EPP-F01B implementation, dependencies, product runtime changes, benchmark collection or execution, network or external mutation, push, PR, merge, dev integration, publication, and release remain unauthorized.
