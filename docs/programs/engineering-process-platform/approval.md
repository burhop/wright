...
**Implementation is blocked pending V4 exact approval.** The v3 records [`APR-EPP-F01-MC-003`](evidence/approvals/APR-EPP-F01-MC-003.json) and [`APR-EPP-F01-IMPL-003`](evidence/approvals/APR-EPP-F01-IMPL-003.json) remain immutable historical decisions for commit `d308d03bc59bf83afc260cf60a9b9bd1729f9b96`, repository tree `e6a04a4de8c52148331b5d3a75209ef58122ad79`, program tree `e3ffc793a3dea088b27d9067eaf219f668c68c2e`, and the 43 artifact digests in `TR-0022`; they grant no present mutation authority after the stable committed-identity failure and 69-task material amendment.
...
The v1 and v3 bundles are stale for current implementation. V4 must provide separate same-subject `material_change` and `feature_implementation` approvals, accept DEC-P0-016, bind the exact correction-profile digest and frozen transition manifest, and limit authority to the revised 69 local tasks and renewed lease. Feature-owner mutation remains stopped until those records are committed and the coordinator completes the legal recovery/preflight transitions. EPP-F01B implementation, dependencies, product runtime changes, benchmark collection or execution, network or external mutation, push, PR, merge, dev integration, publication, and release remain unauthorized.
