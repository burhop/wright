...
*   **Zero-Cloud Authentication**: Uses Wright's opaque local bearer credential and an HMAC-SHA256-derived browser session, validated with constant-time comparisons. Browser sessions use HttpOnly, SameSite=Strict cookies; the control-plane middleware rejects disallowed origins before protected handlers run. Native installation manages the credential locally. JWT signing and verification are not part of this authentication path. No external telemetry or cloud identity providers (e.g., Auth0, Cognito) are permitted.
