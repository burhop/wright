...
- Added ten versioned engineering workflow templates to the workspace Workflows editor, with editable instances, recorded template origins, readiness disclosures, durable approval checkpoints, and local demo-capture evidence.
- Added persisted engineering dataset campaign state, bounded retry and model-usage records, incremental dashboard reconciliation, and native application lifecycle adapters for Blender and Solid Edge.

### Changed

- Hardened model and MCP completion handling so workflow stages finish only from schema-declared successful terminal receipts and preserve ambiguous outcomes for reconciliation.
- Updated engineering MCP catalog evidence and qualification records for the current campaign while leaving output-content validation disabled.

### Fixed

- Preserved required Windows architecture variables in supervised model subprocesses and made Hermes configuration use permission-safe Wright-owned paths.
- Isolated workflow API and end-to-end test databases, aligned schema compatibility at revision 21, and corrected FreeCAD submodule tests to install the declared development test dependencies.
...
- Prevented the published native lifecycle harness from blocking on Windows when background runtime processes inherit redirected standard handles.
- Added bounded native doctor readiness checks with actionable failed-check diagnostics for slower hosted runners.
- Blocked production publication unless the public Hermes adapter mirror defaults to stable `main`.

## [0.1.7] - 2026-07-30

### Fixed

- Allowed the intentional self-contained multi-package wheel layout while keeping every other wheel-content validation fatal.
- Added the release wheel-content check to the local production merge gate so CI and local release validation enforce the same policy.
