...
class Segment:
    ...
    def bounds_points(self):
        ...
        if self.center:
            for angle in (0.0, math.pi / 2, math.pi, 3 * math.pi / 2):
                distance = (
                    (angle - self.angle) if self.sweep > 0 else (self.angle - angle)
                ) % TAU
    ...
    def length(self):
        return (
            self.radius * abs(self.sweep)
            if self.center
            else math.dist(self.start, self.end)
        )
...
def _sections(pairs):
    ...
    while index < len(pairs):
        ...
        if (code, value) == (0, "SECTION"):
            if (
                current is not None
                or index + 1 >= len(pairs)
                or pairs[index + 1][0] != 2
            ):
    ...
    for code in (30, 31, 38, 39):
        if _number(fields, code, "0") != 0:
            raise NotImplementedError(
                "Nonzero elevation/thickness requires explicit plane normalization."
            )
    if tuple(
        _number(fields, code, default)
        for code, default in ((210, "0"), (220, "0"), (230, "1"))
    ) != (0, 0, 1):
        raise NotImplementedError(
            "Non-default extrusion axes require explicit coordinate transformation."
        )
    if int(_first(fields, 67, "0")) != 0:
        raise NotImplementedError(
            "Paper-space geometry is not a flat-pattern cutting contour."
        )
...
def _arc(layer, center, radius, angle, sweep):
    if radius <= 0 or not math.isfinite(radius) or not 0 < abs(sweep) < TAU:
        raise ValueError(
            "Arc needs positive radius and a nonzero sweep below 360 degrees."
        )
    start = (center[0] + radius * math.cos(angle), center[1] + radius * math.sin(angle))
    end = (
        center[0] + radius * math.cos(angle + sweep),
        center[1] + radius * math.sin(angle + sweep),
    )
...
def _polyline(layer, vertices, closed):
    ...
    for index in range(len(vertices) if closed else len(vertices) - 1):
        ...
        if bulge == 0:
            ...
            center = (
                (start[0] + end[0]) / 2 - (end[1] - start[1]) / chord * offset,
                (start[1] + end[1]) / 2 + (end[0] - start[0]) / chord * offset,
            )
            segment = _arc(
                layer,
                center,
                chord * (1 + bulge * bulge) / (4 * abs(bulge)),
...
def _vertices(fields):
    ...
    for vertex in vertices:
        if _number(vertex, 40, "0") != 0 or _number(vertex, 41, "0") != 0:
            raise NotImplementedError(
                "Variable-width polylines are not unambiguous cut paths."
            )
        result.append(
            (_number(vertex, 10), _number(vertex, 20), _number(vertex, 42, "0"))
        )
    return result
    ...
    while index < len(groups):
        ...
        try:
            ...
            if name == "LINE":
                segment = Segment(
                    layer,
                    (_number(fields, 10), _number(fields, 20)),
                ...
                if name == "CIRCLE":
                    circles.append(
                        {"layer": layer, "center": list(center), "radius": radius}
                    )
                else:
                    angle, end = (
                        math.radians(_number(fields, 50)),
                        math.radians(_number(fields, 51)),
                    )
                    segments.append(
                        _arc(layer, center, radius, angle, (end - angle) % TAU)
                    )
            elif name in {"LWPOLYLINE", "POLYLINE"}:
                flags = int(_first(fields, 70, "0"))
                if flags & ~129:
                    raise NotImplementedError(
                        "Fitted, three-dimensional or mesh polylines are unsupported."
                    )
                if any(
                    float(value) != 0 for code, value in fields if code in (40, 41, 43)
                ):
                    raise NotImplementedError(
                        "Wide polylines are not unambiguous cut paths."
                    )
                if name == "LWPOLYLINE":
                    vertices = _vertices(fields)
                    if len(vertices) != int(_first(fields, 90)):
                        raise ValueError(
                            "LWPOLYLINE vertex count does not match its declaration."
                        )
                else:
                    vertices = []
                    index += 1
                    while index < len(groups) and groups[index][0] == "VERTEX":
                        ...
                        if int(_first(vertex_fields, 70, "0")) != 0:
                            raise NotImplementedError(
                                "Fitted or mesh VERTEX records are unsupported."
                            )
                ...
                raise NotImplementedError(
                    f"Unsupported entity type {name}; its geometry was not verified."
                )
        ...
        if len(segments) > MAX_SEGMENTS:
            raise NotImplementedError(
                "DXF exceeds the bounded contour inspection limit."
            )
...
def _topology(segments, tolerance):
    ...
    def node(point):
        ...
        if len(candidates) > 1:
            raise NotImplementedError(
                "Multiple contour vertices fall within the closure tolerance."
            )
        if candidates:
            return candidates[0]
        result = len(points)
    ...
    for segment in segments:
        ...
        if a == b:
            raise ValueError(
                "Contour segment is shorter than the verification tolerance."
            )
        edges.append((a, b))
    ...
    return {
        ...
        "open_or_branched_components": opened,
...
def _cut_integrity(segments, circles, tolerance, report):
    ...
    if layers - {"OUTER_LOOP", "INTERIOR_LOOPS"}:
        issue(
            "unverified",
            "unknown_cut_layer",
            "Unrecognized cut-layer roles: "
            + ", ".join(sorted(layers - {"OUTER_LOOP", "INTERIOR_LOOPS"})),
        )
    if any(s.center for s in segments):
        issue(
            "unverified",
            "curved_contour_intersections",
            "ARC/bulged-polyline metrics were read, but their intersections and nesting are not supported by this bounded verifier.",
        )
        return
    if len(segments) + len(circles) > 1000:
        issue(
            "unverified",
            "intersection_limit",
            "The contour exceeds the bounded pairwise intersection check limit.",
        )
        return
            ...
            if abs(denominator) <= epsilon:
                ...
                if min(amax, bmax) - max(amin, bmin) > tolerance:
                    issue(
                        "errors",
                        "overlapping_cuts",
                        "Cutting LINE segments overlap or duplicate one another.",
                    )
                    return
                continue
            t, u = cross(delta, s) / denominator, cross(delta, r) / denominator
            if (
                -tolerance / a.length <= t <= 1 + tolerance / a.length
                and -tolerance / b.length <= u <= 1 + tolerance / b.length
            ):
                if not any(
                    math.dist(x, y) <= tolerance
                    for x in (a.start, a.end)
                    for y in (b.start, b.end)
                ):
                    issue(
                        "errors",
                        "intersecting_cuts",
                        "Cutting LINE segments intersect away from a shared contour endpoint.",
                    )
                    return
    for i, circle in enumerate(circles):
        ...
        for segment in segments:
            ...
            t = max(
                ...
                min(
                    1,
                    sum((center[j] - segment.start[j]) * delta[j] for j in (0, 1))
                    / segment.length**2,
                ),
            )
            closest = tuple(segment.start[j] + t * delta[j] for j in (0, 1))
            if (
                math.dist(center, closest) <= radius + tolerance
                and max(
                    math.dist(center, segment.start), math.dist(center, segment.end)
                )
                >= radius - tolerance
            ):
                issue(
                    "errors",
                    "intersecting_hole",
                    "A circular cut touches or intersects another cut contour.",
                )
                return
            ...
            if distance <= radius + other["radius"] + tolerance:
                if distance >= abs(radius - other["radius"]) - tolerance:
                    issue(
                        "errors",
                        "intersecting_holes",
                        "Circular cuts overlap or touch.",
                    )
                    return
                issue(
                    "unverified",
                    "nested_circular_cuts",
                    "Nested circular cuts need an explicit material-region check.",
                )
    ...
    if not outer or any(c["layer"].upper() == "OUTER_LOOP" for c in circles):
        issue(
            "unverified",
            "outer_region",
            "This bounded verifier requires a straight-segment OUTER_LOOP for region containment.",
        )
        return
    if _topology(outer, tolerance)["closed_components"] != 1:
        issue(
            "unverified",
            "outer_region",
            "Exactly one closed outer region has not been established.",
        )
        return
    ...
    if any(not inside(c["center"], outer) for c in circles) or any(
        not inside(s.start, outer) for s in interior
    ):
        issue(
            "errors",
            "cut_outside_outer",
            "An interior cut contour lies outside the outer boundary.",
        )
    if interior:
        issue(
            "unverified",
            "interior_region_nesting",
            "Noncircular interior contour nesting needs an explicit material-region check.",
        )
...
def _region_metrics(segments, circles, tolerance):
    ...
    while remaining:
        matches = [
            (i, s.end if math.dist(polygon[-1], s.start) <= tolerance else s.start)
            for i, s in enumerate(remaining)
            if min(math.dist(polygon[-1], s.start), math.dist(polygon[-1], s.end))
            <= tolerance
        ]
        if len(matches) != 1:
    ...
    result = {
        "net_area": net_area,
        "outer_perimeter": sum(s.length for s in outer),
        "inner_perimeter": sum(TAU * c["radius"] for c in circles),
        "inner_loop_count": len(circles),
    }
...
def _native_scale(report, evidence, path, provider_id, document_id, metrics):
    ...
    def reject(code, message, *, inconclusive=False):
        ...
        report["unverified" if inconclusive else "errors"].append(
    ...
    if not isinstance(evidence, dict):
        return reject(
            ...
            "Native flat measurement evidence is not a structured provider report.",
        )
    status = evidence.get("evidenceStatus")
    if (
        not isinstance(status, dict)
        or status.get("provenance") != "observed"
        or status.get("disposition") != "not_evaluated"
    ):
        return reject(
            "native_evidence_unavailable",
            "The CAD provider did not return observed native flat measurements.",
            inconclusive=True,
        )
    if evidence.get("nativeFlatUpToDate") is not True or evidence.get("unit") != "mm":
        return reject(
            "native_evidence_unavailable",
            "Native flat measurements require an up-to-date model and explicit millimeter units.",
            inconclusive=True,
        )
    if (
        not provider_id
        or not document_id
        or evidence.get("providerId") != provider_id
        or evidence.get("documentId") != document_id
    ):
        return reject(
            "native_document_mismatch",
            "Native flat evidence does not match the bound CAD provider and document.",
        )
    output_path = evidence.get("outputPath")
    if (
        not isinstance(output_path, str)
        or os.path.normcase(os.path.realpath(output_path))
        != os.path.normcase(os.path.realpath(path))
        or evidence.get("outputSha256") != report["file"].get("sha256")
    ):
        return reject(
            "native_export_mismatch",
            "Native flat evidence does not match the exact exported DXF path and SHA-256.",
        )
    names = ("netAreaMm2", "outerPerimeterMm", "innerPerimeterMm")
    if any(
        type(evidence.get(key)) not in (int, float) or not math.isfinite(evidence[key])
        for key in names
    ):
        return reject(
            "invalid_native_measurement",
            "Native area and perimeter measurements must be finite numbers.",
        )
    if (
        evidence["netAreaMm2"] <= 0
        or evidence["outerPerimeterMm"] <= 0
        or evidence["innerPerimeterMm"] < 0
        or type(evidence.get("innerLoopCount")) is not int
        or evidence["innerLoopCount"] < 0
    ):
        return reject(
            "invalid_native_measurement",
            "Native flat area, perimeter or loop count is invalid.",
        )
    if metrics is None:
        return reject(
            "native_comparison_unsupported",
            "The exported contour is not supported for native area/perimeter comparison.",
            inconclusive=True,
        )
    if evidence["innerLoopCount"] != metrics["inner_loop_count"]:
        return reject(
            "native_loop_mismatch",
            "Exported inner-loop count disagrees with the native flat model.",
        )
    ...
    for scale in candidates:
        observed = {
            "netAreaMm2": metrics["net_area"] * scale**2,
        ...
        if not all(math.isfinite(value) for value in observed.values()):
            return reject(
                "native_comparison_range",
                "Scaled DXF area/perimeters exceed the supported numeric range.",
            )
        delta = {name: abs(observed[name] - evidence[name]) for name in names}
        passed = all(
            delta[name]
            <= max(1e-3 if name == "netAreaMm2" else 1e-4, abs(evidence[name]) * 1e-6)
            for name in names
        )
        comparisons.append(
            {"scale_to_mm": scale, "matches": passed, "absolute_deltas": delta}
        )
        if passed:
    ...
    if len(matches) != 1:
        return reject(
            ...
            "No unique millimeter/inch scale matches the native flat area and both perimeters.",
            inconclusive=len(matches) > 1,
        )
    scale = matches[0]
    if header_scale is not None and scale != header_scale:
        return reject(
            "native_unit_conflict",
            "The DXF $INSUNITS header contradicts the measured native flat geometry.",
        )
    if report["units"]["insunits"] not in (None, 0) and header_scale is None:
        return reject(
            "unsupported_explicit_units",
            "An unsupported explicit DXF unit cannot be overridden by inferred scale.",
            inconclusive=True,
        )
    ...
    if header_scale is None:
        report["units"].update(
            name="mm" if scale == 1 else "in",
            scale_to_mm=scale,
            source="hash_bound_native_flat_geometry",
        )
        report["unverified"] = [
            item for item in report["unverified"] if item["code"] != "units_unverified"
        ]
    return scale
...
def verify_flat_dxf(
    ...
    *,
    ...
    native_evidence: dict | None = None,
    ...
    report = {
        "schema_version": 1,
        "verification": "flat_dxf_export_integrity",
        "status": "inconclusive",
        "file": {"path": relative_path or path.name},
        "units": {},
        "entity_counts": {},
        "contours": {},
        "holes": {},
        ...
        "unverified": [],
        "not_checked": [
            "Design-dependent dimensions and feature counts",
            "Material and bend allowance",
            "Native flat-model geometry equivalence",
    ...
    try:
        if path.stat().st_size > MAX_BYTES:
            raise NotImplementedError(
                "DXF exceeds the 20 MiB bounded inspection limit."
            )
        ...
        report["units"] = {
            ...
            "name": unit,
            "scale_to_mm": scale,
            "source": "$INSUNITS" if unit else None,
        }
        if unit is None:
            report["unverified"].append(
                {
                    "code": "units_unverified",
                    "message": "DXF has no supported explicit $INSUNITS length unit. $MEASUREMENT does not establish millimeters.",
                }
            )
        groups = _groups(sections["ENTITIES"])
        report["entity_counts"] = dict(
            Counter(f"{_first(fields, 8, '0')}:{name}" for name, fields in groups)
        )
        ...
        cuts = [
            segment for segment in segments if segment.layer.upper() not in BEND_LAYERS
        ]
        bends = [
            segment for segment in segments if segment.layer.upper() in BEND_LAYERS
        ]
        if any(s.center for s in bends) or any(
            c["layer"].upper() in BEND_LAYERS for c in circles
        ):
            report["unverified"].append(
                {
                    "code": "curved_bend_annotation",
                    "message": "A bend-centerline layer contains curved entities.",
                }
            )
        ...
        if not cuts and not cut_circles:
            report["errors"].append(
                {
                    "code": "no_cut_contours",
                    "message": "DXF contains no supported cutting contours.",
                }
            )
        # Keep the tolerance explicit, and never label raw unknown-unit values mm.
        tolerance = 1e-5 / scale if scale else 1e-8
        report["contours"] = {
            **_topology(cuts, tolerance),
            "circle_count": len(cut_circles),
        ...
        if report["contours"]["open_or_branched_components"]:
            report["errors"].append(
                {
                    "code": "open_contour",
                    "message": "Cutting geometry contains an open or branched contour.",
                }
            )
        _cut_integrity(cuts, cut_circles, tolerance, report)
        signatures = [
            (s.layer, s.start, s.end, s.center, s.radius, s.angle, s.sweep)
            for s in cuts
        ]
            ...
            report["errors"].append(
                {
                    ...
                    "message": "DXF contains duplicate cutting entities.",
                }
            )
        circle_signatures = [
            (c["layer"], *c["center"], c["radius"]) for c in cut_circles
        ]
        if len(circle_signatures) != len(set(circle_signatures)):
            report["errors"].append(
                {
                    "code": "duplicate_entity",
                    "message": "DXF contains duplicate circles.",
                }
            )
        if not report["errors"] and all(
            item["code"] == "units_unverified" for item in report["unverified"]
        ):
            report["geometry_raw"] = _region_metrics(cuts, cut_circles, tolerance)
        if native_evidence is not None:
            verified_scale = _native_scale(
                report,
                native_evidence,
                path,
                provider_id,
        ...
        if points:
            ...
            scaled_bounds = (
                {key: [v * scale for v in values] for key, values in bounds.items()}
                if scale
                else None
            )
            if scaled_bounds and not all(
                math.isfinite(v) for values in scaled_bounds.values() for v in values
            ):
                raise ValueError(
                    "Scaled geometry bounds exceed the supported numeric range."
                )
        ...
        if not all(math.isfinite(v) for v in (diameters or []) + (bend_lengths or [])):
            raise ValueError(
                "Scaled geometry lengths exceed the supported numeric range."
            )
        report["holes"] = {
            "circular_contour_count": len(cut_circles),
            "circles_raw": cut_circles,
            "diameters_mm": diameters,
            "interpretation": "Circular cut contours; design role and nesting not verified.",
        }
        report["bends"] = {
            "centerline_count": len(bends),
            "layers": dict(Counter(s.layer for s in bends)),
