...
class ApplicationLease:
    def __init__(self, identity, *, timeout=30, on_wait=None, directory=None):
        ...
        self.directory=Path(directory) if directory else Path(tempfile.gettempdir())/'wright-workflow-application-locks'
        self.handle=None

    async def __aenter__(self):
        self.directory.mkdir(parents=True,exist_ok=True)
        path=self.directory/(hashlib.sha256(self.identity.encode()).hexdigest()+'.lock')
            ...
            async with asyncio.timeout(self.timeout):
                ...
                while not _try_acquire_file_lock(handle):
                    if not announced and self.on_wait:
                        await self.on_wait();announced=True
                    await asyncio.sleep(.1)
            return self
        except TimeoutError as error:
            handle.close();self.handle=None
            raise _error('APPLICATION_BUSY','Another workflow is using this application.',
                         'Wait for its task to finish before running this task again.') from error
    ...
    async def __aexit__(self,*args):
        if self.handle:
            try:_release_file_lock(self.handle)
...
def lease_is_active(identity, *, directory=None):
    ...
    root=Path(directory) if directory else Path(tempfile.gettempdir())/'wright-workflow-application-locks'
    path=root/(hashlib.sha256(identity.encode()).hexdigest()+'.lock')
    try:handle=path.open('r+b')
    except FileNotFoundError:return None
    with handle:
        if not _try_acquire_file_lock(handle):return True
