...
class WorkspaceFileUseCases:
    ...
    async def write_generated(self, workspace_dir: str, path: str, content: str, policy: str) -> str:
        return await self.write_generated_bytes(workspace_dir, path, content.encode("utf-8"), policy)

    async def write_generated_bytes(self, workspace_dir: str, path: str, content: bytes, policy: str) -> str:
        return await self._executor.run(
            "workspace.files.write_generated",
            lambda: self._files_factory(workspace_dir).write_generated(path, content, policy),
            timeout_seconds=self._timeout,
        )
    ...
    async def read_reference(self, workspace_dir: str, path: str) -> bytes:
        ...
        def work():
            ...
            return content
        return await self._executor.run("workspace.files.reference", work, timeout_seconds=self._timeout)

    async def upload_workflow_image(self, workspace_dir: str, name: str, content: bytes) -> str:
        ...
        return await self._executor.run("workspace.files.upload_image",
            lambda: self._files_factory(workspace_dir).write_generated(name, content, "indexed"), timeout_seconds=self._timeout)
