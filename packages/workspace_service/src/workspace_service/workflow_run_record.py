...
def _execution_snapshot(record, state):
    ...
    def artifact(value, depth=0):
        ...
        if not out.get('id') or not out.get('kind') or not out.get('name'):
            truncated = True
            return None
        out['provenance'] = fields(value.get('provenance'), ('run_id', 'task_id', 'output_port'))
        revisions = value.get('provenance', {}).get('input_revisions', [])
        out['provenance']['input_revisions'] = [list(pair) for pair in revisions[:64]
            if isinstance(pair, (list, tuple)) and len(pair) == 2
            and isinstance(pair[0], str) and len(pair[0]) <= 4096
            and (pair[1] is None or isinstance(pair[1], str) and len(pair[1]) <= 4096)] if isinstance(revisions, (list, tuple)) else []
        truncated |= not isinstance(revisions, (list, tuple)) or len(revisions) > 64
        reps = value.get('representations', [])
        if not isinstance(reps, list):
            truncated = True
            return None
        out['representations'] = [fields(r, ('kind', 'location', 'format', 'provider_id', 'resource_id',
            'revision', 'durability', 'sha256', 'size_bytes')) for r in reps[:64] if isinstance(r, dict)]
        out['representations'] = [r for r in out['representations'] if r.get('location') and r.get('kind')]
        exports = value.get('exports', [])
        if not isinstance(exports, list):
            exports = []
            truncated = True
        out['exports'] = [a for item in exports[:16] if isinstance(item, dict)
                          if (a := artifact(item, depth+1))] if depth < 2 else []
        truncated |= len(reps) > 64 or len(exports) > 16 or (depth >= 2 and bool(exports))
    ...
    output_fields = ('task_id', 'task_title', 'output_path', 'output_bytes', 'output_format', 'sha256', 'cad_role', 'artifact_role')
    event_fields = ('kind', 'at', 'task_id', 'task_title', 'execution_kind', 'message', 'status', 'tool', 'revision')
    ...
    for event in events:
        ...
        if kind == 'step_started' and task_id:
            ...
            invalidate({i for i in ids if isinstance(i, str)} if isinstance(ids, list) else set())
            active = None  # The restart is active only after its step_started event.
            revisions += 1
        elif kind == 'tool_started':
            tools += 1
        elif (kind == 'task_progress' and execution_kinds.get(task_id) == 'mcp_task'
        ...
        if kind in {'run_started','step_started','step_completed','task_progress','operation_progress',
                    'tool_started','tool_completed','design_check','design_revision','review_requested'}:
            progress = fields(event, event_fields)

    result = record.get('result') or {}
    if isinstance(result.get('results'), list):
        current_results = {r['id']: r for r in result['results'] if isinstance(r, dict) and isinstance(r.get('id'), str)}
    elif not events:
        current_results = record.get('partial_results', {})
    if isinstance(result.get('outputs'), list):
        outputs = {o['output_path']: fields(o, output_fields) for o in result['outputs']
                   if isinstance(o, dict) and isinstance(o.get('output_path'), str)}
    compact_results = [a for r in current_results.values() if isinstance(r, dict) if (a := artifact(r))]
    ...
    if isinstance(run_id, str) and len(run_id) > 4096:
        run_id = None
        truncated = True
    snapshot = {'active_task_id': active if state == 'running' else None,
                'completed_task_ids': list(completed), 'event_count': len(events),
                'model_call_count': model_calls, 'tool_call_count': tools,
                'tool_completed_count': tool_ends, 'revision_count': revisions,
                'outputs': list(outputs.values()), 'last_progress': progress, 'truncated': truncated}
    # Bound the complete artifact/event projection, even for unusually wide graphs.
    while len(json.dumps([snapshot, compact_results], ensure_ascii=True).encode()) > 64*1024:
...
async def record_workflow_run(*, service, workspace_dir, source_path, source_digest, execute, on_event=None, workspace_id=None):
    from .workflow_resource_lease import ApplicationLease
    identity='workflow-run:'+uuid4().hex
    async with ApplicationLease(identity):
        return await _record_run(service=service,workspace_dir=workspace_dir,source_path=source_path,
            source_digest=source_digest,execute=execute,on_event=on_event,identity=identity,workspace_id=workspace_id)


async def _record_run(*, service, workspace_dir, source_path, source_digest, execute, on_event, identity, workspace_id):
    started = datetime.now(timezone.utc)
    slug = PurePosixPath(source_path).name.removesuffix(".workflow.wflow")
    filename = f"runs/{slug}/{started.strftime('%Y%m%dT%H%M%SZ')}-{uuid4().hex[:12]}.json"
    record = {"workflow_path": source_path, "source_digest": source_digest,
              "owner": {"host": socket.gethostname(), "lease": identity},
              "started_at": started.isoformat(), "status": "running", "events": []}
    ...
    filename = await service.files.write_generated(workspace_dir, filename, json.dumps(record, ensure_ascii=False, indent=2), "indexed")

    async def checkpoint():
        await service.files.write_generated(workspace_dir, filename, json.dumps(record, ensure_ascii=False, indent=2), "overwrite")
    ...
    async def emit(event):
        ...
        if event.get('kind') == 'design_revision':
            ...
            record['partial_results'] = {key: result for key, result in record.get('partial_results', {}).items()
    ...
    try:
        await emit({'kind': 'run_started', 'at': started.isoformat(), 'task_id': '',
                    'task_title': slug, 'run_log_path': filename})
        ...
        if review_request:
            ...
            review = await service.workflow_artifact_reviews.create(workspace_id=workspace_id,
                workspace_dir=workspace_dir, workflow_path=source_path, run_log_path=filename,
                result=result, request=review_request)
            result.update(status='pending_review', review=review)
            record.update(status='pending_review', result=result, review=review)
            await emit({'kind':'review_requested', 'at':datetime.now(timezone.utc).isoformat(),
                'task_id':review['task_id'], 'task_title':review['task_title'], 'review':review})
        else:
            result['status'] = 'completed'
            record.update(status="completed", result=result)
    except asyncio.CancelledError:
        record.update(status="cancelled", error="Cancelled; completed tool operations remain.")
        raise
    except Exception as error:
        record.update(status="failed", error=str(error), code=getattr(error, "code", "EXECUTION_FAILED"))
        raise
    finally:
        ...
        try:
            ...
            if cleanup.cancel_called:
                raise TimeoutError('Final workflow checkpoint exceeded its cleanup deadline')
        except Exception:
            ...
            _logger.exception('Unable to persist final workflow checkpoint: %s', filename)
...
def recent_workflow_runs(workspace_dir, source_path, *, limit=10, latest_only=False):
    ...
    for index, entry in enumerate(folder.iterdir()):
        ...
        if entry.suffix=='.json':candidates.append(entry.name)
    results=[]
    latest_group=None
    for name in sorted(candidates,reverse=True)[:100]:
        ...
        try:
            ...
            with path.open('rb') as stream:data=stream.read(16*1024*1024+1)
            if len(data)>16*1024*1024:continue
            record=json.loads(data)
            if not isinstance(record,dict):continue
            if record.get('workflow_path')!=source_path:continue
            events=record.get('events',[])
            if not isinstance(events,list) or any(not isinstance(e,dict) for e in events):continue
            state=record.get('status','unknown')
            if not isinstance(state,str) or state not in {'running','completed','pending_review','failed','cancelled','interrupted','unknown'}:continue
            if any(record.get(key) is not None and (not isinstance(record[key],str) or len(record[key])>128)
                   for key in ('started_at','completed_at','execution_ended_at','source_digest')):continue
            if record.get('error') is not None and not isinstance(record['error'],str):continue
            if state=='running':
                owner=record.get('owner',{})
                try:
                    active=lease_is_active(owner['lease']) if owner.get('host')==socket.gethostname() and isinstance(owner.get('lease'),str) else None
                except OSError:
                    active=None  # An unreadable ownership probe cannot prove interruption.
                state='running' if active is True else 'interrupted' if active is False else 'unknown'
            result=record.get('result') or {}
            if not isinstance(result,dict) or not isinstance(record.get('partial_results',{}),dict):continue
            if result.get('results') is not None and not isinstance(result['results'],list):continue
            summary={'path':relative,'status':state,'started_at':record.get('started_at'),
                'completed_at':record.get('completed_at'),'source_digest':record.get('source_digest'),
                'error':record.get('error'),'results':result.get('results') or list(record.get('partial_results',{}).values()),
                'last_event':events[-1] if events else None}
            if latest_only:
                snapshot, artifacts, run_id = _execution_snapshot(record,state)
                summary.update(execution=snapshot, results=artifacts, run_id=run_id,
                    execution_ended_at=record.get('execution_ended_at'),
                    source_matches_current=record.get('source_digest')==current_digest,
                    last_event=snapshot['last_progress'])
                if isinstance(summary.get('error'),str):summary['error']=summary['error'][:4096]
                latest_group=name.split('-',1)[0]
            results.append(summary)
        except (OSError,ValueError,TypeError,KeyError,AttributeError):continue
        if not latest_only and len(results)>=min(20,max(1,limit)):break
    return sorted(results,key=lambda r: (r.get('started_at') or '',r['path']),reverse=True)[:1] if latest_only else results
