...
class Representation:
    ...
    def __post_init__(self):
        if self.kind not in {
            ...
            "cloud_resource",
        } or self.durability not in {"persistent", "session", "run"}:
        ...
        if self.kind in {"application_document", "cloud_resource"} and not (
            self.provider_id and self.resource_id
        ):
            raise ValueError(
                "Application resources need explicit provider and resource identity."
            )
...
def select_representation(
    result: EngineeringResult, requirement: InputRequirement
) -> Representation:
    if result.kind not in requirement.kinds:
        raise ValueError(
            f"{result.name} is {result.kind}; this input needs {', '.join(requirement.kinds)}."
        )
    for rep in result.representations:
        if (
            requirement.representation_kinds
            and rep.kind not in requirement.representation_kinds
        ):
            continue
    ...
    correction = (
        f"Configure an export in the producing block as {formats} and connect that export."
        if formats
        else "Select a resource supported by the consuming application."
    )
    raise ValueError(f"{result.name} has no compatible representation. {correction}")


def validate_result_input(
    value: EngineeringResult | ResultCollection, requirement: InputRequirement
):
    ...
    if isinstance(value, ResultCollection) != requirement.collection:
        raise ValueError(
            "Connect a collection to a collection input, or select an individual result."
        )
...
def file_representation(output: dict) -> Representation:
    return Representation(
        ...
        size_bytes=output["output_bytes"],
    )


def file_result(output: dict, provenance: Provenance) -> EngineeringResult:
    return EngineeringResult(
        f"{provenance.run_id}:{provenance.task_id}:{provenance.output_port}",
        "file",
        output["output_path"],
