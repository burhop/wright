...
def verify_task_files(workspace_dir: str, paths: tuple[str, ...], before: dict | None = None) -> list[dict]:
    from .workspace_path import WorkspacePath
    ...
    for path in paths:
        try:
            ...
            files.append({"output_path": path, "output_bytes": size,
                          "output_format": target.suffix.lstrip("."), "sha256": digest})
        except (OSError, ValueError) as error:
            raise _invalid(f"The task did not produce the expected file: {path}.",
                           "Inspect the tool results and correct the task or expected filename.") from error
    return files
...
def image_media_type(path: str, data: bytes) -> str:
    ...
    if not match or not match[1]:
        raise ValueError("Choose a PNG, JPEG, GIF or WebP image whose contents match its extension.")
    return match[0]
...
class WorkflowReference:
    ...
    @classmethod
    def from_bytes(cls, path: str, data: bytes, *, image: bool, allow_binary: bool = False):
        identity = hashlib.sha256(data).hexdigest()
        if image:
            mime = image_media_type(path, data)
            return cls(path, identity, image_url=f"data:{mime};base64,{base64.b64encode(data).decode('ascii')}", size_bytes=len(data))
        try:
            text = data.decode("utf-8-sig")
        except UnicodeError as error:
            if allow_binary and data:
                return cls(path, identity, file_only=True, size_bytes=len(data))
            raise ValueError("This document format cannot be read by this task yet. Choose a UTF-8 text, Markdown, HTML or JSON file.") from error
...
async def reference_from_result(result, *, service, workspace_dir, task_title):
    ...
    if result.kind not in {'file', 'image', 'text', 'structured'}:
        raise _invalid(f'{task_title}: {result.name} is an application resource, not document contents.',
    ...
    if len(files) != 1:
        raise _invalid(f'{task_title}: select one saved workspace file for {result.name}.',
                       'Choose a named export saved in this workspace. A cloud link or a group of files cannot supply document contents.')
    rep = files[0]
    image = rep.format.lower() in {'png', 'jpg', 'jpeg', 'gif', 'webp', 'screenshot_jpeg'}
    if not image and rep.format.lower() not in {'txt', 'text', 'md', 'markdown', 'html', 'htm', 'json', 'csv', 'yaml', 'yml', 'xml'}:
        raise _invalid(f'{task_title}: {rep.format or "this file format"} cannot be read as document contents.',
                       'Connect it to an application that imports this format, or request a text/image export from the producer.')
    if not rep.sha256:
        raise _invalid(f'{task_title}: the exported file has no verified content identity.',
    ...
    try:
        ...
        if hashlib.sha256(data).hexdigest() != rep.sha256 or (rep.size_bytes is not None and len(data) != rep.size_bytes):
            raise _invalid(f'{task_title}: {rep.location} changed after it was produced.',
        ...
        raise _invalid(f'{task_title}: the exported workspace file could not be read.',
                       str(error) if isinstance(error, ValueError) else 'Restore the file or rerun its producing task.') from error
