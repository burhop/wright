        ...
        cases.append(
            {
                ...
                "state": "planned_migration",
                "authored": False,
                "runnable": False,
                "live_verified": False,
                "applications": scenario["domains"],
                "capabilities": scenario["capabilities"],
                "inputs": scenario["inputs"],
                "expected_outputs": scenario["artifacts"],
                "verification_criteria": scenario["assertions"],
                "fixture_provenance": scenario["provenance"],
                "blockers": [
                    "Legacy Rivet fixture; author a workspace .wflow and bind real application capabilities before a live run."
                ],
            }
        )
    return cases
    ...
    for path in sorted((root / "workflows").glob("*.wflow")):
        ...
        case = {
            ...
            "verification_criteria": [
                "Source revision matches inventory",
                "Returned files exist with matching digest and valid format",
            ],
            "model_call_bound": 0,
        ...
        try:
            ...
            case.update(
                name=definition["fields"].get("name", path.name),
                purpose=definition["fields"].get("purpose", ""),
            ...
            case["compiles"] = True
            case["model_call_bound"] = sum(
                0
                if s.human_review
                else s.max_tool_calls + 1
                if s.agent_task
                else 0
                if s.tool_name
                else 1
            ...
            case["ai_task_count"] = sum(
                not s.human_review and not s.tool_name for s in plan.steps
            )
                ...
                case["inputs"].append(
                    {
                        ...
                        "path": settings.get("workspace_file"),
                        "inline": bool(settings.get("input_text")),
                    }
                )
                if settings.get("workspace_file"):
                    target = WorkspacePath(str(root)).resolve(
                        settings["workspace_file"]
                    )
                    if not target.is_file():
                        case["blockers"].append(
                            f"Missing input file: {settings['workspace_file']}"
                        )
            for step in plan.steps:
                if step.server_id:
                    case["applications"].append(step.server_id)
                if step.save:
                    case["expected_outputs"].append(
                        {
                            "kind": "file",
                            "path": step.output_path,
                            "format": step.output_format,
                            "policy": step.file_policy,
                        }
                    )
                if step.application:
                    case["expected_outputs"].append(
                        {
                            "kind": step.application.get(
                                "kind", "application_resource"
                            ),
                            "source": step.application["source"],
                        }
                    )
                    case["verification_criteria"].append(
                        "Inspect the resulting resource identity and revision through the application; completion text is insufficient."
                    )
                if step.cad:
                    case["expected_outputs"].append(
                        {
                            "kind": "cad_model",
                            "source": step.cad["source"],
                            "edit_mode": step.cad.get("edit_mode"),
                        }
                    )
                    case["expected_outputs"].extend(
                        {
                            "kind": "file",
                            "path": e["path"],
                            "format": e["format"],
                            "policy": e.get("policy", "indexed"),
                        }
                        for e in step.cad.get("exports", [])
                    )
                    if step.cad["source"] == "workspace":
                        ...
                        item = {
                            "name": "CAD model",
                            "path": step.cad["file"],
                            "preserve": step.cad.get("edit_mode") == "copy",
                        }
                        if f.is_file():
                            item["sha256"] = sha(f.read_bytes())
                        else:
                            case["blockers"].append(
                                f"Missing CAD file: {step.cad['file']}"
                            )
                        case["inputs"].append(item)
                    if step.cad["source"] == "session":
                        case["blockers"].append(
                            "Select and verify the intended open application document before this run."
                        )
                    case["verification_criteria"].append(
                        "Review task-specific dimensions/properties independently; file format alone is insufficient."
                    )
            case["applications"] = sorted(set(case["applications"]))
            case["blockers"].append(
                "Live application/model availability and engineering oracle not yet verified for this source revision."
            )
        except (WorkflowSourceExecutionError, ValueError, StopIteration) as error:
            case.update(compiles=False)
            case["blockers"].append(str(error))
        cases.append(case)
    return {
        "schema_version": 1,
        ...
        "target": 100,
        "qualification_counts_unchanged": True,
        "created_at": datetime.now(timezone.utc).isoformat(),
        "workspace": str(root),
        "cases": cases,
        "planned_cases": planned_scenarios(),
    }
...
def verify_output(root, output):
    ...
    data = path.read_bytes()
    if not data or len(data) != output["output_bytes"]:
        raise ValueError("Output size differs from the run record.")
    if output.get("sha256") and sha(data) != output["sha256"]:
        raise ValueError("Output digest differs from the run record.")
    suffix = path.suffix.lower()
    if suffix in {".html", ".htm"}:
        text = data.decode("utf-8").lower()
        if "<html" not in text or "</html>" not in text:
            raise ValueError("Output is not a complete HTML document.")
    elif suffix == ".json":
        json.loads(data)
    elif suffix in {".step", ".stp"}:
        if (
            not data.lstrip().startswith(b"ISO-10303-21;")
            or b"END-ISO-10303-21;" not in data
        ):
            raise ValueError("Invalid STEP exchange structure.")
    elif suffix in {".psm", ".par", ".asm", ".dft"}:
        if not data.startswith(bytes.fromhex("d0cf11e0a1b11ae1")):
            raise ValueError("Invalid native compound-document header.")
        ...
        raise ValueError(
            f"No format oracle registered for {suffix}; cannot count as verified."
        )
    return {
    ...
    try:
        original = {
            i["path"]: sha(
                ...
                .resolve(i["path"], must_exist=True)
                .read_bytes()
            )
        ...
        async with asyncio.timeout(timeout):
            response = await client.get(
                api + "/api/workspace/workflow-sources",
                params={"session_id": session, "path": case["path"]},
            )
            response.raise_for_status()
            document = response.json()
            if sha(document["source"].encode()) != case["source_sha256"]:
                raise ValueError(
                    "Workflow changed since inventory. Refresh and review its manifest entry."
                )
            async with client.stream(
                "POST",
                api + "/api/workspace/workflow-sources/run",
                json={
                    "session_id": session,
                    "path": case["path"],
                ...
                async for line in response.aiter_lines():
                    ...
                    if event["kind"] == "failed":
                        raise ValueError(event.get("message", "Workflow failed"))
                    if event["kind"] in {"completed", "pending_review"}:
                        if "result" in attempt:
                            raise ValueError(
                                "Duplicate terminal workflow receipt; inspect the saved run before retrying."
                            )
                        result = event["result"]
                        if event["kind"] == "pending_review":
                            ...
                            if (
                                ...
                                raise ValueError(
                                    "Invalid pending-review receipt; no human decision or acceptance is inferred."
                                )
                        elif result.get("status") == "pending_review":
                            raise ValueError(
                                "Completed receipt conflicts with pending-review result."
                            )
                        attempt["result"] = result
                        attempt["terminal_event"] = event["kind"]
            if "result" not in attempt:
                raise ValueError(
                    "Connection ended without a terminal workflow receipt; inspect the saved run log before retrying."
                )
            for output in attempt["result"].get("outputs", []):
                attempt["checks"].append(verify_output(root, output))
            ...
            attempt["checks"].extend(
                verify_engineering_assertions(attempt["result"], assertions)
            )
            for path, digest in original.items():
                if (
                    sha(
                        WorkspacePath(str(root))
                        .resolve(path, must_exist=True)
                        .read_bytes()
                    )
                    != digest
                ):
                    raise ValueError(f"Original model changed: {path}")
                attempt["checks"].append(
                    {"path": path, "check": "original_unchanged", "passed": True}
                )
            if not attempt["checks"]:
                raise ValueError(
                    "No output oracle ran; completion alone is insufficient."
                )
            ...
            attempt["engineering_review_required"] = pending or any(
                o["kind"] in {"cad_model", "analysis", "application_resource"}
                for o in case["expected_outputs"]
            )
            if pending:
                attempt.update(
                    review=attempt["result"]["review"],
                    live_verified=False,
        ...
        attempt.update(
            status="timed_out",
            error="Observation timed out. The server may have partial operations; inspect its run record before rerunning.",
        )
    except Exception as error:
        attempt.update(status="failed", error=str(error))
    ended = datetime.now(timezone.utc).isoformat()
    attempt["observation_ended_at"] = ended
    if attempt.get("terminal_event") in {"completed", "pending_review"}:
        attempt["execution_ended_at"] = ended
...
def observed_call_metrics(events):
    ...
    """
    active = {}
    ai_started = ai_completed = agent_started = decisions = direct_tools = 0
    ...
    return dict(
        ai_calls_started=ai_started,
        ai_calls_completed=ai_completed,
        agent_tasks_started=agent_started,
        agent_decisions_started=decisions,
        direct_tool_steps_started=direct_tools,
        mcp_tool_calls_started=tool_started,
        mcp_tool_results_recorded=tool_recorded,
        mcp_tool_calls_succeeded=tool_succeeded,
        mcp_tool_calls_failed=tool_failed,
        model_transport_calls=None,
...
async def run_campaign(
    ...
    *,
    ...
    if (
        ...
        or not 1 <= max_model_calls <= 500
    ):
        raise ValueError(
            "Use bounded runs (1–20), concurrency (1–4), timeout (1–3600s) and model calls (1–500)."
        )
    ...
    if (
        len(cases) > max_runs
        or sum(c["model_call_bound"] for c in cases) > max_model_calls
    ):
        raise ValueError("Selected workflows exceed the campaign budget.")
    if any(not c["compiles"] for c in cases):
        raise ValueError("A selected workflow does not compile.")
    if any(c.get("authoring_shape_valid") is False for c in cases):
        raise ValueError("A selected workflow has invalid workspace authoring fields.")
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        ...
        async def run(case):
            ...
            async with semaphore, AsyncExitStack() as stack:
                for app in sorted(case["applications"]):
                    await stack.enter_async_context(locks[app])
