...
def setup_record(root, events=(), **extra):
    ...
    record = {
        ...
        "status": "running",
        "started_at": "2026-09-08T10:00:00.100Z",
        "events": list(events),
        "owner": {"host": socket.gethostname(), "lease": "controlled-test"},
        **extra,
    }
    folder = root / "runs/example"
...
def test_active_snapshot_has_recorded_predecessor_and_compact_counts(
    tmp_path, monkeypatch
):
    monkeypatch.setattr(
        "workspace_service.workflow_resource_lease.lease_is_active", lambda _: True
    )
    source, path, _ = setup_record(
        tmp_path,
        [
            event("step_started", "design", execution_kind="ai"),
            event("step_completed", "design"),
            event("step_started", execution_kind="mcp_task", prompt="PRIVATE PROMPT"),
            event("task_progress", available_tools=[], tool_calls=0),
            event("tool_started", arguments={"large": "SECRET"}),
            event("tool_completed", result={"native": "SECRET"}),
            event(
                "task_progress",
    ...
    assert (s["model_call_count"], s["tool_call_count"], s["tool_completed_count"]) == (
        3,
        1,
        1,
    )
    ...
    assert "SECRET" not in json.dumps(summary) and "PRIVATE PROMPT" not in json.dumps(
        summary
    )
...
def test_revision_invalidates_completion_and_results_until_explicit_restart():
    events = [
        ...
        event("result_ready", engineering_result=artifact("cad", "old.psm")),
        event("output_saved", output_path="old.psm", output_format="psm"),
        event("step_completed", "check"),
...
def test_unconfirmed_or_released_lease_never_pulses(
    tmp_path, monkeypatch, lease, state
):
    monkeypatch.setattr(
        "workspace_service.workflow_resource_lease.lease_is_active", lambda _: lease
    )
    source, _, _ = setup_record(
        tmp_path, [event("step_started", execution_kind="mcp_task")]
    )
    summary = recent_workflow_runs(tmp_path, source, latest_only=True)[0]
...
@pytest.mark.parametrize(
    "state", ["completed", "failed", "pending_review", "cancelled"]
)
def test_terminal_snapshot_retains_partial_outputs_and_end_but_clears_active(
    tmp_path, state
):
    source, _, _ = setup_record(
        tmp_path,
        [
            event("step_started", execution_kind="ai"),
            event("result_ready", engineering_result=artifact("cad", "report.html")),
        ],
        status=state,
        error="Example failure" if state == "failed" else None,
        execution_ended_at="2026-09-08T10:01:00Z",
    )
    summary = recent_workflow_runs(tmp_path, source, latest_only=True)[0]
...
def test_stale_saved_source_keeps_exact_run_identity(tmp_path):
    ...
    summary = recent_workflow_runs(tmp_path, source, latest_only=True)[0]
    assert (
        summary["source_matches_current"] is False
        and summary["source_digest"] == record["source_digest"]
    )


def test_unreadable_lease_is_unknown_not_missing_history(tmp_path, monkeypatch):
    def unavailable(_):
        raise PermissionError("Controlled ownership probe denial")

    monkeypatch.setattr(
        "workspace_service.workflow_resource_lease.lease_is_active", unavailable
    )
    source, _, _ = setup_record(
        tmp_path, [event("step_started", execution_kind="mcp_task")]
    )
    ...
    assert (
        ...
        and summary["execution"]["active_task_id"] is None
    )


def test_latest_reads_one_timestamp_group_and_orders_same_second_by_start(
    tmp_path, monkeypatch
):
    source, path, record = setup_record(tmp_path, status="completed")
    for second in range(10):
        (path.parent / f"20260908T0959{second:02d}Z-ffffffffffff.json").write_text(
            json.dumps(record)
        )
...
def test_latest_ignores_malformed_foreign_and_escaping_records(tmp_path):
    ...
    (path.parent / "20260908T100002Z-a.json").write_text(
        json.dumps({**record, "events": ["not-an-event"]})
    )
    (path.parent / "20260908T100001Z-a.json").write_text(
        json.dumps({**record, "workflow_path": "other/example.workflow.wflow"})
    )
    assert recent_workflow_runs(tmp_path, source, latest_only=True)[0]["path"].endswith(
        path.name
    )
    with pytest.raises(ValueError):
        recent_workflow_runs(tmp_path, "../outside.workflow.wflow", latest_only=True)


def test_projection_is_byte_bounded_and_native_payloads_never_appear():
    results = [
        artifact("task" + str(i), "reports/" + ("p" * 2000) + str(i) + ".html")
        for i in range(100)
    ]
    s, projected, _ = _execution_snapshot(
        {
            "events": [event("tool_completed", result={"native": "X" * 1000000})],
    ...
    assert (
        s["truncated"] is True
        and len(json.dumps([s, projected], ensure_ascii=True).encode()) <= 64 * 1024
    )
...
def test_default_history_keeps_original_last_event_and_no_snapshot(tmp_path):
    source, _, _ = setup_record(
        tmp_path,
        [event("tool_completed", result={"native": "full evidence"})],
        status="completed",
    )
    summary = recent_workflow_runs(tmp_path, source)[0]
    assert (
        summary["last_event"]["result"] == {"native": "full evidence"}
        and "execution" not in summary
    )
...
def test_actual_cad_export_progress_does_not_count_as_model_decisions():
    fixture = json.loads(
        (
            Path(__file__).parent / "fixtures/run-progress-dispatch-20260908.json"
        ).read_text()
    )
