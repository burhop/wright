...
def test_cross_process_lock_deadline_fails_before_commit_and_never_commits_later(
    ...
    try:
        ...
        store = WorkspaceWorkflowSourceStore(
            str(tmp_path), lock_timeout_seconds=0.15
        )
...
def test_failed_definition_replace_preserves_prior_bytes_and_head(
    ...
    assert not (
        metadata / "revisions" / "00000000000000000002.json"
    ).exists()
...
def test_bounded_malicious_metadata_maps_parser_limits_to_integrity(
    tmp_path, payload
):
...
def test_committed_reads_never_call_unbounded_path_read_bytes(
    tmp_path, monkeypatch
):
    store = WorkspaceWorkflowSourceStore(str(tmp_path))
    created = store.create(SOURCE_PATH, "source")
...
async def test_write_use_case_defers_cancellation_until_worker_finishes(tmp_path):
    ...
    task = asyncio.create_task(
        use_cases.create(str(tmp_path), SOURCE_PATH, "source")
    )
