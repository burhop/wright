...
def test_asgi_disconnect_persists_cancelled_end_and_partial_without_completed_tool(
    tmp_path, monkeypatch, blocked_storage
):
    monkeypatch.setattr(
        "workspace_service.workflow_run_record._FINAL_CHECKPOINT_SECONDS", 0.05
    )
            ...
            await emit(
                {
                    ...
                    "engineering_result": {
                        "id": "partial",
                        "provenance": {"task_id": "design"},
                    },
                }
            )
            await emit(
                {"kind": "tool_started", "task_id": "quote", "tool": "browser_click"}
            )
            started.set()
    ...
    if not blocked_storage:
        assert (
            raw["execution_ended_at"]
            and raw["completed_at"] == raw["execution_ended_at"]
        )
    ...
    assert recent_workflow_runs(tmp_path, source)[0]["status"] == (
        "interrupted" if blocked_storage else "cancelled"
    )
