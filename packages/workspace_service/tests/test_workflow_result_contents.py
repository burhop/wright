...
def test_named_export_contents_reach_downstream_prompt_with_identity(
    tmp_path, prompt_source
):
    ...
    step = replace(
        workflow.steps[0],
        prompt_from="producer.export" if prompt_source else None,
    ...
    result = asyncio.run(
        execute_prompt_workflow(
            ...
            plan=workflow,
            input_values={"producer.export": exported(data)},
            response_generator=generate,
        )
    )
    assert result["steps"][0]["references"] == [
        {
            "path": "export.md",
            "sha256": hashlib.sha256(data).hexdigest(),
            "kind": "text",
...
def test_named_image_export_reaches_model_as_pixels(tmp_path):
    ...
    workflow = replace(
        workflow,
        steps=(replace(workflow.steps[0], references=(("View", "producer.export"),)),),
    )
    ...
    asyncio.run(
        execute_prompt_workflow(
            ...
            plan=workflow,
            input_values={
                "producer.export": exported(
                    data, path="view.jpg", fmt="screenshot_jpeg"
                )
            },
            response_generator=generate,
        )
...
@pytest.mark.parametrize(
    "problem",
    ["changed", "missing", "unverified", "binary", "cloud", "ambiguous", "model"],
)
def test_result_contents_fail_explicitly_without_model_call(tmp_path, problem):
    ...
    if problem == "changed":
        (tmp_path / "export.md").write_bytes(b"Use a 99 mm flange.")
    if problem == "missing":
        (tmp_path / "export.md").unlink()
    if problem == "unverified":
        value = replace(
            value, representations=(replace(value.representations[0], sha256=None),)
        )
    if problem == "binary":
        value = replace(
            value, representations=(replace(value.representations[0], format="step"),)
        )
    if problem == "cloud":
        value = replace(
            value,
            representations=(
                Representation(
                    "cloud_resource",
                    "https://example.invalid/export.md",
                    "markdown",
                    provider_id="cloud",
                    resource_id="export",
                ),
    ...
    if problem == "ambiguous":
        value = replace(
            ...
            representations=(
                ...
                replace(value.representations[0], location="second.md"),
            ),
        )
    if problem == "model":
        value = replace(value, kind="cad_model")
...
def test_compiled_cad_export_connection_reads_pixels_or_rejects_binary(
    tmp_path, image_export
):
    ...
    fmt, path = (
        ("screenshot_jpeg", "view.jpg") if image_export else ("step", "model.step")
    )
    data = (
        b"\xff\xd8\xfftest-cad-preview"
        if image_export
        else b"ISO-10303-21;\nEND-ISO-10303-21;"
    )
    config = settings()
    config.update(
        save_native=False,
        exports=[{"format": fmt, "path": path, "port": "cad_response"}],
    )
    cad = task(
        "cad",
        fmt="text",
        ...
        mcp_server="cad",
        cad=json.dumps(config),
    )
    cad = cad.replace(
        '"kind":"engineering_document","name":"Response"',
        '"kind":"workspace_file","name":"Export"',
    )
    ...
    async def decide(messages, tools, **kwargs):
        ...
        if decisions == 1:
            selected = next(
                t for t in tools if "cad.set_variable" in t["function"]["description"]
            )
            return {
                "role": "assistant",
                "tool_calls": [
        ...
        return {
            "role": "assistant",
            "content": json.dumps(
                {"status": "completed", "response": "Model inspected.", "evidence": [1]}
            ),
        }
    ...
    if image_export:
        ...
        assert result["steps"][1]["references"] == [
            {"path": path, "sha256": hashlib.sha256(data).hexdigest(), "kind": "image"}
        ]
        assert (
            result["results"][0]["exports"][0]["representations"][0]["sha256"]
            == hashlib.sha256(data).hexdigest()
        )
    else:
        with pytest.raises(
            WorkflowSourceExecutionError, match="cannot be read as document contents"
        ):
            asyncio.run(execute())
...
def test_exact_mcp_consumes_verified_exported_text_or_json(
    tmp_path, whole_arguments, changed
):
    ...
    opts = (
        {"mcp_arguments_source": "connection", "mcp_arguments_input": "search_query"}
        if whole_arguments
        else {}
    )
    workflow = compile_prompt_workflow(
        source(task("producer", fmt="json"), mcp(**opts))
        + edge("producer.producer_response", "search.search_query")
    )
    ...
    async def execute():
        return await execute_prompt_workflow(
            ...
            plan=workflow,
            input_values={
                "producer.producer_response": exported(
                    data, path=filename, fmt="json" if whole_arguments else "text"
                )
            },
    ...
    if changed:
        with pytest.raises(
            WorkflowSourceExecutionError, match="changed after it was produced"
        ):
            asyncio.run(execute())
        assert not gateway.calls
    else:
        result = asyncio.run(execute())
        assert gateway.calls[0][0][3] == {
            "query": "bend relief",
            "limit": 2 if whole_arguments else 3,
        }
        assert result["steps"][0]["references"] == [
            {
                "path": filename,
