...
"""
    async def scenario():
        async with ApplicationLease('cad',directory=tmp_path):
            child=await asyncio.to_thread(subprocess.run,[sys.executable,'-c',code,str(tmp_path)],capture_output=True,text=True,timeout=15)
            assert child.returncode==0,child.stderr
            assert child.stdout.strip()=='busy'
        async with ApplicationLease('cad',directory=tmp_path,timeout=.1):pass
...
def test_cancelled_waiter_does_not_release_the_owner(tmp_path):
    async def scenario():
        async with ApplicationLease('cad',directory=tmp_path):
            waiter=asyncio.create_task(ApplicationLease('cad',directory=tmp_path).__aenter__())
            await asyncio.sleep(.03);waiter.cancel()
