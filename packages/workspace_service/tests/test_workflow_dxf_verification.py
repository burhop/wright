...
def dxf(geometry=None, units=4, measurement=1):
    ...
    pairs = [*header, (0, 'ENDSEC'), (0, 'SECTION'), (2, 'ENTITIES'),
...
def test_verified_mm_contour_holes_and_bends_have_exact_file_identity(tmp_path):
    data = dxf(rectangle() + circle() + circle(60, 80) + line((0, 50), (80, 50), 'UP_CENTERLINES'))
...
def test_explicit_inches_are_scaled_without_changing_file(tmp_path):
    data = dxf(rectangle(2, 3) + circle(.5, .5, .1), units=1)
    report = verify(tmp_path, data)
    assert report['status'] == 'pass'
...
@pytest.mark.parametrize('geometry, expected', [
    ...
    (line((0, 0), (10, 10)) + line((10, 10), (0, 10)) + line((0, 10), (10, 0)) + line((10, 0), (0, 0)), 'intersecting_cuts'),
...
def test_overflowing_derived_bend_length_fails_with_writable_report(tmp_path):
    report = verify(tmp_path, dxf(rectangle() + line((-1e308, 0), (1e308, 0), 'UP_CENTERLINES')))
    assert report['status'] == 'fail'
    json.dumps(report, allow_nan=False)


@pytest.mark.parametrize('geometry, expected', [
    (rectangle() + entity('SPLINE', 'INTERIOR_LOOPS', [(10, 1), (20, 2)]), 'unsupported_geometry'),
...
def test_arc_metrics_are_retained_but_curve_intersections_are_inconclusive(tmp_path):
    ...
    arcs += entity('ARC', 'OUTER_LOOP', [(10, 0), (20, 0), (40, 10), (50, 180), (51, 360)])
...
def test_cad_export_gate_preserves_report_and_stops_later_export(tmp_path, units, succeeds):
    cfg = settings()
    cfg.update(save_native=False, exports=[{'format': 'flat_dxf', 'path': 'flat.dxf', 'port': 'flat'},
    ...
    async def call(session, request, name, arguments, **kwargs):
        ...
        if name == 'cad.list_providers':
            result.structured_content['result'][0]['capabilities']['exports'].append('flat_dxf')
    ...
    async def scenario():
        ...
        if succeeds:
            ...
            descriptor = next(file for file in files if file['output_path'] == 'flat.dxf')
    ...
    assert report['file']['sha256'] == hashlib.sha256((tmp_path / 'flat.dxf').read_bytes()).hexdigest()
...
def native_evidence(path, *, width=80, height=100, radius=3, scale=1):
    ...
    return {
        ...
        'nativeFlatUpToDate': True, 'unit': 'mm',
        'providerId': 'solid_edge', 'documentId': 'original',
        'outputPath': str(path), 'outputSha256': hashlib.sha256(path.read_bytes()).hexdigest(),
        'netAreaMm2': (width * height - math.pi * radius ** 2) * scale ** 2,
        'outerPerimeterMm': 2 * (width + height) * scale,
        'innerPerimeterMm': 2 * math.pi * radius * scale, 'innerLoopCount': 1,
        'selectedFaceIds': ['face-1', 'face-2'], 'nativeMethods': ['Face.Area', 'Loop.Edges'],
    }
...
def verify_with_native(tmp_path, *, units=None, scale=1, patch=None, geometry=None):
    ...
    report = verify_flat_dxf(path, native_evidence=evidence, provider_id='solid_edge', document_id='original')
    json.dumps(report, allow_nan=False)  # Evidence remains writable even for bad provider values.
    return report


@pytest.mark.parametrize('units, scale', [(None, 1), (None, 25.4), (0, 1), (4, 1), (1, 25.4), (5, 10)])
def test_native_flat_metrics_establish_unique_scale_without_rewriting_dxf(tmp_path, units, scale):
    ...
    assert report['native_geometry']['tolerances'] == {'length_abs_mm': 1e-4, 'area_abs_mm2': 1e-3, 'relative': 1e-6}
    assert report['bounds_mm']['size'] == pytest.approx([80 * scale, 100 * scale])
    assert report['units']['source'] == ('hash_bound_native_flat_geometry' if units in (None, 0) else '$INSUNITS')
    assert (tmp_path / 'flat.dxf').read_bytes() == data
...
def test_native_evidence_must_match_bound_export_and_all_geometry_metrics(tmp_path, patch, expected):
...
@pytest.mark.parametrize('patch', [
    {'evidenceStatus': {'provenance': 'ai_assertion', 'disposition': 'not_evaluated'}},
    {'evidenceStatus': {'provenance': 'unavailable', 'disposition': 'not_evaluated'}},
    {'evidenceStatus': {'provenance': 'observed', 'disposition': 'approved'}},
    {'nativeFlatUpToDate': False}, {'nativeFlatUpToDate': 'true'}, {'unit': 'm'},
])
def test_native_measurement_provenance_and_up_to_date_flat_are_required(tmp_path, patch):
    report = verify_with_native(tmp_path, patch=patch)
    assert report['status'] == 'inconclusive'
...
@pytest.mark.parametrize('patch', [
    {'netAreaMm2': float('nan')}, {'outerPerimeterMm': float('inf')}, {'innerPerimeterMm': -1},
    {'netAreaMm2': None}, {'netAreaMm2': True}, {'netAreaMm2': 0}, {'innerLoopCount': 1.0},
])
...
def test_native_oracle_does_not_approve_unknown_geometry(tmp_path):
    report = verify_with_native(tmp_path, geometry=rectangle() + circle() + entity('SPLINE', 'INTERIOR_LOOPS', [(10, 1), (20, 2)]))
    assert report['status'] == 'inconclusive'
...
def test_cad_gate_uses_actual_export_report_for_native_scale_and_publishes_evidence(tmp_path, spoof_document):
    cfg = settings()
    cfg.update(save_native=False, exports=[{'format': 'flat_dxf', 'path': 'flat.dxf', 'port': 'flat'}])
    gateway, run = runner(tmp_path, cfg)
    original_call = gateway.call_tool
...
@pytest.mark.parametrize('expected_status, data', [('inconclusive', dxf(units=None)),
                                                  ('fail', dxf(rectangle() + circle(100, 20, 3)))])
def test_failed_dxf_report_is_clickable_partial_result_without_releasing_export_or_quote(tmp_path, expected_status, data):
    ...
    quote = replace(step, id='quote', title='Request supplier quote', references=(('DXF', 'edit.flat'),))
    with pytest.raises(WorkflowSourceExecutionError) as error:
        asyncio.run(execute_prompt_workflow(service=service(tmp_path), workspace_dir=str(tmp_path),
            plan=PromptWorkflow('DXF to quote', (step, quote), {}), input_values={}, response_generator=None,
            on_event=on_event, tool_runtime=runtime, action_generator=object()))
    assert error.value.code == 'CAD_EXPORT_VERIFICATION_FAILED'
    ...
    assert result['provenance']['output_port'] == 'diagnostic:flat.dxf.verification.json'
    representation, = result['representations']
    report_path = tmp_path / representation['location']
    assert representation['kind'] == 'workspace_file' and representation['durability'] == 'persistent'
    assert representation['sha256'] == hashlib.sha256(report_path.read_bytes()).hexdigest()
