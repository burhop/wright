...
def test_real_reader_releases_and_checkpoint_replacement_succeeds(
    ...
    with ThreadPoolExecutor(max_workers=1) as pool:
        ...
        assert held.wait(2)
        assert (
            LocalWorkspaceFiles(str(tmp_path)).write_generated(
                "run.json", new, "overwrite"
            )
            == "run.json"
        )
...
def test_persistent_windows_errors_have_exact_retry_bound(
...
@pytest.mark.parametrize(
    "winerror,code", [(None, errno.EACCES), (112, errno.ENOSPC), (3, errno.ENOENT)]
)
...
def test_indexed_concurrent_publications_preserve_every_complete_payload(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=8) as pool:
        paths = list(
            pool.map(
                lambda data: files.write_generated("report.json", data, "indexed"),
                payloads,
            )
        )
