...
from packages.workspace_service.tests.test_workflow_design_check import (
    FakeCad,
    plan,
    report,
)
...
def captured_report():
    ...
    return json.loads(
        (
            Path(__file__).parent / "fixtures/design-check-mixed-verdict-20260908.json"
...
def test_actual_mixed_response_is_preserved_for_bounded_correction():
    ...
    assert [
        sum(c["status"] == s for c in validated["checks"])
...
def test_mixed_response_still_requires_evidenced_failure_and_correction(fault):
    ...
    if fault == "no_failure":
        ...
        failures[0]["evidence"] = [9]
    elif fault == "bool_citation":
        failures[0]["evidence"] = [True]
    elif fault == "no_corrections":
        value["corrections"] = []
    elif fault == "empty_correction":
        value["corrections"] = [" "]
...
def test_mixed_revision_never_releases_unknowns_or_exceeds_bound(
    monkeypatch, tmp_path, end
):
    monkeypatch.setattr("workspace_service.workflow_cad.CadTask", FakeCad)
    mixed = captured_report()
    responses = (
        [mixed, report("needs_input")]
        if end == "needs_input"
        else [mixed, mixed, mixed]
    )
    ...
    async def run_task(step, prompt, *args, **kwargs):
        ...
        return (
            json.dumps(next(sequence)) if step.design_check else "Created"
        ), successful_calls()
    ...
    assert error.value.code == (
        "DESIGN_CHECK_NEEDS_INPUT" if end == "needs_input" else "DESIGN_CHECK_REJECTED"
    )
    ...
    assert (
        len([e for e in events if e["kind"] == "design_revision"]) == len(responses) - 1
    )
