...
def safe_conflict():
    ...
    issue = {'code': code, 'stage': 'preflight_before_recipe_mutation_cleanup_confirmed',
             'retryable': False, 'argument': 'parameters'}
    return {'providerId': 'solid_edge', 'operationStatus': 'failed', 'isSuccess': False,
            'isSaved': False, 'document': None, 'outputPath': None,
            'createdReferences': [], 'warnings': [], 'error': issue, 'issues': [dict(issue)]}
...
def test_confirmed_clean_recipe_rejection_requires_changed_arguments_and_success_evidence(mode):
    ...
    gateway.result = GatewayToolResult(content=(), structured_content=report, is_error=True)
    cad = BoundCad()
    decisions, events = [], []
    ...
    async def decide(messages, *args, **kwargs):
        ...
        if len(decisions) == 2:
            ...
            gateway.result = GatewayToolResult(content=(), structured_content={'isSuccess': True})
            return call('BendAllowance' if mode == 'unchanged' else 'Design_BendAllowance')
    ...
    if mode == 'changed':
        ...
        assert [record['status'] for record in records] == ['invalid_arguments', 'succeeded']
        assert cad.observed == [{'isSuccess': True}]
    else:
        with pytest.raises(WorkflowSourceExecutionError,
                           match='same rejected recipe' if mode == 'unchanged' else 'valid execution evidence'):
            asyncio.run(execute)
    assert len(gateway.calls) == (1 if mode == 'unchanged' else 2)


@pytest.mark.parametrize('field,value', [
    ('warnings', ['Cleanup uncertain']), ('createdReferences', ['variable:width']),
    ('document', {'id': 'still-open'}), ('outputPath', 'partial.psm'),
    ('isSuccess', True), ('isSaved', True), ('providerId', 'other'),
    ('operationStatus', 'partial'), ('issues', []), ('error', {}),
])
def test_adverse_or_incomplete_native_report_remains_fatal(field, value):
    ...
    with pytest.raises(WorkflowSourceExecutionError) as error:
        asyncio.run(runner.run_task(plan().steps[0], 'Create', decide, emit, cad=BoundCad()))
...
def test_report_contract_without_bound_cad_does_not_enable_correction():
    gateway, runner = runtime()
    gateway.list_tools = lambda *_: (replace(TOOL, tool_name='cad.create_sheet_metal_from_recipe'),)
    gateway.result = GatewayToolResult(content=(), structured_content=safe_conflict(), is_error=True)
...
def test_failed_mutation_records_full_report_then_stops_without_another_decision(large):
    ...
    with pytest.raises(WorkflowSourceExecutionError) as failure:
        asyncio.run(runner.run_task(plan().steps[0], 'Create the requested output', decide, emit))
