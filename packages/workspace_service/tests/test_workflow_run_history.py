...
def test_unfinished_record_is_interrupted_only_when_known_host_lease_is_released(
    tmp_path,
):
    source = workflow(tmp_path)
    ...
    record = {
        ...
        "owner": {
            ...
            "lease": "history-test-" + tmp_path.name,
        },
    }
    ...
    assert json.loads(path.read_text())["status"] == "running", (
        "History must not rewrite execution evidence"
    )
...
def test_history_is_scoped_to_exact_workflow_and_rejects_path_escape(tmp_path):
    ...
    (folder / "foreign.json").write_text(
        json.dumps(
            {"workflow_path": "other/example.workflow.wflow", "status": "completed"}
        )
    )
    ...
    with pytest.raises(ValueError):
        recent_workflow_runs(str(tmp_path), "../outside.wflow")
