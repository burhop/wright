...
from workspace_service.workflow_campaign import (
    inventory,
    observed_call_metrics,
    run_campaign,
    run_case,
)
...
class Client:
    ...
    async def get(self, url, params):
        ...
        return Reply(
            {
                "source": self.entries[params["path"]]["source"],
...
def fixture(tmp_path, identity="rfq", terminal="pending_review"):
    text = source()
    path = f"workflows/{identity}.workflow.wflow"
    ...
    result = {
        ...
        "outputs": [
            {
                "output_path": f"{identity}.html",
                "output_bytes": len(data),
                "sha256": hashlib.sha256(data).hexdigest(),
            }
        ],
    }
    if terminal == "pending_review":
        result["review"] = {"review_id": "a" * 32, "state": "pending", "actor": None}
    events = [
        {"kind": "step_started", "task_id": key, "execution_kind": "ai"}
        for key in ("extract", "draft")
    ]
    events += [
        {"kind": "step_completed", "task_id": key} for key in ("extract", "draft")
    ]
    events += (
        [{"kind": "review_requested", "task_id": "engineer"}]
        if terminal == "pending_review"
        else []
    )
    ...
    case = {
        ...
        "inputs": [],
        "expected_outputs": [{"kind": "file"}],
        "model_call_bound": 2,
        "compiles": True,
        "applications": ["shared-server"],
    }
    return case, {"source": text, "events": events}


def test_pending_review_releases_campaign_slot_without_becoming_a_pass_or_submitting_decision(
    tmp_path, monkeypatch
):
    first, a = fixture(tmp_path)
    second, b = fixture(tmp_path, "second", "completed")
    ...
    results = asyncio.run(
        run_campaign(
            {"workspace": str(tmp_path), "cases": [first, second]},
            ids=["rfq", "second"],
    ...
    assert (
        ...
        and completed["status"] == "structure_verified"
    )
    ...
    assert (
        pending["human_decision_submitted"] is False
        and pending["engineering_review_required"] is True
    )
    assert pending["checks"][0][
        "passed"
    ]  # File structure is evidence, not human acceptance.
    assert (
        pending["call_metrics"]["ai_calls_started"]
        == pending["call_metrics"]["ai_calls_completed"]
        == 2
    )
    assert pending["call_metrics"]["mcp_tool_results_recorded"] == 0
    assert (
        len(client.requests) == 4
        and len(list((tmp_path / "attempts").glob("*.json"))) == 2
    )


@pytest.mark.parametrize(
    "mutation", ["missing-id", "approved", "duplicate", "wrong-event"]
)
def test_invalid_or_conflicting_terminal_review_receipts_do_not_create_acceptance(
    tmp_path, mutation
):
    case, entry = fixture(tmp_path)
    if mutation == "missing-id":
        entry["events"][-1]["result"]["review"].pop("review_id")
    if mutation == "approved":
        entry["events"][-1]["result"]["review"]["state"] = "approved"
    if mutation == "duplicate":
        entry["events"].append(copy.deepcopy(entry["events"][-1]))
    if mutation == "wrong-event":
        entry["events"][-1]["kind"] = "completed"
    result = asyncio.run(
        run_case(
            case,
            root=tmp_path,
...
def test_call_metrics_count_actual_attempts_and_keep_agent_transport_unknown():
    ...
    metrics = observed_call_metrics(events)
    assert metrics["ai_calls_started"] == metrics["ai_calls_completed"] == 2
    assert (
        metrics["agent_tasks_started"] == 1 and metrics["agent_decisions_started"] == 2
    )
    assert (
        metrics["mcp_tool_results_recorded"] == metrics["mcp_tool_calls_started"] == 2
    )
