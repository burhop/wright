...
def task(key="report", prompt="Write a short report.", fmt="html", **settings):
    values = {
        ...
        + {"html": "html", "text": "txt", "markdown": "md", "json": "json"}[fmt],
        **settings,
    }
    return f'''task {key}
  name: "{key}"
...
def service(root, text=""):
    ...
    return SimpleNamespace(
        files=SimpleNamespace(
            write_generated=write_generated, write_generated_bytes=write_generated_bytes
        ),
        workflow_sources=SimpleNamespace(read=read),
    )
...
def test_model_timeout_preserves_completed_output_and_names_current_failure(tmp_path):
    ...
    async def generate(prompt, fmt):
        if fmt == "html":
            raise TimeoutError(
                "The model did not finish responding within 180 seconds."
            )
...
def test_chained_response_is_prompt_and_only_terminal_step_saves(tmp_path):
    ...
    assert calls == [
        ("Write a short report.", "text"),
        ("Write an HTML report about steel.", "html"),
    ]
...
def test_intermediate_copy_and_each_terminal_branch_saves(tmp_path):
    plan = compile_prompt_workflow(
        source(
            task("first", fmt="text", save_output=True),
            task("a", prompt_source="connection", prompt_input="a_prompt"),
            task(
                "b", fmt="markdown", prompt_source="connection", prompt_input="b_prompt"
            ),
            links=[("first", "a"), ("first", "b")],
        )
    )

    async def generate(prompt, fmt):
        return HTML if fmt == "html" else "Response"
    ...
    assert {item["output_path"] for item in result["outputs"]} == {
        "first.txt",
        "a.html",
        "b.md",
    }
...
def test_indexed_names_are_exclusive_across_concurrent_runs(tmp_path):
    ...
    assert {r["output_path"] for r in results} == {
        "report.html",
        *(f"report-{n:03d}.html" for n in range(1, 6)),
    }
...
def test_completed_document_survives_downstream_failure_without_replacing_previous_file(
    tmp_path, policy
):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"output_filename": ".git/report.html"},
...
def test_invalid_settings_rejected_before_generation(settings):
    with pytest.raises(WorkflowSourceExecutionError):
        compile_prompt_workflow(source(task(**settings)))
...
def test_cycle_and_shared_overwrite_rejected():
    with pytest.raises(WorkflowSourceExecutionError, match="cycle"):
        compile_prompt_workflow(
            source(
                task("a", prompt_source="connection", prompt_input="a_prompt"),
                task("b", prompt_source="connection", prompt_input="b_prompt"),
                links=[("a", "b"), ("b", "a")],
            )
        )
    with pytest.raises(WorkflowSourceExecutionError, match="overwrite"):
        compile_prompt_workflow(
            source(
                task(
                    "a",
                    output_filename="same.html",
                    file_policy="overwrite",
                    save_output=True,
                ),
                task("b", output_filename="same.html"),
                links=[("a", "b")],
            )
        )
    ...
    with pytest.raises(WorkflowSourceExecutionError, match="changed"):
        asyncio.run(
            prepare_prompt_workflow(
                ...
                workspace_dir=str(tmp_path),
                path="workflows/test.workflow.wflow",
...
def test_response_formats():
    ...
    with pytest.raises(WorkflowSourceExecutionError):
        validate_response("{broken", "json")
    with pytest.raises(WorkflowSourceExecutionError):
        validate_response("NaN", "json")
