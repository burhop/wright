...
def done(response, evidence=(1,)):
    return {
        "role": "assistant",
        "content": json.dumps(
            {"status": "completed", "response": response, "evidence": list(evidence)}
        ),
    }
...
def test_ai_task_discovers_tools_calls_twice_and_feeds_downstream(tmp_path):
    workflow = compile_prompt_workflow(
        source(
            task(
                "research", fmt="text", authoring_template="mcp-task", mcp_server="docs"
            ),
            task(
                "report",
                fmt="text",
                prompt="",
                prompt_source="connection",
                prompt_input="report_prompt",
            ),
    ...
    replies = iter(
        [call("bracket"), call("sheet metal"), done("Verified findings", (1, 2))]
    )
    seen, events = [], []
...
def test_task_refreshes_catalog_before_deciding_but_checks_contract_again_before_call(
    tmp_path, change_during_decision
):
    ...
    updated = replace(
        ...
        provenance={"server_revision": "live"},
    )
    gateway, runner = runtime()
    catalog = [TOOL]
    ...
    async def decide(messages, tools, **kwargs):
        ...
        return (
            call("second query")
            if actions == 2
            else done("Both current tools used", (1, 2))
        )
    ...
    if change_during_decision:
        with pytest.raises(
            WorkflowSourceExecutionError, match="definition has changed"
        ):
            asyncio.run(execute())
...
def test_task_refuses_unsupported_or_unevidenced_completion(
    tmp_path, replies, expected
):
    gateway, runner = runtime()
...
def test_browser_navigation_can_repeat_without_allowing_duplicate_mutations(
    tmp_path, name, official, allowed
):
    from dataclasses import replace
    from packages.workspace_service.tests.test_workflow_mcp_execution import TOOL
...
def test_full_32_call_budget_completes_or_blocks_33rd_call(tmp_path, finish_at_limit):
    ...
    replies = iter(
        [
            *(call(f"observation {index}") for index in range(32)),
            done("All observations collected", tuple(range(1, 33)))
            if finish_at_limit
            else call("33rd"),
        ]
    )
...
def test_task_cannot_claim_a_file_it_did_not_produce(tmp_path, state):
    from workspace_service.workflow_references import (
        snapshot_task_files,
        verify_task_files,
    )
    ...
    if state != "missing":
        target.write_text("old CAD" if state == "unchanged" else "")
...
def test_verified_tool_file_is_exposed_with_digest(tmp_path):
    ...
    async def decide(*args, **kwargs):
        ...
        if not action.get("tool_calls"):
            (tmp_path / "part.step").write_text("tool-created CAD fixture")
        return action
    ...
    result = asyncio.run(
        execute_prompt_workflow(
            service=service(tmp_path),
            workspace_dir=str(tmp_path),
            plan=plan(expected_files="part.step"),
            input_values={},
            response_generator=None,
            action_generator=decide,
            tool_runtime=runner,
...
def test_reference_contents_and_image_bytes_reach_the_model(tmp_path):
    ...
    step = replace(
        workflow.steps[0], references=(("Document", "doc"), ("Image", "image"))
    )
    workflow = replace(workflow, steps=(step,))
    image = WorkflowReference.from_bytes(
        "fixture.png", b"\x89PNG\r\n\x1a\nfixture", image=True
    )
    doc = WorkflowReference.from_bytes(
        "brief.md", b"Actual design brief: 12 mm flange", image=False
    )
    ...
    asyncio.run(
        execute_prompt_workflow(
            ...
            workspace_dir=str(tmp_path),
            plan=workflow,
            input_values={"doc": doc, "image": image},
            response_generator=generate,
        )
    )


def test_mcp_task_receives_image_on_each_decision_and_records_reference_identity(
    tmp_path,
):
    from workspace_service.workflow_references import WorkflowReference
...
"""
    research = task(
        ...
        '"key":"research_prompt","kind":"engineering_document"',
        '"key":"research_prompt","kind":"reference_images"',
    )
    workflow = compile_prompt_workflow(
        source(image_input, research, links=[("image", "research")])
    )
    image = WorkflowReference.from_bytes(
        "sketch.png", b"\x89PNG\r\n\x1a\nfixture", image=True
    )
    ...
    assert result["steps"][0]["references"] == [
        {"path": "sketch.png", "sha256": image.sha256, "kind": "image"}
    ]
...
def test_unreadable_reference_fails_explicitly(path, data, image):
    ...
    with pytest.raises(ValueError):
        WorkflowReference.from_bytes(path, data, image=image)
    ...
    async def decide(*args, **kwargs):
        return {
            "content": json.dumps(
                {
                    "status": "blocked",
                    "response": "Provide the sheet thickness before creating the design.",
                    "evidence": [],
                }
            )
        }
        ...
        await emit(
            {
                "kind": "tool_started",
                "tool": "docs__search",
                "arguments": {"query": "bracket"},
            }
        )
        # Inspect the on-disk checkpoint while execution is still in progress.
        checkpoint = json.loads(
            next((tmp_path / "runs/example").glob("*.json")).read_text()
        )
        assert (
            checkpoint["status"] == "running"
            and checkpoint["events"][-1]["kind"] == "tool_started"
        )
        if state == "failed":
