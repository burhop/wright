    ...
    return dict(
        ...
        checks=[
            dict(
                ...
                expected="2 mm",
                observed="2 mm",
                status={"pass": "pass", "revise": "fail", "needs_input": "unverified"}[
                    verdict
                ],
                evidence=[1],
            )
...
def native_inspection():
    ...
    observation = {
        "observationId": "thickness-observed",
        "kind": "thickness",
        "evidenceStatus": {"provenance": "observed", "disposition": "pass"},
        "value": {"scalar": {"value": "2", "unit": "mm"}},
    }
    return {
        "operationId": "inspect-model",
        "allCriticalRequirementsPass": True,
        "inspection": {
            "evidenceKind": "inspection",
            "scopeId": "inspect-model",
            "evidenceStatus": {"provenance": "observed", "disposition": "pass"},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "evaluation_blocking",
        "evaluation_unsatisfied",
        "evaluation_not_evaluated",
        "evaluation_observation_missing",
        "evaluations_empty",
        "is_valid_false",
        "overall_fail",
        "overall_unverified",
...
def test_native_rejection_cannot_be_relabelled_as_pass(fault):
    ...
    if fault == "critical_false":
        native["allCriticalRequirementsPass"] = False
    if fault == "critical_string":
        native["allCriticalRequirementsPass"] = "true"
    if fault == "report_unavailable":
        native["inspection"]["evidenceStatus"]["provenance"] = "unavailable"
    if fault == "report_not_evaluated":
        native["inspection"]["evidenceStatus"]["disposition"] = "not_evaluated"
    if fault == "observation_fail":
        native["inspection"]["observations"][0]["evidenceStatus"]["disposition"] = (
            "fail"
        )
    if fault == "observation_inferred":
        native["inspection"]["observations"][0]["evidenceStatus"]["provenance"] = (
            "inferred"
        )
    if fault == "evaluation_blocking":
        native["requirementEvaluations"][0]["isBlocking"] = True
    if fault == "evaluation_unsatisfied":
        native["requirementEvaluations"][0]["isSatisfied"] = False
    if fault == "evaluation_not_evaluated":
        native["requirementEvaluations"][0]["disposition"] = "not_evaluated"
    if fault == "evaluation_observation_missing":
        native["requirementEvaluations"][0]["observation"] = None
    if fault == "evaluations_empty":
        native["requirementEvaluations"] = []
    if fault == "is_valid_false":
        native["isValid"] = False
    if fault == "overall_fail":
        native["overall"] = "fail"
    if fault == "overall_unverified":
        native["overall"] = {"status": "unverified"}
    if fault == "verdict_fail":
        native["verdict"] = "fail"
    calls = [{"status": "succeeded", "result": {"result": native}}]
    with pytest.raises(
        WorkflowSourceExecutionError,
        match="native evidence that failed or is unverified",
    ):
...
def test_native_pass_and_raw_observations_remain_available_for_independent_review():
    assert (
        inspect_verdict(
            json.dumps(report()),
            [{"status": "succeeded", "result": native_inspection()}],
        )["verdict"]
        == "pass"
    )
    ...
    assert (
        inspect_verdict(json.dumps(report()), [{"status": "succeeded", "result": raw}])[
            "verdict"
        ]
        == "pass"
    )
...
def test_failed_native_evidence_can_support_revise_or_needs_input():
    native = native_inspection()
    native["allCriticalRequirementsPass"] = False
    native["requirementEvaluations"][0].update(
        isSatisfied=False, isBlocking=True, disposition="fail"
    )
    for verdict in ("revise", "needs_input"):
        assert (
            inspect_verdict(
                json.dumps(report(verdict)), [{"status": "succeeded", "result": native}]
            )["verdict"]
            == verdict
        )
...
def test_omitting_failed_native_verification_from_citations_does_not_release():
    ...
    calls = [
        {"status": "succeeded", "result": native},
        {"status": "succeeded", "result": raw_face},
    ]
    with pytest.raises(
        WorkflowSourceExecutionError,
        match="native evidence that failed or is unverified",
...
def test_uncited_capability_limit_is_not_a_failed_model_verification():
    capability = {
        ...
        "capabilities": [{"kind": "motion", "isSupported": False}],
        "overall": "unverified",
    }
    check = report()
    check["checks"][0]["evidence"] = [2]
    calls = [
        {"status": "succeeded", "result": capability},
        {"status": "succeeded", "result": native_inspection()},
    ]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"result": None},
        {"allCriticalRequirementsPass": True},
        {"result": {"evidenceKind": "inspection"}},
    ],
)
...
def test_known_native_verification_requires_structured_report(native_result, cited):
    ...
    calls = [
        {
            "tool": "solid-edge__cad.verify_inspection_requirements",
            "status": "succeeded",
            "result": native_result,
        },
        {
            "tool": "solid-edge__cad.measure_body_extents",
            "status": "succeeded",
            "result": {
                "evidenceStatus": {
                    "provenance": "observed",
...
def test_local_argument_correction_before_native_inspection_is_allowed():
    ...
    calls = [
        {
            "tool": "cad.verify_inspection_requirements",
        ...
        },
        {
            "tool": "cad.verify_inspection_requirements",
            "status": "succeeded",
            "result": {"result": native_inspection()},
        },
    ]
    assert inspect_verdict(json.dumps(check), calls)["verdict"] == "pass"
...
def plan():
    ...
    check = PromptStep(
        ...
        True,
        "indexed",
        output_ports=(("model_checked", "cad_model"),),
        agent_task=True,
        server_id="cad",
        cad={"source": "upstream", "from_port": "input", "save_native": False},
        cad_from="make.model",
        design_check=True,
        max_revisions=2,
    )
...
def test_rework_cannot_replay_in_place_or_overwrite_edits():
    workflow = plan()
    for config in (
        {"source": "session", "policy": "indexed"},
        {"source": "new", "policy": "overwrite"},
    ):
        steps = (replace(workflow.steps[0], cad=config), *workflow.steps[1:])
        with pytest.raises(WorkflowSourceExecutionError):
            validate_rework(steps, workflow.revisions)
...
class FakeCad:
    ...
    async def finish(self):
        return CadDocument("cad", {"documentId": "test"}), []

    def engineering_result(self, run_id):
        return EngineeringResult(
            f"{run_id}:{self.step.id}",
            "cad_model",
            "Model",
            (
                Representation(
...
@pytest.mark.parametrize(
    "verdicts,expected_calls,error_code",
    [
...
def test_actual_scheduler_gates_revises_and_stops(
    monkeypatch, tmp_path, verdicts, expected_calls, error_code
):
    ...
    async def run_task(step, prompt, *args, **kwargs):
        ...
        return (
            json.dumps(report(next(sequence))) if step.design_check else "Created"
        ), [{"status": "succeeded"}]

    async def generate(*args):
        calls.append("quote")
        return "Quote handoff"
    ...
    async def run():
        return await execute_prompt_workflow(
            ...
            tool_runtime=SimpleNamespace(run_task=run_task),
            action_generator=lambda: None,
            on_event=event,
        )
...
def test_native_fail_mislabeled_pass_stops_before_export_and_quote(
    monkeypatch, tmp_path
):
    monkeypatch.setattr("workspace_service.workflow_cad.CadTask", FakeCad)
    workflow = plan()
    export = PromptStep(
        "export", "Flat export", "Export", None, (), "text", "flat.txt", True, "indexed"
    )
    ...
    async def run_task(step, *args, **kwargs):
        calls.append(step.id)
        return (json.dumps(report()) if step.design_check else "Created"), [
            {"status": "succeeded", "result": native}
        ]
