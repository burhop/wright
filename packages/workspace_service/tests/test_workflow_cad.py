...
class CadGateway(Gateway):
    def __init__(self):
        self.calls = []
        self.docs = [
            {"documentId": "original", "displayName": "Original", "isDirty": True},
            {"documentId": "other", "displayName": "Other", "isDirty": False},
        ]
    ...
    def list_tools(self, *args):
        ...
        return tuple(
            GatewayTool(
                ...
                annotations={"readOnlyHint": n.startswith("cad.list_")},
            )
            for n in names
        )
            ...
            value = [
                {
                    ...
                    "isDefault": True,
                    "capabilities": {"exports": ["step"]},
                }
            ]
        elif name == "cad.list_documents":
            value = list(self.docs)
        elif name == "cad.open_document":
            value = self.docs[0]
        else:
            doc = next(d for d in self.docs if d["documentId"] == args["documentId"])
            if args.get("copy"):
                doc = {**doc, "documentId": "copy", "displayName": "Copy"}
                self.docs.append(doc)
            if "outputPath" in args:
                Path(args["outputPath"]).write_bytes(
                    b"CAD result " + str(len(self.calls)).encode()
                )
...
def runner(root, config=None, responses=None):
    ...
    step = PromptStep(
        ...
        server_id="cad",
        agent_task=True,
        cad=config or settings(),
        cad_from="create.model",
    )
...
def test_active_window_change_does_not_redirect_edits_or_exports(tmp_path):
    config = settings()
    config["exports"] = [{"format": "step", "path": "model.step", "port": "export"}]
    ...
    async def scenario():
        ...
        tool = next(
            t
            for t in run.runtime.task_tools(run.step)
            if t.tool_name == "cad.set_variable"
        )
        assert run.guard(tool, {"active": True}) == {
            "active": False,
            "providerId": "solid_edge",
            "documentId": "original",
        }
        ...
        assert all(
            a["documentId"] == "original"
            for n, a in g.calls
            if n in {"cad.save_document", "cad.export_document"}
        )
...
def test_copy_uses_in_memory_save_and_leaves_original_dirty(tmp_path):
    cfg = settings()
    cfg.update(edit_mode="copy", copy_path="copy.psm")
    (tmp_path / "copy.psm").write_bytes(b"previous")
    g, run = runner(tmp_path, cfg)

    async def scenario():
        await run.start()
        assert run.document["documentId"] == "copy"
        ref, files = await run.finish()
        assert ref.document["documentId"] == "copy"
...
def test_upstream_document_cannot_cross_server_or_reopen_by_filename(tmp_path):
    cfg = settings()
    cfg.update(source="upstream", from_port="model")
    _, run = runner(
        tmp_path,
        cfg,
        {"create.model": CadDocument("different", {"documentId": "original"})},
    )
    with pytest.raises(WorkflowSourceExecutionError, match="different server"):
        asyncio.run(run.start())
    g, run = runner(
        tmp_path, cfg, {"create.model": CadDocument("cad", {"documentId": "closed"})}
    )
    with pytest.raises(WorkflowSourceExecutionError, match="no longer open"):
        asyncio.run(run.start())
...
def test_closed_model_and_unsupported_export_fail_without_saving(tmp_path):
    ...
    async def scenario():
        await run.start()
        g.docs = []
        with pytest.raises(WorkflowSourceExecutionError, match="closed"):
            await run.finish()

    asyncio.run(scenario())
    assert not list(tmp_path.iterdir())
    cfg = settings()
    cfg["exports"] = [{"format": "invented", "path": "file.bad"}]
    _, run = runner(tmp_path, cfg)
    with pytest.raises(WorkflowSourceExecutionError, match="unavailable"):
        asyncio.run(run.start())
...
@pytest.mark.parametrize(
    "patch",
    [
        {"native_path": "../escape.psm"},
        {"source": "session", "document_id": ""},
        {"source": "upstream", "from_port": ""},
        {"save_native": False, "exports": []},
    ],
)
def test_missing_target_or_final_deliverables_rejected_before_execution(patch):
    ...
    with pytest.raises(WorkflowSourceExecutionError):
        parse_cad(json.dumps(cfg), terminal=True)
...
def test_retargeting_another_document_is_rejected(tmp_path):
    ...
    async def scenario():
        ...
        with pytest.raises(
            WorkflowSourceExecutionError, match="different CAD document"
        ):
            run.guard(tool, {"documentId": "other"})

    asyncio.run(scenario())


@pytest.mark.parametrize(
    "selector", [None, {}, {"active": True}, {"documentId": "original", "active": True}]
)
def test_inspection_pins_nested_document_to_task_model(tmp_path, selector):
    ...
    tool = GatewayTool(
        name="inspect",
        server_id="cad",
        tool_name="cad.verify_inspection_requirements",
        description="Inspect native geometry",
        input_schema={
            "type": "object",
            "properties": {
                "request": {"type": "object"},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [],
        {"document": []},
        {"document": {"documentId": "other"}},
        {"document": {"documentId": "other", "active": True}},
    ],
)
def test_inspection_rejects_invalid_or_unrelated_nested_document(
    tmp_path, inspection_request
):
    _, run = runner(tmp_path, settings())
    run.caps = {"provider_id": "solid_edge"}
    run.document = {"documentId": "original"}
...
def test_model_reference_passes_through_two_tasks_independently_of_text(tmp_path):
    ...
    first = PromptStep(
        ...
        False,
        "indexed",
        (("model", "cad_model"), ("response", "text")),
        server_id="cad",
        agent_task=True,
        cad=cfg,
    )
    second_cfg = settings()
    second_cfg.update(source="upstream", from_port="model")
    second = replace(
        first, id="second", title="Second edit", cad=second_cfg, cad_from="first.model"
    )
    gateway = CadGateway()
    runtime = WorkflowMcpRuntime(
        gateway, workspace_id="workspace", session_id="session"
        ...
        if decisions % 2:
            selected = (
                next(
                    t
                    for t in tools
                    if "cad.list_variables" in t["function"]["description"]
                )
                if any(
                    "cad.list_variables" in t["function"]["description"] for t in tools
                )
                else next(
                    t
                    for t in tools
                    if "cad.set_variable" in t["function"]["description"]
                )
            )
            return {
                "role": "assistant",
    ...
    result = asyncio.run(
        execute_prompt_workflow(
            ...
            plan=PromptWorkflow("Chain", (first, second), {}),
            input_values={},
    ...
    assert [s["cad_document"]["documentId"] for s in result["steps"]] == [
        "original",
        "original",
    ]
    assert [r["kind"] for r in result["results"]] == ["cad_model", "cad_model"]
    assert (
        len(result["results"][1]["representations"]) == 2
    )  # live document + saved native model
    assert result["results"][1]["provenance"]["input_revisions"] == (
        ("original", None),
    )
    assert [f["output_path"] for f in result["outputs"]] == ["model.psm"]
    assert not list(tmp_path.glob("*.txt"))
    assert all(
        a["documentId"] == "original" and a["active"] is False
        for n, a in gateway.calls
        if n == "cad.set_variable"
    )
...
def test_upstream_revision_change_rejected_before_modifying(tmp_path):
    ...
    g, run = runner(
        ...
        cfg,
        {
            "create.model": CadDocument(
                "cad", {"documentId": "original", "revision": "v1"}
            )
        },
    )
...
def test_handoff_retains_only_unchanged_native_representation(tmp_path, state):
    ...
    result = EngineeringResult(
        ...
        "Model",
        (live, native),
        Provenance("run", "first", "model"),
    )
    cfg = settings()
    cfg.update(source="upstream", from_port="model", save_native=False, exports=[])
    gateway, run = runner(
        tmp_path,
        cfg,
        {"create.model": CadDocument("cad", {"documentId": "original"}, result)},
    )
...
def test_export_returns_post_export_document_state_without_saving_it(tmp_path):
    ...
    config.update(
        save_native=False,
        exports=[{"format": "step", "path": "model.step", "port": "export"}],
    )
...
def test_sheet_metal_task_excludes_unrelated_creation_contracts(tmp_path):
    ...
    sheet = next(
        t for t in tools if t.tool_name == "cad.create_sheet_metal_from_recipe"
    )
    assert run.allows_tool(sheet)
    assert not run.allows_tool(replace(sheet, tool_name="cad.create_part_from_recipe"))
    assert not run.allows_tool(
        replace(sheet, tool_name="cad.create_assembly_from_recipe")
    )
...
def test_created_model_switches_to_bound_inspection_and_cannot_be_recreated(tmp_path):
    ...
    inspect = replace(
        sheet,
        name="cad.measure_body_extents",
        tool_name="cad.measure_body_extents",
        annotations={"readOnlyHint": True},
    )
    assert run.allows_tool(sheet)
    run.caps = {"provider_id": "solid_edge"}
...
def test_rejected_recipe_returns_correction_before_any_mutation(tmp_path):
    ...
    template = next(
        t for t in g.list_tools() if t.tool_name == "cad.create_sheet_metal_from_recipe"
    )
    validator = replace(
        template,
        name="cad.validate_sheet_metal_recipe",
        tool_name="cad.validate_sheet_metal_recipe",
        annotations={"readOnlyHint": True},
    )
        ...
        return GatewayToolResult(
            ...
            structured_content={
                ...
                "issues": [
                    {
                        "code": "dimension_side",
                        "argument": "steps[0].dimensionSide",
                        "message": "Only outside is supported.",
                        "suggestedFix": "Use outside.",
                    }
                ],
            },
        )
    ...
    async def scenario():
        ...
        g.call_tool = reject
        with pytest.raises(
            WorkflowSourceExecutionError, match="Only outside is supported"
        ):
