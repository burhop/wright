...
def _contains_marker(value):
    if isinstance(value, dict):
        return OBSERVATION_REFERENCE in value or any(
            _contains_marker(item) for item in value.values()
        )
...
def project_tool_message(message: dict[str, Any]) -> tuple[dict[str, Any], bool]:
    ...
    def visit(node, pointer):
        ...
        if isinstance(node, list):
            ...
            observations = (
                inspection.get("observations") if isinstance(inspection, dict) else None
            )
            if isinstance(observations, list) and isinstance(evaluations, list):
                locations = {}
                for index, observation in enumerate(observations):
                    if isinstance(observation, dict) and isinstance(
                        observation.get("observationId"), str
                    ):
                        locations.setdefault(
                            _compact(observation),
                            f"{pointer}/inspection/observations/{index}",
                        )
...
def decode_tool_message(message: dict[str, Any]) -> tuple[dict[str, Any], bool]:
    ...
    try:
        ...
        if (
            ...
            or _strict_json(_compact(value)) != original
        ):
            return message, False
    except (ValueError, TypeError, RecursionError, OverflowError):
        return message, False
