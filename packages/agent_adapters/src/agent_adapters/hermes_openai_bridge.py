...
_TRANSLATION_SYSTEM_PROMPT = "Follow the protocol translation contract exactly; do not execute any tool."
        ...
        if isinstance(value, list):
            ...
            if (
                ...
                or any(key in value for key in (
                    "inspection", "requirementEvaluations", "allCriticalRequirementsPass", "overall", "verdict",
                ))
                or any(key in value and value[key] is not True for key in ("isValid", "isSuccess", "isSatisfied"))
                or value.get("isError") is True
                or value.get("status") in (
                    "failed", "error", "blocked", "unverified", "needs_input",
                    "rejected", "cancelled", "canceled", "timeout",
                )
            ):
                return True
...
class HermesOpenAICompatibilityBridge:
    ...
    def _translation_prompt(self, request: _ValidatedRequest) -> str:
        ...
        if self.settings.workflow_task:
            preamble = (
                ...
                + ('When the requested work is evidenced, use {"kind":"message","content":"the requested response"}. '
                   if optional else "Choose a tool call; execution evidence is required. ")
        ...
        def render(messages: list[dict[str, Any]], omissions: list[dict[str, Any]] | None = None,
                   reference_calls: list[str] | None = None,
                   decoded_calls: list[str] | None = None) -> str:
            transcript = {
            ...
            if reference_calls:
                transcript["wright_observation_references"] = observation_reference_guide(reference_calls)
            if decoded_calls:
                transcript[TOOL_CONTENT_ENCODING] = tool_content_encoding_guide(decoded_calls)
    ...
    def _workflow_translation_prompt(
        ...
        for index, group in enumerate(groups):
            ...
            results = {message.get("tool_call_id"): message for message in group if message.get("role") == "tool"}
            if not calls:
                if results:
                    pinned.add(index)
                continue
        ...
        def within_text_budget(prompt: str) -> bool:
            try:
                ...
                transport_bytes = len("\n".join(self._workflow_transport_text_parts(prompt)).encode("utf-8"))
        ...
        def candidate(indexes: set[int]) -> tuple[str, bool]:
            retained = [message for index in sorted(indexes) for message in groups[index]]
            omitted = [item for index, metadata in exchange_metadata.items() if index not in indexes for item in metadata]
            prompt = render(retained, omitted)
            text_fits = within_text_budget(prompt)
            if not text_fits and projected is not None:
                reference_calls = [message["tool_call_id"] for message in retained if projected[id(message)][1]]
                prompt = render([projected[id(message)][0] for message in retained], omitted, reference_calls)
                text_fits = within_text_budget(prompt)
                if not text_fits and decoded is not None:
                    decoded_calls = [message["tool_call_id"] for message in retained if decoded[id(message)][1]]
                    prompt = render([decoded[id(message)][0] for message in retained], omitted, reference_calls, decoded_calls)
                    text_fits = within_text_budget(prompt)
        ...
        decoded = {id(message): decode_tool_message(projected[id(message)][0]) for message in messages}
        prompt, fits = candidate(selected)
        if fits:
            return prompt
        ...
        eviction_order = sorted(range(len(groups)), key=lambda index: (index not in exchange_metadata, index))
        ...
        if len(selected) < len(groups):
            logger.info(
                ...
                omitted_messages=sum(len(group) for index, group in enumerate(groups) if index not in selected),
    ...
    def _upstream_payload(
        ...
        if translate:
            ...
            for message in request.messages:
                content = message.get("content")
                if self.settings.workflow_task and message.get("role") == "user" and isinstance(content, list):
                    parts = []
                    for part in content:
                        if isinstance(part, dict) and part.get("type") == "image_url":
                            image = part.get("image_url")
                            if not isinstance(image, dict) or not isinstance(image.get("url"), str) or not image["url"].startswith("data:image/"):
                                raise HermesBridgeError("invalid_request", "Workflow image references must contain workspace image data.")
                            images.append(part)
                            parts.append({"type": "text", "text": f"[Reference image {len(images)} attached to this request]"})
                        else:
                            parts.append(part)
                    message = {**message, "content": parts}
                translated_messages.append(message)
            prompt = self._translation_prompt(replace(request, messages=translated_messages))
            text_parts = self._workflow_transport_text_parts(prompt) if self.settings.workflow_task else [prompt]
            content = (
                [{"type": "text", "text": part} for part in text_parts] + images
                if images or len(text_parts) > 1 else prompt
            )
            messages = [
                {
                    "role": "system",
    ...
    def _translation_repair_payload(
        ...
        if self.settings.workflow_task:
            ...
            reserve = sum(len(_compact(message).encode("utf-8")) for message in repair_messages)
            ...
            payload = retry_bridge._upstream_payload(request, translate=True, stream=False)
            ...
            if self.settings.workflow_task:
                ...
                for issue in [error, *error.context[:3]]:
                    summary: dict[str, Any] = {
                        ...
                        "schema_path": [str(part)[:80] for part in issue.absolute_schema_path],
                        "constraint": issue.validator,
                    }
                    if issue.validator == "required" and isinstance(issue.instance, dict):
                        summary["missing"] = [
                            key[:80] for key in issue.validator_value if key not in issue.instance
                        ][:10]
                    if issue.validator == "type":
                        summary["expected_type"] = issue.validator_value
                    issues.append(summary)
                logger.info("workflow_tool_arguments_rejected", tool=name, issues=issues)
