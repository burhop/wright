...
async def test_workflow_images_stay_multimodal_through_translation_and_repair():
    ...
    async def handler(request):
        ...
        assert content[1] == image  # Full pixels, even above transcript truncation limit.
        calls.append(payload)
        decision = "invalid decision" if len(calls) == 1 else '{"kind":"tool_call","name":"create_graph","arguments":{"title":"From sketch"}}'
    ...
    bridge = HermesOpenAICompatibilityBridge(HermesOpenAIBridgeSettings(
        base_url="http://127.0.0.1:8642",api_key="fixture-key",workflow_task=True),transport=httpx.MockTransport(handler))
    result = await bridge.complete(_request(messages=[{"role":"user","content":[{"type":"text","text":"Create from this sketch"},image]}], tools=[_tool()]))
    assert len(calls) == 2
    assert result["choices"][0]["message"]["tool_calls"][0]["function"]["name"] == "create_graph"
...
async def test_workflow_does_not_fetch_an_arbitrary_image_url():
    ...
    with pytest.raises(HermesBridgeError, match="workspace image data"):
        await bridge.complete(_request(messages=[{"role":"user","content":[
            {"type":"image_url","image_url":{"url":"https://example.invalid/private.png"}}]}],tools=[_tool()]))
