...
def observation_exchange(identity, result, *, name="inspect_model", arguments=None, number=None, status="succeeded"):
    """Use the actual workflow executor envelope for evictable observations."""
    messages = exchange(identity, json.dumps({
        "tool_call_number": number, "status": status, "result": result, "text": "",
    }))
...
def bridge(handler, **settings):
    return HermesOpenAICompatibilityBridge(
        HermesOpenAIBridgeSettings(
            base_url="http://127.0.0.1:8642", api_key="test-only",
            workflow_task=True, maximum_translation_prompt_bytes=120_000, **settings,
        ),
...
async def test_complete_large_task_and_middle_geometry_survive_transport_with_image():
    task = "Préface μm. " * 1400 + "HOLES MUST BE 25 mm FROM THE FREE EDGE." + " Finish." * 1700
    faces = [{"face_id": f"face-{index}", "observed": "surface " * 32} for index in range(100)]
    faces[50] = {"face_id": "critical-hole", "distance_to_free_edge_mm": 23.3998}
    measurement = json.dumps({"faces": faces})
    image = {"type": "image_url", "image_url": {"url": "data:image/png;base64," + "AAAA" * 16000}}
    messages = [
        {"role": "system", "content": "Compare actual dimensions; do not assume acceptance."},
...
async def test_compaction_removes_whole_old_exchanges_and_preserves_instructions(maximum_messages):
    instructions = [
        {"role": "system", "content": "Preserve the source model."},
        {"role": "developer", "content": "Failed evidence cannot release manufacturing."},
    ...
    latest = json.dumps({"faces": [{"id": index, "geometry": "x" * 200} for index in range(140)]})
    messages.extend(exchange("latest", latest))
    correction = {"role": "user", "content": "Keep all four holes; report failures accurately."}
    ...
    async def handler(sent):
        ...
        calls = {call["id"] for message in actual for call in message.get("tool_calls", [])}
        results = {message["tool_call_id"] for message in actual if message["role"] == "tool"}
...
@pytest.mark.asyncio
@pytest.mark.parametrize("overflow", ["task", "observation", "schemas", "message_count"])
async def test_required_current_context_overflow_fails_before_model_call(overflow):
    ...
    if overflow == "schemas":
        tools = [{"type": "function", "function": {
            **TOOL["function"],
            "parameters": {"type": "object", "properties": {"value": {"enum": ["x" * 121000]}}},
        }}]
    if overflow == "message_count":
        settings["maximum_messages"] = 3
    messages.extend(exchange("latest", "x" * 121000 if overflow == "observation" else "current"))
...
@pytest.mark.asyncio
@pytest.mark.parametrize("rejected", ["x" * 8192, "μ" * 8192], ids=["ascii", "multibyte"])
async def test_retry_overflow_does_not_send_incomplete_current_evidence(rejected):
    ...
    async def handler(request):
        ...
        return httpx.Response(200, json={"choices": [{"message": {"content": rejected}}]})

    with pytest.raises(HermesBridgeError) as error:
        await bridge(handler).complete(request([{"role": "user", "content": task}, {"role": "user", "content": task}]))
    assert error.value.code == "workflow_context_limit"
    assert len(sent) == 1
...
async def test_retry_budget_evicts_only_older_exchanges_and_preserves_latest_evidence():
    ...
    async def handler(request):
        ...
        if len(sent) == 1:
            return httpx.Response(200, json={"choices": [{"message": {"content": "μ" * 8192}}]})
        assert len(actual) < len(conversation(sent[0]))
        assert sum(len(("\n".join(p["text"] for p in message["content"] if p.get("type") == "text")
                        if isinstance(message["content"], list) else message["content"]).encode("utf-8"))
                   for message in payload["messages"]) <= 120000
        calls = {call["id"] for message in actual for call in message.get("tool_calls", [])}
        results = {message["tool_call_id"] for message in actual if message["role"] == "tool"}
        assert calls == results
        return reply()
...
async def test_compaction_preserves_each_tool_and_discloses_different_argument_history():
    ...
    instructions = [
        ...
        {"role": "user", "content": [{"type": "text", "text": "Requirements " + "x" * 52000}, image]},
    ]
    messages = [*instructions]
    records = [
    ...
    for number, unit in ((6, "mm"), (7, "in")):
        messages.extend(observation_exchange(f"faces-{unit}", {
            "geometry": "g" * 30000,
            "unit": unit,
            "evidenceStatus": {"disposition": "not_evaluated", "provenance": "observed"},
        }, name="faces", arguments={"documentId": "model-1", "unit": unit}, number=number))
    original = json.dumps(messages)

    async def handler(sent):
        upstream = json.loads(sent.content)
        actual = conversation(upstream)
        retained = {call["id"] for message in actual for call in message.get("tool_calls", [])}
        ...
        assert json.loads(omitted["arguments"]) == {"documentId": "model-1", "unit": "mm"}
        assert json.loads(actual[-2]["tool_calls"][0]["function"]["arguments"])["unit"] == "in"
...
@pytest.mark.parametrize("earlier", [
    ...
    observation_exchange("earlier", {"evidenceStatus": {"disposition": "fail", "provenance": "observed"}}),
    observation_exchange("earlier", {"evidenceStatus": {"disposition": "not_evaluated", "provenance": "unavailable"}}),
...
async def test_earlier_unsuccessful_or_uncertain_evidence_is_not_evicted_by_later_success(earlier):
...
async def test_required_distinct_tool_floor_overflow_fails_without_model_request():
    ...
    for name in ("features", "variables", "faces"):
        messages.extend(observation_exchange(name, {"evidence": "x" * 45000}, name=name))
...
async def test_omission_metadata_is_part_of_required_context_budget():
    ...
    messages = [
        ...
        *observation_exchange("earlier", {"old": True}, arguments={"selector": "x" * 120000}),
...
async def test_schema_repair_identifies_nested_missing_field():
    ...
    async def handler(request):
        ...
        decision = {"kind": "tool_call", "name": "inspect_model", "arguments": arguments}
        return httpx.Response(200, json={"choices": [{"message": {"content": json.dumps(decision)}}]})

    result = await bridge(handler).complete(request([{"role": "user", "content": "Inspect."}], [tool]))
    assert len(sent) == 2
    assert json.loads(result["choices"][0]["message"]["tool_calls"][0]["function"]["arguments"])["recipe"]["height"] == 60
...
async def test_schema_failure_reports_location_and_type_without_argument_values():
    ...
    async def handler(request):
        decision = {"kind": "tool_call", "name": "inspect_model", "arguments": {"height": secret}}
        return httpx.Response(200, json={"choices": [{"message": {"content": json.dumps(decision)}}]})

    with pytest.raises(HermesBridgeError) as error:
        await bridge(handler).complete(request([{"role": "user", "content": "Inspect."}], [tool]))
