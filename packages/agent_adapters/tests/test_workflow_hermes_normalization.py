...
from test_workflow_transcript_evidence import bridge, request, reply, TOOL, observation_exchange
...
def bounded_normalize(content):
    ...
    parts = [({**part, "text": part["text"][:65_536]} if part["type"] == "text" else part)
             for part in content]
    return "\n".join(part["text"] for part in parts) if all(part["type"] == "text" for part in parts) else parts
...
def normalize(request):
    ...
    source = Path(os.environ.get("LOCALAPPDATA", "")) / "hermes/hermes-agent/gateway/platforms/api_server.py"
    ...
    constants = {node.targets[0].id: ast.literal_eval(node.value) for node in tree.body
                 ...
                 and node.targets[0].id in {"MAX_NORMALIZED_TEXT_LENGTH", "MAX_CONTENT_LIST_SIZE"}}
    assert constants["MAX_NORMALIZED_TEXT_LENGTH"] == 65_536
    assert len(functions) == 2
    namespace = {"Any": Any, "Dict": Dict, "List": List, **constants,
...
async def test_complete_semantic_parts_survive_real_hermes_boundary(normalize, with_image, repair):
    image = {"type": "image_url", "image_url": {"url": "data:image/png;base64,AAAA"}}
    task = 'Reference μm; preserve "quoted" paths C:\\parts\\design. ' * 700
    faces = [{"faceId": f"model1:face{index}", "surfaceType": "plane" if index <= 8 else "cylinder",
              "radius": .125 if index > 8 else None, "observation": "measured " * 170}
             for index in range(1, 15)]
    faces[10]["radius"] = .118
    faces[11]["radius"] = .038
    variables = {"NeutralFactor": .48, "BendCalculationMethod": 0, "native_rows": "v" * 13000}
    messages = [{"role": "system", "content": "Inspect every required native observation."},
                {"role": "user", "content": [{"type": "text", "text": task}, image] if with_image else task},
                *observation_exchange("variables", variables, name="variables", number=4),
                *observation_exchange("faces", {"result": faces}, number=5)]
    tools = [TOOL, {"type": "function", "function": {**TOOL["function"], "name": "variables",
    ...
    async def handler(call):
        ...
        assert (actual[1]["content"][0]["text"] if with_image else actual[1]["content"]) == task
        assert json.loads(actual[-1]["content"])["result"]["result"] == faces
        assert json.loads(actual[-3]["content"])["result"] == variables
        assert [t["name"] for t in transcript["tools"]] == ["inspect_model", "variables"]
...
async def test_indivisible_oversized_item_fails_before_http(item):
    ...
    if item == "task":
        ...
        tools = [{"type": "function", "function": {**TOOL["function"], "parameters": {
            "type": "object", "properties": {"selector": {"type": "string", "enum": ["x" * 66_000]}}}}}]
