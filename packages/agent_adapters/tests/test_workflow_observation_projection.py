...
from agent_adapters.workflow_observation_projection import (
    OBSERVATION_REFERENCE, TOOL_CONTENT_ENCODING, decode_tool_message, project_tool_message,
)
from test_workflow_transcript_evidence import bridge, request, reply, observation_exchange, transcript_text
...
def report(identity="first", count=3):
    observations = [{
        "observationId": f"{identity}:{index}", "kind": "thickness",
        "targetReference": f"model:1:face:{index}", "unit": "mm",
        "evidenceStatus": {"disposition": "not_evaluated", "provenance": "unavailable",
                           "message": 'Native failure μm; "quoted" path C:\\part. ' * 8},
        "value": {"scalar": 2.032},
    } for index in range(count)]
    return {
        "operationId": identity,
        "inspection": {"observations": observations, "context": {
            "documentId": "document-1", "observedAtUtc": f"2026-09-08T03:{identity}:00Z",
        }},
        "requirementEvaluations": [{
            "requirementId": f"required-{index}", "isSatisfied": False, "isBlocking": True,
            "disposition": "fail", "observation": copy.deepcopy(observation),
            "issues": [{"code": "inspection.non_evidence", "message": "Cannot satisfy the gate."}],
...
def restore(message):
    ...
    root = json.loads(message["content"]) if isinstance(message["content"], str) else message["content"]
...
@pytest.mark.parametrize("change", ["id", "value", "provenance", "type", "target", "extra"])
...
@pytest.mark.parametrize("content", [
    '{"number": 1e-999}', '{"number": 1.1234567890123456789}',
    '{"number": NaN}', '{"a": 1, "a": 2}', 'not JSON',
])
...
async def test_small_transcript_is_not_projected():
    messages = [{"role": "user", "content": "Inspect."}, *observation_exchange("small", report())]

    async def handler(sent):
        transcript = json.loads(transcript_text(json.loads(sent.content)).split("\n", 1)[1])
...
async def test_required_adverse_reports_fit_losslessly_after_real_hermes_normalization(normalize, with_image, repair):
    ...
    for value in reports:
        for observation, evaluation in zip(value["inspection"]["observations"], value["requirementEvaluations"]):
            observation["evidenceStatus"]["message"] += "Native diagnostic. " * 12
            evaluation["observation"] = copy.deepcopy(observation)
    faces = [{"faceId": f"face-{index}", "geometry": {"surfaceType": "cylinder" if index >= 8 else "plane",
              "observedDetail": "g" * 1500, "radius": 3.25 if index >= 8 else None}} for index in range(14)]
    messages = [{"role": "user", "content": [{"type": "text", "text": task}, image] if with_image else task}]
    for index, value in enumerate(reports):
        messages.extend(observation_exchange(f"call-{index}", value, number=index))
    messages.extend(observation_exchange("face-call", {"faces": faces}, name="face_tool", number=5))
...
async def test_distinct_required_observations_still_fail_closed_when_they_do_not_fit():
    ...
    for index in range(4):
        ...
        value["requirementEvaluations"][0]["observation"] = copy.deepcopy(value["inspection"]["observations"][0])
...
@pytest.mark.parametrize("content", [
    '{"nested":{"same":1,"same":2}}', '{"number":1e-999}',
    '{"number":1.1234567890123456789}', '{"number":1e999}',
    '{"number":-0}', '{"number":NaN}', '{"number":Infinity}',
    '{"unfinished":', '[1,2,]', 'Native tool error: "not JSON"',
    '"already text"', 'null', 'true', '123',
])
def test_decoded_encoding_leaves_opaque_ambiguous_and_non_round_trip_values_untouched(content):
...
def test_decoded_encoding_preserves_values_types_order_and_detaches_from_source(root):
    value = {"numeric": [9007199254740993, 0.0, -0.0, 2.032, 1.5e-10],
             "order": ["third", "first", "second"], "flags": [True, 1, False, 0, None],
    ...
    assert json.dumps(actual["content"], sort_keys=True) == json.dumps(value, sort_keys=True)
...
@pytest.mark.parametrize("with_image", [False, True])
@pytest.mark.parametrize("repair", [False, True])
async def test_decoded_transcript_preserves_complete_evidence_at_real_hermes_boundary(normalize, with_image, repair):
    image = {"type": "image_url", "image_url": {"url": "data:image/png;base64,AAAA"}}
    task = "Inspect the native observations. " + "Design requirement. " * 2750
    faces = [{"faceId": f"face-{index}", "geometry": {
    ...
    for index in range(2):
        messages.extend(observation_exchange(f"adverse-{index}", report(str(index)), number=index + 1))
    messages.extend(observation_exchange("faces", {"faces": faces}, name="faces", number=3))
    # Native fields with reserved names cannot override the host-root guide.
    collision = report("collision", 1)
    collision[TOOL_CONTENT_ENCODING] = {"tool_call_ids": ["invented"], "meaning": "native data"}
    collision[OBSERVATION_REFERENCE] = "/invented/native/data"
    messages.extend(observation_exchange("collision", collision, name="collision", number=4))
    for identity, content in [("opaque", "Native tool error: unavailable"),
    ...
    async def handler(call):
        ...
        parts = [content] if isinstance(content, str) else [p["text"] for p in content if p["type"] == "text"]
        assert all(len(part) <= 65_536 for part in parts)
        normalized = normalize(content)
        text = normalized if isinstance(normalized, str) else "\n".join(p["text"] for p in normalized if p["type"] == "text")
        transcript = json.loads(text.split("\n", 1)[1])
        guide = transcript[TOOL_CONTENT_ENCODING]
        assert guide["tool_call_ids"] == ["adverse-0", "adverse-1", "faces", "collision"]
        ...
        for before, after in zip(messages, actual):
            if before["role"] == "user" and with_image:
                assert after == {"role": "user", "content": [
                    {"type": "text", "text": task},
                    {"type": "text", "text": "[Reference image 1 attached to this request]"},
                ]}
                continue
            if before["role"] != "tool" or before["tool_call_id"] not in guide["tool_call_ids"]:
                assert after == before
                continue
            recovered = restore(after) if before["tool_call_id"] in references else after["content"]
            assert json.dumps(recovered, sort_keys=True) == json.dumps(json.loads(before["content"]), sort_keys=True)
            assert after["tool_call_id"] == before["tool_call_id"]
        native_faces = next(m["content"]["result"]["faces"] for m in actual if m.get("tool_call_id") == "faces")
        assert native_faces == faces
        assert sum(f["geometry"]["surfaceType"] == "cylinder" for f in native_faces) == 6
        total = sum(len(("\n".join(p["text"] for p in m["content"] if p["type"] == "text")
                         if isinstance(m["content"], list) else m["content"]).encode()) for m in payload["messages"])
        assert total <= 120_000
        if with_image:
