...
def route_to_transport(monkeypatch, handler):
    ...
    monkeypatch.setattr(
        ...
        lambda: SimpleNamespace(
            base_url="http://invalid.test-only", api_key="test-secret"
        ),
    )
    ...
    def construct(settings, **kwargs):
        ...
        return actual_bridge(
            settings, transport=kwargs.get("transport", httpx.MockTransport(handler))
        )

    monkeypatch.setattr(
        hermes_openai_bridge, "HermesOpenAICompatibilityBridge", construct
    )


@pytest.mark.asyncio
@pytest.mark.parametrize("repair", [False, True])
async def test_workflow_policy_preserves_twenty_messages_and_image_through_hermes(
    normalize, monkeypatch, repair
):
    image = {"type": "image_url", "image_url": {"url": "data:image/png;base64,AAAA"}}
    messages = [
        {"role": "system", "content": "Keep unknown and adverse observations."},
        {
            "role": "user",
            "content": [{"type": "text", "text": "Requirements μm. " * 1700}, image],
        },
    ]
    for index, size in enumerate(
        (5394, 3784, 13047, 31159, 4354, 18094, 215, 397, 34247)
    ):
        messages.extend(
            observation_exchange(
                f"native-{index}",
                {
                    "observationId": f"distinct-{index}",
                    "nativeDetail": str(index) * size,
                    "evidenceStatus": {
                        "provenance": "unavailable" if index == 5 else "observed",
                        "disposition": "fail" if index == 8 else "not_evaluated",
                    },
                    "isSatisfied": False,
                    "measured": -0.0 if index == 5 else 2.032,
                    "targetReference": f"model:1:face:{index + 35}",
                },
                number=index + 1,
                arguments={"unit": "mm" if index < 8 else "in"},
            )
        )
    ...
    assert (
        HermesOpenAIBridgeSettings(
            base_url="http://invalid", api_key="test"
        ).maximum_translation_prompt_bytes
        == 60_000
    )
...
async def test_workflow_policy_still_fails_before_http_for_unrepresentable_evidence(
    monkeypatch, limit
):
        ...
        messages.extend(
            observation_exchange(
                f"required-{index}",
                {
                    "isSatisfied": False,
                    "evidence": str(index)
                    * (31000 if limit == "mandatory_floor" else 66000),
                },
                number=index + 1,
            )
        )
