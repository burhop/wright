...
def test_review_migration_is_additive_and_repository_survives_restart(tmp_path):
    ...
    with connect_state_db(database) as db:
        ...
        db.execute(
            "INSERT INTO existing_artifacts VALUES (?,?)",
            ("draft.html", b"exact original"),
        )
    ...
    package = {
        "snapshots": [
            {
                "path": "draft.html",
                "sha256": hashlib.sha256(b"exact original").hexdigest(),
            }
        ]
    }
    ...
    with connect_state_db(database) as db:
        assert (
            db.execute("SELECT content FROM existing_artifacts").fetchone()[0]
            == b"exact original"
        )
    with pytest.raises(KeyError):
        restarted.get("another-workspace", "review")


def test_decision_compare_and_set_leaves_immutable_package_and_original_timestamp(
    tmp_path,
):
    repository = WorkflowArtifactReviewRepository(str(tmp_path / "state.db"))
    repository.create(
        "review", "workspace", "workflow.wflow", "a" * 64, {"original": "bytes"}
    )
    ...
    record = repository.decide(
        "workspace", "review", "a" * 64, {**decision, "decided_at": "retry"}
    )
    assert json.loads(record["decision_json"])["decided_at"] == "first"
    assert record["package_json"] == before
    with pytest.raises(ArtifactReviewConflict):
        repository.decide("workspace", "review", "b" * 64, decision)
    with pytest.raises(ArtifactReviewConflict):
        repository.decide(
            "workspace", "review", "a" * 64, {**decision, "state": "changes_requested"}
        )
