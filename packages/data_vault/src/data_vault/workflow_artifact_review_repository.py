...
class WorkflowArtifactReviewRepository:
    ...
    def create(self, review_id, workspace_id, path, digest, package):
        raw = json.dumps(package, ensure_ascii=False, separators=(',', ':'), allow_nan=False)
        ...
        with connect_state_db(self.db_path, ensure_parent=True) as db:
            ...
            db.execute('INSERT INTO workflow_artifact_reviews VALUES (?,?,?,?,?,?,?)',
                       (review_id, workspace_id, path, digest, raw, 'pending', None))

    def get(self, workspace_id, review_id):
        with connect_state_db(self.db_path, ensure_parent=True) as db:
            db.execute(REVIEW_SCHEMA)
            row = db.execute('SELECT * FROM workflow_artifact_reviews WHERE workspace_id=? AND review_id=?',
                             (workspace_id, review_id)).fetchone()
        if row is None:
    ...
    def list(self, workspace_id, path):
        with connect_state_db(self.db_path, ensure_parent=True) as db:
            db.execute(REVIEW_SCHEMA)
            rows = db.execute('SELECT * FROM workflow_artifact_reviews WHERE workspace_id=? AND workflow_path=? ORDER BY rowid DESC LIMIT 20',
                              (workspace_id, path)).fetchall()
        ...
        with connect_state_db(self.db_path, ensure_parent=True) as db:
            ...
            try:
                ...
                if row['state'] != 'pending':
                    ...
                    if any(previous.get(key) != decision.get(key) for key in ('state', 'actor', 'reason')):
                        raise ArtifactReviewConflict('A different decision was already recorded')
                else:
                    db.execute('UPDATE workflow_artifact_reviews SET state=?, decision_json=? WHERE review_id=? AND state=?',
