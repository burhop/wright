...
def test_representative_graph_and_layout_cover_each_semantic_identity_once() -> None:
    ...
    assert {
        item.id: item.produced_by_block_id for item in draft.semantic.intended_artifacts
    } == {
    ...
    assert {item.semantic_id: (item.x, item.y) for item in draft.layout.positions} == {
