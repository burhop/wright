...
def test_command_application_is_atomic_and_accepts_one_revision() -> None:
    ...
    assert (
        next(
            block for block in current.blocks if block.id == "block.generate-geometry"
        ).title
    ...
    assert (
        next(
            block for block in accepted.blocks if block.id == "block.generate-geometry"
        ).title
        == "Generate approved bracket geometry"
    )
