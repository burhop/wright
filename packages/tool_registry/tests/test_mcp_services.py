...
async def test_activation_preserves_approval_for_lifecycle_check(db_path):
    ...
    server = _insert_server(
        db_path, risk_level="high", approval_gates=["workspace_write_approval"]
    )
    ...
    await services.toggle_server_activation(
        engine, server.server_id, True, approval_context=approval
    )
