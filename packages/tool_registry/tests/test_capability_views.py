...
def test_platform_filter_uses_catalog_support_without_machine_probes():
    ...
    assert paginate_capabilities(
        [entry], views, filters=CapabilityFilters(platforms=frozenset({"linux_x64"}))
    ).total == 1
    assert paginate_capabilities(
        [entry], views, filters=CapabilityFilters(platforms=frozenset({"windows_11_x64"}))
    ).total == 0
