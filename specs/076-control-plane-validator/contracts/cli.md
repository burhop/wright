...
Only explicitly listed producer/consumer versions are accepted. Legacy v1 is limited to `epp-bootstrap-v1-r1-r9` and the enumerated `epp-bridge-v1-r10-r19`; no later v1 record and no second v1-to-v2 migration is accepted. The committed-identity correction is only the exact V4-approved `COR-EPP-F01-US1-COMMITTED-IDENTITY-001`. The separate transition input-origin correction is only the exact V5-approved `COR-EPP-F01-US1-TR0027-INPUT-ORIGIN-001` and its single TR-0027 `/inputs/3` claim. An older reader, different profile, partial/extra target set, origin mismatch, or unauthorized profile fails closed. Unknown major and undeclared newer minor versions fail closed. Seed and generated dashboard bytes never claim committed validity; only the external validation delivery envelope can prove it.
