...
**Status**: Blocked — V5 exact material-change and implementation approval required
...
**Resolved boundary**: In EPP-F01, “dashboard” means the governed `dashboard.json` snapshot and equivalent CLI report only. The browser-accessible program-status page required by the parent program is the separate dependency-ordered EPP-F01B feature and is not one of these 72 tasks. The proposed V7 amendment adds T070–T072 only for one exact two-claim repair-evidence correction; it does not authorize implementation.
