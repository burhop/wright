...
**Status**: Blocked — V9 planning/re-analysis only; no implementation lease
...
**Resolved boundary**: In EPP-F01, “dashboard” means the governed `dashboard.json` snapshot and equivalent CLI report only. The browser-accessible program-status page remains separate EPP-F01B work. T070 and T071 are complete; T072 failed closed. The approved V8 implementation was interrupted before T073 completed because clean-subject preflight exposed two additional immutable evidence defects. V9 plans only their closed append-only disposition through T077–T080: the schema-less exact `EPP-F01-V8-discovery.json` blob and TR-0051's complete-but-self-last changed-path manifest. T073–T076, the roadmap-policy defect, T066–T068, EPP-F01B implementation, and every excluded action remain unauthorized until separately reactivated.
...
4. **Given** the exact approved correction profile `COR-EPP-F01-US1-COMMITTED-IDENTITY-001`, **When** validation replays the immutable history, **Then** it recomputes and visibly disposes exactly 37 factual metadata claims, retains the original records and findings, and rejects any extra, missing, future, circular, readiness, gate, benchmark, release, authority, or correction target.
5. **Given** the exact approved profile `COR-EPP-F01-US1-TR0027-INPUT-ORIGIN-001`, **When** validation reaches TR-0027 `/inputs/3`, **Then** it proves the referenced approval is absent at the declared source and is the exact blob first added with TR-0027's container, retains the original finding and bytes, and resolves only that one input-origin finding.
...
- A correction profile is absent, unapproved, partly matched, self-referential, targets a non-ancestor or forbidden class, contains a wildcard/range, or disagrees with committed Git-object bytes.
- A repair-evidence correction includes any state occurrence other than revisions 45 and 46 at the exact recovery pointer, or treats the operational state pointer as historical evidence.
- The TR-0044 repair claim points anywhere except `/inputs/1/sha256`, accepts a value other than the exact 63-character recording, or cannot recompute the exact TR-0043 blob and 64-character SHA-256.
- The TR-0027 input-origin profile points anywhere except `/inputs/3`, permits another transition or input, finds the approval at the declared source, fails to find both exact blobs in the unique container, or changes any manifest, authority, readiness, benchmark, candidate, freshness, or release result.
...
- **FR-016**: Findings MUST use stable reason codes, severity, affected artifact, exact JSON pointer when applicable, invariant, evidence, consequence, bounded recovery, resolution status, and nullable correction reference; output ordering MUST be deterministic, all safely discoverable independent findings MUST be reported in one run, and correction disposition MUST retain rather than suppress the original finding.
...
- **FR-025**: The validator MAY resolve historical committed-identity findings only through the single closed profile `COR-EPP-F01-US1-COMMITTED-IDENTITY-001`. The profile MUST enumerate exactly six immutable v2 transition output-digest claims and exactly 31 tree-value JSON-pointer occurrences across immutable state revisions 1–26; bind each target to its repository-relative path, introducing commit/tree, Git blob, raw SHA-256, canonical state digest where applicable, recorded value, authoritative value, and recomputation method; require every target container to be a strict ancestor of the correction-containing commit; and reject any substitution, omission, addition, wildcard, range, same/future/circular target, or correction-of-correction. Applying the profile MUST preserve original bytes and findings, report their resolution with the correction reference, affect only program-health factual-metadata disposition, and leave all readiness areas, gate evidence, benchmark counts, freshness, candidate identity, approvals, and release eligibility unchanged. Validators that do not understand the profile MUST fail closed.
- **FR-026**: The validator MAY resolve the one historical TR-0027 input-origin finding only through `COR-EPP-F01-US1-TR0027-INPUT-ORIGIN-001`. It MUST bind the exact TR-0027 bytes/blob and `/inputs/3`, the exact approval path/bytes/blob, declared source commit `c3012733d358dbbeb4821a2fbf5449d6d1b12c47`, unique containing commit `88481d57f1258f59f303f507eafc4e352569bc11` and tree, prove source absence plus container introduction, preserve the original bytes/finding, and reject every added, omitted, substituted, wildcard, range, manifest, authority, readiness, benchmark, candidate, freshness, release, or correction target. The profile MUST be separately bound by exact V5 `material_change` and `feature_implementation` approvals; unsupported readers fail closed.
- **FR-027**: The validator MAY resolve the two repair-evidence findings discovered at `b61b069c6a68688193a60c7f39d9f2b8044027bf` only through `COR-EPP-F01-REPAIR-EVIDENCE-001`. Claim `REPAIR-CAUSE-ID-001` MUST bind exactly the two immutable occurrences at state revisions 45 and 46 `/active_mutating_lease/recovery/active_cause_id`, their raw bytes/blobs, introducing commits/trees and canonical state digests, and the invalid-to-valid spelling relation without changing original bytes or the successor operational state. Claim `TR0044-INPUT-DIGEST-001` MUST bind exact TR-0044 bytes/blob and `/inputs/1/sha256`, the recorded 63-character value, exact TR-0043 bytes/blob and recomputed 64-character SHA-256. The profile MUST reject every third occurrence, other pointer, path, state, transition, wildcard, range, present/future record, manifest, lifecycle/action, authority, readiness, benchmark, candidate, delivery, release, or correction target; require both claims or fail; retain original findings; prove correction-off/on projection equality; and remain ineffective until separate same-subject V7 `material_change` and `feature_implementation` approvals bind its exact digest. Unsupported readers fail closed.
- **FR-028**: V8 MAY dispose exactly three later immutable findings only through `COR-EPP-F01-T072-CHECKPOINT-EVIDENCE-001`: TR-0047 `/outputs/0/sha256`, TR-0047 `/outputs/1/sha256`, and the exact TR-0050 event tuple. The two digest claims MUST hash raw committed Git blob bytes, never checkout Markdown bytes; the event claim MUST bind only TR-0050 and map it to the already-defined `repair/repair_checkpoint/BLOCKED/BLOCKED` rule with its exact required-evidence mapping, without editing TR-0050 or broadening lifecycle policy. Separately, the gate-evidence repair MUST change only `/catalog_digest` to the SHA-256 of exact committed gate-catalog blob bytes, and the two walkthrough causes MUST replace stale fixed authority/state assumptions with current machine-derived truth and emit repository-relative resolvable finding paths. Every other source, test, transition, policy, claim, readiness value, benchmark value, dashboard byte, candidate, approval, delivery, and release result MUST remain unchanged. The separately failing roadmap-policy inversion test is an explicit excluded P0 question and blocks T066 after V8.
- **FR-029**: V9 MAY disposition exactly two immutable preflight findings only through `COR-EPP-F01-V9-PREFLIGHT-EVIDENCE-001`. Claim `V8-DISCOVERY-SCHEMA-REFERENCE-001` MUST externally validate the exact committed schema-less discovery blob against the exact-value `v8-discovery-evidence.schema.json` contract while retaining the missing-`/$schema` finding and original bytes; it MUST NOT infer a schema for any other artifact. Claim `TR0051-MANIFEST-ORDER-001` MUST bind the exact TR-0051 blob and `/git/changed_paths_manifest`, prove 35 unique paths, exact equality with the containing commit's complete changed-path set, no missing/extra/duplicate path, and only the recorded self-path-at-index-34 versus canonical index-9 difference. Any changed identity, path, count, set, order defect other than that exact historical ordering, wildcard, future record, generic waiver, or correction-of-correction MUST fail closed. This planning subject MAY replace the stale V8 approval action with the V9 approval action in lifecycle policy; after freeze, T077–T080 correction-off/on verification MUST keep those V9 policy bytes and the roadmap-policy result unchanged. V9 MUST leave V8 claims, T073–T076 implementation, all four readiness areas, 0/100 benchmark truth, gate rows, candidate, authority, delivery, dashboard bytes, and release eligibility unchanged.
...
- **Committed-Identity Correction**: One exact, approval-bound, append-only profile that records a closed set of recomputable factual metadata dispositions without rewriting historical evidence or changing readiness authority.
- **Repair-Evidence Correction**: The separate exact, approval-bound, two-claim profile for the immutable revision-45/revision-46 cause-ID and TR-0044 digest defects; it is not an extension or generic form of either earlier correction.
- **Preflight-Evidence Correction**: The exact approval-bound V9 two-claim profile for one immutable schema-less discovery blob and one immutable complete-but-self-last transition manifest; it is not schema inference, manifest sorting tolerance, or an extension of another correction.
...
- **SC-011**: Correction verification proves `37/37` exact claims, rejects every mutated target-set or forbidden-class fixture, and demonstrates byte-for-byte equality of all four readiness areas, benchmark counters, freshness, and release eligibility before and after applying the profile.
- **SC-012**: Input-origin correction verification proves exactly `1/1` claim, rejects every identity, pointer, origin, target-set, manifest, or authority mutation, and demonstrates unchanged readiness, benchmark, freshness, candidate, approval, and release results.
- **SC-013**: Repair-evidence correction verification proves exactly `2/2` claims and `2/2` cause-ID occurrences, rejects every added, omitted, substituted, reordered, wildcard, range, pointer, identity, ancestry, authority, forbidden-class, or correction-of-correction mutation, retains both original findings, and demonstrates byte-for-byte equality of readiness, gate, benchmark, freshness, candidate, approval, delivery, and release projections before and after disposition.
- **SC-014**: V8 verification proves exactly `3/3` historical claims; a one-field gate-evidence diff at `/catalog_digest`; both walkthrough causes with all three affected walkthrough test functions passing; committed-byte equivalence across Windows/POSIX line endings; repository-relative paths for TR-0050 and gate-catalog findings; and correction-off/on plus pre/post-rebind deep equality for all four readiness areas, all 34 gate rows, candidate/approval/delivery/release fields, dashboard bytes, and both honest `0/100` and isolated non-empty synthetic benchmark fixtures. The excluded roadmap-policy inversion test remains reported and T066 remains ineligible.
- **SC-015**: V9 verification proves exactly `2/2` claims, validates the immutable discovery object only through its exact external schema, reproduces TR-0051's 35-path complete-set equality and sole self-path position difference, rejects every near miss and unsupported reader, retains both original findings, and demonstrates byte-identical readiness, benchmark, gate, roadmap-policy, candidate, authority, delivery, dashboard, and release projections before and after disposition. T073–T076 and the separate roadmap-policy repair remain ineligible after V9 until separately authorized.
