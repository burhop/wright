...
12. Add a third, independent closed profile, `COR-EPP-F01-REPAIR-EVIDENCE-001`, for exactly two repair-evidence claims discovered after the first T066 candidate: the invalid underscore spelling of one stable cause identifier at two exact historical state pointers, and the truncated TR-0043 digest at TR-0044 `/inputs/1/sha256`. The profile binds every original record to raw SHA-256, Git blob, introducing commit/tree, exact pointer, and authoritative replacement value; it permits no rewrite, wildcard, new record, state/lifecycle/authority/readiness effect, or generic evidence waiver. T070–T072 raise the append-only plan to 72 tasks and remain unexecuted until exact V7 `material_change` and `feature_implementation` approvals accept DEC-P0-018 and bind the frozen profile digest.
...
4. **Validate semantics**: check normalized safe references, raw and canonical digests, lifecycle/event graphs, revision chain, complete transition manifests, roadmap DAG and eligibility, approvals, P0 due dates, WIP, pointer, and lease identity/scope. For the committed-identity correction, require exact V4 binding, strict ancestry and `37/37` recomputation. Separately, for the TR-0027 input-origin correction, require exact V5 binding, source absence, unique container introduction, literal `1/1` identity/pointer equality and unchanged manifest. For the repair-evidence correction, require exact V7 binding, literal `2/2` claim equality, exact `2/2` historical cause-ID occurrences, and exact TR-0043 source-digest recomputation. Every profile retains the original finding, has zero readiness/authority effect, and fails closed on any mismatch.
