...
- **SC-002**: Starting from a prior verified snapshot without the Onshape Labs record, a valid approved catalog update adds the Onshape Labs FeatureScript MCP without a code change, while 100% of custom entries, explicit disablement, install state, workspace grants, and credential references remain unchanged through activation, restart, and rollback. The feature's final bundled recovery snapshot MAY also include the same validated record for new offline installations.
