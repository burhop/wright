...
**Current workspace-owned correction subject**: exact commit, tree, walkthrough, and manifest pending
...
| Immutable revisions/CAS | retained production foundation | simulated | one visible workspace `.workflow.wflow` source uses side-effect-free read, idempotent create-if-absent bootstrap on Workflows entry, and compare-and-swap save; re-entry cannot overwrite existing source, and host revision/digest/integrity records remain outside engineer-authored source | focused persistence proof is required before binding the current correction to an exact subject; T052–T054 historical production sidecars remain separate |
| Renderer adapter | retained | renderer-specific experiments | reused unchanged via `DraftCanvasRenderer` | retain |
| Block/flow visual hierarchy | lane/form dominated | strong | nine-step canvas without decorative phase lanes or phase stripes; groups are optional rather than mandatory workflow phases | historical `f9237763` direction approval and prior `c5fb7d8e` correction evidence are preserved; current exact walkthrough is pending |
| Workspace ownership and entry | absent | not addressed | select a real workspace, then open **Workflows** at `/workspace/<real-id>?workflow=canonical`; that explicit action bootstraps and immediately opens the default only when absent, while ordinary workspace entry creates nothing and workflow creation/viewing has no global destination | routing, idempotent first/repeated entry, existing-source preservation, and absence of global navigation require focused and walkthrough proof on the new exact subject |
...
| Direct block movement | bounded custom interaction | disabled | node moves live before mouse-up while revision and accepted digests stay fixed; release commits only the layout digest | prior constrained Chromium walkthrough and digest assertions pass; fresh workspace-subject evidence pending |
...
| Output recognition/lineage | excluded | visual concept | bracket preview, report, STEP download, producer/run/revision/type/digest | prior browser download passes; fixture is simulated and current exact workspace walkthrough is pending |
| Accessibility | later checkpoint | partial | stable controls, focus trap/return, keyboard handles/component disclosure, 2× scale, named accessibility tree, axe zero serious/critical, reduced motion | the prior automated Chromium matrix passes 9/9; the workspace-owned subject needs fresh evidence, and one requesting engineer's formative review is not the five-participant protocol or real screen-reader evidence, so T056 remains open |
| Fixed-height desktop containment | page-oriented shell | unproven | 1070×791 application has no document/page scrolling; bounded columns own any necessary internal scrolling | implementation and automated viewport contract present; fresh exact walkthrough pending |
...
The recovery concept reaches functional parity with the useful canvas-first direction and surpasses the frozen prototype on direct manipulation, explicit source provenance, contextual downstream work, mixed-authority inspection, paired editing, proposal review, invalid containment, bounded run recovery, reusable-component inspection, and stable-identity navigation. The [approval record](evidence/product-approval.md) keeps T051 bound to its historical exact `f9237763` baseline. The [digest-bound comparison](evidence/prototype-side-by-side.md) and continuation-5 walkthrough remain prior automated, evidence-based material-equivalence and correction verification for `c5fb7d8e`; they do not invent a second human approval or claim that the requesting user personally reran that subject. The current correction moves authoring into a real workspace, makes Source an engineer-facing script rather than internal IR, and adds one visible `.workflow.wflow` file that is idempotently bootstrapped on Workflows entry and protected by compare-and-swap thereafter. Its next exact commit and full walkthrough are explicitly pending. T056's five-participant moderated usability and real assistive-technology evidence, qualifying engineering oracles, and public/cross-platform release gates remain downstream obligations.
