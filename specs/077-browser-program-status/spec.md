...
- **FR-043**: The page MUST provide a canonical all-use-case funnel with total, not-started, in-progress, implemented, independently verified, and remaining counts derived from a closed governed use-case registry. Each stage record MUST use the source classes allowed for that stage by the digest-bound source catalog; resolve its exact `subject_id` through that source's named parser; and preserve the resolved verdict. `implemented` requires a resolved passing user-visible acceptance record; code-only or partial work remains `in_progress`; `independently_verified` requires a resolved passing independent verdict bound to that exact acceptance subject whose verifier differs from its evidence author; and `remaining` means exactly `total - implemented`. A wrong source class, missing subject, verdict mismatch, broken acceptance binding, or equal author/verifier MUST reject publication.
- **FR-044**: The page MUST provide a separate 100-process subset funnel with defined, in-progress, implemented, tested, independently verified, and benchmark-qualified counts. Process identities MUST be unique and limited to exactly `EPP-PROC-001` through `EPP-PROC-100`. Qualification MUST resolve the exact process subject and passing acceptance/independent-verification relation and reconcile to the authoritative governed dashboard; its verifier MUST differ from the evidence author. Definition, implementation, test, and independent-verification evidence MUST remain orthogonal and MUST NOT upgrade qualification.
...
- **FR-047**: The canonical test-counting rule MUST retain all attempts for traceability but aggregate only the latest terminal run for each `(commit, suite_id, population_id)` counting key; carry the exact unique framework-collected test-case identities and their deterministic set digest for each selected source; count each parametrized identity once; exclude `summary_only` populations from aggregation; reject any test-case identity shared by two selected component populations; require per-source `total = len(test_case_ids) = passed + failed + skipped + not_run` and the same outcome arithmetic at checkpoint level; and define pass rate as `passed / (passed + failed)`, unavailable when that denominator is zero. Every string used by the three digest rules MUST already be Unicode NFC and contain no NUL. The identity-set digest sorts unique IDs by unsigned UTF-8 byte order and hashes `wright-test-id-set-v1\n` followed by repeated decimal-byte-length, colon, raw UTF-8 bytes, and LF frames. The run key hashes `wright-test-run-key-v1\n` followed by the same length framing for commit, suite ID, population ID, and ASCII-decimal attempt in that order. The runs digest hashes the complete stored-order `runs` array serialized as UTF-8 canonical JSON with recursively sorted object keys, preserved array order, NFC strings, no insignificant whitespace, comma/colon separators, and unescaped non-ASCII. All SHA-256 values are lowercase hexadecimal, and cross-language golden fixtures MUST prove identical bytes and digests.
