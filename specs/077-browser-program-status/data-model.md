...
The committed registry contains bounded stable use-case identities and orthogonal evidence lists for definition, progress, user-visible acceptance, tests, independent verification, and benchmark qualification. Use-case IDs are unique. Non-null process IDs are unique and limited to `EPP-PROC-001` through `EPP-PROC-100`. The publisher derives all governed use-case counts (`total`, `not_started`, `in_progress`, `implemented`, `independently_verified`, `remaining`) and the 100-process subset (`defined`, `in_progress`, `implemented`, `tested`, `independently_verified`, `benchmark_qualified`). `remaining` is exactly `total - implemented`.
...
Every stage record carries an exact evidence class, catalog source name, verdict, subject, acceptance-subject binding and independent verifier where required, plus a resolvable evidence identity. The bundle carries the bounded per-use-case records rather than aggregate counts alone. Publisher, runtime, and browser therefore reject duplicate use-case/process identities, evidence reused across incompatible stages, a stage path outside its named source rule, verification not bound to one passing acceptance subject, non-independent verification, or qualification not bound to the authoritative process/dashboard evidence; runtime/browser recompute both funnels from the projected items.

## TestRunLedger and TestHistory

The ledger has a monotonic revision, prior committed ledger identity, canonical runs digest, and unique run IDs/run-key digests. The publisher compares the current file with its exact committed predecessor, requires the prior run list as an unchanged prefix, and rejects deletion, rewrite, duplicate keys, or tied/conflicting maximum attempts. Each attempt carries exact commit, time, suite, population, category, attempt, terminal flag, aggregate role, exact unique framework-collected test identities, sorted-set digest, counts, and evidence. A canonical checkpoint selects the unique latest terminal attempt for each `(commit, suite_id, population_id)`, sums only `component` populations whose identity sets are pairwise disjoint, and keeps `summary_only` runs for detail. The bundle carries the three declared digest rules, a publisher append-only/latest-selection attestation, and each selected source's time, terminal/role, identities, digest, and counts. Source-free runtime/browser validate the attestation/evidence binding and independently recompute projected identity digests, disjointness, arithmetic, category/overall totals, and pass rate; they do not claim access to absent ledger history. Each source satisfies `total = len(test_case_ids) = passed + failed + skipped + not_run`; pass rate is `passed / (passed + failed)` or unavailable when the denominator is zero.

All digest inputs reject non-NFC strings and NUL. The identity digest sorts unique normalized IDs by unsigned UTF-8 byte order and hashes ASCII `wright-test-id-set-v1\n` plus, for each ID, ASCII decimal UTF-8 byte length, `:`, raw UTF-8 bytes, and LF. The run key uses the same framing after ASCII `wright-test-run-key-v1\n` for commit, suite ID, population ID, and ASCII-decimal attempt in that order. The runs digest hashes the complete stored-order array using `wright-json-c14n-v1`: recursively sorted object keys, arrays retained, NFC strings, UTF-8, no insignificant whitespace, comma/colon separators, and unescaped non-ASCII. SHA-256 output is lowercase hexadecimal; Python and TypeScript golden fixtures must agree byte-for-byte.

Use-case stage evidence is not self-declared authority. The digest-bound source catalog maps each stage to closed allowed source classes. The publisher resolves the exact subject through the named parser, compares the resolved evidence class and verdict, and enforces acceptance bindings. Definition is admitted only from roadmap/story catalog; progress from transitions/work registry/feature tasks; acceptance from gate or verification evidence; tests from the test ledger or verification evidence; independent verification only from verification evidence; and qualification from dashboard, benchmark coverage, gate evidence, or verification evidence. Passing independent verification and qualification require a recorded evidence author and a different verifier. Any wrong source, missing subject, verdict mismatch, broken binding, or equal author/verifier rejects publication.
...
- all-use-case or 100-process counts cannot be derived from unique registry identities and orthogonal evidence, `remaining != total - implemented`, a process ID is duplicated/outside `001..100`, implementation lacks acceptance, independent verification is not passing/bound, or qualification disagrees with the dashboard;
- a canonical test checkpoint uses a non-terminal or non-latest attempt, an identity-set digest cannot be recomputed, selected component identity sets overlap, a summary-only population is aggregated, selected-source/checkpoint count arithmetic or pass rate is wrong, or unavailable history is replaced with zero; or
