...
The full endpoint's ETag/304 behavior is canonical. Implementations MUST NOT add a second identity endpoint unless analysis proves this bounded route misses the performance target and the contract is amended before implementation.
...
- Only contracted repository-relative evidence paths and allowlisted HTTPS GitHub PR/check links are returned.
...
- Additive optional fields within schema `1.x` are safe to ignore.
