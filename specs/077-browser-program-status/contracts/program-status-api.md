...
- Governed all-use-case and 100-process funnel stages are independently derived from unique use-case IDs and unique `EPP-PROC-001..100` identities plus orthogonal definition, progress, acceptance, test, independent-verification, and benchmark evidence. `remaining = total - implemented`. The proposed story catalog is separate, and only the dashboard governs qualification.
- Each projected use-case stage carries source name, evidence class, verdict, exact subject, acceptance-subject binding and independent verifier where required, and one resolvable evidence identity. Runtime/browser recompute funnel counts and reject duplicate identities, cross-stage evidence reuse, source-rule mismatch, unbound/non-independent verification, and qualification inconsistent with the dashboard.
- Test history selects the latest terminal attempt per `(commit, suite_id, population_id)`, exposes each selected source's exact test identities/time/terminal/role/counts, recomputes sorted-set digests, counts each parametrized identity once, rejects overlapping component populations and bad count arithmetic, excludes `summary_only` runs from totals, and renders missing categories unavailable. Pass rate is `passed/(passed+failed)` or unavailable when that denominator is zero.
