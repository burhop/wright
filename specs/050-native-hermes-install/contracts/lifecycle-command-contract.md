...
Every command returns a structured `LifecycleResult` and a concise Hermes-facing
projection. Errors are stable codes plus redacted remediation; traceback,
...
1. Validate Hermes/plugin/runtime/platform/data-schema compatibility.
...
Start MUST NOT inspect a repository, `WRIGHT_REPO_DIR`, frontend source, or invoke
Git, Docker, Node, npm, or a frontend build.

## `/wright status`

Returns within the bounded probe window and includes:

- plugin, active runtime, predecessor, Hermes, Python, and compatibility versions;
- lifecycle state and active operation;
- process identity and challenged API/UI health;
- Hermes connection, MCP transport, catalog, data-schema, configuration, and
  workspace summary;
...
ports, data permissions/integrity, backup readiness, compatibility, Hermes model
route, MCP transport/catalog, and secret-permission checks. Optional external
...
- Clear process state after confirmed exit; never stop Hermes or another port
  owner.
...
- Validate artifact integrity, plugin/Hermes compatibility, and data schema before
...
- Cooperate with Hermes package removal through the package-plugin lifecycle hook.
...
- Reject symlinks, external workspaces, home/root directories, Hermes-owned data,
  and ambiguous configuration.
