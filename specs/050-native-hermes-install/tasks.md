# Tasks: Native Hermes Installation
...
- [X] T003 Create the native runtime package/test directory skeleton in `src/wright_engineering/hermes_plugin/`, `src/wright_engineering/runtime/`, `src/wright_engineering/static/`, and `tests/native_runtime/`
...
- [X] T007 [P] Add contained Windows/POSIX Hermes-home, runtime/data separation, symlink, root, and external-workspace path tests in `tests/native_runtime/test_layout.py` (FR-021-FR-024, SC-008)
...
- [X] T009 [P] Add plugin/runtime/Hermes/Python/platform/data-schema compatibility accept/reject tests in `tests/native_runtime/test_compatibility.py` (FR-017-FR-020, FR-028, FR-035, SC-009)
...
- [X] T014 Implement canonical contained layout and owned-data scope resolution in `src/wright_engineering/runtime/layout.py` (FR-021-FR-024)
...
- [X] T016 Add machine-readable compatibility metadata and fail-closed checks in `src/wright_engineering/compatibility.json` and `src/wright_engineering/hermes_plugin/compatibility.py` (FR-017, FR-028, FR-035)
...
through first healthy UI using one candidate wheel and no Git/Docker/Node/source.

### Tests first

- [X] T019 [P] [US1] Add Hermes base-distribution entry-point discovery and runtime-dependency absence tests in `tests/native_runtime/test_hermes_entrypoint.py` (FR-001, FR-002, FR-006, SC-001)
...
- [X] T022 [P] [US1] Add `/wright start` command projection tests with no repo detection/build calls in `tests/native_runtime/test_commands_start.py` (FR-003, FR-005, FR-006)
- [ ] T023 [US1] Add subprocess-audited clean first-install/start integration test in `tests/native_runtime/test_clean_install.py` that forbids Git, Docker, Node/npm, source checkout, `WRIGHT_REPO_DIR`, and preinstalled Wright (FR-031-FR-033, SC-001-SC-003)

### Implementation

- [X] T024 [US1] Convert root metadata into the base plugin plus complete `runtime` extra, bundle all Wright modules/resources, and register the Hermes entry point in `pyproject.toml` (FR-001, FR-004, FR-027-FR-030)
...
- [X] T027 [US1] Implement Hermes entry-point registration and the thin `/wright` dispatcher in `src/wright_engineering/hermes_plugin/__init__.py` and `src/wright_engineering/hermes_plugin/commands.py` (FR-001, FR-006)
- [X] T028 [US1] Implement install-on-start, compatibility, activation, challenge, failure recovery, and UI result orchestration in `src/wright_engineering/runtime/lifecycle.py` (FR-002-FR-007, FR-012, FR-014-FR-017)
- [X] T029 [US1] Extend the public CLI with hidden/stable runtime serve and lifecycle dispatch without importing runtime extras in `src/wright_engineering/cli.py` (FR-004-FR-006)
- [X] T030 [US1] Convert `hermes-plugin-wright/` into a one-release legacy delegate with migration diagnostics and remove repository/build behavior from `hermes-plugin-wright/commands.py`, `hermes-plugin-wright/bridge.py`, and `hermes-plugin-wright/plugin.yaml` (FR-003, FR-005, FR-027, FR-028)
...
- [X] T031 [P] [US2] Add structured status and doctor state/health/remediation tests in `tests/native_runtime/test_status_doctor.py` (FR-009, FR-010, SC-005)
...
- [X] T034 [P] [US2] Add packaged catalog, MCP transport/progress/cancellation, workspace creation/reopen, and Hermes health integration tests in `tests/native_runtime/test_packaged_integration.py` (FR-011-FR-013, FR-018, SC-011)
...
- [X] T036 [US2] Implement read-only status and bounded doctor checks across plugin/runtime/process/API/UI/Hermes/MCP/catalog/data/config/workspaces in `src/wright_engineering/runtime/diagnostics.py` (FR-009, FR-010, FR-025, FR-026)
...
- [X] T038 [US2] Project lifecycle results and existing open/catalog commands through the packaged API in `src/wright_engineering/hermes_plugin/commands.py` (FR-006, FR-009-FR-013)
...
- [X] T049 [US3] Expose update/rollback through Hermes and public CLI projections in `src/wright_engineering/hermes_plugin/commands.py` and `src/wright_engineering/cli.py` (FR-006, FR-017-FR-020)
...
- [X] T050 [P] [US4] Add running/stopped uninstall, runtime/cache removal, preserved data, and Hermes callback tests in `tests/native_runtime/test_uninstall.py` (FR-021, FR-024, SC-008)
...
- [X] T053 [US4] Add `/wright uninstall` and `/wright purge` command/callback tests in `tests/native_runtime/test_commands_remove.py` (FR-006, FR-021-FR-024)
...
- [X] T056 [US4] Register Hermes pre-remove lifecycle integration and command projections in `src/wright_engineering/hermes_plugin/__init__.py` and `src/wright_engineering/hermes_plugin/commands.py` (FR-006, FR-021-FR-024)
...
- [X] T062 [P] [US5] Add real-Hermes capability probe tests that reject current Git-only install/update/remove in `tests/native_runtime/test_hermes_package_plugin_contract.py` (FR-001, FR-003, FR-036, FR-037)
...
- [X] T065 [US5] Implement clean base/lifecycle/matrix harness with subprocess audit and source isolation in `scripts/test-native-hermes-install.py` (FR-031-FR-035)
- [X] T066 [US5] Add mandatory native candidate workflow on claimed platforms with local wheelhouse and no publish permissions in `.github/workflows/native-hermes-pr.yml` (FR-030-FR-035, FR-041)
...
- [X] T069 [US5] Add build-once runtime-extra validation, stable Hermes channel activation, and published native lifecycle jobs to `.github/workflows/release.yml` while retaining mandatory PyPI/GHCR/Docker Hub/docs/GitHub Release gates (FR-036-FR-040)
- [X] T070 [US5] Reclassify `.github/workflows/sync-hermes-plugin-mirror.yml` as legacy migration-only and prevent it from satisfying native release evidence (FR-027, FR-036-FR-040)
...
- [ ] T073 [US5] Validate and record the minimum released Hermes package-plugin lifecycle capability in `src/wright_engineering/compatibility.json` and `docs/getting-started/hermes-plugin.md`; do not mark complete until the real released interface passes published-artifact install/update/rollback/remove tests (FR-001, FR-003, FR-017, FR-035-FR-037)
...
**Goal**: Make native Hermes the clear primary path, preserve Docker as turnkey,
and document exact lifecycle/data/package behavior.
...
- [X] T074 [P] [US6] Update documentation release-gate tests for primary native path, one package role, exact lifecycle commands, data semantics, platform evidence, and mandatory Docker path in `tests/test_docs_release_gate.py` and `tests/test_hermes_plugin_mirror_docs.py` (FR-027, FR-028, FR-039, FR-042-FR-044, SC-014)
- [X] T075 [P] [US6] Add command/example drift checks between compatibility metadata, package metadata, plugin help, install docs, and release runbook in `tests/test_native_docs_contract.py` (FR-017, FR-027, FR-028, FR-042)

### Implementation

- [X] T076 [US6] Replace helper-only and Docker-primary claims with verified native-primary plus mandatory Docker language in `README.md`, `docs/getting-started/overview.md`, and `docs/getting-started/install-matrix.md` (FR-027, FR-028, FR-039, FR-042, SC-014)
- [X] T077 [US6] Document exact native install/start/status/doctor/stop/update/rollback/uninstall/purge and troubleshooting flows in `docs/getting-started/hermes-plugin.md` and `docs/hermes-desktop-wright.md` (FR-001-FR-010, FR-017-FR-024, FR-042)
- [X] T078 [US6] Document application distribution/runtime-extra/private-package roles and compatibility policy in `docs/release/community-release-readiness.md` and `docs/release/hermes-plugin-mirror.md` (FR-027-FR-030, FR-042)
- [X] T079 [US6] Document offline cache/index operation, platform support boundaries, external MCP prerequisites, and Docker coexistence in `docs/getting-started/prerequisites.md`, `docs/getting-started/quickstart-docker.md`, and `docs/mcp-catalog/mcp-server-testing-process.md` (FR-035, FR-039, FR-042-FR-044)
...
## Phase 9: Cross-Cutting Validation and Delivery
...
- [X] T080 Run `/speckit-analyze`, remediate all CRITICAL/HIGH findings in `specs/050-native-hermes-install/spec.md`, `plan.md`, and `tasks.md`, and rerun until coverage is complete
- [X] T081 [P] Run focused Python unit/contract/integration/security suites including `uv run pytest -q tests/native_runtime hermes-plugin-wright/tests tests/release` and record results in `specs/050-native-hermes-install/validation.md`
...
| Requirement | Tasks |
| --- | --- |
| FR-001 | T019, T024, T027, T062, T073 |
| FR-002 | T010, T017, T019, T021, T023, T028 |
| FR-003 | T022, T030, T039, T062, T065 |
...
| FR-017 | T009, T016, T042, T044, T047-T049, T073, T075, T077 |
...
| FR-025 | T006, T012, T031, T036, T040, T052, T055, T085 |
| FR-026 | T012, T031, T036, T040 |
| FR-027 | T006, T024, T030, T058, T064, T070, T074-T078 |
...
| FR-031 | T010, T023, T061, T065, T084 |
| FR-032 | T010, T023, T061, T062, T065, T073, T084 |
| FR-033 | T023, T034, T041, T061, T065-T067, T084 |
| FR-034 | T023, T033, T043, T044, T052, T061, T065, T084 |
| FR-035 | T009, T016, T061, T065-T067, T073, T079, T084 |
| FR-036 | T059, T060, T062, T068-T073, T087, T090 |
| FR-037 | T059, T060, T062, T068, T069, T073, T084 |
| FR-038 | T058, T059, T064, T068, T069, T085 |
| FR-039 | T060, T068-T071, T074, T076, T079, T083, T087 |
| FR-040 | T059, T060, T068-T071, T083, T087, T090 |
| FR-041 | T060, T063, T066, T069, T085 |
| FR-042 | T071, T074-T079 |
| FR-043 | T060, T069, T074, T076, T079, T083, T087 |
| FR-044 | T074, T079, T083 |
| FR-045 | T067, T072, T081-T090 |
| SC-001 | T019, T023, T027-T030, T065, T084 |
| SC-002 | T023, T061, T065-T067, T084 |
| SC-003 | T021, T023, T028, T061, T065, T084 |
| SC-004 | T011, T021, T028, T032, T033, T037, T084 |
| SC-005 | T012, T031, T035, T036, T040, T084 |
| SC-006 | T034, T041, T042, T046-T048, T051, T057, T084 |
| SC-007 | T043, T044, T047, T048, T084 |
| SC-008 | T007, T014, T050-T057, T084 |
| SC-009 | T009, T016, T044, T048, T062, T073, T084 |
| SC-010 | T006, T010, T012, T024, T025, T058, T064, T065, T085 |
| SC-011 | T020, T034, T039, T083 |
| SC-012 | T059, T060, T068-T070, T073, T084, T087, T090 |
| SC-013 | T009, T016, T061, T065-T067, T073, T079, T084 |
| SC-014 | T074-T079 |
| SC-015 | T067, T072, T080-T090 |

The released-Hermes prerequisite is explicit coverage, not waived scope: T062
must fail against Git-only Hermes, and T073 cannot complete until a released
package-plugin lifecycle satisfies the external contract.
...
- User Story 5 consumes the exact artifact and lifecycle built by User Stories
  1-4; production completion also depends on a released Hermes implementation of
  the external package-plugin contract.
- User Story 6 depends on executable contracts from User Stories 1-5 so docs do
  not make premature claims.
- Phase 9 depends on all stories and remains incomplete until real released-Hermes
  and public-artifact evidence exists.
...
- Final focused Python, frontend, and provider-neutral/Docker validations
  T081-T083 may run in parallel before the full matrix and merge gates.
...
T062: Real Hermes capability probe tests
...
6. Run the complete completion audit and delivery phase; do not substitute the
   Hermes fixture for released-Hermes production evidence.
