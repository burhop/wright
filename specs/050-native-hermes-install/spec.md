# Feature Specification: Native Agent-Manager Installation
...
**Input**: User description: "Deliver a production-ready native Wright runtime
that users can install from Hermes and connect to from Codex. The design stays
open to a future OpenClaw adapter without claiming it in this delivery.
Manager-specific prerequisites belong to each thin adapter: Hermes may require
Git, while Wright's runtime must not require a source checkout, Docker, a
frontend toolchain, `WRIGHT_REPO_DIR`, or user-issued Python package commands.
Preserve Docker, provider-neutral MCP behavior, and all production release
gates."
...
- Q: May Wright assume Git is present when Hermes installs plugins, and must the
  architecture remain usable by other managers? -> A: Yes; Git is a Hermes
  prerequisite, while Wright core must remain manager-neutral for Hermes,
  Codex, and future managers.
- Q: Is OpenClaw a supported installation target for this delivery? -> A: No.
  Preserve a manager-neutral design, but treat OpenClaw integration and host
  validation as future work. It MUST NOT block this feature or its release.
...
**Independent Test**: Begin with a clean supported Hermes environment containing
Hermes' documented prerequisite, Git, but no Docker, Node.js, npm, Wright
checkout, `WRIGHT_REPO_DIR`, or preinstalled Wright package; install the
candidate adapter through the real Hermes Git plugin interface, run `/wright
start`, and verify the UI, Wright health, Hermes connectivity, MCP transport,
and catalog.

**Acceptance Scenarios**:

1. **Given** a supported clean Hermes installation with Git, **When** the user
   installs Wright through Hermes' supported Git plugin interface, **Then** the
   thin adapter automatically obtains a compatible, complete Wright runtime
   without requiring a Wright checkout or user-issued package command.
...
3. **Given** the Hermes adapter has already been installed, **When** Wright
   runtime installation and start are exercised, **Then** neither operation
   invokes Git, Docker, Node.js, npm, `WRIGHT_REPO_DIR`, or Wright source files.
...
1. **Given** an installed Wright runtime with user workspaces, **When** the user runs `/wright uninstall` and then removes the Hermes-owned plugin through Hermes, **Then** Wright processes, runtime artifacts, and plugin integration are removed while user data and user-authored workspace files remain.
...
A release owner publishes a Wright version only when the managed runtime and
every publicly claimed manager adapter are distributed and verified alongside
the existing Python, container, documentation, and release artifacts.

**Why this priority**: A release is incomplete if its primary Hermes path or any
other publicly claimed manager integration cannot be used by a third party.
...
2. **Given** a production release candidate, **When** release gates run, **Then**
   manager-adapter distribution and connection checks plus runtime distribution,
   clean install, start, upgrade, rollback, uninstall, and public-install
...
A user or operator can select Hermes, Codex, or Docker, understand what
each runtime and adapter artifact does, and follow concise installation,
connection, upgrade, rollback, uninstall, and troubleshooting guidance.
...
2. **Given** a Codex user, **When** they read the installation guide,
   **Then** it configures the manager to connect directly to Wright's MCP service
   and does not require Hermes.
3. **Given** a Docker user, **When** they read the installation guide, **Then** the turnkey isolated path remains complete and is clearly distinguished from native manager installation.
4. **Given** any public Wright package or plugin artifact, **When** its documentation and metadata are inspected, **Then** it has one unambiguous role, lifecycle owner, compatibility contract, and supported installation audience.
5. **Given** troubleshooting or removal needs, **When** the user follows the documentation, **Then** commands and expected data effects agree with automated lifecycle tests.

---

### User Story 7 - Use the Same Wright Runtime from Other Agent Managers (Priority: P1)

A Codex user connects to the same installed Wright runtime and
provider-neutral MCP service through that manager's supported configuration or
plugin mechanism. Wright core does not contain copied Hermes branches or use a
Hermes-owned data location.

**Why this priority**: Hermes is the first distribution adapter, not Wright's
architecture. Keeping the runtime and MCP service manager-neutral prevents each
new agent manager from creating another application implementation.

**Independent Test**: Install one Wright runtime in a Wright-owned home, generate
or load the supported Hermes and Codex integration profiles, and prove that
both resolve the same runtime identity and MCP service contract while
their manager-specific prerequisites and configuration shapes remain confined
to their adapters.

**Acceptance Scenarios**:

1. **Given** a healthy native Wright runtime, **When** Codex loads the Wright MCP
   configuration, **Then** it can discover and call the provider-neutral Wright
   tools without routing through Hermes.
3. **Given** a new manager adapter, **When** it invokes the public Wright
   lifecycle and MCP contracts, **Then** no manager-specific condition is added
   to the lifecycle, catalog, workspace, or MCP core.
...
- **FR-001**: Wright MUST provide a supported Hermes Git plugin installation
  interface for third-party users that matches the released Hermes CLI.
- **FR-002**: Installing the plugin MUST automatically obtain and configure a compatible complete Wright runtime.
- **FR-003**: Hermes adapter installation MAY invoke Git because Git is a
  documented Hermes prerequisite. After the adapter is installed, Wright
  runtime installation and start MUST NOT invoke Git, Docker, Node.js, npm, a
  Wright source checkout, `WRIGHT_REPO_DIR`, or user-issued Python package
  commands.
...
- **FR-006**: A manager adapter MUST translate manager actions into Wright's
  public lifecycle contract; Wright MUST own runtime installation, activation,
  health verification, update, rollback, stop, uninstall, and purge while the
  runtime remains isolated from every manager process.
...
- **FR-009**: Wright status MUST report the invoking manager adapter, installed
  and active runtime versions, lifecycle state, compatibility state, API and UI
  health, active manager connection, MCP transport, catalog availability, and
  relevant data location without exposing secrets.
- **FR-010**: `/wright doctor` MUST diagnose installation integrity, version compatibility, process ownership, ports, configuration, data access, Wright health, invoking-manager connectivity, MCP transport, catalog health, and available recovery actions.
...
- **FR-012**: A successful start MUST provide a usable Wright UI location and the UI MUST accurately report Wright API and active-manager connectivity.
...
- **FR-017**: Each manager adapter and the Wright runtime MUST publish an
  explicit, machine-verifiable compatibility contract and fail closed on
  unsupported combinations without making runtime core manager-specific.
...
- **FR-021**: The supported uninstall procedure MUST remove managed runtime
  artifacts and active Wright processes through `/wright uninstall`, preserve
  user data by default, and then remove the manager-owned plugin integration
  through that manager's supported removal interface. Wright MUST NOT delete or
  mutate manager-owned plugin state itself.
...
- **FR-027**: Every public Wright runtime or manager-adapter artifact MUST have
  one documented purpose, owner, compatibility relationship, update path, and
  uninstall behavior.
...
- **FR-031**: Automated clean-install validation MUST begin with only the
  documented prerequisites of the manager under test. Hermes validation MUST
  include Git; after Hermes adapter installation, validation MUST remove Git
  from the runtime PATH and begin without Docker, Node.js, npm, a Wright
  checkout, `WRIGHT_REPO_DIR`, or preinstalled Wright packages.
- **FR-032**: Clean-install validation MUST use the same supported manager
  adapter installation interface offered to third parties and MUST prove
  automatic Wright runtime installation.
- **FR-033**: Native acceptance validation MUST cover install, start, health, UI, manager connection, MCP transport, catalog, workspace creation and reopening, stop, restart, update from previous stable, rollback, uninstall, reinstall with preserved data, and explicit purge.
...
- **FR-035**: Validation MUST cover every operating-system and architecture combination publicly claimed for the Wright runtime and each manager adapter; unvalidated combinations MUST NOT be newly claimed.
- **FR-036**: Production release orchestration MUST distribute the compatible
  Hermes adapter, every other claimed manager adapter, and the managed Wright
  runtime, and MUST block release completion unless their published-artifact
  install and connection checks plus the Wright runtime lifecycle verification
  succeed.
- **FR-037**: Public-install verification MUST consume published immutable
  manager-adapter identities and Wright runtime artifacts rather than a local
  Wright checkout or substituted files. For Hermes, an immutable published Git
  tag or commit is the adapter identity.
...
- **FR-040**: A production release MUST NOT be reported complete when any required managed-runtime, manager-adapter, or Docker distribution, lifecycle, connection, or public-install gate is missing, skipped, or failing.
...
- **FR-046**: Wright lifecycle storage, commands, artifact resolution, and
  process management MUST be manager-neutral and rooted in a Wright-owned home,
  not `HERMES_HOME`, Codex state, or another manager directory.
- **FR-047**: Codex MUST be able to consume Wright directly through
  the same provider-neutral STDIO or Streamable HTTP MCP service used by the
  Hermes adapter; the integration may not require Hermes as an intermediary.
- **FR-048**: Git, marketplace, archive, and other installation
  prerequisites specific to one manager MUST remain confined to that manager's
  adapter and acceptance harness and MUST NOT become Wright runtime
  prerequisites.

### Key Entities

- **Manager Adapter Installation**: A Hermes-, Codex-, or future
  manager-visible integration with its installation source, installed version,
  declared Wright compatibility, lifecycle projection, and installation status.
...
- **Compatibility Contract**: Machine-verifiable rules that map manager adapter,
  runtime, manager protocol, platform, and data-format versions to supported or
...
- **Wright-Owned Data Scope**: Documented directories and records managed by Wright, separated from user-authored external workspaces and unrelated manager data.
- **Release Evidence**: The immutable manager-adapter/runtime identities, compatibility result, platform matrix, native lifecycle verification, existing artifact verification, and completion decision for one product release.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: A new Hermes user on every supported native platform completes the
  supported Git plugin installation and reaches a healthy Wright UI using that
  one install operation followed by `/wright start`.
- **SC-002**: One hundred percent of Hermes clean-install tests use Git only for
  the real plugin-install phase; after that phase, runtime install and start
  pass with Git, Docker, Node.js, npm, Wright source files, `WRIGHT_REPO_DIR`,
  and preinstalled Wright packages absent from the execution environment.
...
- **SC-005**: One hundred percent of status and doctor test scenarios report the correct manager adapter, runtime, compatibility, process, API, UI, manager connection, MCP, catalog, and recovery state without exposing secret values.
...
- **SC-012**: Every production release reports each claimed manager-adapter
  identity and one compatible Wright runtime identity, and all required
  published-artifact adapter and lifecycle checks pass before completion.
...
- **SC-014**: A third-party evaluator can select native Hermes, Codex, or Docker
  installation and complete the documented supported path
  without consulting repository-development instructions.
...
- **SC-016**: Hermes and Codex integration contract tests resolve one
  Wright-owned runtime identity and the same provider-neutral MCP tool surface,
  with zero manager-specific branches in Wright lifecycle or MCP core.

## Assumptions

- Hermes 0.19's supported Git plugin installation is the production Hermes
  adapter path. Git is assumed present for that install and for Hermes-managed
  adapter updates; it is not a Wright runtime prerequisite.
- The native runtime may use a public package repository internally, but that
  mechanism is hidden behind Wright lifecycle commands invoked by each manager
  adapter.
...
- Before a previous native Wright release exists, first-release acceptance uses
  two immutable candidate versions to prove update and rollback. Starting with
  the second native release, the previous stable public version is mandatory.
...
- Internet access may be required to obtain public adapter and runtime artifacts, but installed Wright core operation retains the project's offline-first behavior and provides documented offline installation or caching guidance where each manager mechanism supports it.
- Codex consumes Wright directly through its supported MCP configuration
  surface and does not route through Hermes. OpenClaw integration is future
  work and is not a supported path or release gate for this delivery.
